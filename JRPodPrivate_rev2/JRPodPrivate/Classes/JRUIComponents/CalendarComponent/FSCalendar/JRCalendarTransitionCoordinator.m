//
//  JRCalendarTransitionCoordinator.m
//  JRCalendar
//
...
@interface JRCalendarTransitionCoordinator ()

@property (readonly, nonatomic) JRCalendarTransitionAttributes *transitionAttributes;
@property (strong  , nonatomic) JRCalendarTransitionAttributes *pendingAttributes;
@property (assign  , nonatomic) CGFloat lastTranslation;

- (void)performTransitionCompletionAnimated:(BOOL)animated;
- (void)performTransitionCompletion:(JRCalendarTransition)transition animated:(BOOL)animated;
...
- (void)performForwardTransition:(JRCalendarTransition)transition fromProgress:(CGFloat)progress;
- (void)performBackwardTransition:(JRCalendarTransition)transition fromProgress:(CGFloat)progress;
...
- (CGRect)boundingRectForScope:(JRCalendarScope)scope page:(NSDate *)page;

- (void)boundingRectWillChange:(CGRect)targetBounds animated:(BOOL)animated;

@end

@implementation JRCalendarTransitionCoordinator
...
{
    if (self.state != JRCalendarTransitionStateIdle) {
        return NO;
    }
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    ...
    if (gestureRecognizer == self.calendar.scopeHandle.panGesture) {
        ...
        return self.calendar.scope == JRCalendarScopeWeek ? velocity >= 0 : velocity <= 0;
    }
    if ([gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]] && [[gestureRecognizer valueForKey:@"_targets"] containsObject:self.calendar]) {
        ...
        BOOL shouldStart = self.calendar.scope == JRCalendarScopeWeek ? velocity.y >= 0 : velocity.y <= 0;
...
- (void)scopeTransitionDidBegin:(UIPanGestureRecognizer *)panGesture
{
    if (self.state != JRCalendarTransitionStateIdle) return;
    
    CGPoint velocity = [panGesture velocityInView:panGesture.view];
    switch (self.calendar.scope) {
        case JRCalendarScopeMonth: {
            if (velocity.y < 0) {
                self.state = JRCalendarTransitionStateChanging;
                self.transition = JRCalendarTransitionMonthToWeek;
            }
            break;
        }
        case JRCalendarScopeWeek: {
            if (velocity.y > 0) {
                self.state = JRCalendarTransitionStateChanging;
                self.transition = JRCalendarTransitionWeekToMonth;
            }
            break;
        }
        default:
...
- (void)scopeTransitionDidEnd:(UIPanGestureRecognizer *)panGesture
{
    ...
    self.state = JRCalendarTransitionStateFinishing;
...
- (void)performScopeTransitionFromScope:(JRCalendarScope)fromScope toScope:(JRCalendarScope)toScope animated:(BOOL)animated
{
    ...
    self.transition = ({
        JRCalendarTransition transition = JRCalendarTransitionNone;
        if (fromScope == JRCalendarScopeMonth && toScope == JRCalendarScopeWeek) {
            transition = JRCalendarTransitionMonthToWeek;
        } else if (fromScope == JRCalendarScopeWeek && toScope == JRCalendarScopeMonth) {
            transition = JRCalendarTransitionWeekToMonth;
        }
        transition;
    });
    ...
    // Start transition
    self.state = JRCalendarTransitionStateFinishing;
    JRCalendarTransitionAttributes *attr = self.transitionAttributes;
...
{
    if (self.calendar.scope != JRCalendarScopeMonth) return;
    ...
    if (lastRowCount != currentRowCount) {
        ...
        CGRect bounds = (CGRect){CGPointZero,[self.calendar sizeThatFits:self.calendar.frame.size scope:JRCalendarScopeMonth]};
        self.state = JRCalendarTransitionStateChanging;
        void (^completion)(BOOL) = ^(BOOL finished) {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(MAX(0, duration-animationDuration) * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                self.state = JRCalendarTransitionStateIdle;
            });
        };
...
- (void)performTransitionCompletion:(JRCalendarTransition)transition animated:(BOOL)animated
{
    ...
    self.state = JRCalendarTransitionStateIdle;
    self.transition = JRCalendarTransitionNone;
...
- (JRCalendarTransitionAttributes *)transitionAttributes
{
    JRCalendarTransitionAttributes *attributes = [[JRCalendarTransitionAttributes alloc] init];
    ...
    switch (self.transition) {
        ...
        case JRCalendarTransitionMonthToWeek: {
            ...
            NSDate *focusedDate = ({
                ...
                NSArray<NSDate *> *visibleCandidates = [candidates filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(NSDate *  _Nullable evaluatedObject, NSDictionary<NSString *,id> * _Nullable bindings) {
                    NSIndexPath *indexPath = [self.calendar.calculator indexPathForDate:evaluatedObject scope:JRCalendarScopeMonth];
                    NSInteger currentSection = [self.calendar.calculator indexPathForDate:self.calendar.currentPage scope:JRCalendarScopeMonth].section;
                    return indexPath.section == currentSection;
                }]];
                NSDate *date = visibleCandidates.firstObject;
                date;
            });
            NSInteger focusedRow = [self.calendar.calculator coordinateForIndexPath:[self.calendar.calculator indexPathForDate:focusedDate scope:JRCalendarScopeMonth]].row;
            
            NSDate *currentPage = self.calendar.currentPage;
            NSIndexPath *indexPath = [self.calendar.calculator indexPathForDate:currentPage scope:JRCalendarScopeMonth];
            ...
            attributes.targetBounds = [self boundingRectForScope:JRCalendarScopeWeek page:attributes.targetPage];
        ...
        case JRCalendarTransitionWeekToMonth: {
            ...
            NSDate *focusedDate = ({
                ...
                NSArray<NSDate *> *visibleCandidates = [candidates filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(NSDate *  _Nullable evaluatedObject, NSDictionary<NSString *,id> * _Nullable bindings) {
                    NSIndexPath *indexPath = [self.calendar.calculator indexPathForDate:evaluatedObject scope:JRCalendarScopeWeek];
                    NSInteger currentSection = [self.calendar.calculator indexPathForDate:self.calendar.currentPage scope:JRCalendarScopeWeek].section;
                    return indexPath.section == currentSection;
                }]];
                NSDate *date = visibleCandidates.firstObject;
                date;
            ...
            attributes.targetBounds = [self boundingRectForScope:JRCalendarScopeMonth page:attributes.targetPage];
...
- (JRCalendarScope)representingScope
{
    switch (self.state) {
        case JRCalendarTransitionStateIdle: {
            return self.calendar.scope;
        }
        case JRCalendarTransitionStateChanging:
        case JRCalendarTransitionStateFinishing: {
            return JRCalendarScopeMonth;
        }
    }
}
...
- (CGRect)boundingRectForScope:(JRCalendarScope)scope page:(NSDate *)page
{
    ...
    switch (scope) {
        case JRCalendarScopeMonth: {
            if (self.calendar.placeholderType == JRCalendarPlaceholderTypeFillSixRows) {
                ...
                contentSize = CGSizeMake(self.calendar.fs_width,
                                         ...
                                         self.calendar.scopeHandle.fs_height + _collectionViewLayout.sectionInsets.top + _collectionViewLayout.sectionInsets.bottom);
            }
            break;
        }
        case JRCalendarScopeWeek: {
...
- (void)performForwardTransition:(JRCalendarTransition)transition fromProgress:(CGFloat)progress
{
    JRCalendarTransitionAttributes *attr = self.pendingAttributes;
    switch (transition) {
        case JRCalendarTransitionMonthToWeek: {
            
            [self.calendar willChangeValueForKey:@"scope"];
            [self.calendar fs_setUnsignedIntegerVariable:JRCalendarScopeWeek forKey:@"_scope"];
        ...
        case JRCalendarTransitionWeekToMonth: {
            ...
            [self.calendar willChangeValueForKey:@"scope"];
            [self.calendar fs_setUnsignedIntegerVariable:JRCalendarScopeMonth forKey:@"_scope"];
            [self.calendar didChangeValueForKey:@"scope"];
    ...
    switch (transition) {
        ...
        case JRCalendarTransitionWeekToMonth: {
            ...
            [self performAlphaAnimationFrom:progress to:0 duration:0.3 exception:self.pendingAttributes.focusedRowNumber completion:^{
                ...
                [self performTransitionCompletion:JRCalendarTransitionMonthToWeek animated:YES];
            }];
...
- (void)performAlphaAnimationFrom:(CGFloat)fromAlpha to:(CGFloat)toAlpha duration:(CGFloat)duration exception:(NSInteger)exception completion:(void(^)())completion;
{
    [self.calendar.visibleCells enumerateObjectsUsingBlock:^(JRCalendarCell *cell, NSUInteger idx, BOOL *stop) {
...
- (void)performAlphaAnimationWithProgress:(CGFloat)progress
{
    CGFloat opacity = self.transition == JRCalendarTransitionMonthToWeek ? MAX((1-progress*1.1),0) : progress;
    [self.calendar.visibleCells enumerateObjectsUsingBlock:^(JRCalendarCell *cell, NSUInteger idx, BOOL *stop) {
        if (CGRectContainsPoint(self.collectionView.bounds, cell.center)) {
            BOOL shouldPerformAlpha = NO;
            NSIndexPath *indexPath = [self.collectionView indexPathForCell:cell];
...
- (void)performPathAnimationWithProgress:(CGFloat)progress
{
    ...
    self.collectionView.fs_top = (-self.pendingAttributes.focusedRowNumber*self.calendar.collectionViewLayout.estimatedItemSize.height)*(self.transition == JRCalendarTransitionMonthToWeek?progress:(1-progress));
...
@implementation JRCalendarTransitionAttributes
