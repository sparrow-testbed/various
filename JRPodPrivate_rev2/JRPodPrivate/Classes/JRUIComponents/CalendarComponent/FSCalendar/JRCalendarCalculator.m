...
@interface JRCalendarCalculator ()
...
@implementation JRCalendarCalculator
...
- (instancetype)initWithCalendar:(JRCalendar *)calendar
{
    ...
    if (self) {
        ...
        self.titleHeight = JRCalendarAutomaticDimension;
        self.subtitleHeight = JRCalendarAutomaticDimension;
...
- (CGFloat)titleHeight
{
    if (_titleHeight == JRCalendarAutomaticDimension) {
...
{
    if (_subtitleHeight == JRCalendarAutomaticDimension) {
...
{
    return [self indexPathForDate:date atMonthPosition:JRCalendarMonthPositionCurrent scope:self.calendar.transitionCoordinator.representingScope];
}

- (NSIndexPath *)indexPathForDate:(NSDate *)date scope:(JRCalendarScope)scope
{
    return [self indexPathForDate:date atMonthPosition:JRCalendarMonthPositionCurrent scope:scope];
}
...
- (NSIndexPath *)indexPathForDate:(NSDate *)date atMonthPosition:(JRCalendarMonthPosition)position scope:(JRCalendarScope)scope
{
    ...
    switch (scope) {
        case JRCalendarScopeMonth: {
            ...
            if (position == JRCalendarMonthPositionPrevious) {
                section++;
            } else if (position == JRCalendarMonthPositionNext) {
                section--;
            }
...
{
    if (self.calendar.transitionCoordinator.transition == JRCalendarTransitionWeekToMonth) {
...
- (NSInteger)numberOfHeadPlaceholdersForMonth:(NSDate *)month
{
    ...
    NSInteger number = ((currentWeekday- self.gregorian.firstWeekday) + 7) % 7 ?: (7 * (!self.calendar.floatingMode&&(self.calendar.placeholderType == JRCalendarPlaceholderTypeFillSixRows)));
    return number;
}
...
{
    if (!month) return 0;
    if (self.calendar.placeholderType == JRCalendarPlaceholderTypeFillSixRows) return 6;
...
{
    if (self.calendar.transitionCoordinator.representingScope == JRCalendarScopeWeek) return 1;
...
{
    if (!indexPath) return JRCalendarMonthPositionNotFound;
    if (self.calendar.transitionCoordinator.representingScope == JRCalendarScopeWeek) {
        return JRCalendarMonthPositionCurrent;
    }
    ...
    switch (comparison) {
        case NSOrderedAscending:
            return JRCalendarMonthPositionPrevious;
        case NSOrderedSame:
            return JRCalendarMonthPositionCurrent;
        case NSOrderedDescending:
            return JRCalendarMonthPositionNext;
    }
}

- (JRCalendarCoordinate)coordinateForIndexPath:(NSIndexPath *)indexPath
{
    JRCalendarCoordinate coordinate;
