...
<%
	...
	multilang_id.addElement("WO_201");
...
var G_SERVLETURL    = "<%=POASRM_CONTEXT_NAME%>/servlets/ev.evmaster_mst";
var GridObj         = null;
var MenuObj         = null;
var myDataProcessor = null;

function initAjax(){
	doRequestUsingPOST( 'W201', '<%=to_day.substring(0,4)%>' ,'p_ev_gubun', '', false);
}
	
function nextAjax(){
	var p_ev_gubun_id            = eval(document.getElementById('p_ev_gubun'));
	
	p_ev_gubun_id.options.length = 0;
	
	var nodePath                 = document.getElementById("p_ev_gubun");
	var ooption                  = document.createElement("option");
	
	ooption.text             = "전체";
	ooption.value            = "";
	nodePath.add(ooption);
	doRequestUsingPOST( 'W201', '<%=to_day.substring(0,4)%>' ,'p_ev_gubun', '' );
}
...
function doInsert(){
	...
	if (confirm("<%=text.get("MESSAGE.1018")%>")){
		...
		param           = "?mode=insert&cols_ids="+cols_ids;                                //파라미터
...
function doDelete(){
	...
	for( var i = 0; i < grid_array.length; i++ ){
		EV_M_ITEM = GridObj.cells(grid_array[i], GridObj.getColIndexById("CODE")).getValue(); // 대분류코드
...
<s:grid screen_id="WO_201" grid_obj="GridObj" grid_box="gridbox"/>
<s:footer/>
