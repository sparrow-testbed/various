...
<%-- 		location.href = "ven_bd_con2.jsp?flag=update&vendor_code=<%=company_code%>&mode=<%="3".equals(SOLE_PROPRIETOR_FLAG) ? "resident_no" : "irs_no"%>&irs_no=<%=IRS_NO%>&popup=T&resident_no="+form1.resident_no.value --%>
		var tmp = "ven_bd_con2_j_ict.jsp?flag=update&vendor_code=<%=company_code%>&mode=<%="3".equals(SOLE_PROPRIETOR_FLAG) ? "resident_no" : "irs_no"%>&irs_no=<%=IRS_NO%>&popup=T&resident_no="+form1.resident_no.value;
// 		alert("tmp : " + tmp);
