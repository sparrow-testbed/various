var __iccardlist = function(__SANDBOX) {
...
XWC.getLocaleResource("iccardlist", XWC.lang(), "UTF8");

function onload(aDialogObj) {
    gDialogObj = aDialogObj;

	XWC.UI.setDragAndDrop(__2);
	    gICCardTableView = new XWC.UI.TableView(__6);

    gICCardTableView.onSelectRow = function(aObject) {
		var aSelectedRowElement = gICCardTableBody.rows[aObject.rowIndex-1],
			aSelectedRow = aSelectedRowElement.getAttribute("providerName"),
			i;
		
		for(i = 0; i < gICCardList.length; i++){
			if(aSelectedRow == gICCardList[i]){
    ...
    gICCardTableView.onRefresh = function(aData) {
		var providerInfo,
			i;

		gICCardTableBody = __6.getElementsByTagName("table")[0].tBodies[0];
		gICCardList = aData;
		...
		for (i = 0; i < aData.length-1; i++) {
			...
			td.appendChild(gICCardTableView.createImageTextCell(AnySign.mBasePath+"/img/cert_csp.png", providerInfo));
			
			tr.setAttribute("tabindex", 4, 0);
		    tr.appendChild(td);

		    gICCardTableBody.appendChild(tr);
		}
    }

	__SANDBOX.upInterface().getMediaList (XWC.CERT_LOCATION_ICCARD, 0, 1, refreshTableView);

	if (AnySign.mWBStyleApply)
	{
		var aButton = XWC.UI.createElement("a");
		var aParent = __16.parentNode.parentNode;
...
{
	gICCardTableView.refresh(aMediaList.split("\t\n"));

		var aTableRows = gICCardTableBody.getElementsByTagName('tr');
	if (aTableRows.length > 0) {
    	gICCardTableView.selectRow (aTableRows[0]);
	}
}
...
function onOKButtonClick() {
	var aTableRows = gICCardTableBody.getElementsByTagName('tr');
	if (aTableRows.length > 0) {
		gDialogObj.onconfirm(Number(gSelectedRowIndex));
	}
}
...
__4.className = 'xwup-iccardlist';
var __5 = XWC.UI.createElement('DIV');
__5.appendChild(document.createTextNode(XWC.S.iccard_select));
__4.appendChild(__5);
