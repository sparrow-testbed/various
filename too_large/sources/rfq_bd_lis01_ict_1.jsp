...
<%
	...
	multilang_id.addElement("I_SRQ_101");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "I_SRQ_101";
	...
	String WISEHUB_PROCESS_ID="I_SRQ_101";
...
function setRfqSubmit(){
	...
	var REDIRECT = "new";
    var url="/ict/s_kr/bidding/rfq/rfq_bd_ins01_ict.jsp?flag=Y&rfq_no=" + rfq_no + "&rfq_count=" + rfq_count + "&rfq_seq=" + rfq_seq + "&item_no=" + item_no;	
	
    document.location = url;
}

function setRfqGiveUp(){
	checked_count = 0;
	rowcount      = GridObj.GetRowCount();
	rfq_no        = "";
	rfq_count     = "";
	...
	for(row=rowcount-1; row>=0; row--){
		...
		if( GD_GetCellValueIndex(GridObj,row, IDX_RFQ_STATUS) != "B" ) { // 진행상태에 따라 견적 수정불가(작성중)
			alert("진행상태가 [견적중] 인 경우에만 포기가 가능합니다.");
	...
	var REDIRECT = "new";
    var url="/ict/s_kr/bidding/rfq/rfq_bd_ins01_ict.jsp?flag=N&rfq_no=" + rfq_no + "&rfq_count=" + rfq_count + "&rfq_seq=" + rfq_seq + "&item_no=" + item_no;	
...
<s:header>
	<form name="form1" action="">
		...
		<input type="hidden" name="rfq_status" id="rfq_status" value="B">
...
btn("javascript:doSelect()","조 회");
							...
							<TD>
<script language="javascript">
btn("javascript:setRfqSubmit()","견적서 (제출)");
</script>
							</TD>
							<TD>
<script language="javascript">
btn("javascript:setRfqGiveUp()","견적포기");
</script>
							</TD>
						</TR>
					</TABLE>
...
<%-- <s:grid screen_id="I_SRQ_101" grid_obj="GridObj" grid_box="gridbox"/> --%>
