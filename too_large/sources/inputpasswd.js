var __inputpasswd = function(__SANDBOX) {
...
inputpasswd_tk1 = null; var gDialogObj;
var gErrCallback;
var gDialogParam;
var gInputHandler;

XWC.getLocaleResource("inputpasswd", XWC.lang());
...
function onload(aDialogObj) {
	...
	var aDescLabel = XWC.S[gDialogParam.messageType].desc ;
	var aPasswdLabel = XWC.S[gDialogParam.messageType].passwd;

	XWC.UI.setDragAndDrop(__2);
	__2.firstChild.appendChild(document.createTextNode(XWC.S[gDialogParam.messageType].title));
	if (gDialogParam.descType == "copy" && AnySign.mEnhancedPW._copy == true) {
		__11.innerHTML = aDescLabel + XWC.S.info;
		__9.style.margin = "15px 15px 10px 15px";
	}
	else {
		__11.innerHTML = aDescLabel;
	}

	if (gDialogParam.descType == "pfx") {
		__18.style.display = "";
				__19.style.marginRight = "5px";
		__19.onclick = gDialogParam.func;
		if (gDialogParam.isSave == true) {
			__19.checked = true;
			__19.disabled = true;
		}
		
		if (__SANDBOX.browserName == "trident")
			__18.appendChild(document.createTextNode(" (Internet Explorer)"));
		else if (__SANDBOX.browserName == "edge")
			__18.appendChild(document.createTextNode(" (Edge)"));
		else if (__SANDBOX.browserName == "firefox")
			__18.appendChild(document.createTextNode(" (Firefox)"));
		else if (__SANDBOX.browserName == "chrome")
			__18.appendChild(document.createTextNode(" (Chrome)"));
		else if (__SANDBOX.browserName == "safari")
			__18.appendChild(document.createTextNode(" (Safari)"));
		else if (__SANDBOX.browserName == "opera")
			__18.appendChild(document.createTextNode(" (Opera)"));
	}

	__22.innerHTML = aPasswdLabel;
	
	if (gDialogParam.inputType == "lite") {
		gInputHandler = new __SANDBOX.inputKeyHandler ("inputpasswd", __23, 1, -190, 5, "qwerty_crt", 30, 120, "lite");
	} else if (gDialogParam.inputType == "xfs") {
        gInputHandler = new __SANDBOX.inputKeyHandler ("inputpasswd", __23, 1, -190, 5, "qwerty_crt", 30, 120, "e2e");
	} else {
		gInputHandler = new __SANDBOX.inputKeyHandler ("inputpasswd", __23, 1, -190, 5, "qwerty_crt", 30, 120);
	}
	...
	gInputHandler.onEnterKeyPress (__25);

	if (!(__SANDBOX.isIE() && navigator.userAgent.indexOf('Windows NT 6.2') >= 0)) {
		if (__SANDBOX.isIE() <= 7) {
			setCapsLockToolTip(__23, __13, 0, -30);
		}
		else {
			setCapsLockToolTip(__23, __13, 0, -20);
		}
	}
	...
	if (AnySign.mWBStyleApply)
	{
		...
		var aParent = __25.parentNode.parentNode;
		...
		checkbox.setAttribute('tabindex', '2', 0);
		if (__SANDBOX.IEVersion <= 8) {
				checkbox.mergeAttributes(XWC.UI.createElement("<INPUT name='inputpasswd_tek_check1'>"), false);
		} else {
				checkbox.setAttribute('name', 'inputpasswd_tek_check1', 0);
		}

		checkbox.setAttribute('id', 'xwup_inputpasswd_tek_check1', 0);
		...
		__23.parentNode.appendChild (label);
		__23.parentNode.firstChild.style.minWidth= "80px";
		__23.parentNode.firstChild.style.width = "80px";
		__23.style.width = "145px";
		...
		if (AnySign.mPlatform == AnySign.mPlatformList[0] ||AnySign.mPlatform == AnySign.mPlatformList[1] || (__SANDBOX.browserName == "opera" && __SANDBOX.browserVersion >= 12.0))
		{
			...
			inputpasswd_tk1.useTransKey = true;
		}
	}
...
function onOKButtonClick(e)
{
	...
	if (gInputHandler.getLength() == 0) {
		alert(XWC.S[gDialogParam.messageType].nokeyword);
		__25.disabled = false;
		return;
	}
	...
	if (gDialogParam.descType == "copy" && AnySign.mEnhancedPW._copy == true) {
		aLength = 10;
		if (gInputHandler.getLength() < 10) {
			gInputHandler.clear();
			alert(XWC.S[gDialogParam.messageType].nokeyword2);
			__25.disabled = false;
			return;
		}
	}

	_checkPasswordLenExt = function (aResult) {
	...
	{
		aKeyword = gInputHandler.getValue(result);
		if (aLength == 10) {
			__SANDBOX.upInterface().checkPasswordLenExt(aKeyword, aKeyword, 0, _checkPasswordLenExt);
		} else {
			gDialogObj.onconfirm(aKeyword);
		}
	}

	if (gDialogParam.messageType == "certificate2")
...
function onInputMouseCheckBoxClick(aEvent) {
	...
	aTransKey = inputpasswd_tk1;

	if (aChecked) {
		aTransKey.useTransKey = true;
		__23.readOnly = true;
		showTransKeyBtn("inputpasswd_tk1");
		aTransKey.clear();
	}
	else {
...
__9.className = 'xwup-degap';
var __10 = XWC.UI.createElement('IMG');
__10.setAttribute('src', AnySign.mBasePath+'/img/main_inputpasswd.png', 0);
__10.setAttribute('width', '56', 0);
__10.setAttribute('height', '40', 0);
__10.setAttribute('alt', XWC.S.common.passwd, 0);
...
__12.className = 'xwup-widget-sec';
...
__19.setAttribute('type', 'checkbox', 0);
__19.setAttribute('tabindex', '5', 0);
__18.appendChild(__19);
__18.appendChild(document.createTextNode(XWC.S.save));
__4.appendChild(__18);
var __20 = XWC.UI.createElement('FORM');
if (__SANDBOX.IEVersion <= 8) {
__20.mergeAttributes(XWC.UI.createElement("<FORM name='xwup_inputpasswd_tek_form'>"),false);
} else {
__20.setAttribute('name', 'xwup_inputpasswd_tek_form', 0);
}
__20.setAttribute('id', 'xwup_inputpasswd_tek_form', 0);
__20.className = 'xwup-inpwd-sec';
__20.onsubmit = function(event) {return false;};
var __21 = XWC.UI.createElement('DIV');
__21.className = 'xwup-password-field';
__21.setAttribute('id', 'inputpasswd_tk1', 0);
var __22 = XWC.UI.createElement('LABEL');
__22.htmlFor = 'xwup_inputpasswd_tek_input1';
__22.className = 'xwup-input-name';
__21.appendChild(__22);
var __23 = XWC.UI.createElement('INPUT');
__23.setAttribute('tabindex', '5', 0);
if (__SANDBOX.IEVersion <= 8) {
__23.mergeAttributes(XWC.UI.createElement("<INPUT name='inputpasswd_tek_input1'>"),false);
} else {
__23.setAttribute('name', 'inputpasswd_tek_input1', 0);
}
__23.setAttribute('id', 'xwup_inputpasswd_tek_input1', 0);
__23.setAttribute('type', 'password', 0);
__23.className = 'xwup-input-pwd';
__23.setAttribute('kbd', 'qwerty_crt', 0);
__21.appendChild(__23);
__20.appendChild(__21);
__4.appendChild(__20);
var __24 = XWC.UI.createElement('DIV');
__24.className = 'xwup-buttons-layout';
var __25 = XWC.UI.createElement('BUTTON');
__25.setAttribute('tabindex', '5', 0);
__25.setAttribute('type', 'button', 0);
__25.setAttribute('id', 'xwup_ok', 0);
__25.onclick = function(event) {onOKButtonClick(event)};
...
__26.setAttribute('id', 'xwup_cancel', 0);
