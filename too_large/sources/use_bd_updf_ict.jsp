...
<%
    ...
    String target       = "use_bd_upd2_ict.jsp"; // 사용자 리스트의 수정화면에서의 수정
	if (os_gb.equals("win10")) {
		target       = "use_bd_upd2_ict_window10.jsp"; // 사용자 리스트의 수정화면에서의 수정
	}
...
<frameset rows="100%,*" frameborder="NO" border="0" framespacing="0" cols="*"> 
	<frame name="body" noresize src="/ict/s_kr/admin/user/<%=target%>?flag=<%=flag%>&user_flag=<%=user_flag%>&user_id=<%=user_id%>">
