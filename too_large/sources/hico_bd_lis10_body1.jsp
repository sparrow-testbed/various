...
<input type="hidden" id="s_template_refitem" 	name="s_template_refitem" 	value="<%=s_template_refitem%>">
	...
	<tr>
		<td>
			...
			<input type="hidden" name="s_factor_refitem" value="<%=s_factor_ref%>"><!-- 문제번호-->
            <input type="hidden" name="answered_seq"><!--선택된 값이 담긴다.-->
            
			<% if( "checked".equals(chk) ) { %>
				<script>
				checkData(<%=seq%>, '1');
				</script>
			<% } %>
			  ...
			  <tr height=25>
			   	<td class="c_data_1">
			   	<img src="../../../images//supply/blank.gif" width="10" height="1"><input <%=dis %> type="radio" class="input_data1" name="<%=s_factor_ref%>" <%=chk%> onClick="javascript:checkData(<%=seq%>, '1');"><%=wf.getValue("ITEM_NAME", i)%>
			   	</td>
			<%
					for(int j=2; ; j++)
					{
		   ...
		   <% if( "checked".equals(chk) ) { %>
			...
			<% } %>
				<td>
				<img src="../../../images//supply/blank.gif" width="10" height="1"><input <%=dis %> type="radio" class="input_data1" name="<%=s_factor_ref%>" <%=chk%> onClick="javascript:checkData(<%=seq%>, <%=j%>);"><%=wf.getValue("ITEM_NAME", i)%>
				</td>
			<%
					}else{
						break;
		...
		<table border=0 cellpadding=0 cellspacing=0 width="100%">
			<%
				...
				int i=0;
					...
					int s_factor_ref = Integer.parseInt(wf.getValue("S_FACTOR_REFITEM", 0));
					factor_name= wf.getValue("FACTOR_NAME", 0);
					
					selected = Integer.parseInt(wf.getValue("SELECTED_SEQ", 0));
					if(selected == 1)
					{
						chk = "checked";
					}else
