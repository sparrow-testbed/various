...
<%
	...
	multilang_id.addElement("I_CT_001");
	multilang_id.addElement("I_CT_002");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "I_CT_001";
...
<% String WISEHUB_PROCESS_ID="I_CT_001";%>
...
function doSaveCT()
{
	...
	for(var i = 0; i < grid_array2.length; i++)
	{
		...
		if("" == LRTrim(GridObj2.cells(grid_array2[i], GridObj2.getColIndexById("VENDOR_NAME")).getValue()))
		{
			alert("계약업체를 선택하세요.");
			return;
		}
...
function GridObj2_doOnRowSelected(rowId,cellInd) {
	...
	if(header_name == "ANN_NO")
    {
		...
		var bid_no = LRTrim(GridObj2.cells(rowId, GridObj2.getColIndexById("BID_NO")).getValue());   
		var vendor_code = LRTrim(GridObj2.cells(rowId, GridObj2.getColIndexById("VENDOR_CODE")).getValue());   
        
		if( bid_no == "" || vendor_code == ""){
			var url  = '';
			var title  = '';
			var param  = '';
			//window.open("/common/CO_008_ict.jsp?callback=getCtrlUser", "SP9113", "toolbar=no, location=no, directories=no, status=yes,menubar=no,scrollbars=yes,resizable=no,width=450,height=550,left=0,top=0");		
...
function doSubmitCT() { 
	...
	*/
	...
	f.VENDOR_NAME.value       = LRTrim(VENDOR_NAME);
	...
	f.target                   = "_self"; 
	f.method                   = "post";   
	f.action = "ct_submit_insert_ict.jsp";
	f.submit();
}
...
<s:grid screen_id="I_CT_001" height="350px"/>
<TABLE width="100%" border="0" cellspacing="0" cellpadding="0">
	<TR>
		<TD height="10"></TD>
	</TR>
...
<TABLE width="100%" border="0" cellspacing="0" cellpadding="0">
			<TR>
			    <TD height="15" align="left">
					<TABLE cellpadding="0">
						<TR>
							<td><script language="javascript">btn("javascript:doSubmitCT()", "계약서 업로드")</script></td>		 							
						</TR>
					</TABLE>
				</TD>
				<TD height="15" align="right">
...
<s:grid screen_id="I_CT_002" grid_obj="GridObj2" grid_box="gridbox2" grid_cnt="2" height="300px"/>
</form>
</s:header>
