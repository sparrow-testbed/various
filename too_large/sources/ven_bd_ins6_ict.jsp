...
//info = new WiseInfo(info.getSession("HOUSE_CODE"),"ID=IF^@^LANGUAGE=KO^@^NAME_LOC=SUPPLY^@^NAME_ENG=SUPPLY^@^DEPT=ALL^@^");
	...
	multilang_id.addElement("I_SU_001");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "I_SU_001";
...
<!-- PROCESS ID 선언 -->
<% String WISEHUB_PROCESS_ID="I_SU_001";%>
<!-- 사용 언어 설정 -->
...
<%
	...
	String vendor_code  = JSPUtil.nullToEmpty(info.getSession("COMPANY_CODE"));
	String company_code = JSPUtil.nullToEmpty(info.getSession("COMPANY_CODE"));

	//if ( vendor_code.equals("") )
	if( !JSPUtil.nullToEmpty(request.getParameter("vendor_code")).equals("") )
	{
		vendor_code = JSPUtil.nullToEmpty(request.getParameter("vendor_code"));
		company_code = JSPUtil.nullToEmpty(request.getParameter("company_code"));
	}
...
<!--
var G_SERVLETURL         = "<%=POASRM_CONTEXT_NAME%>/servlets/ict.supply.admin.info.ven_bd_ins6_ict";
...
function setHeader()
{
	...
	INDEX_BIZ_RPS_YN       = GridObj.GetColHDIndex("BIZ_RPS_YN");
...
// 	GridObj.addRow();
	...
	if(row == 1){
		GridObj.cells(GridObj.getRowId(row-1), GridObj.getColIndexById("BIZ_RPS_YN")).cell.wasChanged = true;
		GridObj.cells(GridObj.getRowId(row-1), GridObj.getColIndexById("BIZ_RPS_YN")).setValue("1");		
	}else{
		GridObj.cells(GridObj.getRowId(row-1), GridObj.getColIndexById("BIZ_RPS_YN")).cell.wasChanged = false;
		GridObj.cells(GridObj.getRowId(row-1), GridObj.getColIndexById("BIZ_RPS_YN")).setValue("0");
	}
...
function setSave()
{
	...
	for(var i=0;i<GridObj.GetRowCount();i++)
	{
		...
		/*
		if(GD_GetCellValueIndex(GridObj,i,INDEX_FLAG)!="I")
		{
			GridObj.cells(i+1, GridObj.getColIndexById("SELECTED")).setValue(1);
			GridObj.cells(i+1, GridObj.getColIndexById("SELECTED")).cell.wasChanged = true;					
		}
		*/
		
		if(GD_GetCellValueIndex(GridObj,i,INDEX_BIZ_RPS_YN)==true)
		{
			BizRpsYRow++;	
		}		
	}
	...
	if(BizRpsYRow == 0){
		alert("영업대표여부를 한명 선택하여야 합니다.");
		return;
	}
	
	if(BizRpsYRow > 1){
		alert("영업대표여부는 한명만 선택가능합니다.");
		return;
	}
...
function BackPage()
{
	parent.up.MM_showHideLayers('m1','','hide','m2','','show','m3','','hide','m4','','hide','m5','','hide','m6','','hide','m11','','hide','m22','','hide','m33','','hide','m44','','hide','m55','','hide','m66','','hide');
	parent.up.goPage('gl');
}
function NextPage()
{
	...
	parent.up.MM_showHideLayers('m1','','show','m2','','show','m3','','hide','m4','','hide','m5','','hide','m6','','hide','m11','','hide','m22','','hide','m33','','show','m44','','hide','m55','','hide','m66','','hide');
	parent.up.goPage('pj');
}
//-->
</Script>
...
var myDataProcessor = null;
	...
	{
		/* var gg = getGridSelectedRows(GridObj, "BIZ_RPS_YN");
		if(gg !=0){
			
			for(var j = dhtmlx_start_row_id; j < dhtmlx_end_row_id; j++)
			{
				//GridObj.cells(j+1, GridObj.getColIndexById("SELECTED")).setValue("0");
				GridObj.cells(j+1, GridObj.getColIndexById("BIZ_RPS_YN")).setValue(0);
				GridObj.cells(j+1, GridObj.getColIndexById("BIZ_RPS_YN")).cell.wasChanged = false;
			}
		}
		
		GridObj.cells( rowId, GridObj.getColIndexById("BIZ_RPS_YN")).setValue(1);
		GridObj.cells( rowId, GridObj.getColIndexById("BIZ_RPS_YN")).cell.wasChanged = true;	  */
...
<%if (popup==null||"".equals(popup)||popup.equals("U")||popup.equals("T")) {%>
		※ 정확히 입력바랍니다. -> 행삽입 후 영업담당 정보를 입력하고 등록하십시오.<br/>	
		※ <font color='red'>영업대표여부</font> 체크된 사용자의 핸드폰이 <font color='red'>SMS 수신번호</font>가 됨 (영업대표는 1명만 가능)		
...
<%-- <s:grid screen_id="I_SU_001" grid_obj="GridObj" grid_box="gridbox"/> --%>
