...
<%
	...
	Object[] obj =
	{
	 ...
	 multilang_id.addElement("RT_002");
...
var	servletUrl = "<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.admin.bulletin_list_new";
...
// 		var dept_type = encodeUrl(LRTrim(document.form1.dept_type.value));
		var gongji_gubun = encodeUrl(LRTrim(document.form1.gongji_gubun.value));
		
		
		/* if(gongji_gubun == null || gongji_gubun == "") {
			alert("고객의소리 구분을 선택해주십시오.");
			document.form1.gongji_gubun.focus();
			return;
		} */
		...
		GridObj.post("<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.admin.bulletin_list_new","gubun="+gubun+"&quest="+quest+"&company_code="+company_code+"&gongji_gubun="+gongji_gubun+"&language="+language+"&mode=getSupRpt&grid_col_id="+grid_col_id);
	...
	function checkRows()
	{
		...
		alert("<%=text.get("RT_002.MSG_0101")%>");
		return false;
	}
	...
	function doUpdate()
	{
		...
		if(parseInt(sel_row) > 1) {
			//alert('한행만 선택하십시요.');
			alert("<%=text.get("RT_002.MSG_0102")%>");
			return;
		}

		var select_seq = LRTrim(GridObj.cells(cnt, GridObj.getColIndexById("SEQ")).getValue());

		location.href = "rpt_list_upd_new.jsp?select_seq="+select_seq;
	...
	function doDelete()
	{
	   ...
	   if (confirm("<%=text.get("RT_002.0001")%>")){
		    var cols_ids = "<%=grid_col_id%>";

		    myDataProcessor = new dataProcessor("<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.admin.bulletin_list_new?cols_ids="+cols_ids+"&mode=setRptDelete");
...
/* 	document.form1.method = "POST";
    ...
    document.form1.action = "../admin/bulletin_view_count_update.jsp";

    document.form1.submit(); */
    
	var jqa = new jQueryAjax();
	jqa.action = "bulletin_view_count_update.jsp";
	jqa.submit();
}

function popup(seq)
{
	var url = "../admin/rpt_list_popup_new.jsp?seq="+seq;

	CodeSearchCommon(url,'rpt_list_popup_new','0','0','700','600');
	doSelect();
}
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <%	} %>
				<tr>
		            ...
		            <td class="data_td">
			        <input type="text" name="quest" size="44">
			      </td>
			      <td width="15%" class="title_td"  >&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("RT_002.GONGJI_GUBUN")%></td>
	              <td width="35%" class="data_td">
		          	<select name="gongji_gubun" id="gongji_gubun">
					  <option value="">전체</option>
						<%
							String gongji_gubun = ListBox(request, "SL0018" , info.getSession("HOUSE_CODE")+"#"+"Z003", "X");
							out.println(gongji_gubun);
						%>
					</select>
			      </td>
			    </tr>
<%--
				...
				<tr>
		        	<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("RT_002.COMPANY_NAME")%>
		               </td>
		            <td class="data_td">
...
<!-- 				</tr>				    -->
<!-- 				<tr> -->
<%-- 		        	<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("RT_002.DEPT_TYPE")%> --%>
<!-- 		               </td> -->
<!-- 		            <td class="data_td"> -->
						...
						<TR>
							...
							<TABLE cellpadding="2" cellspacing="0">
							  <TR>
								...
								<td style="display:none;"><script language="javascript">btn("javascript:doUpdate()","<%=text.get("RT_002.button_update")%>")</script></td>
								<td style="display:none;"><script language="javascript">btn("javascript:doDelete()","<%=text.get("RT_002.button_delete")%>")</script></td>			                            	
			                   <%--  <% } %>		 --%>			
							  </TR>
						    </TABLE>
						  </td>
