...
<%
	...
	//HTML에 쓰일 변수들
	String TITLE = String.valueOf(text.get("RT_001.MSG_0104"));// "Q&A 작성";
	...
	if( "Y".equals(load_flag) ) {
		response.sendRedirect("rpt_list_new.jsp");
	}
%>
...
function Create(){
	...
	if (LRTrim(F.SUBJECT.value) == ""){
		//alert("제목은 필수 입력입니다.");
		alert("<%=text.get("RT_001.MSG_0100")%>");
	...
	if (LRTrim(F.CONTENT.value) == ""){
		//alert("내용을 입력하세요.");
		alert("<%=text.get("RT_001.MSG_5000")%>");
	...
	if(!confirm("정말 등록하시겠습니까?")){
 	...
 	F.method = "POST";
    F.target = "childFrame";
    F.action = "rpt_write_wk_new.jsp?con_type="+con_type;
    F.submit();
    
    <%--
 	var jqa = new jQueryAjax();
 	
	jqa.action = "rpt_write_wk_new.jsp";
...
function Cancel(){
    if(!confirm("<%=text.get("RT_001.MSG_0101")%>"))//취소 하시겠습니까?
    	return;
	var con_type = "<%=con_type%>";
    go_list('/admin/rpt_list_new.jsp', 'MUO141000008', 4, '');
    //topMenuClick('/admin/rpt_write_new.jsp', 'MUO141000008', '4', '');
}
			...
			<tr>
				<td width="100%" valign="top">
					<TABLE cellpadding="0" cellspacing="0" border="0" width="100%">
						<TR>
							<td><font color=red>당행 직원과 관련한 Clean계약 위반사례</font> 및<br>공사 및 구매관련 원가절감 등 상생정보(아이디어) 제공<br>단, 등록내용은 총무부장만 열람이 가능하며 비밀이 보장됩니다.</td>
											...
											<table width="100%" border="0" cellspacing="0" cellpadding="0">
												<tr>
													<td width="15%" class="title_td">
														&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("RT_001.MSG_0102")%>
													</td>
													<td class="data_td">
