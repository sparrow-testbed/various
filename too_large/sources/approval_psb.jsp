...
function Init()
{
	if(chk_page(opener.window.location.href) == "ap_wait_list.jsp" ){
		document.all["ctFrm2"].style.display = "block";
		document.all["ctFrm2"].style.width = "100%";
		document.all["ctFrm2"].style.height = "150px";
	}
	
}

function chk_page(url){
	var len = url.length;
	var last = url.lastIndexOf("/");
	return (url.substring(last+1,len)).toLowerCase();
}
...
function rMateFileAttach(att_mode, view_type, file_type, att_no) {
	...
	function btnApprovalAll(arg1,arg2,arg3,arg4){
		$.post(
			...
			function(arg){
				...
				if(result != "0" ){
					...
					opener.btnApprovalAll(arg1,arg2,arg3,arg4);
				}else if(result == "-1" ){ 
...
<body onload="Init();" bgcolor="#FFFFFF" topmargin="0" marginwidth="0"
	leftmargin="0">
	<form name="form1" method="post">
		<input type="hidden" name="CONTRACT_DEPOSIT"> <input
			type="hidden" name="MENGEL_DEPOSIT"> <input type="hidden"
			name="dp_remark">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				<td align="center" class="style1">자 본 예 산 지 급 품 의</td>
			</tr>
			<tr>
				<td height="20">&nbsp;</td>
			</tr>
		...
		<table width="98%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td width="40%" valign="top"><table width="90%" height="92"
						...
						<tr>
							<td class="title">
								요&nbsp;&nbsp;&nbsp;&nbsp;청&nbsp;&nbsp;&nbsp;&nbsp;자&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp;<%=add_user_name%></td>
						</tr>
						<tr>
							<td height="1" bgcolor="#000000"></td>
						</tr>
						<tr>
							<td class="title">자본예산지급명:&nbsp;&nbsp;&nbsp;<%=subject%></td>
						</tr>
						<tr>
							<td height="1" bgcolor="#000000"></td>
						</tr>
				...
				<td width="*" colspan="2" align="right">
					...
					<table border="0" cellspacing="1" cellpadding="0" bgcolor="#000000"
						align="right">
						<tr bgcolor="#FFFFFF">
							<td width="20" align="center" bgcolor="#CCCCCC" class="title1">결<br>재
							</td>
							<%
...
%>
    	...
    	%> 기 안<%--<img src="/images/<%=info.getSession("HOUSE_CODE")%>/stamp_2.gif" height="35" border="0" align="absmiddle">--%>
											<% }else if(APP_STATUS.equals("E")) { // 승인 %> 승 인<%--<img src="/images/<%=info.getSession("HOUSE_CODE")%>/stamp_1.gif" height="35" border="0" align="absmiddle">--%>
											<% }else if(APP_STATUS.equals("R")) { // 반려%> 반 려<%--<img src="/images/<%=info.getSession("HOUSE_CODE")%>/stamp_3.gif" height="35" border="0" align="absmiddle">--%>
											<% }else { %> &nbsp; <% } %>
										</td>
									</tr>
									<tr>
...
%>
						...
						<tr bgcolor="#FFFFFF" style="display: none">
							<td width="32" align="center" bgcolor="#CCCCCC" class="title1">합<br>의
							</td>
							<%
...
APP_STATUS    = "";
//////////////////ClipReport4 조립부 시작///////////////////////////////////////////////////////////
_rptAprvData.append("자 본 예 산 지 급 품 의");

_rptAprvData.append(_RD);	

_rptAprvData.append("지 급 번 호");
...
%>
				...
				<td height="30" align="right">
					<TABLE cellpadding="0">
						<TR>
							...
							<TD><script language="javascript">btn("javascript:btnApprovalAll('2','1','<%=doc_no%>','')","승 인")</script></TD>
							<TD><script language="javascript">btn("javascript:btnApprovalAll('3','1','<%=doc_no%>','')","반 려")</script></TD>
							<%-- 		      		<TD><script language="javascript">btn("javascript:changeApprovalLine('<%=doc_type%>','<%=doc_no%>','<%=doc_seq%>','<%=sign_path_seq%>')",21,"결재선추가")</script></TD> --%>
							<% } %>
							<TD><script language="javascript">btn("javascript:ctFrm.clipPrint('<%=_rptAprvData.toString().replaceAll("\"", "&quot")%>',<%=approvalCnt%>);","출 력")</script></TD>
	...
	<iframe id="ctFrm" name="ctFrm"
		src="/kr/tax/pay_bd_ins1_pop.jsp?pay_send_no=<%=doc_no %>&cont_gl_seq=<%=doc_seq %>&cont_form_no=<%=doc_no %>"
		style="width: 100%; height: 500px; border: 0px;" frameborder="0"
		scrolling="auto"></iframe>
	<%-- <script language="javascript">rMateFileAttach('S','R','PR',form1.attach_no.value);</script> --%>
	<iframe id="ctFrm2" name="ctFrm2"
		src="/kr/tax/pay_audit_list_pop.jsp?pay_send_no=<%=doc_no %>&cont_gl_seq=<%=doc_seq %>&cont_form_no=<%=doc_no %>"
		style="width: 100%; height: 180px; border: 0px; display: none;"
