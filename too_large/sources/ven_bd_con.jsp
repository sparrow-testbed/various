...
<%
	...
	String upframe = "";
	upframe   = "ven_bd_top.jsp";
	
	//if("win10".equals(rdoWin)){
		downframe = "ven_bd_ins2_window10.jsp";
	//}else{
	//	downframe = "ven_bd_ins2.jsp";
	//}
%>

</head>
<frameset rows="0,80,*" cols="*" border="0" framespacing="0" frameborder="NO">
<frame src="ven_bd_prt.jsp" name="prt" frameborder="NO">
<frame src="<%=upframe%>?mode=<%=mode%>&irs_no=<%=irs_no%>&resident_no=<%=resident_no%>&popup=<%=popup%>&vendor_code=<%=vendor_code%>&CompanyCode=<%=CompanyCode%>&status=<%=status%>" name="up" frameborder="NO">
