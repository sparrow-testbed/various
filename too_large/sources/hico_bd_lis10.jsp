...
<frameset rows="130,*,1" frameborder="NO" border="0" framespacing="0" cols="*">
  <frame name="lis10_top" scrolling="NO" noresize src="hico_bd_lis10_top.jsp?flag=popup&irs_no=<%=irs_no%>&status=<%=status%>&sg_refitem=<%=sg_refitem%>&vendor_code=<%=vendor_code%>&buyer_house_code=<%=buyer_house_code%>&popup=<%=popup%>">
