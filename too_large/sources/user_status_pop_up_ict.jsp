...
<%
	...
	if("".equals(i_user_id) == false){
		...
		SepoaOut value = ServiceConnector.doService(info, "I_AD_132", "CONNECTION","getDisplay", obj);
...
function checkData(){
	...
	if(!checkEmpty(f.user_name_loc, "<%=text.get("AD_134.MSG_0106")%>"))//사용자명(영문)을 입력해주십시요.
		return false;
...
%>
	...
	if(!checkKorea(f.language, "<%=text.get("AD_134.MSG_0118")%>",5))//사용언어는 5자 이내입니다.
		return false;

	if(!checkEmpty(f.language, "<%=text.get("AD_134.MSG_0119")%>"))//사용언어를 선택해주십시요.
		return false;
	...
	if(!checkKorea(f.country, "<%=text.get("AD_134.MSG_0123")%>",2))//국가는 2자 이내입니다.
		return false;

	if(!checkEmpty(f.country, "<%=text.get("AD_134.MSG_0124")%>"))//국가를 선택해주십시요.
		return false;
	...
	if(!checkKorea(f.address_eng, "<%=text.get("AD_134.MSG_0128")%>", 200))//주소(영문)은 200자 이내입니다.
		return false;
...
function Save(){
	...
	f.method = "POST";
	f.action = "user_status_pop_up_save_ict.jsp";
	f.submit();
...
function getprofile(){
	...
	<%}else if("MUP141000001".equals(info.getSession("MENU_PROFILE_CODE"))){%>
...
function searchProfile(part){
	...
	if(part == "city"){
		...
		if(!checkEmpty(f.country,"<%=text.get("AD_134.MSG_0140")%>"))
			return;
...
%>
						<TD>
<script language="javascript">
btn("javascript:Save()","<%=text.get("BUTTON.save")%>");	// 저장
</script>
						</TD>
						<TD>
<script language="javascript">
btn("javascript:parent.window.close()","<%=text.get("BUTTON.close")%>");	// 닫기
</script>
						</TD>
<%
	}
						...
						<td width="100%">
							<table width="100%" border="0" cellspacing="0" cellpadding="0">
								...
								<tr>
									...
									<td class="data_td">
										<input type="text" name="user_name_loc" id="user_name_loc" value="<%=user_name_loc%>" size="40" maxlength="40" class="input_re">
									</td>
									<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("AD_134.TEXT_0110")%></td>
									<td class="data_td">
										<input type="text" name="user_name_eng" id="user_name_eng" value="<%=user_name_eng%>" size="20" maxlength="40" class="input_re">
									</td>
								</tr>
								<tr>
									<td colspan="4" height="1" bgcolor="#dedede"></td>
								...
								<tr>
									<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("AD_134.TEXT_0111")%></td>
									<td class="data_td">
										<input type="text" name="text_company_code2" id="text_company_code2" size="20" value="<%=company_code%>" class="input_empty" readonly>
									</td>
									<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("AD_134.TEXT_0112")%></td>
									<td class="data_td">
										<input type="text" name="dept" id="dept" value="<%=dept%>" size="7" maxlength="10" class="input_re" readOnly>
										<a href="javascript:searchProfile('dept')">
											<img src="<%=POASRM_CONTEXT_NAME%>/images/button/query.gif" align="absmiddle" border="0" alt="">
										</a>
										<input type="text" name="text_dept" id="text_dept" value="<%=dept_name_loc%>" size="20" class="input_empty" readOnly>
									</td>
								</tr>
								<tr>
									<td colspan="4" height="1" bgcolor="#dedede"></td>
								...
								<tr  style="display: none;">
									<td  class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("AD_134.TEXT_0120")%></td>
									<td  class="data_td">
										...
										<input type="text" name=language_name id="language_name" value="<%=language_name%>" size="15" maxlength="15" class="input_empty" readOnly>
									</td>
									<td  class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("AD_134.TEXT_0124")%></td>
									<td  class="data_td">
										<input type="text" name="country" id="country" value="KR" size="5" class="input_re" readOnly>
										<a href="javascript:searchProfile('country')">
											<img src="<%=POASRM_CONTEXT_NAME%>/images/button/butt_query.gif" align="absmiddle" border="0" alt="">
										</a>
										<input type="text" name="country_name" id="country_name" value="<%=country_name%>" size="15" maxlength="15" class="input_empty" readOnly>
									</td>
								</tr>
								<tr  style="display: none;">
									<td colspan="4" height="1" bgcolor="#dedede"></td>
								...
								<tr  style="display: none;">
									<td  class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("AD_134.TEXT_0129")%></td>
									<td class="data_td" colspan="3">
										<input type="text" name="address_eng" id="address_eng" value="<%=address_eng%>" size="100" maxlength="200" class="inputsubmit">
									</td>
								</tr>
<%
