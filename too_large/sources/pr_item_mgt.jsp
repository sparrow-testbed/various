...
<%
	...
	HashMap text               = null;
	String  screen_id          = "PR_003_2";
	String  grid_obj           = "GridObj";
	String  WISEHUB_PROCESS_ID = "PR_003_2";
	String  isSingle           = request.getParameter("isSingle");
	boolean isSelectScreen     = false;
	
	multilang_id.addElement("PR_003_2");
...
function setHeader(){
	...
	INDEX_EXCEL_QTY = GridObj.GetColHDIndex("EXCEL_QTY") ;//액셀로 저장한 수량
	INDEX_EXCEL_RD_DATE = GridObj.GetColHDIndex("EXCEL_RD_DATE") ;//액셀로 저장한 납기요청일
	INDEX_EXCEL_VENDOR_CODE = GridObj.GetColHDIndex("EXCEL_VENDOR_CODE") ;//액셀로 저장한 업체코드
	INDEX_EXCEL_DELY_TO_ADDRESS = GridObj.GetColHDIndex("EXCEL_DELY_TO_ADDRESS") ;//액셀로 저장한 납품주소
	INDEX_EXCEL_DELY_TO_ADDRESS_CD = GridObj.GetColHDIndex("EXCEL_DELY_TO_ADDRESS_CD") ;//액셀로 저장한 납품주소코드
}
...
function doSelect(){
	...
	*/
	...
	var params = "mode=getExcelCatalog";
...
function doInsert(){
	...
	for(var i = 0; i < grid_array.length; i++) {
    	...
    	var description_loc          = GD_GetCellValueIndex(GridObj, rowIndex, INDEX_DESCRIPTION_LOC);
    	var excel_dely_to_address    = GD_GetCellValueIndex(GridObj, rowIndex, INDEX_EXCEL_DELY_TO_ADDRESS);
    	var excel_dely_to_address_cd = GD_GetCellValueIndex(GridObj, rowIndex, INDEX_EXCEL_DELY_TO_ADDRESS_CD);
    	var excel_rd_date            = GD_GetCellValueIndex(GridObj, rowIndex, INDEX_EXCEL_RD_DATE);
    	
    	if( item_no == null || item_no == "" ) {
    		alert("품목코드가 없으면 처리할 수 없습니다.");
    		return;
    	}
    	
    	if( description_loc == null || description_loc == "" ) {
    		alert("품목명이 없으면 처리할 수 없습니다.");
    		return;
    	}
    	
        parent.opener.setExcelCatalog(
		item_no,
		description_loc,
		...
		GD_GetCellValueIndex(GridObj, rowIndex, INDEX_EXCEL_QTY),
		...
		excel_dely_to_address,//액셀로 저장한 납품주소
		excel_dely_to_address_cd,//액셀로 저장한 납품주소코드
		GD_GetCellValueIndex(GridObj, rowIndex, INDEX_IMAGE_FILE_PATH),
		excel_rd_date//액셀로 저장한 납기요청일
		);
...
function doTemplate()
{
	location.href = "/template/Excel_Template_01.xls";
}
...
function doUpload(){
    ...
    doQueryDuring();
    document.form1.action = "pr_item_mgt_excel_upload.jsp?transaction_id=<%=transaction_id%>";
		...
		<table width="99%" border="0" cellspacing="0" cellpadding="0" >
			<tr>
				<td class='title_page' height="20" align="left" valign="bottom">
					<span class='location_end'>품목 액셀업로드</span>
				</td>
			</tr>
		</table>
...
<%-- <s:grid screen_id="PR_003_2" grid_obj="GridObj" grid_box="gridbox"/> --%>
<s:footer/>
</body>
</html>