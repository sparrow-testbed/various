...
/*    this.postGrid = function(url, params) {
    ...
    this.postGrid = function(url, params) {
        if(params.indexOf("grid_col_id") == -1) {
            params += "&grid_col_id=" + this.getGridColIds();
        }
        //alert(url);
        //alert(params);
        this.post(url, params);
    ...
    this.params = "";
    this.doData = "";
    this.SetDateFormat = function(colId,format){	//wise에서 변환하는 과정에서
    	this.setDateFormat(colId,format);	
    };
    this.SetParam = function(key, value) {    	
    	if(typeof value == "undefined"){
    		//wisegrid에서는 undefined된 데이터라면 자동으로 null을 보내지만, 일반 javascript는 그렇지 않다.
    		//null로 해주지 않으면 value에 undefined라는 문자열이 들어간다.
    		value = null;	
    		
    	}
    	if(key == "WISETABLE_DOQUERY_DODATA") {
    		this.doData = value;
    	}
    ...
    this.SendData = function(G_SERVLETURL) {
    	this.SetParam("grid_type", "dhtmlx");
    	
    	if(this.doData == "0") {
	    	this.postGrid(G_SERVLETURL, this.params);
	    	this.clearAll(false);
    	} else if(this.doData == "1") {

	        if(this.params.indexOf("grid_col_id") == -1) {
	            this.params += "&grid_col_id=" + this.getGridColIds();
	        }
			var grid_array = getGridChangedRows(this, "SELECTED");
		    var myDataProcessor = new dataProcessor(G_SERVLETURL, this.params);
		    sendTransactionGridPost(this, myDataProcessor, "SELECTED", grid_array);
    		
    	} else {
    		alert("WISETABLE_DOQUERY_DODATA is not declared.");
    	} 
	    this.params = "";
	    this.doData = "";
    };
    
    this.DoQuery = function(url) {
    	this.SetParam("WISETABLE_DOQUERY_DODATA", 0);
    	this.SendData(url);
    }
    ...
    this.AddRow = function() {
		...
		this.addRow(newId, "");
    	this.cells(newId, this.getColIndexById("SELECTED")).cell.wasChanged = true;
		this.cells(newId, this.getColIndexById("SELECTED")).setValue("1");
    };
    
    this.imageListKey = new Array();
    this.imageListValue = new Array();

    /**
	 * document.WiseGrid.AddImageList("SIGN_PATH","/kr/images/button/query.gif");	
	 */
    this.AddImageList = function(key, value) {
    	this.imageListKey.push(key);
    	this.imageListValue.push(value);
    };

    /**
	 * document.WiseGrid.SetCellImage("SIGN_PATH",document.WiseGrid.GetRowCount()-1,0);
	 */
    this.SetCellImage = function(key, rowIdx, imageIdx) {
    	var image = "";
    	for(var i = 0; i < this.imageListKey.length; i++) {
    		if(this.imageListKey[i] == key) {
    			image = this.imageListValue[i];
    			break;
    		}
    	}
    	if(image != "") {
    		this.cells2(rowIdx, this.getColIndexById(key+"_IMG")).setValue(image);
    	}
    };

    /**
	 * document.WiseGrid.SetCellValueIndex(INDEX_SIGN_PATH,document.WiseGrid.GetRowCount()-1,'');    
	 */
    this.SetCellValueIndex = function(colIdx, rowIdx, val) {
		this.cells2(rowIdx, colIdx).setValue(val);
    };

    /**
	 * document.WiseGrid.GetColHDIndex("SELECTED");
	 */
    this.GetColHDIndex = function(colId) {
    	return this.getColIndexById(colId);
    };

    this.GetColHDKey= function(colIdx) {
    	return this.getColumnId(colIdx);
    };
    
    this.GetCellValue = function(colIdx, row) {
    	//colIdx가 문자로 들어와서 숫자, 즉 컬럼 번호로 변경해준다.
    	var colNum = this.getColIndexById(colIdx);
    	return this.cells2(row, colNum).getValue();
    };
   
    // WiseGrid의 헤더클릭시 이벤트 선택. 이용할 일은 없으나 오류방지용.
    this.strHDClickAction = "";
	...
	//WOORI은행에서 추가함.(wise
	this.GetCellValueIndex=function (colId,rowId){
		//wise는 행 데이터가 0부터 시작한다.
		//dthmlx는 행데이터가 1부터 시작하므로 wise에서 전달되는 row값에 +1 한다. 
		return GridObj.cells(rowId+1, colId).getValue();
	};
...
/**
 * document.WiseGrid,document.WiseGrid.GetRowCount()-1,INDEX_SIGN_PATH_NAME
 */
function GD_GetCellValueIndex(gridObj, rowNum, colIdx) {
	// colIdx는 숫자여야 한다.
	return gridObj.cells2(rowNum, colIdx).getValue();
}
function SepoaGridGetCellValueIndex(gridObj, rowNum, colIdx) {
	return GD_GetCellValueIndex(gridObj, rowNum, colIdx);
}
/**
 * 		GD_SetCellValueIndex(document.WiseGrid,document.WiseGrid.GetRowCount()-1, INDEX_SELECTED, 	"true&", 						"&"); //check_box true 로 setting
 */
function GD_SetCellValueIndex(gridObj, rowIdx, colIdx, val, val2) {
	val = val.split(val2).join();
	var type = gridObj.getColType(colIdx);
	if( type == "ch" ) {
		if(val == "true" ) {
	    	gridObj.cells2(rowIdx, colIdx).cell.wasChanged = true;
			gridObj.cells2(rowIdx, colIdx).setValue("1");
		} else {
			// 이부분은 좀 생각해야 한다.
	    	gridObj.cells2(rowIdx, colIdx).cell.wasChanged = false;
			gridObj.cells2(rowIdx, colIdx).setValue("0");
		}
	} else {
		gridObj.cells2(rowIdx, colIdx).setValue(val);
	}
}
function SepoaGridSetCellValueIndex(gridObj, rowIdx, colIdx, val, val2) {
	return GD_SetCellValueIndex(gridObj, rowIdx, colIdx, val, val2);
}

/**
var row_idx = checkedOneRow(INDEX_SELECTED);
*/
function checkedOneRow(colIdx){
	
	var grid_array = getGridChangedRows(this, this.getColumnId(colIdx));
	var checked_count = grid_array.length;
   if(checked_count == 0)  {
        alert(G_MSS1_SELECT);
        return -1;
    }
...
//            this.enableItemEditor(true);
	...
	var that = this;
	this.setSkin('dhx_skyblue');
	this.setImagePath(_POASRM_CONTEXT_NAME+"/dhtmlx/dhtmlx_full_version/imgs/csh_bluefolders/");
