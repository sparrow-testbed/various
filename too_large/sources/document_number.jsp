...
<%
	...
	multilang_id.addElement("AD_100");
...
var strToday	= '<%= to_day %>';
	...
	function doQuery()
	{
		...
		var i_change_user_id = encodeUrl(LRTrim(document.forms[0].i_change_user_id.value));

		var grid_col_id = "<%=grid_col_id%>";

		var param = "i_change_user_id="+i_change_user_id+"&mode=query&grid_col_id="+grid_col_id;
		//alert(param);
		var G_SERVLETURL = "<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.admin.document_number";
    ...
    function doInsert()
	{
		...
		{
			<%--신규생성건만 등록하실 수 있습니다.--%>
			if(LRTrim(GridObj.cells(grid_array[i], GridObj.getColIndexById("flag")).getValue()) == "Y")
			{
				alert("<%=text.get("AD_100.1003")%>");
				return;
			}
			<%--문서코드가 선택되지 않았습니다.--%>
			if(LRTrim(GridObj.cells(grid_array[i], GridObj.getColIndexById("description")).getValue()) == "Y")
			{
				alert("<%=text.get("AD_100.1003")%>");
				return;
			}
	    ...
	    if (confirm("<%=text.get("MESSAGE.1018")%>")){
            ...
            G_SERVLETURL = "<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.admin.document_number";//서블릿 경로
...
//alert("rowId=="+rowId + "cellInd=="+cellInd);
		...
		if(cellInd == GridObj.getColIndexById("doc_type") ){

			if( GridObj.cells(rowId, GridObj.getColIndexById("doc_type")).getValue() == "" ){
				popupcode("M999","setDocType");
				setrow=rowId;
			}
			else
			{
				var i_doc_type = GridObj.cells(rowId, GridObj.getColIndexById("doc_type")).getValue();

				var url = "document_number_popup.jsp?popup_flag=true&i_doc_type=" +i_doc_type;
				var left = 50;
				var top = 100;
				var width = 750;
				var height = 500;
	...
	function setDocType(text,value,arg1,arg2){

		GridObj.cells(setrow, GridObj.getColIndexById("doc_type")).setValue(text);
		GridObj.cells(setrow, GridObj.getColIndexById("description")).setValue(value);
	}
	...
	function doDelete()
	{
		...
		{
			<%--우선 조회를 하세요--%>
			if(LRTrim(GridObj.cells(grid_array[i], GridObj.getColIndexById("flag")).getValue()) == "N")
			{
				alert("<%=text.get("AD_100.1001")%>");
				return;
			}
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#DBDBDB">
					<tr>
						<td width="15%" height="24" class="title_td"><img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;
							<%=text.get("AD_100.add_user_id")%> <!-- 주메뉴코드 -->
						</td>
						<td width="85%" height="24" class="data_td">
							<input type="text" size="15" class="inputsubmit" name="i_change_user_id">
							<input type="text" name="i_change_user_id_hidden" style = "display:none">
						</td>
