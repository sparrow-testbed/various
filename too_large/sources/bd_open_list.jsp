...
<%
	...
	multilang_id.addElement("BD_014");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "BD_014";
...
%>
<!--  /home/user/wisehub/wisehub_package/myserver/V1.0.0/wisedoc/kr/dt/bidd/ebd_bd_lis13.jsp -->
<!--
Title:         개찰  <p>
 Description:  개찰<p>
		...
		if(checkEval()){
			...
			if(confirm(Message) == 1) {
				...
				tech_dq 	= parseFloat(GD_GetCellValueIndex(GridObj,row_idx, INDEX_TECH_DQ) == "" ? "0" : GD_GetCellValueIndex(GridObj,row_idx, INDEX_TECH_DQ));
			 	...
			 	if(tech_dq == 0){	// 입찰공고시 협상에 의한 입찰이면서 기술제안서 점수비율
					alert("입찰공고시 기술제안서 점수비율이 없습니다.");
					return;
				}			 	
...
var G_SERVLETURL   = "<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.sourcing.bd_open_list";
...
<%--         var url =  "<%=POASRM_CONTEXT_NAME%>/sourcing/bd_ann_detail.jsp"; --%>
        ...
        var url = "bd_pp_dis.jsp?BID_NO=" + bid_no + "&BID_COUNT=" + bid_count + "&VOTE_COUNT=" + vote_count + "&BID_OPEN_DATE_VALUE=" + open_date_time;
...
function doBidProgress() {
	...
	*/
	...
	if(Number(h_server_date) < Number(BID_OPEN_DATE.replaceAll("/", "").replaceAll(":", "").replaceAll(" ", ""))){
    ...
    <tr>
 	...
 	<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<TR style="display:none;">
				<td width="3%">&nbsp;</td>
				<td width="20%"> <img src="/images/<%=info.getSession("HOUSE_CODE")%>/ibk_s_arr.gif" width="9" height="9"> 평가대상</td>
				<td > 
				...
				<td width="25%"> <script language="javascript">btn("javascript:doEval()","등록")</script></td>
			</tr>						
						
						
				      		<TR>
							 <TD> <script language="javascript">btn("javascript:doEvalProp()", "제안평가등록")</script></td>
			    	  		</TR>
		      			</TABLE>
		      		</td>
...
<s:grid screen_id="BD_014" grid_obj="GridObj" grid_box="gridbox"/>
<s:footer/> 
</body>
</html>
