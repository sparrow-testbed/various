...
function Query(type){
 ...
 if(type == "close")
 {
	...
	var param = "mode=getWaitList&grid_col_id="+grid_col_id;
	    param += dataOutput();
	var url = "<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.approval.ap_wait_list";
...
/* 결재 버튼을 눌렀을때.. (선택된 문서의 번호, 타입, 차수는 jsp에서 파라미터로 넘겨준다.)*/
function EndApp(){
...
function EndApp3()
{
	...
	var url = "/approval/ap_approval.jsp?app_type="+app_type;
...
// 			winpopup = window.open(vm_url ,"doExplanationMvd","toolbar=no, location=no, directories=no, status=no,menubar=no,scrollbars=yes,resizable=yes,width=800,height=748,left=0,top=0");			
			...
			var default_url = "approval_bid.jsp?doc_no="+doc_no+"&doc_seq="+doc_seq+"&doc_type="+doc_type+"&sign_enable=T"+"&attach_no="+attach_no+"&doc_status="+doc_status+"&proceeding_flag="+proceeding_flag+"&doc_seq="+doc_seq+"&sign_path_seq="+sign_path_seq+"&add_user_name="+add_user_name+"&add_date="+add_date+"&subject="+subject+"&view_content=Y";
  ...
  <tr>
			...
			<TABLE cellpadding="0">
	      		<TR>
    	  			...
    	  			<td><script language="javascript">btn("javascript:doConfirm()","결 재")</script></td>
    	  			<!--
					<TD><script language="javascript">btn("javascript:EndApp()",13,"결 재")</script></TD>
...
<%-- <s:grid screen_id="AP_001" grid_obj="GridObj" grid_box="gridbox"/> --%>
