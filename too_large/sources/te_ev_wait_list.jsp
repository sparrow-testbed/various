...
<%
	...
	multilang_id.addElement("EV_008");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "EV_008";
...
// 		GridObj.SetNumberFormat("PAY_AMT"		,G_format_amt);
	...
	function doSelect()
	{
		...
		var param = "mode=getEtEvWaitList&grid_col_id="+grid_col_id;
		param += dataOutput();
...
var myDataProcessor = null;
	...
	function setGridDraw()
    {
    	...
    	GridObj.attachHeader("#rspan,업체코드,업체명,공종,공종,공종,공종,등급,등급,#rspan,선택,평점,평가자,평가일,#rspan,계획번호,항번,계획명,코드,버전,평가표명");    	
...
function doOnRowSelected(rowId,cellInd)
{
	...
	var et_no = SepoaGridGetCellValueId(GridObj, rowId, "ET_NO");   
	
	if( header_name == "ET_NO_SEL" && et_no == "") {
		var url = "/kr/ev/te_ev_ins.jsp";
		var param = "";
		param += "?popup_flag_header=true";
		param += "&etpl_no="+SepoaGridGetCellValueId(GridObj, rowId, "ETPL_NO");
		param += "&etpl_seq="+SepoaGridGetCellValueId(GridObj, rowId, "ETPL_SEQ");
		param += "&vendor_code="+SepoaGridGetCellValueId(GridObj, rowId, "VENDOR_CODE");
		param += "&rowId="+rowId;		
		PopupGeneral(url+param, "평가상세", "", "", "925", "800");
	}else if( (header_name == "ET_NO" || header_name == "ASC_SUM") && et_no != "") {
		...
		PopupGeneral(url+param, "기술평가상세", "", "", "925", "800");
...
function  ET_NO_callback(pRowId) {
	GridObj.cells(pRowId, GridObj.getColIndexById("ET_NO_SEL")).setValue("<%=POASRM_CONTEXT_NAME%>/images/icon/icon.gif");			
	GridObj.cells(pRowId, GridObj.getColIndexById("ET_NO")).setValue("");
	GridObj.cells(pRowId, GridObj.getColIndexById("ASC_SUM")).setValue("");
	GridObj.cells(pRowId, GridObj.getColIndexById("EVAL_DATE")).setValue("");
	GridObj.cells(pRowId, GridObj.getColIndexById("EVAL_USER_ID")).setValue("");
	GridObj.cells(pRowId, GridObj.getColIndexById("EVAL_USER_NAME_LOC")).setValue("");		
}
...
function eval_yy_Changed(){
	...
	var param = "<%=info.getSession("HOUSE_CODE")%>#<%=info.getSession("ID")%>#" + form1.eval_yy.value;
	doRequestUsingPOST('SL0200', param, 'etpl_no', '');
}
...
function etpl_no_Changed(){
	...
	var param = "<%=info.getSession("HOUSE_CODE")%>#<%=info.getSession("ID")%>#" + form1.eval_yy.value + "#" + form1.etpl_no.value;
	doRequestUsingPOST('SL0201', param, 'group1_code', '');
}
...
function group1_code_Changed(){
	...
	var param = "<%=info.getSession("HOUSE_CODE")%>#<%=info.getSession("ID")%>#" + form1.eval_yy.value + "#" + form1.etpl_no.value + "#" + form1.group1_code.value;
	doRequestUsingPOST('SL0202', param, 'group2_code', '');
}
</script>
</head>
...
<form name="form1" >
	...
	<%
		String listbox1 = ListBox(request, "SL0199", info.getSession("HOUSE_CODE") + "#" + info.getSession("ID"), "");
...
<s:grid screen_id="EV_008" grid_obj="GridObj" grid_box="gridbox"/>
<s:footer/>
</body>
</html>
