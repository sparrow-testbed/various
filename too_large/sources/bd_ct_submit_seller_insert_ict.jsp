...
<%
	...
	Object[]    obj = { mapData };
	SepoaOut    so  = ServiceConnector.doService( info, "I_SBD_011", "CONNECTION", "getBDHeader", obj );
	SepoaFormater wf = new SepoaFormater( so.result[0] );
        ...
        if(!VOTE_COUNT.equals("1")) { // 재입찰한 경우에는, 해당 업체가 전 차수에 투찰한 업체만 업로드 하게끔 한다.
            wf = new SepoaFormater(so.result[1]);

            loading_flag             = wf.getValue("CNT"     ,0);
        }
        so  = ServiceConnector.doService( info, "I_SBD_011", "CONNECTION", "getBDHD_VnInfo", obj );
...
var G_SERVLETURL   = "<%=POASRM_CONTEXT_NAME%>/servlets/ict.sepoa.svl.sourcing.bd_ct_submit_seller_ict";

function init() {
<%--     if("<%=loading_flag%>" == "N") { --%>
//         alert("이전에 입찰서를 업로드하지 않은 업체는 가격입찰을 진행 하실수 없습니다.");
//         history.back(-1);
//     }
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
      <td class="title_td" width="15%">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;계약서류 첨부<img src='/images/mail_point.gif'' border='0'></td>
  ...
  <table width="98%" border="0" cellspacing="0" cellpadding="0">
    <tr> 
      <td width="98%" height="30">
        ...
        <table><tr>
         <% if("S".equals(MODE)){ %>
          <td><script language="javascript">btn("javascript:doSubmit()", "계약서류 업로드")</script></td>
         <% }else if("C".equals(MODE)){ %>          
          <td><script language="javascript">btn("javascript:doCancel()", "계약서류 업로드 취소")</script></td>
          <% } %>         
		  </tr></table>  
        </div>
      </td>
