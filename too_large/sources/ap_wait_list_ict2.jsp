...
<html>
	<head>
<title><%=text.get("MESSAGE.MSG_9999")%>1</title> <%-- 우리은행 전자구매시스템 --%>
...
function Query(type){
 ...
 if(type == "close")
 {
	...
	var param = "mode=getWaitList&grid_col_id="+grid_col_id;
	    param += dataOutput();
	var url = "<%=POASRM_CONTEXT_NAME%>/servlets/ict.sepoa.svl.approval.ap_wait_list_ict";
...
function NextApp1(next_sign_user_id) {
	...
	winobj = window.open("ap_approval.jsp?app_type="+app_type+"&app_person="+app_person, "pop_up1", "status=yes, resizable=yes width=500 height=480");
	winobj.focus();
...
function EndApp(){
    ...
    var tax_cnt = 0;
    var chkCht = getGridSelectedRows(GridObj, "SELECTED");
	for(var i=0; i<chkCht.length;i++){
		
		if(chkCht!=0) {
			next_sign_user_id = GridObj.cells(chkCht[i], GridObj.getColIndexById("NEXT_SIGN_USER_ID")).getValue();
			
			if (next_sign_user_id == "") //차기 결재자가 없는것 = 결재 완료 인것
			{
				if(temp_type_count == 0) {
					temp_type = GD_GetCellValueIndex(GridObj,chkCht[i]-1,INDEX_DOC_TYPE);
					temp_type_count = 1;
				} else if(temp_type != GD_GetCellValueIndex(GridObj,chkCht[i]-1,INDEX_DOC_TYPE)){
					alert("문서형태가 같아야합니다.");
					return;
				}
				if(GD_GetCellValueIndex(GridObj,chkCht[i]-1,INDEX_APP_STATUS) != 'P') {
					alert("문서의 결재상태를 확인해주세요");
					return;
				}
				if(GD_GetCellValueIndex(GridObj,chkCht[i]-1,INDEX_PERSON_APP_STATUS) == '완료') {
					alert("이미 결재하신건입니다.");
					return;
				}
				...
				checkedrowcount++;
				if(GD_GetCellValueIndex(GridObj,chkCht[i]-1,INDEX_STRATEGY_TYPE)== "" )
				    strategy += "";
				else
				    strategy += GD_GetCellValueIndex(GridObj,chkCht[i]-1,INDEX_STRATEGY_TYPE) + " $";

				doc_no += GD_GetCellValueIndex(GridObj,chkCht[i]-1,INDEX_DOC_NO) + " $";
				doc_type += GD_GetCellValueIndex(GridObj,chkCht[i]-1,INDEX_DOC_TYPE) + " $";
				text_doc_type += GD_GetCellValueIndex(GridObj,chkCht[i]-1,INDEX_DOC_TYPE_TEXT ) + " $";
				doc_seq += GD_GetCellValueIndex(GridObj,chkCht[i]-1,INDEX_DOC_SEQ) + " $";

				if(GD_GetCellValueIndex(GridObj,chkCht[i]-1,INDEX_SHIPPER_TYPE) == "")
					shipper_type += "X" + " $";
				else
					shipper_type += GD_GetCellValueIndex(GridObj,chkCht[i]-1,INDEX_SHIPPER_TYPE) + " $";
				company_code += GD_GetCellValueIndex(GridObj,chkCht[i]-1,INDEX_COMPANY_CODE) + " $";

				//세금계산서의 경우 한건씩만 결재가능토록한다.
				if(GD_GetCellValueIndex(GridObj,chkCht[i]-1,INDEX_DOC_TYPE) == 'TAX') {
					tax_cnt++;
				}
            }
		}
...
function EndApp3()
{
	...
	var url = "/ict/approval/ap_approval_ict2.jsp?app_type="+app_type;
	CodeSearchCommon(url,'pop_up2','400','250','400','250');
}
...
function RefundApp()
{
	...
	var url = "/ict/approval/ap_approval_ict2.jsp?app_type="+app_type
	CodeSearchCommon(url,'pop_up3','400','250','400','250');	
}
...
function btnApprovalAll(div,divCnt,doc_no,doc_seq){
    ...
    chkCht=getGridSelectedRows(GridObj, "SELECTED");
...
function EndApp21(){
   ...
   /*  if (tax_cnt > 1)
	...
	var app_type = "E";   /* 결재 타입: 결재,차기 결재  */
	var url = "/approval/ap_approval_pay.jsp?app_type="+app_type;
	CodeSearchCommon(url,'pop_up2','400','250','400','250');
...
//     		FileAttach('NOT',GridObj.cells(rowId, GridObj.getColIndexById("ATTACH_NO")).getValue(),'VI', false);
    ...
    if(header_name=="SIGN_PATH_IMG"){
		...
		var url = "/ict/approval/ap_user_list_ict2.jsp?company_code="+company_code+"&doc_type="+doc_type+"&doc_no="+doc_no+"&doc_seq="+doc_seq;
		CodeSearchCommon(url,'BKWin','550','800','650','400');
    }
	...
	if(header_name == "DOC_NO") {
		...
		if(doc_type == 'PR'){
			var pr_url = "/kr/dt/app/pr1_bd_dis4_approval.jsp?pr_no="+doc_no+"&doc_type="+doc_type+"&sign_enable=T"+"&attach_no="+attach_no+"&doc_status="+doc_status+"&proceeding_flag="+proceeding_flag+"&doc_seq="+doc_seq+"&sign_path_seq="+sign_path_seq;
			winpopup = window.open(pr_url ,"doExplanationM","toolbar=no, location=no, directories=no, status=no,menubar=no,scrollbars=yes,resizable=yes,width=800,height=748,left=0,top=0");
		}
		
		else if(doc_type == 'VM'){//공급업체 승인 결재는 사용하지 않음
			return;
// 			var vm_url = "/kr/master/vendor/sta_bd_dis1_approval.jsp?vendor_code="+doc_no+"&sign_enable=T&doc_seq="+doc_seq+"&sign_path_seq="+sign_path_seq+"&doc_status="+doc_status+"&proceeding_flag="+proceeding_flag;
// 			winpopup = window.open(vm_url ,"doExplanationMvd","toolbar=no, location=no, directories=no, status=no,menubar=no,scrollbars=yes,resizable=yes,width=800,height=748,left=0,top=0");			
		}
		
		else if(doc_type == 'RA'){
			var ra_url = "/kr/dt/rat/rat_pp_dis1.jsp?doc_no="+doc_no+"&doc_status="+doc_status+"&doc_seq="+doc_seq+"&doc_type="+doc_type+"&sign_path_seq="+sign_path_seq+"&proceeding_flag="+proceeding_flag;
			winpopup = window.open(ra_url ,"doExplanationM","toolbar=no, location=no, directories=no, status=no,menubar=no,scrollbars=yes,resizable=yes,width=800,height=748,left=0,top=0");
		}
		
		else if(doc_type == 'INV'){
			var inv_url = "/kr/order/ivdp/inv1_bd_dis1_approval.jsp?inv_no="+doc_no+"&doc_type="+doc_type+"&sign_enable=T"+"&attach_no="+attach_no+"&doc_status="+doc_status+"&add_user_name="+add_user_name+"&add_date="+add_date+"&subject="+subject+"&proceeding_flag="+proceeding_flag+"&doc_seq="+doc_seq+"&sign_path_seq="+sign_path_seq;
			winpopup = window.open(inv_url ,"doExplanationM6","toolbar=no, location=no, directories=no, status=no,menubar=no,scrollbars=yes,resizable=yes,width=800,height=748,left=0,top=0");
		}
			...
			winpopup = window.open(default_url, "_blank", "width=800, height=740, toolbar=no, menubar=no, scrollbars=yes, resizable=no, copyhistory=no" );			
		}
		
		else if(doc_type == 'BID_ICT'){
			var default_url = "approval_bid_ict2.jsp?doc_no="+doc_no+"&doc_seq="+doc_seq+"&doc_type="+doc_type+"&sign_enable=T"+"&attach_no="+attach_no+"&doc_status="+doc_status+"&proceeding_flag="+proceeding_flag+"&doc_seq="+doc_seq+"&sign_path_seq="+sign_path_seq+"&add_user_name="+add_user_name+"&add_date="+add_date+"&subject="+subject+"&view_content=Y";
			...
			var default_url = "approval_pay.jsp?from=&doc_no="+doc_no+"&doc_seq="+doc_seq+"&doc_type="+doc_type+"&sign_enable=T"+"&attach_no="+attach_no+"&doc_status="+doc_status+"&proceeding_flag="+proceeding_flag+"&doc_seq="+doc_seq+"&sign_path_seq="+sign_path_seq+"&add_user_name="+add_user_name+"&add_date="+add_date+"&subject="+subject;
			winpopup = window.open(default_url, "_blank", "width=950, height=950, toolbar=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no" );			
		}
			...
			winpopup = window.open(default_url, "_blank", "width=1050, height=750, toolbar=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no" );			
		}
			...
			winpopup = window.open(default_url, "_blank", "width=1050, height=900, toolbar=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no" );			
		}
		
		else if(doc_type == 'AR'){
			var default_url = "approval_ar.jsp?doc_no="+doc_no+"&doc_seq="+doc_seq+"&doc_type="+doc_type+"&sign_enable=T"+"&attach_no="+attach_no+"&doc_status="+doc_status+"&proceeding_flag="+proceeding_flag+"&doc_seq="+doc_seq+"&sign_path_seq="+sign_path_seq+"&add_user_name="+add_user_name+"&add_date="+add_date+"&subject="+subject+"&view_content=Y";
			winpopup = window.open(default_url, "_blank", "width=800, height=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no" );					
		}
...
function doConfirm() {
	...
	for(var i = 0; i < grid_array.length; i++ ) {
		...
		doc_no            = GridObj.cells(grid_array[i], GridObj.getColIndexById(LRTrim( "DOC_NO            " ) ) ).getValue();
		attach_no         = GridObj.cells(grid_array[i], GridObj.getColIndexById(LRTrim( "ATTACH_NO         " ) ) ).getValue();	
		proceeding_flag   = GridObj.cells(grid_array[i], GridObj.getColIndexById(LRTrim( "PROCEEDING_FLAG   " ) ) ).getValue();	
		doc_seq           = GridObj.cells(grid_array[i], GridObj.getColIndexById(LRTrim( "DOC_SEQ           " ) ) ).getValue();
		sign_path_seq     = GridObj.cells(grid_array[i], GridObj.getColIndexById(LRTrim( "SIGN_PATH_SEQ     " ) ) ).getValue();	
		doc_type          = GridObj.cells(grid_array[i], GridObj.getColIndexById(LRTrim( "DOC_TYPE          " ) ) ).getValue();
		add_user_name     = GridObj.cells(grid_array[i], GridObj.getColIndexById(LRTrim( "ADD_USER_NAME     " ) ) ).getValue();	
		add_date          = GridObj.cells(grid_array[i], GridObj.getColIndexById(LRTrim( "ADD_DATE          " ) ) ).getValue();	
		subject           = GridObj.cells(grid_array[i], GridObj.getColIndexById(LRTrim( "SUBJECT           " ) ) ).getValue();	
		can_approval      = GridObj.cells(grid_array[i], GridObj.getColIndexById(LRTrim( "CAN_APPROVAL      " ) ) ).getValue();	
		next_sign_user_id = GridObj.cells(grid_array[i], GridObj.getColIndexById(LRTrim( "NEXT_SIGN_USER_ID " ) ) ).getValue();
		app_stage        = GridObj.cells(grid_array[i], GridObj.getColIndexById(LRTrim( "SIGN_PATH_SEQ " ) ) ).getValue();
		...
		if(doc_type == 'PR'){
			...
			var default_url = "approval_pay.jsp?doc_no="+doc_no+"&doc_seq="+doc_seq+"&doc_type="+doc_type+"&sign_enable=T"+"&attach_no="+attach_no+"&doc_status="+doc_status+"&proceeding_flag="+proceeding_flag+"&doc_seq="+doc_seq+"&sign_path_seq="+sign_path_seq+"&add_user_name="+add_user_name+"&add_date="+add_date+"&subject="+subject+"&next_sign_user_id="+next_sign_user_id+"&app_stage="+app_stage;
			winpopup = window.open(default_url, "_blank", "width=950, height=950, toolbar=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no" );			
		}
	...
	<input type="hidden" name="subject_1" id="subject_1" value="">
	    ...
	    <tr>
      		...
      		<td class="data_td" width="35%">
        		<s:calendar id_from="from_date"  default_from="<%=SepoaString.getDateSlashFormat(from_date) %>" id_to="to_date" default_to="<%=SepoaString.getDateSlashFormat(to_date) %>" format="%Y/%m/%d"/>
      		</td>
      		<td class="title_td" width="15%">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;최종결재일자</td>
      		<td class="data_td" width="35%">
      			<s:calendar id_from="sign_from_date"  default_from="" id_to="sign_to_date" default_to="" format="%Y/%m/%d"/>
...
//                     out.println(lb_cm_2);
   ...
   <tr>
   			<!-- 2011.3.25 정민석 문서형태 입찰공문, 통합구매품의, 서비스요청, 업체승인, 업체거래중지 항목 주석처리후
   				 구매요청품의, 검수요청품의 추가 -->
      		...
      		<td class="data_td" width="35%">
      			...
      			<!--<option value="RQ">견적의뢰</option>-->
      				<option value="BID_ICT">입찰공문</option>
  ...
  <tr>
			...
			<TABLE cellpadding="0">
	      		<TR>
    	  			...
    	  			<td><script language="javascript">btn("javascript:doConfirm()","승 인")</script></td>
    	  			<!--
    ...
    <tr>
			...
			<input type="hidden" name="content" value="" >
      			...
      			<input type="hidden" name="text_ctrl_person_id" value="" >
			<input type="hidden" size="12" value="" name="text_doc_type" class="inputsubmit">
     		</td>
   	</tr>
 	</table>

	<input type ="hidden" name="ctrl_type" value="">
	<input type ="hidden" name="used_flag_doc" value="off">
	<input type ="hidden" name="used_flag_ctrl" value="off">
</form>
...
<%-- <s:grid screen_id="I_AP_001" grid_obj="GridObj" grid_box="gridbox"/> --%>
