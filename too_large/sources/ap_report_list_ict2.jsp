...
<%
	...
	multilang_id.addElement("I_AP_008");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "I_AP_008";
...
<% String WISEHUB_PROCESS_ID="I_AP_008";%>
<%@ include file="/include/code_common.jsp"%>
...
function Query()
{
	...
	var url	= "<%=POASRM_CONTEXT_NAME%>/servlets/ict.sepoa.svl.approval.ap_report_list_ict";
	param += dataOutput();
...
function Cancel()
{
	...
	for(var i = 0; i < grid_array.length; i++)
	{
		...
		//화면ID는 필수 입력으로 처리한다.
		//if(LRTrim(GridObj.cells(grid_array[i], GridObj.getColIndexById("DOC_TYPE")).getValue()) == "BID_ICT" || LRTrim(GridObj.cells(grid_array[i], GridObj.getColIndexById("DOC_TYPE")).getValue()) == "RA")
		//{
		//	alert("입찰공문, 역경매는 결재취소 하실 수 없습니다.");
		//	return;
		//}
	...
	/* for(var i=0; i<GridObj.GetRowCount();i++){
		...
		if(GD_GetCellValueIndex(GridObj,i,INDEX_SELECTED) == "true"){
			...
			if(doc_type == "BID_ICT" || doc_type == "RA"){
				alert("입찰공문, 역경매는 결재취소 하실 수 없습니다.");
				return;
			}
	...
	params+=dataOutput();
	var grid_array = getGridChangedRows(GridObj, "SELECTED");
 	myDataProcessor = new dataProcessor("<%=POASRM_CONTEXT_NAME%>/servlets/ict.sepoa.svl.approval.ap_report_list_ict",params);
...
*/
    	...
    	} else if (GD_GetCellValueIndex(GridObj,msg2,INDEX_DOC_TYPE) == "BID_ICT") {
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<input type="hidden" name="content" id="content" value="" >
		...
		<tr>
			...
			<!-- 2011.3.25 정민석 문서형태 입찰공문, 서비스요청, 업체승인, 업체거래중지 항목 주석처리후
   				 구매요청품의, 검수요청품의 추가 -->
  			<td class="title_td" width="15%">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;문서형태</td>
      		<td class="data_td" width="35%" colspan="3">
...
<%-- <s:grid screen_id="I_AP_008" grid_obj="GridObj" grid_box="gridbox"/> --%>
