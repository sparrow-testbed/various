...
<%
	...
	multilang_id.addElement("SR_013");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "SR_013";
...
<script language="javascript" type="text/javascript">
	...
	var INDEX_c_template_refitem;

	function setHeader() 
	{
	
/* 		GridObj.AddHeader(				"c_template_refitem",		"","t_text",1020,0,false);
...
*/	
		...
		INDEX_c_template_refitem = GridObj.GetColHDIndex("c_template_refitem"); 
	...
	function getQuery() {
		...
		var servletUrl = "<%=getWiseServletPath("master.template.chk_template_lst")%>";
		...
		var param = "mode=getTempList&grid_col_id="+grid_col_id;
		    param += dataOutput();
		var url = "<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.procure.chk_temp_list";
	...
	function templateUpdate() {
		...
		c_template_refitem = GD_GetCellValueIndex(GridObj,cnt,INDEX_c_template_refitem);
		var url = "chk_temp_insert.jsp?mode=update&c_template_refitem=" + c_template_refitem;
		popup(url);
	}
	...
	function templateDelete(){
		...
		var sel_row = 0;
		var cnt = 0;
		var c_template_refitem = "";
		for(var i=0;i<GridObj.GetRowCount();i++){
			var temp = GD_GetCellValueIndex(GridObj,i,INDEX_choose);
			if(temp == "1") {
				sel_row++;
				c_template_refitem = c_template_refitem + "," + GD_GetCellValueIndex(GridObj,i,INDEX_c_template_refitem);
			}
		}
		if(sel_row == 0) {
			alert("항목을 선택해주세요.");
		...
		c_template_refitem = c_template_refitem.substring(1);
		
		var value = confirm('삭제 하시겠습니까?');
		if(value){
	/* 		this.hiddenframe.location.href = "chk_template_ins.jsp?mode=delete&c_template_refitem=" + c_template_refitem;
 */
	...
	function popup(url) {
		...
		var doc = window.open( url, 'doc', 'left='+dim[1]+', top='+dim[0]+', width='+width+', height='+height+', toolbar='+toolbar+', menubar='+menubar+', status='+status+', scrollbars='+scrollbars+', resizable='+resizable);
	}
...
setHeader();
  	...
  	function JavaCall(msg1,msg2,msg3,msg4,msg5){
		...
		if(msg3 == INDEX_template_name){
			var c_template_refitem = GD_GetCellValueIndex(GridObj,msg2,INDEX_c_template_refitem);
			var url = "chk_template_pop.jsp?mode=view&c_template_refitem=" + c_template_refitem;
			popup(url);
...
var c_template_refitem = GridObj.cells(rowId, GridObj.getColIndexById("c_template_refitem")).getValue();	

window.open("chk_temp_detail.jsp?mode=view&c_template_refitem=" +c_template_refitem, "_blank", "width=800, height=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no" );
...
<s:grid screen_id="SR_013" grid_obj="GridObj" grid_box="gridbox"/>
<s:footer/>
</body>
</html>
