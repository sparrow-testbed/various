...
<script language='JavaScript'>
	...
	function <%=grid_obj%>_setGridDraw()
    {
    	...
    	<%=grid_obj%> = new SepoaDhtmlXGridObject('<%=grid_box%>');	// dhtmlXGridObject 생성자 함수를 sepoa_dhtmlXGridObject함수에서 상속받아서 사용한다.
        ...
        switch(menuitemId) {
            ...
            case "AllCheckLock":
            	if(<%=grid_obj%>.getColType(cInd) == "ch" && <%=isSelectScreen%> == false) {
            		if(<%=grid_obj%>.getRowsNum() > 2000) {
            			alert("데이터 조회 건수가 많습니다. 2000 건 까지만 처리 가능합니다.");
            			break;
            		} else {
