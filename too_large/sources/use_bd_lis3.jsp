...
function setHeader()
{
	...
	document.WiseGrid.AddHeader("SEL"             ,       "¼±ÅÃ","t_checkbox",10,30,false);
	document.WiseGrid.AddHeader("USER_ID"         ,       "»ç¿ëÀÚ ID","t_text",20,90,false);
	document.WiseGrid.AddHeader("USER_NAME_LOC"   ,       "»ç¿ëÀÚ¸í","t_text",100,80,false);
	document.WiseGrid.AddHeader("COMPANY_NAME"    ,       "È¸»ç","t_text",100,90,false);
	document.WiseGrid.AddHeader("TEXT_WORK_TYPE"  ,       "¾÷¹«±ÇÇÑ","t_text",100,80,false);
	document.WiseGrid.AddHeader("DEPT_NAME"       ,       "ºÎ¼­¸í","t_text",100,130,false);
	document.WiseGrid.AddHeader("POSITION"        ,       "Á÷À§","t_text",100,60,false);
	// 2011.3.24 solarb Á÷Ã¥Ç×¸ñ hidden  »çÀÌÁî 0À¸·Î ¼öÁ¤
	document.WiseGrid.AddHeader("MANAGER_POSITION",       "Á÷Ã¥","t_text",100,0,false);
	document.WiseGrid.AddHeader("PHONE_NO"        ,       "ÀüÈ­¹øÈ£","t_text",100,0,false);
	document.WiseGrid.AddHeader("MENU_NAME"       ,       "ÇÁ·ÎÆÄÀÏ¸í","t_imagetext",100,0,false);
	document.WiseGrid.AddHeader("HOUSE_CODE"      ,       "","t_text",1000,0,false);
	document.WiseGrid.AddHeader("USER_TYPE"      ,        "USER_TYPE","t_text",1000,0,false);
...
function Change()
{
	...
	for(var i=0;i<document.WiseGrid.GetRowCount();i++)
	{
		...
		if(temp == "true")
		{
			...
			<%--   // user_flag Y: top¿¡¼­ÀÇ ÀÚ±âÀÚ½ÅÀÇ »ç¿ëÀÚ Á¤º¸¸¦ °íÄ¥¶§ ,N: ¸¶½ºÅÍ »ç¿ëÀÚ¸Þ´º¿¡¼­  »ç¿ëÀÚ ¼öÁ¤ P: ? --%>
			winobj = window.open("/kr/master/user/use_bd_updf.jsp?i_flag=N&user_flag=Y&i_user_id="+GD_GetCellValueIndex(document.WiseGrid,i,IDX_USER_ID),"BKWin","top="+top+",left="+left+",width=840,height=600,resizable=yes,status=yes,scrollbars = yes");
...
function checkcode()
{
	...
	{
		if(GD_GetCellValueIndex(document.WiseGrid,i,IDX_SEL) == "true" && GD_GetCellValueIndex(document.WiseGrid,i,IDX_MENU_NAME) == "")
			rtn = true;
	}
	return rtn;
}
...
function JavaCall(msg1, msg2, msg3, msg4, msg5)  {
	...
	if(msg1 == "doQuery") {
		GD_SetCellValueIndex(document.WiseGrid,0, IDX_SEL,     "1" );
	}
}
...
{
    var user_id = document.forms[0].i_user_id.value;
...
function pwdReset()
{
    ...
    if(chkKorea(document.forms[0].password.value) < 6 || chkKorea(document.forms[0].password.value) > 10) {
        alert("ºñ¹Ð¹øÈ£´Â ¿µ¹®ÀÚ¿Í ¼ýÀÚ¸¦ È¥¿ëÇÏ¿© 6~10ÀÚ¸¸ °¡´ÉÇÕ´Ï´Ù.(1)");
    ...
    if(!chkMixing(document.forms[0].password.value)) {
        alert("ºñ¹Ð¹øÈ£´Â ¿µ¹®ÀÚ¿Í ¼ýÀÚ¸¦ È¥¿ëÇÏ¿© 6~10ÀÚ¸¸ °¡´ÉÇÕ´Ï´Ù.(2)");
        document.forms[0].password.focus();
        document.forms[0].password.select();
        return ;
    }
    ...
    if(!chkIdIsEquals(document.forms[0].password.value)) {
        alert("ºñ¹Ð¹øÈ£´Â ID°¡ Æ÷ÇÔµÇ¾î ÀÖÀ¸¸é ¾ÈµË´Ï´Ù.(3)");
        document.forms[0].password.focus();
        document.forms[0].password.select();
        return ;
    }
    ...
    if(!chkInputRepeat(document.forms[0].password.value)) {
        alert("ºñ¹Ð¹øÈ£´Â °°Àº ¹®ÀÚ¸¦ 3¹ø ÀÌ»ó ¹Ýº¹ ÀÔ·ÂÇÒ ¼ö ¾ø½À´Ï´Ù.(4)");
        document.forms[0].password.focus();
        document.forms[0].password.select();
        return ;
    }

	if(!confirm("»ç¿ëÀÚ("+sel_user_id+") ºñ¹Ð¹øÈ£¸¦ ÃÊ±âÈ­ ÇÕ´Ï´Ù.\nÃÊ±âÈ­ ºñ¹Ð¹øÈ£´Â '"+password+"' ÀÔ´Ï´Ù.")) {
	    return;
	}
...
<input type="hidden" name="edit" value="Y">
	 <tr style="display: none;">
	  ...
	  <td class="c_data_1">
		...
		<option>ÀüÃ¼</option>
	   <%
		 String i_user_type = ListBox(request, "SL0081",  house_code+"#", company_code);
	...
	<tr>
	  ...
	  <td class="c_data_1" width="35%">
		<input type="text" size="15" value="<%=info.getSession("ID") %>" name="i_user_id" readonly class="inputsubmit" onkeydown='entKeyDown()' >
	  ...
	  <td class="c_data_1" width="35%">
		<input type="text" name="i_user_name_loc" readonly size="20" value="<%=info.getSession("NAME_LOC") %>" class="inputsubmit" onkeydown='entKeyDown()' ></td>
	</tr>
  </table>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
      		...
      		<td height="30" align="right">
				<TABLE cellpadding="0">
		      		<TR>
						 ...
						 <TD background="/images/<%=info.getSession("HOUSE_CODE")%>/btn_bg.gif" class="btn" height="25"><a href="javascript:doSelect()" class="btn">Á¶ È¸</a></TD>
	...
	%>                   <!-- TD width="10" align="left"><img src="/images/<%=info.getSession("HOUSE_CODE")%>/btn_left.gif" width="10" height="25" /></TD>
						 <TD background="/images/<%=info.getSession("HOUSE_CODE")%>/btn_bg.gif" class="btn" height="25"><a href="javascript:doDisplay()" class="btn">»ó¼¼Á¤º¸</a></TD>
					 	 <TD width="10" align="left"><img src="/images/<%=info.getSession("HOUSE_CODE")%>//btn_right.gif" width="10" height="25" /></TD>
					 	 <TD width="10" align="left"><img src="/images/<%=info.getSession("HOUSE_CODE")%>/btn_left.gif" width="10" height="25" /></TD>
						 <TD background="/images/<%=info.getSession("HOUSE_CODE")%>/btn_bg.gif" class="btn" height="25"><a href="javascript:doApproval()" class="btn">½Â ÀÎ</a></TD>
					 	 <TD width="10" align="left"><img src="/images/<%=info.getSession("HOUSE_CODE")%>//btn_right.gif" width="10" height="25" /></TD>
					 	 <TD width="10" align="left"><img src="/images/<%=info.getSession("HOUSE_CODE")%>/btn_left.gif" width="10" height="25" /></TD>
						 <TD background="/images/<%=info.getSession("HOUSE_CODE")%>/btn_bg.gif" class="btn" height="25"><a href="javascript:doDelete('setDelete')" class="btn">»è Á¦</a></TD>
					 	 <TD width="10" align="left"><img src="/images/<%=info.getSession("HOUSE_CODE")%>//btn_right.gif" width="10" height="25" /></TD-->
	<%} else {%>
		<%
		if (!gubun.equals("M")) {
		%>				<TD height="25">ºñ¹Ð¹øÈ£ : </TD>
						<TD height="25"><input type="text" value="" name="password" size="12"></td>
						<TD width="10" align="left"><img src="/images/<%=info.getSession("HOUSE_CODE")%>/btn_left.gif" width="10" height="25" /></TD>
						<TD background="/images/<%=info.getSession("HOUSE_CODE")%>/btn_bg.gif" class="btn" height="25"><a href="javascript:pwdReset()" class="btn">ºñ¹Ð¹øÈ£_ÃÊ±âÈ­</a></TD>
					 	<TD width="10" align="left"><img src="/images/<%=info.getSession("HOUSE_CODE")%>//btn_right.gif" width="10" height="25" /></TD>
					 	<TD width="10" align="left"><img src="/images/<%=info.getSession("HOUSE_CODE")%>/btn_left.gif" width="10" height="25" /></TD>
						<TD background="/images/<%=info.getSession("HOUSE_CODE")%>/btn_bg.gif" class="btn" height="25"><a href="javascript:Change()" class="btn">¼ö Á¤</a></TD>
					 	<TD width="10" align="left"><img src="/images/<%=info.getSession("HOUSE_CODE")%>//btn_right.gif" width="10" height="25" /></TD>
		<%} %>
		<%
		if (!gubun.equals("M")) {
