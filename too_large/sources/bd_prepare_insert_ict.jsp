...
<%
	...
	multilang_id.addElement("I_BD_013");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "I_BD_013";
	...
	Object[] obj = {map};
	SepoaOut value = ServiceConnector.doService(info, "I_BD_012", "CONNECTION","getBdPrepareHeader", obj);
...
//                 }
        ...
        if(obj.name == "ESTM_PRICE"){
        	...
        	var tmp_tmp = basicPrice / (basicPrice - estmPrice);
        	var tmp = 0;

        	if (tmp_tmp > "0"){
        		tmp = 100 - (100 / (basicPrice / (basicPrice - estmPrice)));	
        	}
...
var G_SERVLETURL   = "<%=POASRM_CONTEXT_NAME%>/servlets/ict.sepoa.svl.sourcing.bd_prepare_insert_ict";
...
function init() {
	...
	<%
	    	if(!COST_STATUS.equals("EC") && !COST_STATUS.equals("")) {
				...
				if("".equals(ESTM_PRICE1) || ESTM_PRICE1 == null || "0".equals(ESTM_PRICE1)){
	 				ESTM_PRICE1 = "0.00";
	 				ESTM_RATE = "0.00";
	 			}else{
...
function doSave(FLAG) {
	...
	if(estm_price > basic_amt){
		 alert("내정가격은 예정가격 이하만 입력가능합니다.");
		 button_flag = false;
		 document.forms[0].ESTM_PRICE.focus();
	     return;
	}
    ...
    location.href="bd_prepare_list_ict.jsp";
}
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="0">	
								<tr>
									...
									<td class="data_td">&nbsp;
										...
										<%} else {%>
											<input type="text" name="BASIC_AMT" id="BASIC_AMT" value="<%=SepoaMath.SepoaNumberType(Double.parseDouble(BASIC_AMT),"###,###,###,###,###.##")%>" readonly onblur="javascript:setOnBlur(this);" onFocus="javascript:setOnFocus(this);"  style="text-align: right;">
										<%}%>
										&nbsp;원&nbsp;(VAT포함)
									</td>
