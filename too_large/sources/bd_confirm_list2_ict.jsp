...
<%
	...
	multilang_id.addElement("I_BD_019");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "I_BD_019";
...
var G_SERVLETURL   = "<%=POASRM_CONTEXT_NAME%>/servlets/ict.sepoa.svl.sourcing.bd_confirm_list2_ict";
...
{ 
	...
	if( header_name == "ANN_NO")
    {
		...
		var url = "/ict/sourcing/bd_ann_d2_ict_"+ann_version+".jsp?SCR_FLAG=D&ANN_STATUS=AR&ANN_VERSION="+ ann_version;
...
*/
	...
	{ 
		var ANN_NO      = LRTrim(GridObj.cells(grid_array[i], GridObj.getColIndexById("ANN_NO")).getValue());
		var ANN_COUNT   = LRTrim(GridObj.cells(grid_array[i], GridObj.getColIndexById("ANN_COUNT")).getValue());
...
// 	if(confirm(Message) == 1){
    	url =  "bd_confirm_popup2_ict.jsp";
    	url += "?ANN_NO=" + ANN_NO + "&ANN_COUNT=" + ANN_COUNT + "&PFLAG=" + pflag;
	    window.open( url , "bd_confirm_popup","toolbar=no, location=no, directories=no, status=yes,menubar=no,scrollbars=yes,resizable=yes,width=800,height=500,left=0,top=0");
// 	} 
...
// 	if(confirm(Message) == 1){
    	url =  "bd_confirm_popup2_1_ict.jsp";
    	url += "?ANN_NO=" + ANN_NO + "&ANN_COUNT=" + ANN_COUNT;
	    window.open( url , "bd_confirm_popup","toolbar=no, location=no, directories=no, status=yes,menubar=no,scrollbars=yes,resizable=yes,width=800,height=500,left=0,top=0");
// 	} 
// 		document.form.method = "POST";
...
<td width="100%">
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
    	...
    	<tr>
      		...
      		<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp; 공고담당자</td>
      		<td  width="35%" class="data_td">
        		<input type="text" name="CHANGE_USER_NAME" id="CHANGE_USER_NAME"   value="<%=USER_NAME%>" onkeydown='entKeyDown()' size="10" maxlength="10" >
      		</td>
		</tr>
		<tr>
			<td colspan="4" height="1" bgcolor="#dedede"></td>
			...
			<td height="30" align="right">
				<TABLE cellpadding="0">
					<TR>
						...
						<%-- TD><script language="javascript">btn("javascript:doConfirm('INP')", "서류접수")</script></TD --%>
						<%-- TD><script language="javascript">btn("javascript:doConfirm('FINAL');", "적격업체확인")</script></TD --%>
						<TD><script language="javascript">btn("javascript:doConfirm2();", "적격업체확인")</script></TD>			
					</TR>
				</TABLE>
			</td>
		</tr>
...
<%-- <s:grid screen_id="I_BD_019" grid_obj="GridObj" grid_box="gridbox"/> --%>
