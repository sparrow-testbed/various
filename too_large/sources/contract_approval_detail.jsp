...
<%
//////////////////ClipReport4 선언부 시작///////////////////////////////////////////////////////////
String _rptName          = "020644/rpt_contract_detail_print_f"; //리포트명
...
//////////////////ClipReport4 선언부 끝/////////////////////////////////////////////////////////////
	...
	String ct_flag      = JSPUtil.nullToEmpty(request.getParameter("ct_flag"));    //계약완료(CD)
	String sign_status  = JSPUtil.nullToEmpty(request.getParameter("sign_status"));//결재완료(E)
...
//    	CONT_GUBUN     			= JSPUtil.nullToEmpty(wf.getValue("CONT_GUBUN",				0));
		...
		CHK_FG	                = JSPUtil.nullToEmpty(wf.getValue("CHK_FG",				0));     
		CHK_DATE	            = JSPUtil.nullToEmpty(wf.getValue("CHK_DATE",				0));     
		CHK_TIME	            = JSPUtil.nullToEmpty(wf.getValue("CHK_TIME",				0));     	
		CHK_USER_ID	            = JSPUtil.nullToEmpty(wf.getValue("CHK_USER_ID",				0));     
		CHK_USER_NAME           = JSPUtil.nullToEmpty(wf.getValue("CHK_USER_NAME",				0));     
		
		SIGN_STATUS             = JSPUtil.nullToEmpty(wf.getValue("SIGN_STATUS",				0));     
    }
 	// 클린계약체크리스
    String visibilityStyCleanCt = ("1".equals(CHK_FG))?"visible":"hidden";
    String divCleanCtStyle = "";
    /*
    if("CD".equals(ct_flag) && "E".equals(sign_status)){
    	divCleanCtStyle = "visibility:"+visibilityStyCleanCt;    
    }else{
    	divCleanCtStyle = "Z-INDEX:99;POSITION:absolute;width:730px;height:400px;top:0px;left:160px;visibility:"+visibilityStyCleanCt;
    }
    */
    if("CD".equals(CT_FLAG) && "E".equals(SIGN_STATUS)){
    	divCleanCtStyle = "visibility:"+visibilityStyCleanCt;    
	...
	/*계약서상세내용 조회부분 시작*/
    Object[] obj_2 = {cont_form_no, cont_gl_seq};
	SepoaOut value_2 = ServiceConnector.doService(info, "CT_021", "CONNECTION","getContractFormSelectSCPMT", obj_2);
	
	SepoaFormater wf11_2 = new SepoaFormater(value_2.result[0]);
	
	for(int i = 0; i < wf11_2.getRowCount(); i++)
	{
		CONT_CONTENT += wf11_2.getValue("CONTENT", i);
	}
	
	
	Object[] obj1_2 = {cont_no, cont_gl_seq};
	SepoaOut value1_2 = ServiceConnector.doService(info, "CT_021", "CONNECTION","getContractContNoSelect", obj1_2);
	SepoaFormater wf2_2 = new SepoaFormater(value1_2.result[0]);
	...
	String STAMP_TAX_NO = "";
	String STAMP_TAX_AMT = "";
	
    if(wf2_2.getRowCount() > 0) {
    	seller_code						= JSPUtil.nullToEmpty(wf2_2.getValue("seller_code",0));      //업체코드
    	in_val_cont_type				= JSPUtil.nullToEmpty(wf2_2.getValue("IN_VAL_CONT_TYPE",0)); //계약종류
    	in_val_cont_add_date            = JSPUtil.nullToEmpty(wf2_2.getValue("IN_VAL_CONT_ADD_DATE",0));	// 계약일자
    	in_attach_no					= JSPUtil.nullToEmpty(wf2_2.getValue("ATTACH_NO",0));//첨부파일
    	vendor_in_attach_no				= JSPUtil.nullToEmpty(wf2_2.getValue("SE_ATTACH_NO",0));//업체첨부파일
    	
    	RPT_GETFILENAMES1      	        = JSPUtil.nullToEmpty(wf2_2.getValue("RPT_GETFILENAMES1", 0)); //구매부서 첨부파일명들     	
    	RPT_GETFILENAMES2      	        = JSPUtil.nullToEmpty(wf2_2.getValue("RPT_GETFILENAMES2", 0)); //업체 첨부파일명들     	
    	STAMP_TAX_NO = JSPUtil.nullToEmpty(wf2_2.getValue("STAMP_TAX_NO",0));//인지세번호
    	STAMP_TAX_AMT = JSPUtil.nullToEmpty(wf2_2.getValue("STAMP_TAX_AMT",0));//인지세금액
	}
	...
	if( "CC".equals(ct_flag) || "CD".equals(ct_flag) ) { // 업체인증, 우리은행인증
	    Object[] obj2_2 = {"W100", seller_code};
		SepoaOut value2_2 = ServiceConnector.doService(info, "CT_021", "CONNECTION","getContractsignSelect", obj2_2);
		SepoaFormater wf3_2 = new SepoaFormater(value2_2.result[0]);
		
	    if(wf3_2.getRowCount() > 0) {
	    	if( "CC".equals(ct_flag) )   in_vendor_sign_name = JSPUtil.nullToEmpty(wf3_2.getValue("VENDOR_SIGN_NAME",0));
	    	if( "CD".equals(ct_flag) ){
	    		in_woori_sign_name	 = JSPUtil.nullToEmpty(wf3_2.getValue("WOORI_SIGN_NAME",0));
	    		in_vendor_sign_name  = JSPUtil.nullToEmpty(wf3_2.getValue("VENDOR_SIGN_NAME",0));
	    	}
	    }	
    }
	*/
...
<%-- 		doRequestUsingPOST( 'W001', '3' ,'sg_type3', '<%=SG_LEV3%>' );		 --%>
		...
		$("#sg_type_td").html($("#sg_type1 option:selected").text() + " / " + $("#sg_type2 option:selected").text() + " / " + $("#sg_type3 option:selected").text());
		$("#cont_process_flag_td").html($("#cont_process_flag option:selected").text());
		$("#ele_cont_flag_td").html($("#ele_cont_flag option:selected").text());
		if($("#cont_type1_text option:selected").text() != ""){
			$("#cont_type_text_td").html($("#cont_type1_text option:selected").text() + " / " + $("#cont_type2_text option:selected").text());
		}
		$("#assure_flag_td").html($("#assure_flag option:selected").text());
		$("#cont_total_gubun_td").html($("#cont_total_gubun option:selected").text());
		$("#cont_type_td").html($("#cont_type option:selected").text());
...
//			sg_type3_id.fireEvent("onchange"); //onchange 이벤트발생
	...
	function clipPrint(rptAprvData,approvalCnt) {
		...
		if(typeof(rptAprvData) != "undefined"){
			...
			<% if("1".equals(CHK_FG)){ %>
			document.form.rptAprvLastViewYN.value = "Y";
			<% } %>
	    }
...
_rptData.append(_RF);
_rptData.append(STAMP_TAX_NO);
_rptData.append(_RF);
_rptData.append(STAMP_TAX_AMT);
_rptData.append(_RF);
_rptData.append(RPT_GETFILENAMES2);
...
Object[] obj4 = {cont_no, cont_gl_seq};
SepoaOut value4 = ServiceConnector.doService(info, "CT_020", "TRANSACTION","getCleanContractCheckSelect", obj4);
SepoaFormater wf4  = new SepoaFormater(value4.result[0]);
_rptData.append(_RD);
if("1".equals(CHK_FG)){
	if(wf4.getRowCount() > 0) {
		for(int j = 0; j < wf4.getRowCount(); j++)
		{
			_rptData.append(wf4.getValue("CHK_SEQ", j));
			_rptData.append(_RF);
			_rptData.append("<BR>");
			_rptData.append(wf4.getValue("CHK_COMMENT", j));
			_rptData.append(_RF);
			_rptData.append(wf4.getValue("CHK_FLAG", j));
			_rptData.append(_RL);
		}
		_rptData.append(_RD);
		_rptData.append(CHK_DATE);
	}
}
//////////////////ClipReport4 조립부 끝///////////////////////////////////////////////////////////	
%>
<body leftmargin="15" topmargin="6" onload="">
<s:header popup="true">
...
<input type="hidden" name="rptAprvLastViewYN" id="rptAprvLastViewYN">
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
						  	...
						  	<tr>
			        			...
			        			<td height="24" class="data_td" id="sg_type_td" >
						    		<select id="sg_type1" name="sg_type1" class="inputsubmit" disabled="disabled" style="display:none;">
										<option value=""></option>
						    		<%
										String com1 = ListBox(request, "W001", "1", SG_LEV1);
										out.println(com1);						    		
						    		%>
						    		</select>
						    	
						    		<select id="sg_type2" name="sg_type2" class="inputsubmit" disabled="disabled" style="display:none;">
				               			<option value=""></option>						    		
						    		<%
										String com2 = ListBox(request, "W001", "2", SG_LEV2);
										out.println(com2);						    		
						    		%>
						    		</select>
						    	
						    		<select id="sg_type3" name="sg_type3" class="inputsubmit" disabled="disabled" style="display:none;">
				               			<option value=""></option>						    		
						    		<%
										String com3 = ListBox(request, "W001", "3", SG_LEV3);
										out.println(com3);						    		
						    		%>
						    		</select>        			
			        			</td>
			        			...
			        			<td height="24" class="data_td" id="cont_process_flag_td">
			        				<select id="cont_process_flag" name="cont_process_flag" disabled="disabled" style="display:none;">
				               			<option value=""></option>			        				
						    		<%
										String com4 = ListBox(request, "SL0018", info.getSession("HOUSE_CODE") + "#M809", CONT_PROCESS_FLAG);
										out.println(com4);						    		
						    		%>			        				
			        				</select><!-- M809 -->
			        			</td>        		
						  	...
						  	<tr>
			        			...
			        			<td height="24" class="data_td" id="cont_type_text_td">
			        				<select id="cont_type1_text" name="cont_type1_text" disabled="disabled" style="display:none;">
			        					<option value=""></option>
						    		<%
										String com6 = ListBox(request, "SL0018", info.getSession("HOUSE_CODE") + "#M989", CONT_TYPE1_TEXT);
										out.println(com6);						    		
						    		%>				        				
			        				</select><!-- M994 -->&nbsp;&nbsp;
			        				<select id="cont_type2_text" name="cont_type2_text" disabled="disabled" style="display:none;">
				               			<option value=""></option>			        				
						    		<%
										String com7 = ListBox(request, "SL0018", info.getSession("HOUSE_CODE") + "#M930", CONT_TYPE2_TEXT);
										out.println(com7);						    		
						    		%>				        				
			        				</select><!-- M993 -->        			
			        			</td>        		
							...
							<tr>
			        	  	...
			        	  	<tr>
			        			...
			        			<td height="24" class="data_td" id="ele_cont_flag_td">
				               		<select id="ele_cont_flag" name="ele_cont_flag" onchange="chele_cont_flag()" disabled="disabled" style="display:none;">
				               			<option value=""></option>
						    		<%
										String com5 = ListBox(request, "SL0018", info.getSession("HOUSE_CODE") + "#M223", ELE_CONT_FLAG);
										out.println(com5);						    		
						    		%>				               		
				               		</select><!-- M806 -->
			        			</td>
			        			...
			        			<td height="24" class="data_td" id="cont_type_td">
				                	<select id="cont_type" name="cont_type" class="inputsubmit" disabled="disabled" style="display:none;"><!-- M800 -->
										<option value=""></option>
						    		<%
										String com10 = ListBox(request, "SL0018", info.getSession("HOUSE_CODE") + "#M899", CONT_TYPE);
										out.println(com10);						    		
						    		%>											
									</select>
			        			</td>        		
							...
							<tr>
			        			...
			        			<td height="24" class="data_td" id="assure_flag_td">
				               		<select id="assure_flag" name="assure_flag" disabled="disabled" style="display:none;">
				               			<option value=""></option>
						    		<%
										String com8 = ListBox(request, "SL0018", info.getSession("HOUSE_CODE") + "#M355", ASSURE_FLAG);
										out.println(com8);						    		
						    		%>					               		
							...
							<tr>
						  	...
						  	<tr>
			        			...
			        			<td height="24" class="data_td" id="cont_total_gubun_td">
			        				<select id="cont_total_gubun"name="cont_total_gubun" disabled="disabled" style="display:none;">
			        					<option value=""></option>
						    		<%
										String com9 = ListBox(request, "SL0018", info.getSession("HOUSE_CODE") + "#M204", CONT_TOTAL_GUBUN);
										out.println(com9);						    		
						    		%>				        					
			        				</select><!-- M813 -->&nbsp;
			        				<%=CONT_PRICE %>
...
<table width="100%" border="0" cellspacing="0" cellpadding="1">
	<tr><td class=title_sub height=20 valign=bottom align=left>전자계약서 상세내역 &nbsp; </td></tr>
	<tr>
		<td>
			<table width="100%" border="0" cellspacing="1" cellpadding="0"  bgcolor="#dedede">
				<tr>
...
<div id="divCleanCt" style="<%=divCleanCtStyle%>" align="center">
		<table style="BORDER-BOTTOM: black thin solid; BORDER-LEFT: black thin solid; BACKGROUND-COLOR: #ffffff; BORDER-TOP: black thin solid; BORDER-RIGHT: black thin solid;" width="760" border="0" cellspacing="0" cellpadding="0" align="center">
					...
					<table border="0" cellspacing="0" cellpadding="10">
						<tr>
							...
							<td align="right" height="20" width="55">
								<script language="javascript">btn("javascript:document.getElementById('divCleanCt').style.visibility = 'hidden';", "닫 기")</script>
							</td>
							<% } %>
						</tr>				
					</table>
								...
								<table width="700" border="1" cellspacing="0" cellpadding="0">						
									...
									<tr>
										<td height="25" align="center">
											<font style="font-size:12px;" color="black"><%= wf4.getValue("CHK_SEQ", k) %></font>
										</td>
										<td height="25"  align="left">
											<%= "<pre>"+wf4.getValue("CHK_COMMENT", k)+"</pre>" %>	
										</td>
										<td height="25"  align="center">
											<input type="checkbox" id="ckbFg1Y" name="ckbFg1Y" <%=("Y".equals(wf4.getValue("CHK_FLAG", k)))?"checked":""%> disabled/>
										</td>
										<td height="25"  align="center">
											<input type="checkbox" id="ckbFg1N" name="ckbFg1N" <%=("N".equals(wf4.getValue("CHK_FLAG", k)))?"checked":""%> disabled/>
										</td>								
									</tr>
									<% 
										}
		 ...
		 &nbsp;&nbsp;&nbsp;&nbsp;우리은행 행동강령을 숙지하고 이를 준수 • 실천하였음을 확인합니다.</font>
			...
			<tr align="center" >
				<td height="20" colspan="5">
					<span id="spCleanCtYmd" name="spCleanCtYmd" style="font-size:14px; font-weight:bold; color:black" ><%=CHK_DATE%></span>
				</td>
			</tr>
			<tr>
...
<s:footer/>
<script>
setGridDraw();initAjax();doSelect111();doAmtGubunChange();
</script>
</html>