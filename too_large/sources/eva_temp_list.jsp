...
<%
	...
	multilang_id.addElement("SR_017");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "SR_017";
...
//<!--
	...
	function getQuery() {
		...
		var servletUrl = "/servlets/master.template.eva_template_lst";
		...
		//GridObj.setSorterble("true");
		
		var grid_col_id     = "<%=grid_col_id%>";
		var param = "mode=getTempList&grid_col_id="+grid_col_id;
		    param += dataOutput();
		var url = "<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.procure.eva_temp_list";
	...
	function updateEvaTemp() {
		...
		e_template_refitem = GD_GetCellValueIndex(GridObj,cnt,INDEX_e_template_refitem);
		
		var url = "eva_temp_insert.jsp?mode=update&e_template_refitem=" + e_template_refitem;
		popup(url);
	}
	
	
	function deleteEvaTemp(){
		var wise =  GridObj;
		var sel_row = 0;
		var cnt = 0;
		...
		for(var i=0;i<GridObj.GetRowCount();i++){
			...
			if(temp == "1") {
				sel_row++;
				e_template_refitem = e_template_refitem + "," + GD_GetCellValueIndex(GridObj,i,INDEX_e_template_refitem);
			}
		}
		if(sel_row == 0) {
			alert("항목을 선택해주세요.");
		...
		e_template_refitem = e_template_refitem.substring(1);
		
		var value = confirm('삭제 하시겠습니까?');
		if(value){
			/* this.hiddenframe.location.href = "eva_template_ins.jsp?mode=delete&e_template_refitem=" + e_template_refitem;
	 */	
...
setHeader();
  	...
  	function JavaCall(msg1,msg2,msg3,msg4,msg5){
		...
		if(msg3 == INDEX_template_name){
			...
			var url = "eva_template_pop1.jsp?mode=view&e_template_refitem=" + e_template_refitem;
			popup(url);
...
window.open("eva_temp_detail.jsp?mode=view&e_template_refitem=" +e_template_refitem, "_blank", "width=800, height=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no" );
...
<table width="100%" border="0" cellpadding="1" cellspacing="1" bgcolor="#ccd5de">
    <tr>
      <td class="c_title_1" width="15%"><img src="/images/icon/icon_s_arr.gif" width="9" height="9"> 템플릿명
      </td>
    ...
    <tr>
	  ...
	  <TABLE cellpadding="0">
		<TR>
		  ...
		  <td><script language="javascript">btn("javascript:addEvaTemp()","등 록")</script></td>
		  <td><script language="javascript">btn("javascript:updateEvaTemp()","수 정")</script></td>
		  <td><script language="javascript">btn("javascript:deleteEvaTemp()","삭 제")</script></td>
		</TR>
	  </TABLE>
	  </div>
    </td>
...
<s:grid screen_id="SR_017" grid_obj="GridObj" grid_box="gridbox"/>
<s:footer/>
</body>
</html>
