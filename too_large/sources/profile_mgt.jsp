...
<%
	...
	multilang_id.addElement("AD_022");
...
var strToday	= '<%= to_day %>';
	...
	function doQuery()
	{
		...
		var moduleType = encodeUrl(LRTrim(document.form.module_type.value.toUpperCase()));
		...
		//alert(param);
		var G_SERVLETURL = "<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.admin.profile_mgt";
    ...
    function doInsert()
	{
		...
		for(var i = 0; i < grid_array.length; i++)
		{
			...
			if(GridObj.cells(grid_array[i], GridObj.getColIndexById("selected")).getValue() == 1)
			{
				...
				{
					alert('<%=text.get("AD_022.0101")%>');
					return;
				}
				<%-- 주메뉴코드는 필수 입력으로 처리한다.--%>
				if(LRTrim(GridObj.cells(grid_array[i], GridObj.getColIndexById("module_type")).getValue()) == "")
				{
					alert('<%=text.get("AD_022.0102")%>');
					return;
				}
				<%-- 메뉴코드는 필수 입력으로 처리한다.--%>
				if(LRTrim(GridObj.cells(grid_array[i], GridObj.getColIndexById("menu_object_code")).getValue()) == "")
				{
					alert('<%=text.get("AD_022.0103")%>');
					return;
				}

				<%-- 프로파일코드는 필수 입력으로 처리한다.--%>
				if(!LRTrim(GridObj.cells(grid_array[i], GridObj.getColIndexById("menu_profile_code")).getValue()) == "")
				{
					alert('<%=text.get("AD_022.0104")%>');
					return;
				}
			}
	    ...
	    if (confirm("<%=text.get("MESSAGE.1018")%>")){
            ...
            G_SERVLETURL = "<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.admin.profile_mgt";//서블릿 경로
...
//alert("rowId=="+rowId + "cellInd=="+cellInd);
		...
		if(cellInd == GridObj.getColIndexById("module_type")){
		...
		else if(cellInd == GridObj.getColIndexById("menu_profile_code") ){
           	setrow = rowId;
			setcol = cellInd;

			var obj_code = GridObj.cells(rowId, GridObj.getColIndexById("menu_object_code")).getValue();
			var obj_profile_code = GridObj.cells(rowId, GridObj.getColIndexById("menu_profile_code")).getValue();
			var obj_name = GridObj.cells(rowId, GridObj.getColIndexById("menu_name")).getValue();

			/* mu1_pp_ins1 -- > buy_admin_menupop_mgt */
			var url = "profilepop_mgt.jsp?MENU_CODE=" +obj_code+ "&MENU_NAME=" + obj_name + "&MENU_PROFILE_CODE=" + obj_profile_code+ "&FLAG=Y";
...
//alert("text=="+text +"value=="+value );
    ...
    function doCopy()
	{
		...
		var menu_object_code = GridObj.cells(GridObj.getRowId(idx), GridObj.getColIndexById("menu_object_code")).getValue();
