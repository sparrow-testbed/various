...
<%
	...
	multilang_id.addElement("I_BD_011");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "I_BD_011";
	...
	Object[] obj = {map};
	SepoaOut value = ServiceConnector.doService(info, "I_BD_010", "CONNECTION","getBdReqPrepareHeader", obj);
	...
	SepoaFormater wf5 = new SepoaFormater(value.result[4]); 
		...
		if(wf1.getRowCount() > 0){
	        ...
	        PROM_CRIT_NAME               = wf1.getValue("PROM_CRIT_NAME"        ,0);	// 입찰방식(3번째)
	        ...
	        BZBG_AMT                     = wf1.getValue("BZBG_AMT"              ,0);	   
	    }
		...
		/*
		if(ESTM_USER_ID.equals("")){
			if(wf5.getRowCount() > 0){
	    		ESTM_USER_ID        = wf5.getValue("ESTM_USER_ID_DEFAULT"       ,0);
		        ESTM_USER_NAME      = wf5.getValue("ESTM_USER_NAME_DEFAULT"     ,0);	
			}
		}
		*/
...
//		GridObj.SendData(servletUrl, "ALL", "ALL");
	...
	function getUser(){
		window.open("/common/CO_008_ict.jsp?callback=getAddUser", "SP9113", "toolbar=no, location=no, directories=no, status=yes,menubar=no,scrollbars=yes,resizable=no,width=450,height=550,left=0,top=0");
	}
...
var G_SERVLETURL   = "<%=POASRM_CONTEXT_NAME%>/servlets/ict.sepoa.svl.sourcing.bd_req_prepare_insert_ict";
...
function doSaveEnd(obj)
{
    ...
    if(status == "true") {
        alert(messsage);
        if(pflag =="C"){
        	location.href="bd_req_prepare_list_ict.jsp";
        }else{ 
            button_flag = false;
        }
        //doQuery();
...
function checkRows() {
	...
	alert("<%=text.get("MESSAGE.MSG_1001")%>");
	return false;
...
function doSave(flag) {
    ...
    if(LRTrim(document.forms[0].BASIC_AMT.value) == "" || LRTrim(document.forms[0].BASIC_AMT.value) == "0") {
        alert("예상금액을 입력하세요.");
        button_flag = false;
    ...
    if(LRTrim(document.forms[0].BZBG_AMT.value) == "" || LRTrim(document.forms[0].BZBG_AMT.value) == "0") {
        alert("사업예산을 입력하세요.");
        button_flag = false;
        document.forms[0].BZBG_AMT.focus();
        return;
    }
    ...
    document.forms[0].BZBG_AMT.value = del_comma(document.forms[0].BZBG_AMT.value);
...
<form name="form1" method="post" >
					...
					<tr>
						<td width="100%">
							<table width="100%" border="0" cellspacing="0" cellpadding="0">
								...
								<tr>
									<td class="title_td" width="15%">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp; 공문일자</td>
									<td class="data_td">&nbsp;
										<%=ANN_DATE%>
									</td>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td height="30" align="right">
				<table cellpadding="0">
					<tr>
						<%if(!BID_STATUS.equals("NB") && !BID_STATUS.equals("SB")) {	// 낙찰(SB)/유찰(NB)
							if(!COST_STATUS.equals("EC")) {%>
	...
	<%--<s:grid screen_id="I_BD_011" grid_obj="GridObj" grid_box="gridbox" /> --%>
						...
						<td width="100%">
							<table width="100%" border="0" cellspacing="0" cellpadding="0">	
								<tr>
									...
									<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp; 사업예산</td>
									<td class="data_td">&nbsp;
										<input type="text" name="BZBG_AMT" id="BZBG_AMT" value="<%=SepoaMath.SepoaNumberType(Double.parseDouble(BZBG_AMT),"###,###,###,###,###.##")%>" onblur="javascript:setOnBlur(this);" onFocus="javascript:setOnFocus(this);"   style="text-align: right;" >
										&nbsp;원&nbsp;(VAT포함)
									</td>
								</tr>
								<tr>
