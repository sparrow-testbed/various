var __rootverify = function(__SANDBOX) {
...
XWC.getLocaleResource("rootverify", XWC.lang());

function onload(aDialogObj) {
    gDialogObj = aDialogObj;
	gDialogParam = gDialogObj.args;
	gHash = gDialogParam.hash;

		switch (gDialogParam.dialogType)
	{
		case "confirm":
			__5.innerHTML = 
			XWC.S.cainfo1 + XWC.Util.getCNFromRDN(gDialogParam.subjectRDN) + XWC.S.cainfo3;	
			__7.innerHTML = hashFormatting(gHash);
			break;
		case "alert":
			__5.innerHTML =
			XWC.S.cainfo2 + gDialogParam.subjectRDN + XWC.S.cainfo4;
			__8.value = gHash;
			__9.innerHTML = XWC.S.info3 + XWC.S.info4;
			break;
	}
...
function hashFormatting(aHash)
{
	var aIter,
		aResult = "";
	aHash = aHash.toUpperCase();

	for (aIter = 0 ; aIter < aHash.length ; aIter += 4) {
		aResult += aHash.substr(aIter, 4) + " ";
	}
...
function onOKButtonClick()
{
	...
	aInputHash = __8.value;

	if (aInputHash) {
		aInputHash = aInputHash.split(" ").join("");
	}

	if(gDialogParam.dialogType == "confirm"){
		if (aInputHash.toUpperCase() != gHash.toUpperCase()) {
			gFailedCount++;

			if (gFailedCount > 3) {
				aCommonDialogModule = __SANDBOX.loadModule("commondialog");

				AnySign.SetUITarget (__13);
				aCommonDialog = aCommonDialogModule ({
					...
					args: { title: XWC.S.fail_title,
							message : XWC.S.fail_three + "\n\n" + XWC.S.ask_reinstall,
							dialogType: "confirm",
						    align: "left",
							ldapURL: gDialogParam.ldapURL,
							issuerRDN: gDialogParam.issuerRDN,
	...
	else if(gDialogParam.dialogType == "alert")
	{
...
__6.className = 'xwup-boxtitle';
__6.htmlFor = 'xwup_input_hash';
__6.appendChild(document.createTextNode(XWC.S.value));
__4.appendChild(__6);
var __7 = XWC.UI.createElement('DIV');
__7.setAttribute('id', 'xwup_box', 0);
__7.className = 'xwup-content-box2';
__4.appendChild(__7);
var __8 = XWC.UI.createElement('INPUT');
__8.setAttribute('tabindex', '4', 0);
__8.setAttribute('id', 'xwup_input_hash', 0);
__8.className = 'xwup-verify-input';
__8.setAttribute('type', 'text', 0);
__4.appendChild(__8);
var __9 = XWC.UI.createElement('DIV');
__9.className = 'xwup-verify-info';
__9.setAttribute('id', 'xwup_underinfo', 0);
var __10 = XWC.UI.createElement('DIV');
__10.className = 'info';
__10.appendChild(document.createTextNode(XWC.S.info1));
...
__12.className = 'xwup-buttons-layout';
var __13 = XWC.UI.createElement('BUTTON');
__13.setAttribute('tabindex', '4', 0);
__13.setAttribute('type', 'button', 0);
__13.setAttribute('id', 'xwup_ok', 0);
__13.onclick = function(event) {onOKButtonClick()};
__13.appendChild(document.createTextNode(XWC.S.button_ok));
__12.appendChild(__13);
var __14 = XWC.UI.createElement('BUTTON');
__14.setAttribute('tabindex', '4', 0);
__14.setAttribute('type', 'button', 0);
__14.setAttribute('id', 'xwup_cancel', 0);
__14.onclick = function(event) {onCancelButtonClick()};
__14.appendChild(document.createTextNode(XWC.S.button_cancel));
