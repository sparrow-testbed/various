...
<%
	...
	multilang_id.addElement("RT_001");
...
<%
	//HTML에 쓰일 변수들
	String TITLE = String.valueOf(text.get("RT_002.MSG_0105"));//"고객의소리 수정";
%>
...
function Update()
{
	...
	{
		//alert("제목은 필수 입력입니다.");
		alert("<%=text.get("RT_002.MSG_0103")%>");
		return;
	}
	
	if (LRTrim(F.GONGJI_GUBUN.value) == "") {
		alert("고객의소리 구분을 선택해주십시오.");
		F.GONGJI_GUBUN.focus();
	...
	{
		//alert("내용은 필수 입력입니다.");
		alert("<%=text.get("RT_002.5000")%>");
		return;
	}

    F.method = "POST";
    F.target = "child_Work1";
    F.action = "rpt_list_wk_upd_new.jsp?SEQ=<%=SEQ%>";

    F.submit();
    
    <%--
	var jqa = new jQueryAjax();
	jqa.action = "rpt_list_wk_upd_new.jsp";
...
function Cancel()
{
    if(!confirm("<%=text.get("RT_002.MSG_0104")%>"))//취소 하시겠습니까?
		return;

   	location.href = "rpt_list_new.jsp";
}
...
<%
	...
	SepoaOut value = ServiceConnector.doService(info, "MT_014", "CONNECTION","getQuery_RPT_POP_New", obj);
...
<% if(!COMPANY_CODE.equals(info.getSession("COMPANY_CODE"))){  %>
		...
		<TABLE cellpadding="0" cellspacing="0" border="0" width="100%">
         ...
         <table width="100%" border="0" cellspacing="0" cellpadding="1">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
		        	<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("RT_001.MSG_0102")%></td>
		            <td class="data_td">
			    ...
			    <tr>
		        	<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("RT_001.MSG_8801")%></td>
		            <td class="data_td">
			        	<select name="GONGJI_GUBUN" id="GONGJI_GUBUN" class="input_re">
							<option value=""></option>
							<%
								String lb1 = ListBox(request, "SL0018" , info.getSession("HOUSE_CODE")+"#"+"Z003", GONGJI_GUBUN);
								out.println(lb1);
							%>
						</select>
			      </td>
			    ...
			    <tr>
		        	<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("RT_001.MSG_0106")%></td>
		            <td class="data_td">
			    ...
			    <tr>
		        	<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("RT_001.gongji_gubun")%></td>
		            <td class="data_td">
						<select name="GONGJI_GUBUN" id="GONGJI_GUBUN" class="input_re">
								<%
                ...
                <tr>
		        	<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("RT_001.MSG_0103")%></td>
		            <td class="data_td">
						 <TABLE >
		      			<TR>
		      				<td><input type="text" name="ATTACH_TITLE" id="ATTACH_TITLE" class="inputsubmit" size="80" readonly value="<%=FILE_NAME%>"></td>
							<td><script language="javascript">btn("javascript:attach_file(document.forms[0].ATTACH_NO.value,'NOT')","<%=text.get("RT_002.0004")%>")</script></td>
