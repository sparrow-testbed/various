...
<%
	...
	multilang_id.addElement("SRQ_006");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "SRQ_006";
	String grid_obj  = "GridObj";
	boolean isSelectScreen = false;

%>
<% String WISEHUB_PROCESS_ID="SRQ_006";%>
...
<html>
	<head>
		...
		<%
		    ...
		    String send_flag	= JSPUtil.nullToEmpty(request.getParameter("send_flag"));
			...
			String CREATE_TYPE		 	 = "";
			String QTA_H_ATTACH_NO		 = "";
			String QTA_H_ATTACH_COUNT     = "";
		    ...
		    if(rw_cnt > 0) {
				...
				QTA_H_ATTACH_NO            = wf.getValue("ATTACH_NO  	      ", 0);
				QTA_H_ATTACH_COUNT		   = wf.getValue("ATTACH_CNT  	      ", 0);
 ...
 */
	...
	var servletUrl = "<%=POASRM_CONTEXT_NAME%>/servlets/supply.bidding.qta.qta_bd_upd";
...
//                    if(parseFloat(BEFORE_PRICE) < parseFloat(UNIT_PRICE_VALUE) )
        ...
        //if("<%=TERM_CHANGE_FLAG%>" == "Y") {
        //    DELY_TERMS      = wForm.DELY_TERMS.value;
        //    PAY_TERMS       = wForm.PAY_TERMS.value;
        //    CUR             = wForm.CUR.value;
        //    SHIPPING_METHOD = wForm.SHIPPING_METHOD.value;
        //    USANCE_DAYS     = wForm.USANCE_DAYS.value;
        //    ARRIVAL_PORT    = wForm.ARRIVAL_PORT.value;
        //} else {
	...
	function getApprovalSend() {
        ...
        var QTA_H_ATTACH_NO  = wForm.qta_h_attach_no.value;

        <%-- servletUrl = "<%=getSepoaServletPath("supply.bidding.qta.qta_bd_upd")%>"; --%>
        G_SERVLETURL = "<%=POASRM_CONTEXT_NAME%>/servlets/supply.bidding.qta.qta_bd_upd";
        mode = "setQtUpdate";
        $("#pre_send_flag").val("<%=send_flag%>");
		$("#qta_no"       ).val("<%=QTA_NO%>");
 ...
 */
    ...
    /* function setAttach(attach_key, arrAttrach, rowId, attach_count) {
		if(document.form1.attach_gubun.value == "wise"){
		    ...
		    f.qta_attach_no.value = attach_key;
		    f.qta_attach_count.value = attach_count;
		    f.sign_attach_no2.value = attach_key;
		    f.sign_attach_no_count2.value = attach_count;
		}
		document.form1.attach_gubun.value="body";
    }
 */
    ...
    function setPriceInfo(comp_s_date, comp_e_date, comp_s_qty, comp_e_qty, comp_unit_price, max_unitprice)
    {
        ...
        document.forms[0].t_from_date.value =  GD_GetCellValueIndex(GridObj,G_Pos, INDEX_EP_FROM_DATE);
        document.forms[0].t_to_date.value =    GD_GetCellValueIndex(GridObj,G_Pos, INDEX_EP_TO_DATE);
        document.forms[0].t_from_qty.value =   GD_GetCellValueIndex(GridObj,G_Pos, INDEX_EP_FROM_QTY);
        document.forms[0].t_to_qty.value =     GD_GetCellValueIndex(GridObj,G_Pos, INDEX_EP_TO_QTY);
        document.forms[0].t_unit_price.value = GD_GetCellValueIndex(GridObj,G_Pos, INDEX_UNIT_PRICE);
...
//                     return;
//                 } else {
                    location.href= "/s_kr/bidding/qta/qta_bd_lis1.jsp";
//                 }
            }
        }
        ...
        if(msg1 == "doQuery") {
            ...
            for(i=0; i<maxRow; i++) {
            	...
            	//var INDEX_ITEM_QTY            	= GridObj.getColIndexById("ITEM_QTY");			//수량
            	//var INDEX_CUSTOMER_PRICE 		= GridObj.getColIndexById("CUSTOMER_PRICE");	//ListPrice 단가
            	
            	//var INDEX_SUPPLY_PRICE 		= GridObj.getColIndexById("SUPPLY_PRICE");			//ListPrice 단가
            	
            	customer_unitPrice = GridObj.cells(GridObj.getRowId(i),INDEX_CUSTOMER_PRICE).getValue(); //GD_GetCellValueIndex(GridObj,i, INDEX_CUSTOMER_PRICE);
				supply_unitPrice = GridObj.cells(GridObj.getRowId(i),INDEX_SUPPLY_PRICE).getValue(); //GD_GetCellValueIndex(GridObj,i, INDEX_SUPPLY_PRICE);

				
            	if(customer_unitPrice > 0){
        	    		calculate_grid_amt(GridObj, i, INDEX_ITEM_QTY, INDEX_CUSTOMER_PRICE, "1", "CUSTOMER_AMT");
            	}
            	if(supply_unitPrice > 0){
        	    		calculate_grid_amt(GridObj, i, INDEX_ITEM_QTY, INDEX_SUPPLY_PRICE, "1", "ITEM_AMT");
            	}
	...
	<%}else{%>
                Arow = msg2;
			        if(QTA_ATTACH_NO_VALUE == "PF"){
					    var QTA_NO		= "<%=QTA_NO		%>";
					    var RFQ_NO		= "<%=RFQ_NO		%>";
					    var RFQ_COUNT	= "<%=RFQ_COUNT	%>";
					    var RFQ_SEQ     = GD_GetCellValueIndex(GridObj,msg2, INDEX_RFQ_SEQ);
						var loc2 = "qta_pp_ins1.jsp?XPosition="+ msg2 + "&QTA_ATTACH_NO="+QTA_ATTACH_NO_VALUE+ "&QTA_NO="+QTA_NO+ "&RFQ_NO="+RFQ_NO+ "&RFQ_COUNT="+RFQ_COUNT+ "&RFQ_SEQ="+RFQ_SEQ;
...
function calculate_tot_amt()
{
            ...
            var t_discount = 0;
				...
				if( c_amt > 0 && s_amt > 0 )  t_discount = (eval(c_amt) - eval(s_amt))/ eval(c_amt);
                else t_discount = 0;
...
var myDataProcessor = null;
    ...
    {
    	GridObj_setGridDraw();
    	GridObj.attachHeader("#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,단가,합계,단가,합계,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan");
...
function doOnCellChange(stage,rowId,cellInd)
{
    ...
    if(stage==0) {
    	...
    	var INDEX_SUPPLY_PRICE 		= GridObj.getColIndexById("SUPPLY_PRICE");			//ListPrice 단가
    	var INDEX_SUPPLY_AMT          = GridObj.getColIndexById("SUPPLY_AMT");			//ListPrice 금액

    	var rowIndex                 = GridObj.getRowIndex(GridObj.getSelectedId());
    	
    	if(cellInd == INDEX_CUSTOMER_PRICE){
	    	calculate_grid_amt(GridObj, rowIndex, INDEX_ITEM_QTY, INDEX_CUSTOMER_PRICE, "1", "CUSTOMER_AMT");
    	}else if(cellInd == INDEX_SUPPLY_PRICE){
	    	calculate_grid_amt(GridObj, rowIndex, INDEX_ITEM_QTY, INDEX_SUPPLY_PRICE, "1", "SUPPLY_AMT");
    	}
    	return true;
    }
...
function setAttach(attach_key, arrAttrach, rowId, attach_count) {
	...
	$("#qta_h_attach_no").val(attach_key);
	$("#qta_h_attach_no_count").val(attach_count);
...
<form id="form1" name="form1" action="">
	...
	<input type="hidden" name="st_vendor_name"   id="st_vendor_name"         size="30" class="input_data1" value="<%=VENDOR_NAME%>">
	<input type="hidden" name="shipper_type"     id="shipper_type"           value="<%=SHIPPER_TYPE%>">
	<input type="hidden" name="send_flag"        id="send_flag"              >
	<input type="hidden" name="pre_send_flag"    id="pre_send_flag"          >
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;
				...
				<td width="35%" class="data_td">
					<input type="text" name="rfq_no" id="rfq_no" size="20" value="<%=RFQ_NO%>" >
			</td>
			<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;
				 차수
			</td>
			<td width="35%" class="data_td">
				<input type="text" name="rfq_count" id="rfq_count" size="5" value="<%=RFQ_COUNT%>" >
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td width="15%" class="title_td">
				<img src="/images/<//%=info.getSession("HOUSE_CODE")%>/ibk_s_arr.gif" width="9" height="9"> 비교방식
			</td>
			<td width="35%" class="data_td">
				<//%=SETTLE_TYPE_TEXT%>&nbsp;
			</td>
		...
		<tr>
			...
			<td width="35%" class="data_td" colspan="3">
				<textarea name="rfq_remark" id="rfq_remark" style="width:98%;" rows="5" readonly><%=RFQ_REMARK %></textarea>
			</td>
		</tr>
		<tr>
			<td colspan="4" height="1" bgcolor="#dedede"></td>
		...
		<tr>
			<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;
				...
				<td width="35%" class="data_td" colspan="3">
				<textarea name="remark" id="remark" style="width:98%;" rows="5" onKeyUp="return chkMaxByte(4000, this, '특기사항');"><%=REMARK %></textarea>
			</td>
		</tr>
		<tr>
			<td colspan="4" height="1" bgcolor="#dedede"></td>
		...
		<tr>
			...
			<td class="data_td" colspan="3">
				<table>
					<tr>
						<td>
							<script language="javascript">btn("javascript:goAttach($('#qta_h_attach_no').val())", "파일등록")</script>
						</td>
						<td>
							<input type="text" size="3" readOnly class="input_empty" name="qta_h_attach_no_count" id="qta_h_attach_no_count" value="<%=QTA_H_ATTACH_COUNT%>"/>
							<input type="hidden"  name="qta_attach_no" id="qta_h_attach_no" value="<%=QTA_H_ATTACH_NO%>">
						</td>
					</tr>
				</table>
			</td>
...
<s:grid screen_id="SRQ_006" grid_obj="GridObj" grid_box="gridbox"/>
<s:footer/>
</body>
</html>
