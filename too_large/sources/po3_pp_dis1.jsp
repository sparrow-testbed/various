...
<%
	...
	multilang_id.addElement("PO_002_1");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "PO_002_1";
...
<% String WISEHUB_PROCESS_ID="PO_002_1";%>
<%
    String inv_no          = JSPUtil.nullToEmpty(request.getParameter("po_no"));
	//발주구분 : 직발주(D), 품의발주(N), 메뉴얼발주(M), 카테고리 발주(C)
	String prm_po_type = JSPUtil.nullToRef(JSPUtil.nullToEmpty(request.getParameter("po_type")), "N") ;


	if ("".equals(inv_no) || "GET".equals(request.getMethod())) {
		%>
		<script>
...
<%
        ...
        String CONTRACT_FROM_DATE_1	= "";
		String CONTRACT_TO_DATE_1	= "";
		String CONTRACT_NO			= "";
		String PO_TYPE				= "";
		String ATTACH_NO			= "";
		...
		if(wf.getRowCount() > 0) {
			...
			REMARK			    = wf.getValue("REMARK"    		, 0).replaceAll("\r", "").replaceAll("\n", "");
			...
			CONTRACT_FROM_DATE_1= wf.getValue("CONTRACT_FROM_DATE_1", 0);
			CONTRACT_TO_DATE_1  = wf.getValue("CONTRACT_TO_DATE_1"	, 0);
		...
		//////////////////ClipReport4 선언부 시작///////////////////////////////////////////////////////////
		String _rptName          = "020644/rpt_po3_pp_dis1"; //리포트명
		...
		_rptData.append(_RF);
		_rptData.append(SepoaString.getDateSlashFormat(PO_CREATE_DATE));
		_rptData.append(_RF);
		...
		if(prm_po_type != null && prm_po_type.length() > 0){
			...
			if(!prm_po_type.equals("D") && !prm_po_type.equals("M") && !prm_po_type.equals("C") && !prm_po_type.equals("E")){
				_rptData.append(SepoaString.getDateSlashFormat(CONTRACT_FROM_DATE_1));
				_rptData.append(" ~ ");
				_rptData.append(SepoaString.getDateSlashFormat(CONTRACT_TO_DATE_1));
				_rptData.append(_RF);
				_rptData.append(delay_remark);
				_rptData.append(_RF);
				_rptData.append(warranty_month);
				_rptData.append(_RF);
				_rptData.append(first_percent);				
				_rptData.append(_RF);
				_rptData.append(contract_percent);
				_rptData.append(_RF);
				_rptData.append(mengel_percent);				
			}else{
				_rptData.append(_RF);
				_rptData.append(_RF);
				_rptData.append(_RF);
		...
		if(wf2 != null) {
			if(wf2.getRowCount() > 0) { //데이타가 있는 경우
				for(int i = 0 ; i < wf2.getRowCount() ; i++){
					...
					_rptData.append(_RF);			
					_rptData.append(wf2.getValue("REMARK", i).replaceAll("\r", "").replaceAll("\n", ""));
					_rptData.append(_RL);			
				}
			}
		}
...
// 		GridObj.SetDateFormat("INPUT_TO_DATE"		,"yyyy/MM/dd");
	    ...
	    GridObj.setColumnLabel(IDX_ITEM_NO			, item_no			, 0);
		GridObj.setColumnLabel(IDX_DESCRIPTION_LOC	, description_loc	, 0);
		GridObj.setColumnLabel(IDX_PO_QTY			, po_qty			, 0);
		GridObj.setColumnLabel(IDX_DELY_TO_LOCATION	, dely_to_location	, 0);	
	...
	function doQuery()
	{
		...
		var param = "mode=getPoDetailLine&grid_col_id="+grid_col_id;
		    param += dataOutput();
	...
	function JavaCall(msg1, msg2, msg3, msg4, msg5)
	{
        if(msg1 == "doData") { // 전송/저장시 Row삭제
			...
			if(msg3 == IDX_ITEM_NO){
					...
					var item_no = GridObj.cells(GridObj.getRowId(msg2),msg3).getValue();
				//if(pr_type=="I")
				...
				var exec_no = GridObj.cells(GridObj.getRowId(msg2),msg3).getValue();
				if (exec_no != "") {
            		window.open("/kr/dt/app/app_bd_ins1.jsp?exec_no="+exec_no+ "&pr_type=I&editable=N&req_type=P","execwin","left=0,top=0,width=1024,height=768,resizable=yes,scrollbars=yes, status=yes");
				}
...
function doQueryEnd() {
    ...
    if(GridObj.GetRowCount() > 0){
    	...
    	document.form1.rptData.value = "<%=_rptData.append(_RD)%>" + GridObj.cells(GridObj.getRowId(0), GridObj.getColIndexById("DELY_TO_ADDRESS")).getValue();    	
    }
...
<s:header popup="true">
	<form id="form1" name="form1" method="post">
		...
		<input type="hidden" name="rptData" id="rptData" value="<%=_rptData.toString().replaceAll("\"", "&quot").replaceAll("\r", "").replaceAll("\n", "")%>">
								...
								<table width="100%" border="0" cellspacing="0" cellpadding="0">
									<tr>
										...
										<td width="35%" class="data_td">
											<input type="text" name="PO_NO" value="<%=po_no%>" readOnly style="width:92%;">
										</td>
										<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;발주명</td>
										<td width="35%" class="data_td">
											<input type="text" name="SUBJECT" value="<%=SUBJECT%>" readOnly style="width:92%;">
										</td>
    								</tr>
    								<tr>
									...
									<tr>
										...
										<td width="35%" class="data_td">
											<input type="text" name="VENDOR_CODE" value="<%=VENDOR_CODE%>" readOnly style="width:32%;">
											<input type="text" name="VENDOR_NAME" value="<%=VENDOR_NAME%>" readOnly style="width:59%;">
										</td>
										<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;발주일자</td>
										<td width="35%" class="data_td">
											<input type="text" name="PO_CREATE_DATE" size="20" value="<%=SepoaString.getDateSlashFormat(PO_CREATE_DATE)%>" readOnly style="border:0">
										</td>
									</tr>
									<tr>
										<td colspan="4" height="1" bgcolor="#dedede"></td>
									...
									<tr>
										...
										<td width="35%" class="data_td">
											<input type="text" name="EXEC_AMT_KRW" value="<%=EXEC_AMT_KRW%>" readOnly style="width:95%;">
										</td>
									</tr>
									<tr>
										<td colspan="4" height="1" bgcolor="#dedede"></td>
									...
									<tr>
										...
										<td width="35%" class="data_td">
											<input type="text" name="TAKE_USER_NAME" value="<%=TAKE_USER_NAME%>" readOnly style="width:95%;">
										</td>
...
%>
									...
									<tr>
										...
										<td width="35%" class="data_td">
											<%=SepoaString.getDateSlashFormat(CONTRACT_FROM_DATE_1)%>&nbsp;~&nbsp;<%=SepoaString.getDateSlashFormat(CONTRACT_TO_DATE_1)%>
										</td>
										<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;지체상금율(%)</td>
										<td width="35%" class="data_td"><%=delay_remark  %></td>
									</tr>
									<tr>
										<td colspan="4" height="1" bgcolor="#dedede"></td>
									</tr>
									<tr>
										...
										<td width="35%" class="data_td">검수일로부터 ( <%=warranty_month%> )개월</td>
...
%>
									...
									<tr>
								      	...
								      	<td class="data_td" id="dely_address_loc" colspan=3></td>
								    </tr>
    								<tr>
										<td colspan="4" height="1" bgcolor="#dedede"></td>
									</tr>
...
<%-- <s:grid screen_id="PO_002_1" grid_obj="GridObj" grid_box="gridbox"/> --%>
