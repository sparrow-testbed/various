...
<% String WISEHUB_PROCESS_ID="I_p0010"; %>
...
<%
	...
	String VENDOR_ID = JSPUtil.nullToEmpty(JSPUtil.CheckInjection3(request.getParameter("USER_ID")));	
	String VENDOR_PASSWORD = JSPUtil.nullToEmpty(pluginfreeRequest.getParameter("PASSWORD"));
	String CONFIRM_PASSWORD = JSPUtil.nullToEmpty(pluginfreeRequest.getParameter("CONFIRM_PASSWORD"));
	
	if(!"".equals(VENDOR_PASSWORD)) {		
		if(!VENDOR_PASSWORD.equals(CONFIRM_PASSWORD)) {
			out.print("<script>");
			out.print("alert('새 패스워드 두개가 일치하지 않습니다.');");
			out.print("</script>");    	 
			return;
		}
		
		String msgValidPwd = pwdPolicy.isNewValidPwd(VENDOR_ID, VENDOR_PASSWORD); 
	    if(!"".equals(msgValidPwd)){
	    	out.print("<script>");
			out.print("alert('"+msgValidPwd+"');");
			out.print("</script>");    	   
	    	return;
	    }
	}
...
//	String VENDOR_PASSWORD			= JSPUtil.nullToEmpty(request.getParameter("PASSWORD"));
	...
	String IRS_NO                 	= JSPUtil.nullToEmpty(request.getParameter("ENTRY_TEL")               );
	...
	Logger.err.println("ihstone : mode=" + mode);
	Logger.err.println("ihstone : irs_no=" + irs_no);
	Logger.err.println("ihstone : flag=" + FLAG);
	if("irs_no".equals(mode)){
		...
		String resident_no	= JSPUtil.nullToEmpty(request.getParameter("resident_no"));
			   RESIDENT_NO1	= resident_no.substring(0,6);
			   RESIDENT_NO2 = resident_no.substring(6);
	}
	
	String PHONE_NO1              = JSPUtil.nullToEmpty(request.getParameter("PHONE_NO1"));
	String EMAIL                  = JSPUtil.nullToEmpty(request.getParameter("ENTRY_EMAIL"));
	...
	String PHONE_NO2              = JSPUtil.nullToEmpty(request.getParameter("ENTRY_TEL"));
...
// 	String BANK_ACCT              = JSPUtil.nullToEmpty(KeyboardSecurity.getTouchEnKey(request, "BANK_ACCT"));
	...
	String PHONE_NO1_O		 = JSPUtil.nullToEmpty(request.getParameter("ENTRY_TEL_O"));
	...
	String PHONE_NO2_O		 = JSPUtil.nullToEmpty(request.getParameter("ENTRY_TEL_O"));
	...
	if("".equals(VENDOR_CODE)) {
		if("irs_no".equals(mode)) {
			...
			Object[] params= {};
			SepoaOut wo = ServiceConnector.doService(info, "I_p0010", "TRANSACTION","getMaxVedorCode", params);
...
// 	String RES = enc.encrypt(RESIDENT_NO1+ RESIDENT_NO2); // 암호화
	String RES = RESIDENT_NO1+ RESIDENT_NO2; // 암호화
...
// 	String VENDOR_PASSWORD_ENC = encrypt.getEncryptData();
	...
	String[][] args_vncp = new String[1][];
	String[][] args_vngl_old = new String[1][];
	Object[] obj = null;
	String strMethodName = "setInsert_icomvngl_j";
	...
	if(info == null){
		Logger.err.println("user null value");
	}
	
	SepoaOut value = ServiceConnector.doService(info, "I_p0010", "TRANSACTION",strMethodName, obj);
