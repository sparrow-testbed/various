...
<%
    String screen_id = (String)request.getAttribute("SCREEN_ID");
	String grid_obj = (String)request.getAttribute("GRID_OBJ");
	String grid_box = (String)request.getAttribute("GRID_BOX");
	boolean isSelectScreen = (Boolean)request.getAttribute("SELECT_SCREEN");
	boolean isRowsMergeable = (Boolean)request.getAttribute("ROW_MERGEABLE");
		
	String POASRM_CONTEXT_NAME = (String)request.getAttribute("POASRM_CONTEXT_NAME");
	Vector dhtmlx_read_cols_vec = (Vector)request.getAttribute("DHTMLX_READ_COLS_VEC");
	Vector dhtmlx_back_cols_vec = (Vector)request.getAttribute("DHTMLX_BACK_COLS_VEC");
	String col_del = (String)request.getAttribute("COL_DEL");
	String row_del = (String)request.getAttribute("ROW_DEL");
		
	SepoaInfo info = (SepoaInfo) request.getAttribute("SEPOA_INFO");
	if(info == null) {
	...
	for(int depth=0; depth < sf.getRowCount(); depth++)
	{
    	...
    	if(screen_id.equals(sf.getValue("SCREEN_ID", depth)) && sf.getValue("TYPE", depth).equals("B"))
    	{
    		...
    		grid_header_style  = grid_header_style + "'background-color:#dddee1;height: 34px;',";
    		...
    		if(grid_column_type.equals("coro") && sf.getValue("COL_COMBO", depth).length() > 0) {
    			grid_combo_data += " doRequestUsingPOST_MultidhtmlxGrid( " + sf.getValue("COL_COMBO", depth) + ", '', "+grid_obj+" ); \n";
    		}
...
<script language='JavaScript'>
    var <%=grid_obj %>_grid_col_id = '<%=grid_col_id%>';
    var <%=grid_obj %>_col_ids = '<%=grid_col_id%>';
    ...
    function <%=grid_obj%>_doOnMouseRightButtonClick(menuitemId)
    {
        ...
        switch(menuitemId) {
            ...
            case "ColFrozen":
                ...
                break;
            case "ExcelSave":
            	<%=grid_obj%>_gridExport(<%=grid_obj%>);
                break;
            case "PrintView":
            	<%=grid_obj%>_PrintView(<%=grid_obj%>);
                break;
            case "Undo":
                <%=grid_obj%>.doUndo();
                break;
	...
	function <%=grid_obj%>_doOnPageChanged(cur_page, first_page, last_page)
	{
		...
		if(<%=grid_obj%>.getRowsNum() <= <%=page_count%>) {
			dhtmlx_end_row_id = <%=grid_obj%>.getRowsNum();
			dhtmlx_cur_page_last_idx = <%=grid_obj%>.getRowsNum();
		} else {
			dhtmlx_cur_page_last_idx = cur_page * <%=page_count%>;
		}

		if(dhtmlx_end_row_id > <%=grid_obj%>.getRowsNum()) {
			dhtmlx_end_row_id = <%=grid_obj%>.getRowsNum();
		}
	...
	function <%=grid_obj%>_gridExport(mygrid)
	{
	    var html="";
	    var headText="";
	    var gridColids="";
	...
	function <%=grid_obj%>_PrintView(mygrid)
	{
	    var html="";
	    var headText="";
	    var gridColids="";
	...
	function <%=grid_obj%>_doQueryModalEnd(<%=grid_obj%>, RowCnt)
	{
		...
		if(prg_win == null) {
			...
			prg_win.button("park").hide();
			dhxWins.window("prg_win").setModal(true);
			prg_win.attachURL(_POASRM_CONTEXT_NAME + "/common/progress_ing.htm");
		}
		...
		dhtmlx_last_row_id = <%=grid_obj%>.getRowsNum();
		return true;
	}
