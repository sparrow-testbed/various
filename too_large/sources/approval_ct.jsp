    ...
    int approvalCnt = wf_sign.getRowCount()-wf_agree.getRowCount() > 0 ?  wf_sign.getRowCount() : wf_sign.getRowCount()-wf_agree.getRowCount() == 0 ? wf_sign.getRowCount() :  wf_agree.getRowCount();   //결재라인수
 	for(int i = 0 ; i<approvalCnt; i++) {
		if(i < wf_sign.getRowCount()){
			if("A".equals(wf_sign.getValue("APP_STATUS", i))) { //기안
				add_date = ("".equals(add_date))?wf_sign.getValue("SIGN_DATE", i):add_date;
				add_user_name = ("".equals(add_user_name))?wf_sign.getValue("USER_NAME_LOC", i):add_user_name;
			}
		}
 	}
...
<form name="form1" method="post">
  ...
  <td width="1%" valign="top" ></td>
    <td width="40%" valign="top" ><table width="90%" height="92" border="0" cellpadding="0" cellspacing="0">
	  		...
	  		<tr>
        		<td class="title"> 계&nbsp;약&nbsp;번&nbsp;호&nbsp;(차수)&nbsp;:&nbsp;&nbsp;&nbsp;<%=doc_no%> (<%=doc_seq%>)</td>
      		</tr>
      		<tr>
      		...
      		<tr>
        		<td class="title"> 계&nbsp;&nbsp;&nbsp;&nbsp;약&nbsp;&nbsp;&nbsp;&nbsp;명&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp;<%=subject%></td>
      		</tr>
      <tr>
        <td height="1" bgcolor="#000000"></td>
      </tr>
...
if("CD".equals(ct_flag) && "E".equals(sign_status)){
	_rptAprvData.append("계 약 품 의 완 료"); 
}else{
	_rptAprvData.append("계 약 품 의");
}     	

_rptAprvData.append(_RD);	

_rptAprvData.append("계약번호 (차수)");
_rptAprvData.append(_RF);
_rptAprvData.append(doc_no);
_rptAprvData.append(" (");
_rptAprvData.append(doc_seq);
_rptAprvData.append(")");
...
<%-- 		      		<TD><script language="javascript">btn("javascript:changeApprovalLine('<%=doc_type%>','<%=doc_no%>','<%=doc_seq%>','<%=sign_path_seq%>')",21,"결재선추가")</script></TD> --%>
		      	<% } %>
					...
					<TD style="display: none;"><script language="javascript">btn("javascript:RFQComparison()","소싱비교")</script></TD>
	      			<TD style="display: none;"><script language="javascript">btn("javascript:goIReport()","출 력")</script></TD>
...
<iframe id="ctFrm" name="ctFrm" src="/contract/contract_approval_detail.jsp?cont_no=<%=doc_no %>&cont_gl_seq=<%=doc_seq %>&cont_form_no=<%=doc_no %>&ct_flag=<%=ct_flag%>&sign_status=<%=sign_status%>" style="width: 100%;height: 740px; border: 0px;" frameborder="0" scrolling="auto"></iframe>
