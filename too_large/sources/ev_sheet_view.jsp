...
<Script language="javascript">
	...
	function doQuery(){
		...
		GridObj.post("<%=POASRM_CONTEXT_NAME%>/servlets/ev.ev_sheet_view?mode=query&grid_col_id="+grid_col_id+ param);
		GridObj.clearAll(false); --%>

		var G_SERVLETURL = "<%=POASRM_CONTEXT_NAME%>/servlets/ev.ev_sheet_view";
    ...
    function doQueryEnd(GridObj, RowCnt){
		...
		if(RowCnt>0) {
			GridObj.cells(GridObj.getRowId(0), GridObj.getColIndexById("LEVEL")).setValue(GridObj.getRowsNum());
			rid_rowspan("1-3,6-6", "EV_M_ITEM,EV_D_ITEM");
		}
	...
	function doInsert(){
		...
		var grid_col_id = "<%=grid_col_id%>";
		 ...
		 if (confirm("<%=text.get("MESSAGE.1018")%>")) {
            ...
            var cols_ids = "<%=grid_col_id%>";
		    myDataProcessor = new dataProcessor("<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.admin.ev_sheet_view?cols_ids="+cols_ids+"&mode=insert"+param);
  ...
  <%--   function initAjax(){
		...
		doRequestUsingPOST( 'SL0018', 'M218' ,'period', '<%=period%>' );
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td align="left" class="title_page">심사표상세화면</td>
		</tr>
	</table>
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<tr>
		<td height="5"></td>
		<input type="hidden" name="ev_year_p"  id="ev_year_p"  size="15" readonly="readonly" class="input_empty" value="<%=ev_year%>" >
    </tr>
    <tr>
    	<td width="100%" valign="top">
			<table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#DBDBDB">
		    <tr>
        		<td width="20%" height="24" class="se_cell_title"><%=text.get("WO_017.sheet_num")%></td>
				<td width="30%" height="24" class="se_cell_data">
					<input type="text" name="ev_no"  id="ev_no"  size="15" maxlength="15" class="input_empty" value="<%=ev_no %>" readonly="readonly">
				 </td>
				<td width="20%" height="24" class="se_cell_title"><%=text.get("WO_017.s_group")%></td>
				<td width="30%" height="24" class="se_cell_data">
					<input type="button" value="연결업체 소싱그룹팝업" class="input_btn" onclick="sg_pop();"/>
				</td>
			</tr>
		    <tr>
        		<td width="20%" height="24" class="se_cell_title"><%=text.get("WO_017.kind")%></td>
				<td width="30%" height="24" class="se_cell_data">
				...
				<td width="20%" height="24" class="se_cell_title"><%=text.get("WO_017.v_group")%></td>
				<td width="30%" height="24" class="se_cell_data">
					<select name="sg_kind" id="sg_kind" class="inputsubmit" <%=dis_view%>>
					<option value="">
								전체
							</option>
							<%
		    ...
		    <tr>
        		<td width="20%" height="24" class="se_cell_title"><%=text.get("WO_017.time")%></td>
				<td width="30%" height="24" class="se_cell_data">
					<select name="period" id="period" class="inputsubmit" <%=dis_view%>>
					<%
							 String lp = ListBox(request, "SL0018" ,house_code+"#M218#", "");
							 out.println(lp);
							 %>
					</select>
				</td>
				<td width="20%" height="24" class="se_cell_title"><%=text.get("WO_017.apply_yn")%></td>
				<td width="30%" height="24" class="se_cell_data">
					<input type="checkbox" name="use_flag" id="use_flag" class="inputsubmit"  <%=use_flag %> <%=dis_view%>>
				</td>
			</tr>
		    <tr>
        		<td width="20%" height="24" class="se_cell_title"><%=text.get("WO_017.status")%></td>
				<td width="30%" height="24" class="se_cell_data">
					<select name="sheet_status" id="sheet_status" class="inputsubmit" <%=dis_view%>>
					<%
				...
				<td width="20%" height="24" class="se_cell_title"><%=text.get("WO_017.conf_score")%></td>
				<td width="30%" height="24" class="se_cell_data">
					<input type="text" name="accept_value"  id="accept_value"  size="15" maxlength="15" class="input_empty" value="<%=accept_value %>" >
				</td>
			</tr>
		    <tr>
        		<td width="20%" height="24" class="se_cell_title"><%=text.get("WO_017.year")%></td>
				<td width="30%" height="24" class="se_cell_data">
					<input type="text" name="ev_year" id="ev_year"  size="15" maxlength="15" class="input_empty" value="<%=ev_year %>" >
				</td>
				<td width="20%" height="24" class="se_cell_title"><%=text.get("WO_017.exi_shheet_num")%></td>
				<td width="30%" height="24" class="se_cell_data">
					<input type="text" name="aaaaa"  id="aaaaa"  size="15" maxlength="15" class="input_empty" value="" >
				</td>
			</tr>
		    <tr>
        		<td width="20%" height="24" class="se_cell_title"><%=text.get("WO_017.start_date")%></td>
				<td width="30%" height="24" class="se_cell_data">
					<input type="text" name="st_date" id="st_date"   size="15" maxlength="15" class="input_empty" value="<%=SepoaString.getDateSlashFormat(st_date) %>" >
				</td>
				<td width="20%" height="24" class="se_cell_title"><%=text.get("WO_017.ends_date")%></td>
				<td width="30%" height="24" class="se_cell_data">
					<input type="text" name="end_date" id="end_date"  size="15" maxlength="15" class="input_empty" value="<%=SepoaString.getDateSlashFormat(end_date) %>" >
				</td>
			</tr>
		    <tr>
        		<td width="20%" height="24" class="se_cell_title"><%=text.get("WO_017.title")%></td>
				<td height="24" class="se_cell_data" colspan="3">
					<input type="text" name="subject" id="subject" style="width:500px;" class="input_empty" value="<%=subject %>" >
				</td>
			</tr>																		
			</table>
		  	...
		  	<tr>
				...
				<table cellpadding="2" cellspacing="0">
				    <tr>
						<td >
				    	<%if(!"I".equals(flag)){ %>
							<script language="javascript">btn("doInsert()", "<%=text.get("BUTTON.sheet_perfect")%>")</script>
						<%} %>
						</td>
					</tr>
			    </table>
