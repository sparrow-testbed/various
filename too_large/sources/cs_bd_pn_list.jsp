...
<%
	...
	multilang_id.addElement("PN_001");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "PN_001";
...
<!--
	var	servletUrl = "<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.sourcing.cs_bd_pn_list";
...
// 		GridObj.SetNumberFormat("PAY_AMT"		,G_format_amt);
		...
		*/
		...
		doRequestUsingPOST('SL0150', '0#1', 'group1_code', '');
	}
	...
	function doQuery()
	{
		...
		var param = "mode=getCsBdPnList&grid_col_id="+grid_col_id;
		param += dataOutput();
...
function doOnRowSelected(rowId,cellInd)
{
	...
	var url = '';
	var param = '';
	if( header_name == "VENDOR_NAME_LOC" ) {
	...
	}else if(header_name == "DS_TCN" || header_name == "BID_TCN" || header_name == "NML_BID_TCN" || header_name == "NOT_NML_BID_TCN"){    	
		var vendor_code = SepoaGridGetCellValueId(GridObj, rowId, "VENDOR_CODE");
		var vendor_name_loc = SepoaGridGetCellValueId(GridObj, rowId, "VENDOR_NAME_LOC");
        ...
        var url    = '/sourcing/cs_bd_pn_desc_list.jsp';
		var title  = '공사입찰패널티부과검토상세';
		var param  = 'popup=Y';
		param     += '&vendor_code=' + vendor_code;
		param     += '&vendor_name_loc=' + vendor_name_loc;
		param     += '&pn_yy=' + pn_yy;
		param     += '&pn_ud=' + pn_ud;
		param     += '&pn_ud_loc=' + pn_ud_loc;
		 popUpOpen01(url, title, '1400', '768', param);	                   
    }
...
function bd_yy_Changed(){
	...
	$("select#hy_ds").append("<option value=''>선택</option>");
...
function group1_code_Changed(){
	...
	doRequestUsingPOST('SL0150', form1.group1_code.value+'#2', 'group2_code', '');
}
...
<form name="form1" >
	...
	%>
        ...
        <td width="25%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp; 반기&nbsp;&nbsp;    	
				<select name="hy_ds" id="hy_ds" class="inputsubmit" onChange="javascript:hy_ds_Changed();">
					<option value=''>선택</option>
					<option value="0106" <%=selected0106%>>상반기</option>
                    <option value="0712" <%=selected0712%>>하반기</option>								
				</select>
		 </td>
		 <td width="25%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp; 공종&nbsp;&nbsp;      	
				<select name="group1_code" id="group1_code" class="inputsubmit" onChange="javascript:group1_code_Changed();">
					<option value=''>전체</option>					
				</select>
		 </td>      	 
        <td width="25%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp; 공사업체 등급&nbsp;&nbsp;     	
...
<s:grid screen_id="PN_001" grid_obj="GridObj" grid_box="gridbox"/>
<s:footer/>
</body>
</html>
