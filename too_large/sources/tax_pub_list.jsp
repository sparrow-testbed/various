...
<%
	...
	multilang_id.addElement("TX_002");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "TX_002";
...
// 		GridObj.SetNumberFormat("PAY_AMT"		,G_format_amt);
	...
	function doSelect()
	{
		...
		var param = "mode=getTrList&grid_col_id="+grid_col_id;
		param += dataOutput();
		var url = "<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.tax.tax_pub_list";
	...
	function insert_tax(){
		...
		for(var i=0;i<iRowCount;i++)
		{
			...
			{
// 				if(GridObj.GetCellValue("JOB_STATUS", i) != ""){
// 					alert("선택하신 거래명세서 [" + GridObj.GetCellValue("PAY_NO", i) +  "] 는 이미 처리되었습니다.");
// 					return;
// 				}

				if(GridObj.GetCellValue("TAX_TGT_YN", i) != '발행대상'){
					alert("세금계산서 발행대상 건이 아닙니다. ");
					retun;
				}
				
				if(Number(GridObj.GetCellValue("ITEM_AMT", i)) == 0){
					alert((i+1) + "행의 검수완료 금액이 0원 입니다.\n0원인 검수완료건은 세금계산서를 발행할 수 없습니다.");
					return;
				}


				
				for(var j = i+1 ; j < iRowCount ; j++){
					if(GD_GetCellValueIndex(GridObj,j,IDX_SEL) == "1")
					{
						if(GridObj.GetCellValue("COMPANY_CODE", i) !=  GridObj.GetCellValue("COMPANY_CODE", j)){
							alert("세금계산서 발행은 같은 업체끼리만 할수 있습니다.");
							return;
						}
					}
				}
				...
				pay_no += GridObj.GetCellValue("INV_NO", i) + ",";
				pay_seq += GridObj.GetCellValue("INV_SEQ", i) + ","
			}
		}
...
// 		}
    	...
    	location.href="/kr/tax/tax_pub_ins.jsp?inv_no="+pay_no+"&inv_seq="+pay_seq;
...
function tax_tgt( yn ){
	...
	if(confirm((yn == 'N')?"세금계산서 발행대상에서 제외하시겠습니까?":"세금계산서 발행대상으로 환원하시겠습니까?") == 1) {
		...
		myDataProcessor = new dataProcessor("<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.tax.tax_pub_list" + params);
...
<form name="form1" >
    ...
    <tr>
      <td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp; 검수일자</td>
...
<s:grid screen_id="TX_002" grid_obj="GridObj" grid_box="gridbox"/>
