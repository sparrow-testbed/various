...
<%
	...
	multilang_id.addElement("I_AP_011");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "I_AP_011";
...
function RFQComparison(){
	...
	*/
	...
	//type  입찰 : BID_ICT   견적 RFQ  역경매 RAT
...
<form name="form1" method="post">
  ...
  <td width="1%" valign="top" ></td>
    <td width="29%" valign="top" ><table width="90%" height="92" border="0" cellpadding="0" cellspacing="0" style="display:none;">
...
<%
    ...
    SepoaOut value_sign = ServiceConnector.doService(info,"I_AP_001","CONNECTION","getSignPath",obj_sign);
    SepoaFormater wf_sign = new SepoaFormater(value_sign.result[0]);

    Object[] obj_agree = {doc_no,doc_type,doc_seq}; //합의자
    SepoaOut value_agree = ServiceConnector.doService(info,"I_AP_001","CONNECTION","getSignAgree",obj_agree);
...
<%-- 		      		<TD><script language="javascript">btn("javascript:changeApprovalLine('<%=doc_type%>','<%=doc_no%>','<%=doc_seq%>','<%=sign_path_seq%>')",21,"결재선추가")</script></TD> --%>
		      	<% } %>
	      			...
	      			<TD><script language="javascript">btn("javascript:bidFrm.clipPrint('<%=_rptAprvData.toString().replaceAll("\"", "&quot")%>',<%=approvalCnt%>);","인 쇄")</script></TD>
...
<iframe id="bidFrm" name="bidFrm" src="/ict/sourcing/bd_ann_d_ict_<%=ANN_VERSION %>.jsp?BID_NO=<%=doc_no %>&BID_COUNT=<%=doc_seq %>&SCR_FLAG=D&BID_STATUS=AR&BID_TYPE=<%=BID_TYPE %>&ANN_VERSION=<%=ANN_VERSION %>&VIEW_TYPE=P&view_content=<%=view_content %>" style="width: 98%;height: 550px; border: 0px;" frameborder="0" scrolling="yes"></iframe>
