...
<%
	//HTML에 쓰일 변수들
	String TITLE = String.valueOf(text.get("I_MT_014.MSG_0105"));//"공지사항 수정";
%>
...
function Update()
{
	...
	{
		//alert("제목은 필수 입력입니다.");
		alert("<%=text.get("I_MT_014.MSG_0103")%>");
		return;
	}
	...
	{
		//alert("내용은 필수 입력입니다.");
		alert("<%=text.get("I_MT_014.5000")%>");
		return;
	}

    F.method = "POST";
    F.target = "child_Work1";
    F.action = "bulletin_list_wk_upd_new_ict.jsp?SEQ=<%=SEQ%>";
...
function Cancel()
{
    if(!confirm("<%=text.get("I_MT_014.MSG_0104")%>"))//취소 하시겠습니까?
		return;

   	location.href = "bulletin_list_new_ict.jsp";
}
...
<%
	...
	SepoaOut value = ServiceConnector.doService(info, "I_MT_014", "CONNECTION","getQuery_NOTICE_POP_New", obj);
		            ...
		            <td class="data_td">
						 <TABLE >
		      			<TR>
							...
							<td><script language="javascript">btn("javascript:attach_file(document.forms[0].ATTACH_NO.value,'NOT')","<%=text.get("I_MT_014.0004")%>")</script></td>
