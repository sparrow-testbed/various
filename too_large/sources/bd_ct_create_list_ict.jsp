...
<%
	...
	multilang_id.addElement("I_BD_031");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "I_BD_031";
    ...
    function JavaCall(msg1, msg2, msg3, msg4, msg5) {
	    if(msg1 == "doQuery"){
            ...
            if(msg3 == INDEX_ANN_NO) {
				...
				if(FINAL_FLAG == "N") {
					alert("적합업체로 선정되지 못한 업체는 가격공문을 하실수 없습니다.");
					return;
				}
                if(CONT_TYPE2 != "LP") {
                	if(PARTICIPATE_FLAG == "0"){
						alert("참가신청을 하지 않거나 공문 취소하신 건입니다.");
						return;
					}
                    if(SPEC_FLAG != "Y") {
                        alert("1차평가에서 부적합업체로 선정되어 가격공문을 하실수 없습니다.");
                        return;
                    }
...
var G_SERVLETURL   = "<%=POASRM_CONTEXT_NAME%>/servlets/ict.sepoa.svl.sourcing.bd_qt_confirm_ict";
...
{ 
	...
	if(header_name == "VENDOR") {
		...
		var vote_count = LRTrim(GridObj.cells(rowId, GridObj.getColIndexById("VOTE_COUNT")).getValue());
		var attach_cnt = LRTrim(GridObj.cells(rowId, GridObj.getColIndexById("ATTACH_CNT")).getValue());
		var attach_cnt2 = LRTrim(GridObj.cells(rowId, GridObj.getColIndexById("ATTACH_CNT2")).getValue());
		var attach_cnt3 = LRTrim(GridObj.cells(rowId, GridObj.getColIndexById("ATTACH_CNT3")).getValue());
		var crud = LRTrim(GridObj.cells(rowId, GridObj.getColIndexById("CRUD")).getValue());
		
		if(crud == "C" || (vote_count == "" && attach_cnt == "0" && attach_cnt2 == "0" && attach_cnt3 == "0") ){
			var url  = '';
			var title  = '';
			var param  = '';
			//window.open("/common/CO_008_ict.jsp?callback=getCtrlUser", "SP9113", "toolbar=no, location=no, directories=no, status=yes,menubar=no,scrollbars=yes,resizable=no,width=450,height=550,left=0,top=0");		
...
function doQtConfirm() { 
    ...
    var cols_ids = "<%=grid_col_id%>";
    var params = "mode=setBdQtConfirm";
      ...
      <td height="30">
        ...
        <table><tr>
          <td><script language="javascript">btn("javascript:doQuery()", "조 회")</script></td>
           <%-- 
          <td><script language="javascript">btn("javascript:doQtConfirm()", "확 인")</script></td>
          --%>
          </tr></table>  
        </div>
      </td>      
