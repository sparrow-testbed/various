...
<%
	...
	multilang_id.addElement("SU_103");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "SU_103";
...
<!-- PROCESS ID 선언 -->
<% String WISEHUB_PROCESS_ID="SU_103";%>
...
<%
	...
	SepoaOut value = null;
	SepoaRemote ws = null;
	String CLASS_GRADE_YN ;
	
	if(menu_profile_code1.equals("MUP100100001")){
		CLASS_GRADE_YN = "true";
	}else{
		CLASS_GRADE_YN = "false";
	}
	//String nickName		= "p0030";
	//String conType 		= "CONNECTION";
	//String MethodName 	= "getUserMenuProfile";
	...
	for(int i=0; i < wf.getRowCount(); i++) {
		menu_profile_code = wf.getValue("MENU_PROFILE_CODE", i);
	}
%>
...
<%
    ...
    try {
    	...
    	if (menu_profile_code.equals(admin_menu_profile_code))
    		admin_flag = true;
    	if (!admin_flag) {
...
<!--
	...
	function setHeader()
	{
		//GridObj.AddHeader("sel", 					"선택",				"t_checkbox",	10,	40,	true);
		
		//GridObj.AddHeader("vendor_code",			"업체코드",			"t_text",		20,	90,	false);
		//GridObj.AddHeader("name_loc",				"업체명",			"t_imagetext",	50,	200,false);
		//GridObj.AddHeader("IRS_NO",				"사업자번호",		"t_text",		20,	90,false);
		//GridObj.AddHeader("credit_grade",			"신용등급",			"t_imagetext",	90,	0,	false);
		
		//GridObj.AddHeader("RESIDENT_NO",			"주민등록번호",		"t_text",		150,0,	false);

		
		//GridObj.AddHeader("business_type",		"업태",				"t_text",		100,120,false);
		//GridObj.AddHeader("industry_type",		"업종",				"t_text",		200,120,false);
		//GridObj.AddHeader("ceo_name_loc",			"대표자",			"t_text",		100,100,false);
		//GridObj.AddHeader("RESIDENT_NO",			"주민등록번호",		"t_text",		150,0,	false);
		//GridObj.AddHeader("APPROVAL_REASON",		"등록사유",			"t_imagetext",	500,0,false);
		

		//GridObj.SetColCellAlign("RESIDENT_NO","center");
		<% if(menu_profile_code1.equals("MUP100100001")){  %> 
		<%  }  %>
		
		//GridObj.SetColCellSortEnable("CLASS_GRADE"			,false);
	...
	function doSelect()
	{
		var G_SERVLETURL = "<%=POASRM_CONTEXT_NAME%>/servlets/master.register.vendor_reg_lis2";
		...
		//var RESIDENT_NO = document.form1.RESIDENT_NO.value;
		//var irs_no 		= document.form1.irs_no.value;

		/* GridObj.SetParam("level", level);
		GridObj.SetParam("sg_refitem", sg_refitem); */
		
		//GridObj.SetParam("company_code",company_code);

		//GridObj.SetParam("RESIDENT_NO",RESIDENT_NO);
		/* GridObj.SetParam("irs_no",irs_no);
		GridObj.SetParam("class_grade",form1.class_grade.value);
		GridObj.SetParam("pjt_name",form1.pjt_name.value); */
...
//	    	return;
	...
	function setKG(PURCHASE_BLOCK_FLAG){		//경고,경고해제
		...
		/* P:결재진행중 N:거래 Y:중지 */
		var PURCHASE_BLOCK_FLAG_STR = PURCHASE_BLOCK_FLAG == "K" ? "입찰경고" : "입찰경고해제"				
		for(var i=0;i<sepoa.GetRowCount();i++)
		{
			...
			if(temp == true) {
				...
				if(PURCHASE_BLOCK_FLAG == 'K' && GridObj.GetCellValue("PURCHASE_BLOCK_FLAG", i) != 'N'){
					alert("거래상태가 정상인 경우에만 입찰경고 하실 수 있습니다.");
					return;
				}				
				
				if(PURCHASE_BLOCK_FLAG != 'K' && GridObj.GetCellValue("PURCHASE_BLOCK_FLAG", i) != 'K'){
					alert("거래상태가 경고인 경우에만 입찰경고해제 하실 수 있습니다.");
					return;
				}
		...
		if(value){
			this.hiddenframe.location.href = "vendor_reg_ins.jsp?mode=kg&vendor_code=" + vendor_code + "&purchase_block_flag=" + PURCHASE_BLOCK_FLAG + "&purchase_block_note=" + PURCHASE_BLOCK_NOTE;
		}
	}
	
	function setNotBd(PURCHASE_BLOCK_FLAG){		
		var sepoa       =  GridObj;
		var vendor_code = "";
		var sel_row     = 0;
		...
		/* P:결재진행중 N:거래 Y:중지 */
		var PURCHASE_BLOCK_FLAG_STR = PURCHASE_BLOCK_FLAG == "B" ? "입찰금지" : "입찰금지해제"				
		for(var i=0;i<sepoa.GetRowCount();i++)
		{
			var temp = GD_GetCellValueIndex(GridObj,i,INDEX_SELECTED);
			...
			if(temp == true) {
				...
				if(PURCHASE_BLOCK_FLAG == 'B' && (GridObj.GetCellValue("PURCHASE_BLOCK_FLAG", i) != 'N' && GridObj.GetCellValue("PURCHASE_BLOCK_FLAG", i) != 'K')){
					alert("거래상태가 정상 또는 입찰경고인 경우만 입찰금지 하실 수 있습니다.");
					return;
				}				
				
				if(PURCHASE_BLOCK_FLAG != 'B' && GridObj.GetCellValue("PURCHASE_BLOCK_FLAG", i) != 'B'){
					alert("거래상태가 입찰금지인 경우에만 입찰금지해제 하실 수 있습니다.");
					return;
				}
				
				if(PURCHASE_BLOCK_FLAG == 'B' && GridObj.GetCellValue("PURCHASE_BLOCK_NOTE", i) == ''){
					alert("입찰금지사유를 입력하세요.");
					return;
				}
		...
		var value = confirm(PURCHASE_BLOCK_FLAG_STR + " 하시겠습니까?");
		if(value){
			this.hiddenframe.location.href = "vendor_reg_ins.jsp?mode=nbd&vendor_code=" + vendor_code + "&purchase_block_flag=" + PURCHASE_BLOCK_FLAG + "&purchase_block_note=" + PURCHASE_BLOCK_NOTE;
		}
	}
  	...
  	function setBlock(PURCHASE_BLOCK_FLAG){		//거래개시
		...
		var value = confirm(PURCHASE_BLOCK_FLAG_STR + " 하시겠습니까?");
		if(value){
			this.hiddenframe.location.href = "vendor_reg_ins.jsp?mode=block&vendor_code=" + vendor_code + "&purchase_block_flag=" + PURCHASE_BLOCK_FLAG + "&purchase_block_note=" + PURCHASE_BLOCK_NOTE;
		}
	}
	...
	*/
	function doChgAcct() {
		...
		var scrollbars = 'no';
		var resizable = 'no';
		var url = "/kr/master/register/vendor_reg_lis2_pop2.jsp?code=SP0278&function=selectCode&values=<%=info.getSession("HOUSE_CODE")%>&vendor_code="+vendor_code;
...
<input type="hidden" name="sg_refitem"    id="sg_refitem" value="">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
     <td class="title_td" width="15%">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;소싱그룹대분류</td>
     ...
     <td class="title_td" width="15%">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;소싱그룹중분류</td>
     <td width="35%" class="data_td" align="left">
    ...
    <tr>
     <td class="title_td" width="15%">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;소싱그룹소분류</td>
     <td width="35%" class="data_td" align="left">
    ...
    <tr>
     <td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;사업자등록번호</td>
     <td width="35%" class="data_td" align="left">
     ...
     <td class="title_td" width="15%">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;거래상태</td>
     <td width="35%" class="data_td" align="left">
		<select name="purchase_block_flag" id="purchase_block_flag" class="inputsubmit" onChange="">
		<option value="">전체</option>
		<%
		    String settle = ListBox(request, "SL0018", info.getSession("HOUSE_CODE") + "#"+"M213", "");
		    out.println(settle);
		%>
		</select>
     </td>
     <%-- <td class="title_td" width="15%">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;등급</td>
     <td width="35%" class="data_td" align="left">
		<select name="class_grade" id="class_grade" class="inputsubmit" onChange="">
		<option value="">전체</option>
		<%
		    String settle = ListBox(request, "SL0018", info.getSession("HOUSE_CODE") + "#"+"M634", "");
		    out.println(settle);
		%>
		</select>
     </td> --%>
   ...
   <!--  <tr>
     <td width="15%" class="title_td"> 참여프로젝트명</td>
     <td width="35%" class="data_td" align="left" colspan="3">
      	<input type="text" size="30" value="" name="pjt_name" id="pjt_name" class="inputsubmit" >
     </td>     
    </tr> -->
<!--  
	<tr style="display: none">
		<td width="15%" class="title_td">
			<img src="/images/<%=info.getSession("HOUSE_CODE")%>/ibk_s_arr.gif" width="9" height="9"> 생성일자
		</td>
		<td width="35%" class="data_td">
			<input type="text" name="add_date_start" size="8" maxlength="8" class="input_re" onkeydown='entKeyDown()' value="<%=SepoaDate.addSepoaDateMonth(SepoaDate.getShortDateString(),-1)%>">
			...
			~
			<input type="text" name="add_date_end" size="8" maxlength="8" class="input_re" onkeydown='entKeyDown()' value="<%=SepoaDate.getShortDateString()%>">
			<a href="javascript:Calendar_Open('add_date_end');">
				<img src="../../images/button/butt_calender.gif" width="22" height="19" align="absmiddle" border="0" alt="">
			</a>
		</td>
     	<td width="15%" class="title_td"><img src="/images/<%=info.getSession("HOUSE_CODE")%>/ibk_s_arr.gif" width="9" height="9"> 업체명</td>
     	<td width="35%" class="data_td" align="left" >
      		<input type="text" size="30" value="" name="vendor_name" class="inputsubmit" onkeydown='entKeyDown()' >
     	</td>
	</tr >
	<tr style="display: none">
     	<td width="15%" class="title_td"><img src="/images/<%=info.getSession("HOUSE_CODE")%>/ibk_s_arr.gif" width="9" height="9"> 사업자등록번호</td>
     	<td width="35%" class="data_td" align="left" colspan="3">
      		<input type="text" size="30" value="" name="IRS_NO" class="inputsubmit" onkeydown='entKeyDown()' >
     	</td>
    </tr>
 -->
  </table>
...
<TR>
	<TD height="30" align="left">
		<TABLE cellpadding="0">		
		<TR>
		    <td><script language="javascript">btn("javascript:setKG('K')","입찰경고")</script></td>
			<td><script language="javascript">btn("javascript:setKG('N')","입찰경고햬제")</script></td>				
  			<td><script language="javascript">btn("javascript:setNotBd('B')","입찰금지")</script></td>
			<td><script language="javascript">btn("javascript:setNotBd('N')","입찰금지햬제")</script></td>				  			
		</TR>
		</TABLE>
	</TD>
	<TD height="30" align="right">
		...
		<TR>
			...
			%>
				<TD><script language="javascript">btn("javascript:doChgAcct()","계좌번호변경");</script></TD>				
				<%-- <TD><script language="javascript">btn("javascript:doCredit()","신용등급등록")</script></TD> --%>
  				<%-- <TD><script language="javascript">btn("javascript:approval()","거래중지")</script></TD> --%>
...
<%-- <s:grid screen_id="SU_103" grid_obj="GridObj" grid_box="gridbox"/> --%>
