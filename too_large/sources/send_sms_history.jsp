...
<%
	...
	multilang_id.addElement("AD_052");
	...
	// Dthmlx Grid 전역변수들..
	String screen_id = "AD_052";
	String grid_obj  = "GridObj";
	boolean isSelectScreen = false;
	isRowsMergeable = false;// 화면에 행머지기능을 사용할지 여부의 구분(true = 사용, false = 미사용)

	String default_finish_date = SepoaDate.getShortDateString();
    String default_start_date = SepoaDate.addSepoaDateDay(default_finish_date, -3);
%>
...
var strToday	= '<%= to_day %>';
	...
	function doQuery()
	{
		...
		var from_date 	= LRTrim(document.form.from_date.value.toUpperCase());
		var to_date 	= LRTrim(document.form.to_date.value.toUpperCase());
		var seller_code = encodeUrl(LRTrim(document.form.seller_code.value.toUpperCase()));
		var send_recv 	= encodeUrl(LRTrim(document.form.send_recv.value.toUpperCase()));
		var sms_send 	= encodeUrl(LRTrim(document.form.sms_send.value.toUpperCase()));
		var content 	= encodeUrl(LRTrim(document.form.content.value.toUpperCase()));

		if(from_date == "" || to_date == "") {
			alert("<%=text.get("AD_052.text1")%>");
			return;
		}
		if(!checkDateCommon(from_date)) {
	  		alert("<%=text.get("AD_052.text2")%>");
		...
		var param = "from_date="+from_date+"&to_date="+to_date+"&seller_code="+seller_code+"&send_recv="+send_recv+"&sms_send="+sms_send+"&content="+content+"&mode=query&grid_col_id="+grid_col_id;
		var G_SERVLETURL = "<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.admin.send_sms_history";
	...
	*/
		...
		var param = "?content="+content+"&senderPhoneno="+senderPhoneno+"&cols_ids="+cols_ids+"&mode=insert";//파라미터
	...
	*/
		...
		var G_SERVLETURL = "<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.admin.send_sms_mgt";//서블릿 경로
		var param = "?content="+content+"&subject="+subject+"&cols_ids="+cols_ids+"&mode=mail_send";//파라미터
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#DBDBDB">
					<tr>
						<td width="15%" height="24" align="right" class="se_cell_title">
							<%=text.get("AD_052.send_date_time")%> <!-- 전송일시 -->
						</td>
						<td width="35%" height="24" class="se_cell_data">
						<s:calendar id_from="from_date" default_from="<%=SepoaString.getDateSlashFormat(default_start_date)%>"  
	        						default_to="<%=SepoaString.getDateSlashFormat(default_finish_date)%>" id_to="to_date" 
	        						format="%Y/%m/%d"/>
						</td>
						<td width="15%" height="24" align="right" class="se_cell_title">
							<%=text.get("AD_052.seller_code")%> <!--  업체코드 -->
						</td>
						<td width="35%" height="24" class="se_cell_data">
							<input type="text" name="seller_code" size="10" class="inputsubmit" maxlength="10">
							<a href="javascript:PopupManager();"><img src="../images/button/query.gif" align="absmiddle" border="0" alt=""></a>
					...
					<tr>
						<td width="15%" height="24" align="right" class="se_cell_title">
							<select name="send_recv" id="send_recv" class="inputsubmit">
		            			<option value="send_user_name"><%=text.get("AD_052.send_user_name")%></option>
		            			<option value="recv_user_name"><%=text.get("AD_052.recv_user_name")%></option>
		            			<option value="send_user_phone_no"><%=text.get("AD_052.send_user_phone_no")%></option>
		            			<option value="recv_user_phone_no"><%=text.get("AD_052.recv_user_phone_no")%></option>
						    </select>
						</td>
						<td width="35%" height="24" class="se_cell_data">

						    <input type="text" name="sms_send" size="10" class="inputsubmit" maxlength="10">
						</td>
						<td width="15%" height="24" align="right" class="se_cell_title">
							<%=text.get("AD_052.content")%> <!-- 내용 -->
						</td>
						<td width="35%" height="24" class="se_cell_data">
						<input type="text" name="content" size="20" class="inputsubmit">
						</td>
					</tr>
				</table>
				<%/* 조회조건 END */ %>
