...
var G_SERVLETURL = "<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.contract.contract_create_list";
    ...
    {	
    	...
    	if( header_name == "CONT_NO")
		{
			...
			if(cont_no != '') {
			 	...
			 	popUpOpen("contract_detail.jsp?"+strParm, 'CONT_NO_DETAIL', '1080', '700');
			}
		}
...
<%-- 		doRequestUsingPOST( 'SL5001', 'M286' ,'ct_flag', 	'' ); 			상태 --%>
	...
	function doQuery()
	{
		...
		var changeContract		= document.form.changeContract.checked;
			...
			if(LRTrim(from_cont_date) == "" || LRTrim(from_cont_date) == "" ) { 
	            alert("<%=text.get("CT_014.0002")%>");
	            return;
	        }
	
	        if(!checkDate(from_cont_date)) { 
	             alert("<%=text.get("CT_014.0003")%>"); // 계약작성일자를 확인하세요.
			...
			{
				alert("<%=text.get("CT_014.0014")%>"); // 계약 작성 일자를 잘못 입력하셨습니다.
				return;
			}
	...
	function doUpdate(flag){
		...
		if(flag == "U"){//수정
			...
			{
				alert("<%=text.get("CT_014.0015")%>"); // 작성중인경우에만 수정가능합니다.
				return;
			}
			...
			if(ct_flag != "CT")
			{
				alert("<%=text.get("CT_014.0016")%>"); // 작성중인경우에만 업체전송이 가능합니다.
				return;
			}
	 		/* 
	 		...
	 		if(ct_flag != "CC")
			{
				alert("<%=text.get("CT_014.0017")%>"); // 업체인증상태에서 인증이 가능합니다.
				return;
			}
			...
			/* 
	 		...
	 		<%-- 
		   	if (confirm("<%=text.get("CT_014.0007")%>"))
		   	{  
			...
			if(ct_flag != "CD")
			{
				alert("<%=text.get("CT_014.0018")%>"); // 한화인증상태에서 폐기가능합니다.
				return;
			}
		   	if (confirm("<%=text.get("CT_014.0009")%>")) // 계약서를 폐기하시겠습니까?
		   	{
			...
			{
				alert("<%=text.get("CT_014.0019")%>"); // 한화인증인경우에만 변경계약서 생성이 가능합니다.
				return;
			}
			/* 
...
<%--			
			if(ct_flag == "CT" || ct_flag == "CL"){				
			}else{
				alert("작성중 또는 계약폐기 만 삭제가능합니다");
				return;
			}
--%>	
	...
	function doChange() {
		...
		if(change_yn == "N")
		{
				alert("<%=text.get("CT_014.0012")%>");
				return;
		} 
		
		if(ct_flag != "CE")
		{
				alert("<%=text.get("CT_014.0011")%>");
				return;
		}
...
<body leftmargin="10" topmargin="6" onload="setGridDraw();initAjax();">
							...
							<tr>
								<td width="100%">
									<table width="100%" border="0" cellspacing="0" cellpadding="0">
										<tr>
											<td width="15%" class="title_td">&nbsp;<img	src="/images/blt_srch.gif" width="7" height="7"	align="absmiddle">&nbsp;&nbsp;<%=text.get("CT_014.T_CONT_ADD_DATE")%></td>
											...
											<%-- 										<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("CT_014.T_CONT_TYPE")%></td> --%>
											<!-- 										<td class="data_td"> -->
											...
											<!-- 									</tr> -->
											<!-- 									<tr>  -->
											<td width="15%" class="title_td">&nbsp;<img	src="/images/blt_srch.gif" width="7" height="7"	align="absmiddle">&nbsp;&nbsp;<%=text.get("CT_014.T_SUBJECT")%></td>
											<td class="data_td"><input type="text" name="subject" id="subject" size="20" value="" onkeydown='entKeyDown()'>
											</td>
											<%-- 										<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("CT_014.T_CT_FLAG")%></td> --%>
											<!-- 										<td class="data_td"> -->
											<%-- 											<select name="ct_flag" id="ct_flag" class="inputsubmit" >  --%>
											<%-- <% --%>
											<!-- // 	String ctFlagSelect = ListBox ( request , "SL9000" , "M286" , ctFlag ); -->
											<!-- // 	out.println(ctFlagSelect); -->
											<%-- %> --%>
											<%-- 											</select> --%>
											<!-- 										</td> -->
										</tr>
					...
					<td height="30" align="right">
						<TABLE cellpadding="0">
							<TR>
								...
								<%-- 조회 --%>
								<%
									if("Y".equals ( contractPrice )) {
								%>
								<td><script language="javascript">btn("javascript:doInsert_price()", "단가계약생성");</script></td>
								<%-- 계약생성 --%>
								<%
									}
								%>
								<td>
									<%-- <script language="javascript"> --%> <%-- btn("javascript:doUpdate('U')", "<%=text.get("BUTTON.update")%>"); --%>
									<%-- </script> --%> 
									<script language="javascript">btn("javascript:doModify()", "확정/수정");</script>
								</td>
								<%-- 수정 --%>
								<td><script language="javascript">btn("javascript:doDelete()", "<%=text.get("BUTTON.deleted")%>");</script></td>
								<%-- 삭제 --%>
								<td><script language="javascript">btn("javascript:doOffMake()", "오프라인");</script></td>
								<td><script language="javascript">btn("javascript:doContractMake()", "전자계약생성");</script></td>
								<!-- 							<td> -->
								<%-- <script language="javascript"> --%>
								<%-- btn("javascript:doUpdate('CB')", "<%=text.get("CT_014.CB")%>"); --%>
								<%-- </script> --%>
								<%-- 							</td> 업체전송    --%>
								<!-- 							<td> -->
								<%-- <script language="javascript"> --%>
								<%-- btn("javascript:doUpdate('CD')", "<%=text.get("CT_014.CD")%>"); --%>
								<%-- </script> --%>
								<%-- 							</td> 풍산인증    --%>
								<!-- 							<td> -->
								<%-- <script language="javascript"> --%>
								<%-- btn("javascript:doUpdate('CE')", "<%=text.get("CT_014.change")%>"); --%>
								<%-- </script> --%>
								<%-- 							</td> 변경계약생성   					  	   --%>
							</TR>
						</TABLE>
	...
	<s:grid screen_id="CT_014" grid_obj="GridObj" grid_box="gridbox" />
	<s:footer />
</body>