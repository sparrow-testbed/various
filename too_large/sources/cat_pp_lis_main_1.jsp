...
<%
	...
	HashMap text               = null;
	String  screen_id          = "PR_003_1";
	String  grid_obj           = "GridObj";
	String  WISEHUB_PROCESS_ID = "PR_003_1";
	String  isSingle           = request.getParameter("isSingle");
	boolean isSelectScreen     = false;
	
	multilang_id.addElement("PR_003_1");
...
// 	INDEX_UNIT_MEASURE    = GridObj.GetColHDIndex("UNIT_MEASURE") ;//단위
	INDEX_UNIT_MEASURE    = GridObj.GetColHDIndex("BASIC_UNIT") ;//단위
}
        	...
        	var grid_array = getGridChangedRows(GridObj, "SEL");
        	 for(var i = 0; i < grid_array.length; i++){
           	...
           	parent.opener.setCatalog(
				...
				GD_GetCellValueIndex(GridObj, rowIndex, INDEX_UNIT_MEASURE)
			);
...
function myCatalog() {
    ...
    for(var i = 0;i < row;i++){
        ...
        //if(check == "true"){ // 문자열이 아닌 boolean으로 비교
        if(check == true){
            sendcnt++;
            sendRow += (i+"&");
        }//end of if(true)
...
%>
								<TD>
									<script language="javascript">
										btn("javascript:myCatalog()", "나의카탈로그담기");
									</script>
								</TD>
<%
	}
...
<%-- <s:grid screen_id="PR_003_1" grid_obj="GridObj" grid_box="gridbox"/> --%>
<s:footer/>
</body>
</html>