...
<!--
	...
	function doSpecFlag() {
        ...
        for(row=0; row<GridObj.GetRowCount(); row++) {
			var test_point 		= parseFloat(LRTrim(GD_GetCellValueIndex(GridObj,row, INDEX_TEST_POINT)));
			var standard_point 	= parseFloat(LRTrim(GD_GetCellValueIndex(GridObj,row, INDEX_STANDARD_POINT)));
			var spec_falg 		= GridObj.GetCellHiddenValue("SPEC_FLAG", row);// GD_GetCellValueIndex(GridObj,row, INDEX_SPEC_FLAG);
			var reason 			= GD_GetCellValueIndex(GridObj,row, INDEX_REASON);
			var vendor 			= GD_GetCellValueIndex(GridObj,row, INDEX_VENDOR_NAME);

			var VENDOR_CODE 	= GD_GetCellValueIndex(GridObj,row, INDEX_VENDOR_CODE);
			var CEO_NAME_LOC 	= GD_GetCellValueIndex(GridObj,row, INDEX_CEO_NAME_LOC);
			var TEL 			= GD_GetCellValueIndex(GridObj,row, INDEX_TEL);
			var USER_NAME 		= GD_GetCellValueIndex(GridObj,row, INDEX_USER_NAME);
			var USER_MOBILE 	= GD_GetCellValueIndex(GridObj,row, INDEX_USER_MOBILE);
			var USER_EMAIL 		= GD_GetCellValueIndex(GridObj,row, INDEX_USER_EMAIL);
        ...
        var url = "ebd_pp_ins7.jsp?BID_NO="+BID_NO+"&BID_COUNT="+BID_COUNT+"&VOTE_COUNT="+VOTE_COUNT+"&TITLE=가격입찰일시등록";
        window.open( url , "ebd_pp_ins7","toolbar=no, location=no, directories=no, status=yes,menubar=no,scrollbars=yes,resizable=no,width=800,height=300,left=50,top=80");
    }

    function setDecision(BID_BEGIN_DATE, BID_BEGIN_TIME, BID_END_DATE, BID_END_TIME, OPEN_DATE, OPEN_TIME)
    {
        servletUrl = "/servlets/dt.bidd.ebd_bd_ins6"; //p1012
        mode = "doSpecFlag";
        GridObj.SetParam("mode", 				mode);
        GridObj.SetParam("BID_NO",            "<%=BID_NO%>"                  );
        GridObj.SetParam("BID_COUNT",         "<%=BID_COUNT%>"               );
...
var G_SERVLETURL   = "<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.sourcing.bd_price_popup";
...
function doQuery() {
    ...
    var params = "mode=getBdPricePopupList";
...
<tr>
  ...
  <table width="100%" border="0" cellpadding="1" cellspacing="1" bgcolor="#ccd5de">
    <tr>
      <td width="15%" class="c_title_1"><img src="/images/<%=info.getSession("HOUSE_CODE")%>/ibk_s_arr.gif" width="9" height="9">&nbsp;공고번호</td>
    ...
    <tr>
      <td class="c_title_1" width="15%"><img src="/images/<%=info.getSession("HOUSE_CODE")%>/ibk_s_arr.gif" width="9" height="9">&nbsp;입찰방법</td>
      <td class="c_data_1" width="85%">&nbsp;
    ...
    <tr>
      <td class="c_title_1" width="15%"><img src="/images/<%=info.getSession("HOUSE_CODE")%>/ibk_s_arr.gif" width="9" height="9">&nbsp;담당자명</td>
      <td class="c_data_1" width="85%">&nbsp;
