...
//////////////////ClipReport4 선언부 끝/////////////////////////////////////////////////////////////
	...
	String next_sign_user_id =  JSPUtil.nullToEmpty(request.getParameter("next_sign_user_id")); //다음결재자
		...
		$.post(
			...
			function(arg){
				...
				if(result != "0" ){
					...
					if("<%=doc_type%>" == "PAY" && "<%=next_sign_user_id%>" == ""){
						opener.btnApprovalAll('21','1','<%=doc_no%>','');			
						return;
						
						var url = "/approval/ap_approval_pay.jsp";
						var params = "?doc_type=<%=doc_type%>";
						params += "&doc_no=<%=doc_no%>";
						params += "&doc_seq=<%=doc_seq%>";
						params += "&app_stage=<%=app_stage%>";
						params += "&next_sign_user_id=<%=next_sign_user_id%>";
						//alert(url+params);
						CodeSearchCommon(url+params,'pop_up2','400','250','400','250');
					}else{
						opener.btnApprovalAll('2','1','<%=doc_no%>','');			
					}
				}else if(result == "-1" ){ 
		...
		$.post(
			...
			function(arg){
				...
				if(result != "0" ){
					...
					opener.btnApprovalAll('3','1','<%=doc_no%>','');
				}else if(result == "-1" ){ 
					alert("휴일 체크중 휴일 데이타가 존재하지 않습니다. ");			
				}else{
					alert("휴일 체크중 알 수 없는 오류발생");
...
<form name="form1" method="post">
  ...
  <td width="1%" valign="top" ></td>
    <td width="40%" valign="top" ><table width="90%" height="92" border="0" cellpadding="0" cellspacing="0">
    
	  		<tr>
        		<td class="title"> 지&nbsp;&nbsp;&nbsp;&nbsp;급&nbsp;&nbsp;&nbsp;&nbsp;번&nbsp;&nbsp;&nbsp;&nbsp;호&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp;<%=doc_no%></td>
      		</tr>
      		<tr>
      		...
      		<tr>
        		<td class="title"> 지&nbsp;&nbsp;급&nbsp;&nbsp;결&nbsp;&nbsp;의&nbsp;&nbsp;명&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp;<%=subject%></td>
      		</tr>
      <tr>
        <td height="1" bgcolor="#000000"></td>
      </tr>
...
APP_STATUS    = "";
//////////////////ClipReport4 조립부 시작///////////////////////////////////////////////////////////
_rptAprvData.append("지 급 결 의 품 의");

_rptAprvData.append(_RD);	

_rptAprvData.append("지급번호");
  		...
  		<td height="30" align="right">
			<TABLE cellpadding="0">
	      		...
	      		<% if ("Y".equals(doc_status)) { %>
	      			<TD><script language="javascript">btn("javascript: btnApprovalAll();","승 인")</script></TD>
		      		<TD><script language="javascript">btn("javascript: btnApprovalAll2();","반 려")</script></TD>
<%-- 		      		<TD><script language="javascript">btn("javascript:changeApprovalLine('<%=doc_type%>','<%=doc_no%>','<%=doc_seq%>','<%=sign_path_seq%>')",21,"결재선추가")</script></TD> --%>
		      	<% } %>
		     	 	<TD><script language="javascript">btn("javascript:payFrm.clipPrint('<%=_rptAprvData.toString().replaceAll("\"", "&quot")%>',<%=approvalCnt%>);","출 력")</script></TD>
...
<iframe id="payFrm" name="payFrm" src="/kr/tax/pay_bd_dis3_pop.jsp?doc_no=<%=doc_no %>" style="width: 100%;height: 550px; border: 0px;" frameborder="0" scrolling="auto"></iframe>
<%-- <script language="javascript">rMateFileAttach('S','R','PR',form1.attach_no.value);</script> --%>
<iframe id="ctFrm2" name="ctFrm2"  src="/kr/tax/pay_audit_list_pop.jsp?pay_send_no=<%=doc_no %>&cont_gl_seq=<%=doc_seq %>&cont_form_no=<%=doc_no %>" style="width: 100%;height: 150px; border: 0px;  display:none;" frameborder="0" scrolling="auto"></iframe>
</body>
</html>
<s:footer/>
