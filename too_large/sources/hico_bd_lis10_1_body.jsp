...
//     	}
        ...
        if (typeof(f.s_factor_refitem) == "undefined") {
        	alert("소싱그룹 소분류까지 선택해주세요.");
        	return;
        }
...
//        alert("스크리닝 평가항목을 작성해 주셔서  감사합니다.\n전략구매팀 드림");
		var value = confirm('소싱정보를 저장하시겠습니까?');
		if(value){
...
//      alert("스크리닝 평가항목을 작성해 주셔서  감사합니다.\n전략구매팀 드림");
  ...
  function init() {
	document.location.href	= "hico_bd_lis10_1_body1.jsp?vendor_code=<%=vendor_code%>&sg_refitem=<%=sg_refitem%>&status=<%=status%>&buyer_house_code=<%=buyer_house_code%>&popup=<%=popup%>"
	return;
<%--
...
<%
    ...
    Logger.debug.println(info.getSession("ID"), this, "#################### getScreenList count : " + wf.getRowCount());
    if (wf.getRowCount() > 0) {
%>
            <td colspan="<%=MAX_SCALE_COUNT%>" class="c_title">
<%--             <strong> * "저장" 버튼을 클릭하여  업체등록평가를 완료하여 주십시요.</strong> --%>
			<% if( "N".equals(IS_EXIST) && "T".equals(popup) ) { %>
            <strong> * "저장" 버튼을 클릭하여  소싱정보등록을 완료하여 주십시요.</strong>
            <%} %>
            </td>
          </tr>
...
%>
            <tr height=25 class="c_title_1" style="display: none;">
          ...
          <tr height=25 class="c_data_1" style="display: none;">
...
%>
            ...
            <input type="hidden" name="answered_seq" value="0"><!--선택된 값이 담긴다.-->
        ...
        <td align="right">
          ...
          <TR>
          	<% if(popup == null || popup.equals("") || popup.equals("T") ){ %>
          		<% if("N".equals(IS_EXIST)){ %>
          			<% if( "T".equals(popup) ) { %>
          				<TD><script language="javascript">btn("javascript:setData3()","저 장3")</script></TD>          			
          			<% }else{ %>
