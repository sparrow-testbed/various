...
<%
	...
	//HTML에 쓰일 변수들
	String TITLE = String.valueOf(text.get("MT_015.MSG_0104"));// "Q&A 작성";
...
function Create(){
	...
	if (LRTrim(F.SUBJECT.value) == ""){
		alert("<%=text.get("MT_015.MSG_0100")%>");
	...
	if (LRTrim(F.CONTENT.value) == ""){
		alert("<%=text.get("MT_015.MSG_5000")%>");
 	...
 	F.method = "POST";
    F.target = "childFrame";
    F.action = "bulletin_write_wk_new.jsp?con_type="+con_type;
    F.submit();
...
function Cancel()
{
    if(!confirm("<%=text.get("MT_015.MSG_0101")%>"))//취소 하시겠습니까?
    	return;
	var con_type = "<%=con_type%>";
