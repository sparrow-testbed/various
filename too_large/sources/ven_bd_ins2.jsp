...
// 	Config conf = new Configuration();
	...
	String session_company_code = info.getSession("COMPANY_CODE");
...
// 		all_admin_profile_code = conf.get("wise.all_admin.profile_code."+info.getSession("HOUSE_CODE"));
	...
	String vendor_code	      = "";
	if("WOORI".equals(session_company_code)){
		vendor_code			  = vendor_code = JSPUtil.nullToEmpty(request.getParameter("vendor_code"));
	}else{
		vendor_code			  = session_company_code;
	}	
	...
	if (flag.equals("update") || !"".equals(vendor_code)) {
		...
		value = ServiceConnector.doService(info, "p0010", "CONNECTION",
				"getDis_icomvngl", obj);
		value2 = ServiceConnector.doService(info, "s6006",
				"CONNECTION", "getScrItem", obj2);
		if (value.status == 1) {
			...
			if (iRowCount > 0) {
				...
				SOLE_PROPRIETOR_FLAG = wf.getValue(
						"SOLE_PROPRIETOR_FLAG", 0);
		...
		if (value2.status == 1) {
			SepoaFormater wf2 = new SepoaFormater(
					value2.result[0]);
...
// 		EncDec enc = new EncDec();
		...
		String[] data = { house_code,irs_no };
		Object[] obj = { (Object[]) data,"irs_no" };
	...
	///////////////////////////////////////////////////////////////////////////
	String message = "";
	String isSuccess = JSPUtil.nullToEmpty(request
			.getParameter("isSuccess"));
	String s_template_refitem = JSPUtil.nullToEmpty(request
			.getParameter("s_template_refitem"));
	String c_template_refitem = JSPUtil.nullToEmpty(request
			.getParameter("c_template_refitem"));

	String[] s_factor_refitem = JSPUtil.koForArray(request
			.getParameterValues("s_factor_refitem"));
	String[] answered_seq = JSPUtil.koForArray(request
			.getParameterValues("answered_seq"));
	String sg_refitem = JSPUtil.nullToEmpty(request
			.getParameter("sg_refitem")); //소싱 그룹 구분키
	///////////////////////////////////////////////////////////////////////////
...
var G_MESSAGE1     = "";
var G_ERROR_MESSAGE = "에러가 발생하였습니다. 시스템 담당자에게 문의하세요";
var toDays = "<%=toDays%>";
...
<%-- 	f.ENGINEER_COUNT.value          = "<%=ENGINEER_COUNT%>"; --%>
	...
	if(ZIP_CODE			>= 10){
		...
		//f.ZIP_CODE2.value = ZIP_CODE.substring(3);
...
%>
	...
	if(isEmpty(f.CONFIRM_PASSWORD.value))
	{
   	...
   	/* 
   	if(!chkInputRepeat(f.PASSWORD.value)) {
	    alert("비밀번호는 같은 문자를 3번 이상 반복 입력할 수 없습니다.");
	    f.PASSWORD.focus();
	    return ;
	} 
	if(chkKorea(f.PASSWORD.value) < 8 || chkKorea(f.PASSWORD.value) > 10) {
	    alert("비밀번호는 영문자와 숫자를 혼용하여 8~10자만 가능합니다.");
	    f.PASSWORD.focus();
	    return ;
	}
	if(!chkMixing(f.PASSWORD.value)) {
	    alert("비밀번호는 영문자와 숫자를 혼용하여 8~10자만 가능합니다.");
	    f.PASSWORD.focus();
	    return ;
	}
...
// 	if(f.ENGINEER_COUNT.value > parseInt(emp1)+parseInt(emp2)){
// 		alert("기술인력은 종업원 수를 초과할 수 없습니다.");
// 	 	f.ENGINEER_COUNT.focus();
// 	 	return false;
// 	}
	
// 	if(f.SEARCH_KEY.value == ""){
// 		alert("검색용어를 입력하여 주십시오.");
// 		f.SEARCH_KEY.focus();
// 		return false;
// 	}
	...
	*/
	...
	if(f.BANK_KEY_O.value != "" && f.DEPOSITOR_NAME_O.value != "" && f.BANK_ACCT_O.value != ""){
		<% if("E".equals(SIGN_STATUS)){ %>
		if(f.BANK_KEY_O.value != f.BANK_KEY.value){
			alert("기입력된 주거래은행 변경은 우리은행 담당자만 변경가능합니다.");
			f.BANK_KEY.value = f.BANK_KEY_O.value; 
			return false;
		}
		
		if(f.DEPOSITOR_NAME_O.value != f.DEPOSITOR_NAME.value){
			alert("기입력된 예금주 변경은 우리은행 담당자만 변경가능합니다.");
			f.BANK_KEY.value = f.BANK_KEY_O.value; 
			return false;
		}
		
		if(f.BANK_ACCT_O.value != f.BANK_ACCT.value){
			alert("기입력된 계좌번호 변경은 우리은행 담당자만 변경가능합니다.");
			f.BANK_KEY.value = f.BANK_KEY_O.value; 
			return false;
		}
		<% } %>
	...
	/* if(f.SOLE_PROPRIETOR_FLAG.value == "1"){
		if(isEmpty(f.COMPANY_REG_NO1.value)||isEmpty(f.COMPANY_REG_NO2.value)){
			alert("법인등록번호를 입력하여 주십시오.");
			f.COMPANY_REG_NO1.focus();
			return false;
		}
	} */
	...
	*/
	...
	/*
	if(f.ADDRESS_LOC2.value == ""){
		alert("나머지 주소를  입력하여 주십시오.");
		f.ADDRESS_LOC2.focus();
		return false;
	}
	*/
	...
	document.form1.sg_refitem.value = s_type3;
...
function NextSave()
{
	...
	f.method = "POST";
 	f.target = G_TARGET;
	f.action = "ven_wk_ins2.jsp?SOLE_PROPRIETOR_FLAG="+f.SOLE_PROPRIETOR_FLAG.value+"&VENDOR_TYPE="+f.VENDOR_TYPE.value;
	
// 	f.submit();
	 
	if(makeEncData(f)){ // 키보드보안 암호화 함수
		f.submit();
	} else{
...
function  selectAddr(zip_code, address_loc1, address_loc2, city ) {
	//form1.ZIP_CODE1.value = zip_code.substring(0,3);
...
function getCity(code, text) {
	...
	function goDuplicate() 
	{
		...
		if(!checkEmpty(f.USER_ID, "사용자 ID를 입력해주십시요."))
			return;
...
<%-- 	<input type="hidden" id="att_no" 		name="att_no" 			value="<%=ATTACH_NO%>">  --%>
<%-- 	<input type="hidden" id="attach_no" 	name="attach_no" 		value="<%=ATTACH_NO%>">  --%>
<%-- 	<input type="hidden" id="attach_count" 	name="attach_count" 	value="<%=ATTACH_COUNT%>"> --%>
...
<% if(!ctrl_code.equals("P99")){ %>
	<tr>
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;승인상태
		</td>
		<td class="data_td" >
		...
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;회사코드
		</td>
		<td class="data_td" >
...
%>
	...
	<tr>
		...
		<TABLE cellpadding="0">
			<tr>
				<td> 
					<input type="text" id="USER_ID" name="USER_ID" size="20" maxlength="10" value="<%=irs_no %>" style="ime-mode: disabled;" class="input_re" onKeyUp="return chkMaxByte(10, this, '사용자ID');"  data-enc="on">
				</td>
		...
		<td class="data_td">
			<input type="password" id="PASSWORD" name="PASSWORD" size="18" maxlength="20" class="input_re"  data-enc="on" style="ime-mode:disabled;">
			<input type="password" id="CONFIRM_PASSWORD" name="CONFIRM_PASSWORD" size="18" maxlength="20" class="input_re" data-enc="on" style="ime-mode:disabled;">
			<br/>
...
%>
	...
	<tr>
		<td width="18%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;회사명
		</td>
		...
		<td width="18%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;등록형태
		</td>
		<td width="32%" class="data_td">
	...
	<tr>
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;내자/외자
		</td>
		...
		<td class="title_td" style="display: none;" >&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;파트너종류
		</td>
	...
	<tr>
			<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;사업자등록번호</td>
			<td class="data_td">
		...
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;이전사업자등록번호</td>
		<td class="data_td">
	...
	<tr>
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;업태</td>
		<td class="data_td">
			<input type="text" id="BUSINESS_TYPE" name="BUSINESS_TYPE" value="" size="15" class="input_re" <%=readOnly_flag%> onKeyUp="return chkMaxByte(100, this, '업태');" maxlength="100">
			<input type="hidden" id="BUSINESS_TYPE_O" name="BUSINESS_TYPE_O" value='<%=BUSINESS_TYPE %>'/>
		 	ex.) 도.소매,서비스 외
		</td>
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;업종</td>
		<td class="data_td">
			<input type="text" id="INDUSTRY_TYPE" name="INDUSTRY_TYPE" value="" size="15" class="input_re" <%=readOnly_flag%> onKeyUp="return chkMaxByte(100, this, '업종');" maxlength="100">
	...
	<!--자본금 , 종업원 수 추가 / 단위, 코멘트 추가-->
	<tr>
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;자본금</td>
		<td class="data_td">
			<input type="text" id="CAPITAL" name="CAPITAL" value="" size="15" class="input_re" <%=readOnly_flag%> style="ime-mode: disabled; text-align: right;" onKeyPress="return onlyNumber(event.keyCode);" data-dataType="n"  onchange="$('#CAPITAL').val(comma(this.value, 0));" maxlength="15"> 원
		</td>
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;종업원 수</td>
		<td class="data_td">
			<input type="text" id="EMP_COUNT1" name="EMP_COUNT1" value="" size="7" class="input_re" <%=readOnly_flag%> style="ime-mode: disabled;"	onKeyPress="return onlyNumber(event.keyCode);" data-dataType="n" maxlength="13" onfocus="setOnFocus(this)" onblur="setOnBlur(this)"> (정규직)
			<input type="text" id="EMP_COUNT2" name="EMP_COUNT2" value="" size="7" class="input_re" <%=readOnly_flag%> style="ime-mode: disabled;"	onKeyPress="return onlyNumber(event.keyCode);" data-dataType="n" maxlength="13" onfocus="setOnFocus(this)" onblur="setOnBlur(this)"> (계약직)
		</td>
	</tr>
	<tr>
		<td colspan="4" height="1" bgcolor="#dedede"></td>
...
<%-- 			 기술인력 --%>
<!-- 		</td> -->
<%-- 		<td class="data_td"><input type="text" id="ENGINEER_COUNT" name="ENGINEER_COUNT" value="" size="15" class="inputsubmit" <%=readOnly_flag%> style="ime-mode: disabled;" onKeyPress="checkNumberFormat('[0-9]', this)"></td> --%>
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;과세구분
		</td>
		<td class="data_td">
			<select id="TAX_DIV" name="TAX_DIV" class="inputsubmit" <%=readOnly_flag%>> <%=LB_TAX_KEY%> </select>
		</td>
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;영업개시일</td>
		<td class="data_td">
			<s:calendar id="START_DATE" default_value="" format="%Y/%m/%d"/>		
<%-- 			<input type="text" id="START_DATE" name="START_DATE" size="10" maxlength="8" class="inputsubmit" <%=readOnly_flag%> onKeyPress="return onlyNumber(event.keyCode);" onBlur="DateCheck(this);">  --%>
<!-- 			<a href="javascript:Calendar_Open('start_date');">  -->
<!-- 				<img src="../../images/button/butt_calender.gif" width="22" height="19" align="absmiddle" border="0" alt="">  -->
<!-- 			</a> -->
			ex.) 2007/07/01
		</td>
		<td class="title_td" style="display: none;">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;검색용어</td>
		<td class="data_td"  style="display: none;"><input type="text" id="SEARCH_KEY" name="SEARCH_KEY" value="" size="30" class="input_re" <%=readOnly_flag%> onKeyUp="return chkMaxByte(20, this, '검색용어');"></td>
	</tr>
	<tr>
		<td colspan="4" height="1" bgcolor="#dedede"></td>
	</tr>	
	<tr>
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;대표자명</td>
		<td class="data_td" colspan="3">
			<input type="text" id="CEO_NAME_LOC" name="CEO_NAME_LOC" value="" size="20" class="input_re" <%=readOnly_flag%> onKeyUp="return chkMaxByte(20, this, '대표자명');" maxlength="20">
	...
	<tr>
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;전화번호</td>
		<td class="data_td">
			<input type="text" id="PHONE_NO1" name="PHONE_NO1" value="" size="20" class="input_re" <%=readOnly_flag%> data-dataType="n"  style="ime-mode: disabled;"  maxlength="20" onKeyUp="return chkMaxByte(20, this, '전화번호');" onKeyPress="return onlyNumber(event.keyCode);">
			<input type="hidden" id="PHONE_NO1_O" name="PHONE_NO1_O" value='<%=PHONE_NO1 %>'/> 
			ex.)021234567
		</td>
		<!-- 홈페이지 추가 -->	
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;홈페이지</td>
		<td class="data_td">
			<input type="text" id="HOME_URL" name="HOME_URL" value="" size="30" class="inputsubmit" <%=readOnly_flag%> onKeyUp="return chkMaxByte(50, this, '홈페이지');" maxlength="50" style="ime-mode: disabled;">
		</td>
		<!-- 홈페이지 추가 -->
	</tr>		
	<tr>
	...
	<tr>
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;작성자부서</td>
		<td class="data_td">
			<input type="text" id="ENTRY_DEPT_NAME" name="ENTRY_DEPT_NAME" value="" size="20" class="input_re" <%=readOnly_flag%> onKeyUp="return chkMaxByte(30, this, '작성자부서');" maxlength="30">
			<input type="hidden" id="ENTRY_DEPT_NAME_O" name="ENTRY_DEPT_NAME_O" value='<%=ENTRY_DEPT_NAME %>'/> 
		</td>
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;작성자성명</td>
		<td class="data_td">
			<input type="text" id="ENTRY_USER_NAME" name="ENTRY_USER_NAME" value="" size="30" class="input_re" <%=readOnly_flag%> onKeyUp="return chkMaxByte(30, this, '작성자성명');" maxlength="30">
	...
	<!-- 작성자 연락처, 이메일 추가 -->	
	<tr>
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;작성자 연락처</td>
		<td class="data_td">
			<input type="text" id="ENTRY_TEL" name="ENTRY_TEL" value="" size="20" class="input_re" <%=readOnly_flag%> onKeyUp="return chkMaxByte(20, this, '작성자 연락처');" data-dataType="n" style="ime-mode: disabled;" onKeyPress="return onlyNumber(event.keyCode);" maxlength="20">
			<input type="hidden" id="ENTRY_TEL_O" name="ENTRY_TEL_O" value='<%=ENTRY_TEL %>'/> 
			ex.)021234567
		</td>
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;작성자 이메일</td>
		<td class="data_td">
			<input type="text" id="ENTRY_EMAIL" name="ENTRY_EMAIL" value="" size="30" class="input_re" <%=readOnly_flag%> onKeyUp="return chkMaxByte(50, this, ' 작성자 이메일');" style="ime-mode:disabled;" maxlength="50">
	...
	<tr>
		<td colspan="4" height="1" bgcolor="#dedede"></td>
			 ...
			 <!-- 작성자 연락처, 이메일 추가 -->				
	<tr>
<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;본사주소
</td>
<td class="data_td" colspan="3">
	<table width="100%" border="0" cellspacing="1" cellpadding="1">
	<tr>
		<td>
			<input type="text" id="ZIP_CODE1" name="ZIP_CODE1" value="" size="5" maxlength="5" class="input_re" data-dataType="n" onKeyPress="return onlyNumber(event.keyCode);"  style="ime-mode: disabled;"  readOnly>
			...
			<input type="checkbox" id="ckbFg"  onclick="javascript:ckbFg_onclick();"> 우편번호 수기입력
		</td>
	</tr>
	<tr>
		<td>
			<input type="text" id="ADDRESS_LOC" name="ADDRESS_LOC" style="width:98%" size="45" class="inputsubmit" <%=readOnly_flag%> onKeyUp="return chkMaxByte(200, this, '본사주소');" maxlength="200">
...
<tr>
	<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;국가</td>
		<td class="data_td">
			<select id="COUNTRY" name="COUNTRY" class="inputsubmit" <%=readOnly_flag%> disabled>
				<%=LB_COUNTRY%>
			</select>
		</td>
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;도시</td>
		<td class="data_td">
<!-- 			<input type="text" id="city_code"  name="city_code" value="" size="30" class="inputsubmit" readOnly> -->
	...
	<tr>
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;SMS 수신번호</td>
		<td class="data_td">
		...
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;본사 FAX</td>
		<td class="data_td">
	...
	<tr>
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;거래구분</td>
		<td class="data_td">상품 
		...
		<td class="data_td">
			<select id="BANK_KEY" name="BANK_KEY" class="input_re" >
...
<%-- 			<input type="text" id="BANK_ACCT" name="BANK_ACCT" value="" size="20" class="input_re" <%=readOnly_flag%> onKeyUp="return chkMaxByte(30, this, '계좌번호');" onKeyPress="return onlyNumber(event.keyCode);"  data-enc="on" data-dataType="n" maxlength="30" style="ime-mode:disabled;"> --%>
	...
	<tr>
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;본사사업자등록번호</td>
		<td class="data_td" colspan="3">
		...
		<td class="title_td" style="display: none;" >&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;법인등록번호</td>
		<td class="data_td"  style="display: none;">
	...
	<tr style="display: none;">
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;조정계정</td>
		<td class="data_td">
	...
	%>
	<tr>
		<td valign="top" class="title_td" style="vertical-align: middle;">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;첨부문서</td>
		<td class="data_td" colspan="3">
			<table border="0" cellspacing="2" cellpadding="0">
				<tr>
					<td><b>※원본 스켄하여 파일을 첨부해 주세요.</b><br> ◎법인사업자 - 사업자등록증
					    <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;사용인감계
					    <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;법인인감증명서
						<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;통장사본
						<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;회사소개서																	
...
<!-- 					<td>&nbsp;&nbsp;  -->
<!-- 						<iframe name="attachFrame" width="510" height="150" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" frameborder="0" scrolling="no"></iframe>  -->
<!-- 						<br>&nbsp; -->
<!-- 					</td> -->
					<%
					if (popup==null||"".equals(popup)||popup.equals("U")||popup.equals("T")) {
...
<%-- 						<script language="javascript">btn("javascript:FileAttach('IMAGE',$('#attach_no').val(),'VI')", "파일보기")</script> --%>
...
<tr>
	...
	<tr>
		<td class="title_td"><img src="/images/<%=info.getSession("HOUSE_CODE")%>/ibk_s_arr.gif"
			width="9" height="9"> 업태</td>
		<td class="data_td">
...
<!-- 		</td> -->
<!-- 		<td class="data_td"> -->
<%-- 			<input type="text" id="ENGINEER_COUNT" name="ENGINEER_COUNT" value="" size="15" class="inputsubmit" <%=readOnly_flag%> style="ime-mode: disabled;" onKeyPress="checkNumberFormat('[0-9]', this)"> --%>
<!-- 		</td> -->
		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;과세구분
		</td>
		<td class="data_td">
...
<%-- 			<input type="text" id="START_DATE" name="START_DATE" size="10" maxlength="8" class="inputsubmit" <%=readOnly_flag%> onKeyPress="return onlyNumber(event.keyCode);" onBlur="DateCheck(this);">  --%>
<!-- 			<a href="javascript:Calendar_Open('start_date');">  -->
<!-- 			<img src="../../images/button/butt_calender.gif" width="22" height="19" align="absmiddle" border="0" alt=""> </a> -->
			ex.) 20070701
		</td>
		<td class="title_td" style="display: none;">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;검색용어</td>
		<td class="data_td" style="display: none;">
...
<tr>
	...
	<tr>
		...
		<td class="title_td"><img src="/images/<%=info.getSession("HOUSE_CODE")%>/ibk_s_arr.gif"	width="9" height="9"> 법인등록번호</td>
		<td class="data_td">
...
<!-- 					<td>&nbsp;&nbsp; <iframe name="attachFrame" width="510"	height="150" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" frameborder="0" scrolling="no"></iframe>  -->
					<%
					if (popup==null||"".equals(popup)||popup.equals("U")||popup.equals("T")) {
					%>
					<td class="data_td" colspan="3" height="200">
