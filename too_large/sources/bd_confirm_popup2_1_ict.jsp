...
<%
	...
	multilang_id.addElement("I_BD_024");			
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "I_BD_024";
...
function doQuery() {
    ...
    var params = "mode=getBdItemDetail2";
...
function doBidStatus2() {
	...
	for(var i = 0; i < grid_array.length; i++) {
		...
		inp_cnf_asis    = GridObj.cells(grid_array[i], GridObj.getColIndexById("INP_CNF_ASIS")).getValue();
		final_flag_asis = GridObj.cells(grid_array[i], GridObj.getColIndexById("FINAL_FLAG_ASIS")).getValue();
		
		if( inp_cnf == "N" && final_flag != ""){
			alert("서류접수여부를 선택하세요.");
			button_flag = false;
			return;
		...
		if( inp_cnf != inp_cnf_asis){
			inp_cnf_cnt = inp_cnf_cnt + 1;
		}
		
		if( final_flag != final_flag_asis ){
			/*
			alert("최종결과를 입력하세요.");
			button_flag = false;
			return;
	...
	if( inp_cnf_cnt == 0 && final_flag_cnt == 0  ){
		alert("변경사항이 없습니다.");
		button_flag = false;
		return;
	}
...
<%  
    ...
    thisWindowPopupScreenName = "입찰서류확인";
%>
...
<tr>
	<td height="30" align="right">
		...
		<TR>
			...
			<TD><script language="javascript">btn("javascript:doBidStatus2();", "확인")</script></TD>
