...
<%
	...
	multilang_id.addElement("EV_001");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "EV_001";
...
<%
	...
	//String       LB_CSKD     = ListBox(request, "SL0018",  house_code+"#M903#", "");
%>
...
// 		GridObj.SetNumberFormat("PAY_AMT"		,G_format_amt);
	...
	function doSelect()
	{
		...
		var param = "mode=getCsEvWaitList&grid_col_id="+grid_col_id;
		param += dataOutput();
	...
	function cs_ev_ins(){
		...
		if(iCheckedCount < 1)
		{
     	...
     	if('<%=info.getSession("ID")%>' != eval_user_id  ){
			alert("평가담당자만 처리할 수 있습니다.");
			return;
		}
     	
    	location.href="/kr/ev/cs_ev_ins.jsp?ev_gb=C&inv_no="+inv_no+"&inv_seq="+inv_seq+"&cskd_gb="+cskd_gb+"&vendor_code="+vendor_code;
...
function SP9113_Popup( type ) {
	...
	}else if( type == "eval_user_id2" ) {
		window.open("/common/CO_008.jsp?callback=eval2_callback", "SP9114", "toolbar=no, location=no, directories=no, status=yes,menubar=no,scrollbars=yes,resizable=no,width=450,height=550,left=0,top=0");
...
function  eval2_callback(id, name) {
	form1.eval_user_id2.value     = id;
	form1.eval_user_name2.value   = name;
}
...
function doRemove( type ){
    ...
    if( type == "eval_user_id2" ) {
    	document.forms[0].eval_user_id2.value = "";
        document.forms[0].eval_user_name2.value = "";
    }
    if( type == "inv_user_id" ) {
...
function doEvalChg() {
	...
	for(var i=0;i<iRowCount;i++)
	{
		...
		{								
			...
			vendor_code = GridObj.GetCellValue("COMPANY_CODE", i);
			eval_user_id = GridObj.GetCellValue("EVAL_USER_ID", i);
		    inv_person_id = GridObj.GetCellValue("INV_PERSON_ID", i);
		}
	}
 	...
 	if(iCheckedCount > 1)
	{
 	...
 	if('<%=info.getSession("ID")%>' != inv_person_id 
 			&& '<%=info.getSession("ID")%>' != eval_user_id 
 	...
 	var eval_user_id2 		= LRTrim(form1.eval_user_id2.value);
 	var eval_user_name2 	= LRTrim(form1.eval_user_name2.value);

	if(eval_user_id2 == "") {
		alert("평가담당을 입력하셔야 합니다.");
		return;
	}

	Message = "평가담당을 "+eval_user_name2+"으로 지정 하시겠습니까?";
...
<form name="form1" >
    ...
    <tr>
      <td width="12%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp; 검수일자</td>
...
%>
    ...
    <tr>
  		<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp; 공사평가담당</td>      	
      	<td class="data_td">
...
%>
	...
	<tr style="display:none">
  	...
  	<table width="100%" border="0" cellspacing="0" cellpadding="0">
    	<tr>
			<td width="370px" style="font-weight:bold;font-size:12px;color:#555555;" align="left">&nbsp;&nbsp;<img src="../../images/blt_srch.gif">&nbsp;평가담당 변경&nbsp;&nbsp;&nbsp;&nbsp;
	      	<input type="text" name="eval_user_id2" id="eval_user_id2" size="12" maxlength="5" class="inputsubmit" readOnly > /
	        <input type="text" name="eval_user_name2" id="eval_user_name2" size="15" class="inputsubmit" readOnly >
	        <a href="javascript:SP9113_Popup('eval_user_id2');"><img src="/images/ico_zoom.gif" align="absmiddle" border="0"></a>
      		</td>
      		<td width="150px" align="left"> <b>
		        <table>
		        <tr>
		        	<td><script language="javascript">btn("javascript:doEvalChg()","평가담당 변경")</script></td>	        	
		        </tr>
		        </table>
      		</td>
      		<td width="160px" align="left">
      			<font color="red" style="font-size:12px">* 입찰담당 기술역으로 지정</font>			
      		</td>	        	       
      		<td height="30" align="right">
				<TABLE cellpadding="0">
		      		<TR>
	    	  			<TD><script language="javascript">btn("javascript:doSelect()","조 회")</script></TD>
	    	  			<TD><script language="javascript">btn("javascript:cs_ev_ins()","공 사 평 가")</script></TD>
...
<s:grid screen_id="EV_001" grid_obj="GridObj" grid_box="gridbox"/>
<s:footer/>
</body>
</html>
