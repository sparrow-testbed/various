...
public class p0080 extends SepoaService {
	...
	public p0080(String opt, SepoaInfo info) throws SepoaServiceException {
		 super(opt,info);
	        setVersion("1.0.0");
	...
	/**
	 * 평가템플릿 쿼리
	 * @method SR_getEvalTempFactorList
	 * @param  header
	 * @return Map
	 * @throws Exception
	 * @desc   ICOYRQDT
	...
	private String SR_getEvalTempFactorList( String e_template_refitem, String temp_type, String factor_type ) throws Exception 
	{
		...
		param.put("e_template_refitem", e_template_refitem);   		
	...
	public SepoaOut getEvaluationValuerList(Map<String, String> header) {
		
		String rtn = null;
		try {
			
			rtn = et_getEvaluationValuerList( header );
			setValue(rtn);
			setStatus(1);
	...
	private String et_getEvaluationValuerList( Map<String, String> header) throws Exception {
		...
		SepoaXmlParser wxp =  new SepoaXmlParser(this,new Exception().getStackTrace()[0].getMethodName());
		wxp.addVar("house_code"		, house_code);
		wxp.addVar("eval_refitem"	, header.get("eval_refitem"));
	...
	public SepoaOut getEvaluationVendorList_1(Map<String, String> header) {
		
		String rtn = null;
		try {
			
			rtn = SR_getEvaluationVendorList_1( header );
			setValue(rtn);
			setStatus(1);
	...
	private String SR_getEvaluationVendorList_1( Map<String, String> header ) throws Exception {
		...
		wxp.addVar("house_code"		, house_code);
		wxp.addVar("eval_refitem"	, header.get("eval_refitem"));
		wxp.addVar("user_id"		, header.get("eval_user_id"));
	...
	public SepoaOut insertEvaluerScore(Map<String, Object> data) 
	{
    	String rtn[] = null;
    	...
    	try 
    	{
    		...
    		rtn = SR_insertEvaluerScore(data);
    		
    		setValue( rtn[0] );
    		setStatus( 1 );
    		
    		if ( rtn[ 1 ] != null && !"".equals(rtn[ 1 ]))
    		{
    		    setMessage( rtn[ 1 ] );
    		    setStatus( 0 );
    		}
  	...
  	private String[] SR_insertEvaluerScore(Map<String, Object> data) throws Exception 
  	{
		...
		try{
			...
			for(int i = 0 ; i < grid.size() ; i++){
				...
				wxp =  new SepoaXmlParser(this,new Exception().getStackTrace()[0].getMethodName()+"_upd");
				sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp.getQuery() );
				rtnIns = sm.doUpdate(gridInfo);
				
				wxp =  new SepoaXmlParser(this,new Exception().getStackTrace()[0].getMethodName()+"_del");
				sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp.getQuery() );
				rtnIns = sm.doDelete(gridInfo);

				if(gridInfo.get("EVAL_SCORE") != null && !"".equals(gridInfo.get("EVAL_SCORE"))){
				...
				wxp =  new SepoaXmlParser(this,new Exception().getStackTrace()[0].getMethodName()+"_upd1");
				sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp.getQuery() );
				rtnIns = sm.doUpdate(gridInfo);
			...
			if(evalCnt < grid.size() ){
				eval_flag = "T";
			}else if(evalCnt == grid.size()){
				eval_flag = "C";
			}
			...
			if(evalCnt != 0){
				...
				updMap.put("house_code"	, info.getSession("house_code"));
				updMap.put("eval_flag"	, eval_flag);
				updMap.put("doc_no"		, header.get("bid_no"));
				updMap.put("doc_count"	, header.get("bid_count"));
				
				wxp = new SepoaXmlParser(this,new Exception().getStackTrace()[0].getMethodName()+"_flag");
				sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp.getQuery() );
				rtnIns = sm.doInsert(updMap);
			}
  	    	Commit();
