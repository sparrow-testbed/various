...
public class p0010 extends SepoaService {
	Message msg = new Message(info, "STDCOMM");

	public p0010(String opt, SepoaInfo info) throws SepoaServiceException {
		super(opt, info);
		setVersion("1.0.0");
	}
	...
	/**
	 * Save icomvngl INI 업체 생성때 결재를 탄다 request_info는 차기결재자,결재전략 등의 내용을 포함한다.
	 */
	public SepoaOut setInsert_icomvngl(String[][] args_vngl,
		...
		try {
			...
			String rtn = et_setInsert_icomvngl(args_vngl, args_addr,
					vendor_code);
			setValue(rtn);
			setStatus(1);
			setMessage(msg.getMessage("0000"));
	...
	private String et_setInsert_icomvngl(String[][] args_vngl,
			String[][] args_addr, String vendor_code) throws Exception,
			DBOpenException {
		int rtn = -1;
		ConnectionContext ctx = getConnectionContext();
	 ...
	 * Change_icomvngl//
	 */
	public SepoaOut setUpdate_vngl(String[][] args_vngl, String[][] args_addr,
			String vendor_code, String[][] args_vngl_old) {
		String user_id = info.getSession("ID");
		try {
			if (args_vngl[0][0].equals("admin_upd")) {
				...
				SepoaOut rtn = et_setUpdate_vngl(args_vngl, args_addr,
						vendor_code, args_vngl_old);
			}
	...
	private SepoaOut et_setUpdate_vngl_adm(String[][] args_vngl,
		...
		try {
			...
			new SMS("NONDBJOB", info).rg2Process(ctx, smsParam);
...
//        	}
	...
	private int et_setVendorProject(Map<String, String> header, List<Map<String, String>> grid) throws Exception {
		...
		try {
			...
			SepoaXmlParser wxp = new SepoaXmlParser(this, new Exception().getStackTrace()[0].getMethodName() + "_insert");
			wxp.addVar("house_code", house_code);
			wxp.addVar("vendor_code", vendor_code);
			...
			SepoaXmlParser wxpup = new SepoaXmlParser(this, new Exception().getStackTrace()[0].getMethodName() + "_update");
	...
	private int et_setSave_icomvncp(Map<String, String>header, List<Map<String, String>> grid) throws Exception {
		...
		String user_id = info.getSession("ID");
		Logger.debug
				.println(info.getSession("ID"), this, "et_setSave_icomvncp");
		SepoaSQLManager sm = null;
		...
		try {
			...
			SepoaXmlParser wxp_u = new SepoaXmlParser(this, new Exception().getStackTrace()[0].getMethodName() + "_update");
	...
	/**
	 */
	private String getDis_icomvngl_2(String[] args, String user_id, String mode)
			throws Exception {
		String rtn = null;
		ConnectionContext ctx = getConnectionContext();
		String house_code = info.getSession("HOUSE_CODE");
		...
		try {
			SepoaXmlParser wxp = new SepoaXmlParser(this,
					new Exception().getStackTrace()[0].getMethodName());
			wxp.addVar("housee_code", house_code);
			wxp.addVar("company_code", company_code);
			wxp.addVar("mode", mode);
