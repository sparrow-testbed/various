...
public class t0002 extends SepoaService {
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ////////////////////////////////////////           Session 정보를 담기위한 변수             //////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public t0002(String opt, SepoaInfo info) throws Exception {
        super(opt, info);
        setVersion("1.0.0");
...
//		System.out.println("tytolee : >" + sxp.getQuery() + "<");
    ...
    private Map<String, String> realSetInsertParam01(String itemNo, String reqItemNo, String[] headerData) throws Exception{
    	...
    	result.put("WID",      headerData[26]);
    	result.put("HGT",      headerData[27]);
...
*/
    ...
    public SepoaOut confirm_getUpdate( String REQ_ITEM_NO, String ITEM_NO, String REMARK, String ATTACH_NO, String[][] HeaderData) {
        ...
        try
        {
        	...
        	rtn_str = et_isCheckedAbolRsn(ITEM_NO,ctx);
        	wf		= new SepoaFormater(rtn_str); 
        	
        	if(!"NULL".equals(wf.getValue("ABOL_RSN", 0))){
        		throw new Exception("폐기건은 수정이 불가합니다.");
        	}
    ...
    private String et_isCheckedAbolRsn(String ITEM_NO ,ConnectionContext ctx) throws Exception 
    { 
        String rtn = null; 
        String HOUSE_CODE = info.getSession("HOUSE_CODE"); 
...
//      sql.append(" and   user_id = '"+ls_id+"'            \n"); 
  ...
  ////////////////////////////////////////////////공급사 계좌조회/////////////////////////////////////////////// 
  public SepoaOut req_ins_getVnglList2(String vendor_code ) 
  {
      try 
      {
          String rtn = null;
          rtn = et_req_ins_getVnglList2( vendor_code);
          setValue(rtn);
          setStatus(1);
          setMessage(msg.get("0000"));
      }catch(Exception e){
  ...
  public String et_req_ins_getVnglList2( String vendor_code) throws Exception 
  {
      String result = null;
      String house_code   = info.getSession("HOUSE_CODE");
      String company_code = info.getSession("COMPANY_CODE");
...
/////////////////////////////////////////////////공급사 변경요청 계좌조회/////////////////////////////////////////////// 
public SepoaOut req_ins_getVnglList3(String vendor_code ) 
{
	try 
	{
		String rtn = null;
		rtn = et_req_ins_getVnglList3( vendor_code);
		setValue(rtn);
		setStatus(1);
		setMessage(msg.get("0000"));
	}catch(Exception e){
...
public String et_req_ins_getVnglList3(String AR_NO) throws Exception 
{
	String result = null;
	String house_code   = info.getSession("HOUSE_CODE");
	String company_code = info.getSession("COMPANY_CODE");
...
////////////////////////////////////////////////공급사 변경요청 계좌조회/////////////////////////////////////////////// 
  ...
  /**
	 ...
	 * @modify
	 */
	public SepoaOut doApprovaAcctP(Map<String, Object> data) throws Exception{
		...
		try {
			...
			so        = DocumentUtil.getDocNumber( info, "AR" );  
			AR_NO = so.result[0]; 
			AR_COUNT = "1";
			
			pflag = header.get("pflag");  
						
			
			
			

			sxp = new SepoaXmlParser(this, "et_icoyarhd_setInsert");
			ssm = new SepoaSQLManager(id, this, ctx, sxp);			
			header.put("AR_NO", AR_NO);			
			rtn = ssm.doInsert(header);
			if(rtn < 1){
				throw new Exception("et_icoyarhd_setInsert 중 에러");
			}
						
			 if("P".equals(pflag)){
	            setBDCreateSignRequestInfo(AR_NO, AR_COUNT, header.get("approval_str"), "계좌번호변경요청 - " + header.get("VENDOR_NAME_LOC"), 0);	            		  
	         }
...
//			e.printStackTrace();
	...
	private void setBDCreateSignRequestInfo(String ar_no, String ar_count, String pflag, String subject, int itemSize) {
...
//        	e.printStackTrace();
	...
	/*  계좌변경요청 내역조회 */
	public SepoaOut getIcoyarhdList(Map<String, String> header) {
		try {
			
			String rtnHD = bl_getIcoyarhdList(header);
			
			SepoaFormater wf = new SepoaFormater(rtnHD);
			if (wf.getRowCount() == 0)
				setMessage("내역이존재하지 않습	니다.");
			else {
				setMessage("조회하였습니다.");
			}
			setStatus(1);
...
private String bl_getIcoyarhdList(Map<String, String> header) throws Exception {
