...
public class p6027 extends SepoaService {
	//Message msg = new Message("FW");  // message 처리를 위해 전역변수 선언
	private Message msg;
	
	public p6027(String opt, SepoaInfo info) throws SepoaServiceException{
		super(opt, info);
		setVersion("1.0.0");
 ...
 */
	public SepoaOut addSignRequest(SignRequestInfo reqInfo) {
		...
		try {
			...
			try 
            { 
				//TOBE 2017-07-01 주석 추가 : 오류시 메시지 코드를 보낸다 예) BEBEB10463
				//시스템관리자 다국어관리 MESSAGE에 등록된 코드설명을 화면에서 알림창으로 보여준다
				Logger.sys.println("p6027 Message b1 : = " + getMessage());
                Rollback(); 
            } 
            catch(Exception d) 
            { 
            	Logger.sys.println("p6027 Message b2 : = " + getMessage());
...
//				wxp.addVar("doc_seq", doc_seq);
				...
				for( int i=0; i < sign_path_data.length; i++){
					...
					if("1".equals(sign_path_data[i][0])){
						next_sign_user_id =sign_path_data[i][1];
					}
...
//				wxp.addVar("doc_seq", doc_seq);
				...
				//R102006115128 - [서비스개발] (변원상) 전자구매시스템 결재알림 시스템 개선
				if ("PR".equals(doc_type)) {
					sbMsg.append("구매요청 결재 바랍니다.");	
					UcMessage.DirectSendMessage(info.getSession("ID"), next_sign_user_id, sbMsg.toString());
				}else if ("RQ".equals(doc_type)) {
					sbMsg.append("견적의뢰 결재 바랍니다.");	
					UcMessage.DirectSendMessage(info.getSession("ID"), next_sign_user_id, sbMsg.toString());
				}else if ("BID".equals(doc_type)) {
					sbMsg.append("입찰공고 결재 바랍니다.");	
					UcMessage.DirectSendMessage(info.getSession("ID"), next_sign_user_id, sbMsg.toString());
				}else if ("POD".equals(doc_type)) {
					sbMsg.append("발주 결재 바랍니다.");	
					UcMessage.DirectSendMessage(info.getSession("ID"), next_sign_user_id, sbMsg.toString());
				}else if ("CT".equals(doc_type)) {
					sbMsg.append("전자계약 결재 바랍니다.");				
					UcMessage.DirectSendMessage(info.getSession("ID"), next_sign_user_id, sbMsg.toString());
				}else if ("PSB".equals(doc_type)) {
					sbMsg.append("자본예산지급 결재 바랍니다.");				
					UcMessage.DirectSendMessage(info.getSession("ID"), next_sign_user_id, sbMsg.toString());
				}else if ("AR".equals(doc_type)) {
...
//				wxp.addVar("doc_seq", doc_seq);
			...
			Logger.sys.println("p6027 pay doc_type : " + doc_type);
			...
			if ("PAY".equals(doc_type)) {
				...
				if(rowCount == 1){
			    	...
			    	Logger.sys.println("p6027 et_setBEB00730T02 a: " + doc_type);
			    	
			    	String  rtn   = this.et_setBEB00730T02(info, data2);
			    	
			    	Logger.sys.println("p6027 et_setBEB00730T02 b: " + doc_type);
			    	
			    	
			    	
			    	if(rtn.length() > 3 && !"ERR".equals(rtn.substring(0,3))){
			    					    						    			
			    		Logger.sys.println("p6027 et_setBEB00730T02 c: " + doc_type);
			    		...
			    		if(iRtnCnt < 1) {
			    			...
			    			Logger.sys.println("p6027 et_setBEB00730T02 d: " + doc_type);
			    			throw new Exception();
			    		}
...
//			    		}
				    	...
				    	//TOBE 2017-07-01 추가 온라인 전문 로그 (성공)
				    	this.insertSinfhd(info, "BEB00730T02", "Y", rtn+": 지급결의승인번호가 발급되었습니다."); //interface 입력
...
//			    		setStatus(0);
			    		...
			    		//TOBE 2017-07-01 추가 온라인 전문 로그 (실패)
				    	this.insertSinfhd(info, "BEB00730T02", "N", rtn); //interface 입력
			    		
			    		Logger.sys.println("p6027 et_setBEB00730T02 e: " + doc_type);
			    		throw new Exception();
			    	}
...
//					if(rowCount == 1){
//			    		
//			    		data2.put("VENDOR_CODE"          ,  sf.getValue("VENDOR_CODE",0));
//			    		data2.put("STD_DATE"             ,  SepoaString.getDateUnSlashFormat( sf.getValue("STD_DATE",0)) );
//			    		data2.put("DEPOSITOR_NAME"       ,  sf.getValue("DEPOSITOR_NAME",0));
//			    		data2.put("BANK_CODE"            ,	 sf.getValue("BANK_CODE",0));
//			    		data2.put("BANK_ACCT"            ,  sf.getValue("BANK_ACCT",0));
//			    		data2.put("SUPPLY_AMT"           ,  sf.getValue("SUPPLY_AMT",0));
//			    		data2.put("TAX_AMT"              ,  sf.getValue("TAX_AMT",0));
//			    		data2.put("TOT_AMT"              ,  sf.getValue("TOT_AMT",0));
//			    		data2.put("BMSBMSYY"             ,  sf.getValue("BMSBMSYY",0));
//			    		data2.put("BUGUMCD"              ,  sf.getValue("BUGUMCD",0));
//			    		data2.put("ACT_DATE"             ,  SepoaString.getDateUnSlashFormat( sf.getValue("ACT_DATE",0)) );
//			    		data2.put("EXPENSECD"            ,  sf.getValue("EXPENSECD",0));
//			    		data2.put("SEMOKCD"              ,  sf.getValue("SEMOKCD",0));
//			    		data2.put("SAUPCD"               ,	 sf.getValue("SAUPCD",0));
//			    		data2.put("DOC_TYPE"             ,	 sf.getValue("DOC_TYPE",0));
//			    		data2.put("PAY_TYPE"             ,	 sf.getValue("PAY_TYPE",0));
//			    		data2.put("PAY_REASON"           ,  sf.getValue("PAY_REASON",0));
//			    		data2.put("TAX_NO"               ,  sf.getValue("TAX_NO",0));
//			    		data2.put("NTS_APP_NO"           ,  sf.getValue("NTS_APP_NO",0));
//			    		data2.put("ACC_TAX_DATE"         ,  SepoaString.getDateUnSlashFormat( sf.getValue("ACC_TAX_DATE",0)) );
//			    		data2.put("ACC_TAX_SEQ"          ,  sf.getValue("ACC_TAX_SEQ",0));
//			    		data2.put("APP_STATUS_CD"        ,  sf.getValue("APP_STATUS_CD",0));
//			    		data2.put("ADD_DATE"             ,  sf.getValue("ADD_DATE",0));
//			    		data2.put("ADD_TIME"             ,  sf.getValue("ADD_TIME",0));
//			    		data2.put("ADD_USER_ID"          ,  sf.getValue("ADD_USER_ID",0));
//			    		data2.put("ISU_DT"               ,  sf.getValue("ISU_DT",0));
//			    		data2.put("SEBUCD"               ,  sf.getValue("SEBUCD",0));
//			    		data2.put("ZIPHANGCD"            ,  sf.getValue("ZIPHANGCD",0));
//			    		data2.put("VENDOR_NAME"          ,  sf.getValue("VENDOR_NAME",0));
//			    		data2.put("IRS_NO"               ,  sf.getValue("IRS_NO",0));
//			    		
//			    		String  rtn   = this.et_setBEB00730T02(info, data2);
//			    		
//			    		if(rtn.length() > 3 && !"ERR".equals(rtn.substring(0,3))){
...
//		n02.SEND.TAITM_CD         = header.get("SEMOKCD").length() == 0 ? header.get("EXPENSECD") : header.get("SEMOKCD");					//"211202";
        ...
        */
	   	...
	   	Logger.sys.println("p6027 et_setBEB00730T02-2 user_trm_no : " +  header.get("user_trm_no"));
        ...
        /* TOBE 2017-07-01 */	   	
	   	...
	   	Logger.sys.println("p6027 et_setBEB00730T02-2.5 addUserId : " + addUserId);
	   	
	   	
	   	BEB00730T02 n02 = new BEB00730T02();
 
	   	Logger.sys.println("p6027 et_setBEB00730T02-3 : " + header.get("user_trm_no"));
		...
		//TOBE 2017-08-24 오픈시 원복해야함 (계정계 테스트 서버가 특정일자밖에 인식을 못해서 밑에 하드코딩) n02.SEND.PAY_DTMN_DT            = header.get("ACT_DATE");       //5.   S8         지급결의일자                
        n02.SEND.PAY_DTMN_DT            = header.get("ACT_DATE");       //5.   S8         지급결의일자
//		n02.SEND.PAY_DTMN_DT            = "20180413";
		...
		Logger.sys.println("p6027 et_setBEB00730T02-4 : " + header.get("user_trm_no"));
		
		try {
			Logger.sys.println("p6027 et_setBEB00730T02-4.5 : " + header.get("user_trm_no"));
			
			int iret = n02.sendMessage("BEB00730T02",send_ip,send_port);
			
			Logger.sys.println("p6027 et_setBEB00730T02-5 : " + header.get("user_trm_no"));
			if(iret == ONCNF.D_OK) {
				n02.RECV.log(ONCNF.LOGNAME, "");	
				//ASIS 2017-07-01 result = n02.RECV.PYDTM_APV_NO;
				//TOBE 2017-07-01
				
				Logger.sys.println("p6027 et_setBEB00730T02-6 : " + header.get("user_trm_no"));
...
//				System.out.println("ERR:["+n02.RECV.bizHdr.ERRCODE+"]"+n02.eRECV.MESSAGE);
				...
				Logger.sys.println("p6027 et_setBEB00730T02-7 : " + header.get("user_trm_no"));
				result = "ERR:["+n02.RECV.msgHdr.MSG_CD+"]"+n02.eRECV.msgHdr.MAIN_MSG_TXT;
				
				//TOBE 2017-07-01 메세지 추가
				tmpMsgCd = n02.RECV.msgHdr.MSG_CD.substring(0,5);
				
				if( ("BEBEB10463".equals(n02.RECV.msgHdr.MSG_CD)) || // 예산잔액이 부족합니다
					("BEBEB10015".equals(n02.RECV.msgHdr.MSG_CD)) || // 미 등록된 사업코드입니다
					("BEBEB10565".equals(n02.RECV.msgHdr.MSG_CD))    // 거래 불가합니다
					) {
					setMessage(n02.RECV.msgHdr.MSG_CD);
				} else if("BEBEB".equals(tmpMsgCd)) { // 비즈니스 오류
					setMessage(tmpMsgCd);
				} else if("BECOM".equals(tmpMsgCd)) { // 업무공통 오류
					setMessage(tmpMsgCd);
				} else {
					setMessage("ETCER"); // 기타 오류
				}
				
				Logger.sys.println("p6027 Message sta1 : = " + n02.RECV.msgHdr.MSG_CD);
				Logger.sys.println("p6027 Message sta2 : = " + getMessage());
			}else {
//				System.out.println("sendMessage call error !!.." + iret);
				
				Logger.sys.println("p6027 et_setBEB00730T02-8 : " + header.get("user_trm_no"));
				result = "ERR:sendMessage call error !!.." + iret;
			}
		}
		catch (Exception e) {
			Logger.sys.println("p6027 et_setBEB00730T02-9 : " + header.get("user_trm_no"));
