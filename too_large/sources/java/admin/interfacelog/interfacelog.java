...
public class interfaceLog extends SepoaService {
	private Message msg;

    public interfaceLog(String opt, SepoaInfo info) throws SepoaServiceException {
        super(opt, info);
	...
	public SepoaOut selectSinfhdList(Map<String, String> header){
		...
		try{
			...
			rtn = this.select(ctx, "selectSinfhdList", header);
	...
	public SepoaOut selectUserlogList(Map<String, String> header){
		ConnectionContext ctx = null;
		String            rtn = null;
		...
		try{
			...
			ctx = getConnectionContext();
			rtn = this.select(ctx, "selectUserlogList", header);
