...
public class TX_001_S extends SepoaService {
	Message msg = new Message(info, "IV_001");  // message 처리를 위해 전역변수 선언

    public TX_001_S(String opt, SepoaInfo info) throws SepoaServiceException{
    	super(opt, info);
		setVersion("1.0.0");
...
//			}
	...
	/* 세금계산서 반려사유 등록 */	
	public SepoaOut setBuyerRejectRemark(Map<String, Object> data){
		int rtn = 0;
			
		 try {
			rtn = et_setBuyerRejectRemark(data);
			 if(rtn == 0) {
	             try {
	                 Rollback();
	             } catch(Exception d) {
	...
	private int et_setBuyerRejectRemark(Map<String, Object> data) throws Exception{
		...
		try {
			...
			SepoaXmlParser wxp = new SepoaXmlParser(this, "et_setBuyerRejectRemark");
			SepoaSQLManager sm = new SepoaSQLManager(info.getSession("ID"), this, ctx, wxp);
	...
	public SepoaOut setTxApp(Map<String, Object> data) throws SepoaServiceException
	{
        ConnectionContext ctx = getConnectionContext();
		setStatus(1);
		setFlag(true);
	    ...
	    try {
			...
			if("C".equals(approval_str)){
				sign_status = "E";
			}else {
				sign_status = "R"; //???반려시에 결제상태 어떻게 해야될까?
				SepoaXmlParser wxp = new SepoaXmlParser(this, "setTrRetrun");
			    SepoaSQLManager sm = new SepoaSQLManager(info.getSession("ID"), this, ctx, wxp);
			    value = sm.doUpdate(gridData, header);
			}
			header.put("sign_status", sign_status);
			 SepoaXmlParser wxp = new SepoaXmlParser(this, "setTxApp");
		     SepoaSQLManager sm = new SepoaSQLManager(info.getSession("ID"), this, ctx, wxp);
		     value = sm.doUpdate(gridData, header);
		     ...
		     if(value == 0) {
	            ...
	            if(approval_str.equals("C")) {
	            	setMessage("승인 되었습니다.");
	            }else if (approval_str.equals("R")) {
	            	setMessage("반려 되었습니다.");
				}
	        	 Commit();
	         }
