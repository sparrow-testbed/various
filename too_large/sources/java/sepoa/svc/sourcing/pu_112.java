...
public class PU_112 extends SepoaService {

    //20131129 sendakun
    private HashMap msg = null;


    public PU_112( String opt, SepoaInfo info ) throws SepoaServiceException {
        super( opt, info );
        setVersion( "1.0.0" );
        ...
        try {
        	...
        	Logger.sys.println( "PU_112 : " + e.getMessage() );
        }
    }
	...
	public SepoaOut getBottomSupiList(String seller_code, String rfq_no, String rfq_count, String sel_flag) {
        ...
        try { 
            ...
            if("".equals ( rfq_no )){ 
                ...
                sqlsb.append("  SELECT * FROM  ( SELECT SELLER_CODE                                                              \n"); 
                sqlsb.append("      , " + SEPOA_DB_OWNER + "GETCOMPANYNAMELOC( SELLER_CODE, 'S' ) AS SELLER_NAME                 \n"); 
                sqlsb.append("      , COMPANY_CODE || '*' || SELLER_CODE || '*' || TO_CHAR(SEQ) AS USER_SEQ \n"); 
                sqlsb.append("      , USER_NAME                                                                 \n"); 
                sqlsb.append("      , MOBILE_NO                                                                 \n"); 
                sqlsb.append("      , EMAIL                                                                     \n"); 
                sqlsb.append("      , NULL AS SUPI_ID                                                           \n"); 
                sqlsb.append("   FROM SSUPI WHERE SELLER_CODE IN (                                              \n"); 
                sqlsb.append("      ''                                                                          \n");  
                for(int i = 0 ; i < seller_codes.length ; i++){
                    sqlsb.append(sm.addSelectString ( "      ,?                                                 \n")); sm.addStringParameter(seller_codes[i].trim()); 
                }
                sqlsb.append("   )) A GROUP BY A.SELLER_CODE, A.SELLER_NAME, A.USER_SEQ, A.USER_NAME, A.MOBILE_NO, A.EMAIL, A.SUPI_ID ORDER BY A.SELLER_CODE  \n");
...
*/
    ...
    public SepoaOut getRfqVedorList ( HashMap < String , String > hashMap ) {
        ...
        try {
            ...
            if ( "SSUGL".equals ( MapUtils.getString ( hashMap , "type" ) ) ) {

    			sxp = new SepoaXmlParser(this, "getRfqVedorList_selectSSUGL");
    			ssm = new SepoaSQLManager(info.getSession("ID"), this, ctx, sxp); 
    		 
    			 
            } else if ( "SPOGL".equals ( MapUtils.getString ( hashMap , "type" ) ) ) {

    			sxp = new SepoaXmlParser(this, "getRfqVedorList_selectSPOGL");
    			ssm = new SepoaSQLManager(info.getSession("ID"), this, ctx, sxp); 
    		  
            }
            else if ( "SSGSL".equals ( MapUtils.getString ( hashMap , "type" ) ) ) {
                
                String sgCode = "";
                if ( !"".equals ( MapUtils.getString ( hashMap , "sg_code1" ) ) ) {
                    sgCode = MapUtils.getString ( hashMap , "sg_code1" );
                }
                if ( !"".equals ( MapUtils.getString ( hashMap , "sg_code2" ) ) ) {
                    sgCode = MapUtils.getString ( hashMap , "sg_code2" );
                }
                if ( !"".equals ( MapUtils.getString ( hashMap , "sg_code3" ) ) ) {
                    sgCode = MapUtils.getString ( hashMap , "sg_code3" );
                }
                if ( !"".equals ( MapUtils.getString ( hashMap , "sg_code4" ) ) ) {
                    sgCode = MapUtils.getString ( hashMap , "sg_code4" );
                }
                if ( !"".equals ( MapUtils.getString ( hashMap , "sg_code5" ) ) ) {
                    sgCode = MapUtils.getString ( hashMap , "sg_code5" );
                }
                hashMap.put("sg_code", sgCode);
                hashMap.put("house_code", info.getSession("HOUSE_CODE"));
                //SSGSL테이블이 존재하지 않아서 견적요청의 견적업체 팝업의 쿼리를 응용해서 만듬.(2015.02.16 next1210)
    			sxp = new SepoaXmlParser(this, "getRfqVedorList_selectSSGSL");
    			ssm = new SepoaSQLManager(info.getSession("ID"), this, ctx, sxp);                                                                                            
            }
            else if ( "SRQGL".equals ( MapUtils.getString ( hashMap , "type" ) ) ) {
    			sxp = new SepoaXmlParser(this, "getRfqVedorList_selectSRQGL");
    			ssm = new SepoaSQLManager(info.getSession("ID"), this, ctx, sxp);  
            }
            else if ( "SGGSL".equals ( MapUtils.getString ( hashMap , "type" ) ) ) {
                
                String gpCode = "";
                if ( !"".equals ( MapUtils.getString ( hashMap , "gp_code1" ) ) ) {
                	gpCode = MapUtils.getString ( hashMap , "gp_code1" );
                }
                if ( !"".equals ( MapUtils.getString ( hashMap , "gp_code2" ) ) ) {
                	gpCode = MapUtils.getString ( hashMap , "gp_code2" );
                }
                hashMap.put("gp_code", gpCode);
                hashMap.put("house_code", info.getSession("HOUSE_CODE"));
                //SSGSL테이블이 존재하지 않아서 견적요청의 견적업체 팝업의 쿼리를 응용해서 만듬.(2015.02.16 next1210)
    			sxp = new SepoaXmlParser(this, "getRfqVedorList_selectSGGSL");
    			ssm = new SepoaSQLManager(info.getSession("ID"), this, ctx, sxp);                                                                                            
            }
			...
			setValue((ssm != null)?ssm.doSelect(hashMap):""); 
        } catch ( Exception e ) {
            setStatus ( 0 ) ;
            setFlag ( false ) ;
            setMessage ( e.getMessage ( ) ) ;
