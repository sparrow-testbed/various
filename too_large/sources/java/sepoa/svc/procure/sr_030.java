...
public class SR_030 extends SepoaService{
	...
	String QUA_FACTOR_NO_19 = "19";			//재거래 의사
  	...
  	public SR_030( String opt, SepoaInfo sinfo ) throws SepoaServiceException{
      	super( opt, sinfo );
      	setVersion( "1.0.0" );
  	}
	...
	private String[] et_setEvabdupd1(String[][] SetData, String eval_refitem, String fromdate, String todate, String flag) throws Exception 
	{
	 ...
	 public SepoaOut getTmpList(String tmp_name)	{
		...
		public SepoaOut getEvaList(Map<String, Object> data) 
		{
			...
			String factor_type = "";
			try {
				rtn = getEvaList( e_template_refitem, temp_type, factor_type );
				setValue(rtn);
				setStatus(1);			
			} catch(Exception e) {
				Logger.err.println("Exception e =" + e.getMessage());
		...
		private String getEvaList( String e_template_refitem, String temp_type, String factor_type ) throws Exception 
		{
			String rtn = null;
	   		ConnectionContext ctx = getConnectionContext();
		...
		public SepoaOut setEvaDetailInsert(Map<String, Object> data) 
		{
	    	String rtn[] = null;
			...
			Map<String, String> customheader2 = new HashMap<String, String>();
			customheader2.put("NOTE", header.get("note"));
			customheader2.put("EVAL_VALUER_REFITEM", header.get("eval_valuer_refitem"));

			int count = Integer.parseInt(header.get("count"));
	    	for (int i = 0; i < count; i++) {
				Map<String, String> customheader = new HashMap<String, String>();
				int answered_score = Integer.parseInt(header.get("answered_score_"+i));
				int weight = Integer.parseInt(header.get("weight_"+i));
				int item_max_score = Integer.parseInt(header.get("item_maxscore_"+i));
				String SELECTED_SEQ_SCORE = String.valueOf((answered_score*(weight/item_max_score)));
				
				customheader.put("E_SELECTED_FACTOR", header.get("e_factor_refitem_"+i));
				customheader.put("SELECTED_SEQ", header.get("answered_num_"+i));
				customheader.put("EVAL_VALUER_REFITEM", header.get("eval_valuer_refitem"));
				customheader.put("SELECTED_SEQ_SCORE", SELECTED_SEQ_SCORE);
	    	...
	    	try 
	    	{
	    		rtn = setEvaDetailSave(headerlist, customheader2);
	    		setFlag(true);
	    		setValue( rtn[0] );
	    		setStatus(1);
	  	...
	  	private String[] setEvaDetailSave(List<Map<String, String>> headerlist, Map<String, String> customheader) throws Exception 
	  	{
	    	String returnString[] = new String[ 2 ];
	    	ConnectionContext ctx = getConnectionContext();
			...
			try{
				...
				SepoaXmlParser wxp =  new SepoaXmlParser(this,"setEvaDetailSave_1");
				sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp);
	  	    	rtnIns = sm.doInsert(headerlist);
	  	    	  	    	
	  	    	wxp =  new SepoaXmlParser(this, "setEvaDetailSave_2");
	    	    sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp);
	    	   	rtnIns = sm.doInsert(customheader);
	  	    	Commit();
	  	...
	  	public SepoaOut checkComplete(Map<String, Object> data) 
	  	{
	    	String rtn[] = null;
			Map<String, String> header = MapUtils.getMap(data, "headerData");
			String eval_item_refitem = header.get("eval_item_refitem");
			String eval_refitem = header.get("eval_refitem");

	    	try 
	    	{
	    		rtn = SR_checkComplete(eval_item_refitem, eval_refitem, "", "");
	  	...
	  	private String[] SR_checkComplete(String eval_item_refitem, String eval_refitem, String dept1, String dept2) throws Exception 
	  	{
	    	...
	    	headerdata.put("eval_refitem",eval_refitem);
	    	headerdata.put("eval_item_refitem",eval_item_refitem);
			...
			try
			{
	    		...
	    		//현재 일반템플릿만 사용.
	    		SepoaXmlParser wxp =  new SepoaXmlParser(this,"SR_checkComplete_1");
	    		
				
				sm = new SepoaSQLManager(info.getSession( "ID" ),this,ctx,wxp);
	  	    	String rtn = sm.doSelect(headerdata);
	  	    	wf =  new SepoaFormater(rtn);
				...
				wxp =  new SepoaXmlParser(this,"SR_checkComplete_2");		
				sm = new SepoaSQLManager(info.getSession( "ID" ),this,ctx,wxp);
	    	    ...
	    	    //해당 업체에 대한 평가가 완료 되었을 경우 
	  	    	...
	  	    	{
		  	    	//해당 업체에 대한 평가 완료 - 점수부여
	  	    		wxp =  new SepoaXmlParser(this,"SR_checkComplete_3");
	  	    		headerdata.put("eval_score", eval_score);
	  	    		headerdata.put("eval_refitem", eval_refitem);
	  	    		headerdata.put("eval_item_refitem", eval_item_refitem);
					
			    	sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp);
			    	rtnIns = sm.doUpdate(headerdata);
		    	    	
			    	//************************************//
			    	//평가 전체가 완료되었는지 체크
					if("IV".equals(doc_type)){	//검수요청 평가일 경우 분기 처리.
						wxp =  new SepoaXmlParser(this,"SR_checkComplete_4_1");
					}else{
				    	wxp =  new SepoaXmlParser(this,"SR_checkComplete_4");
					}
		    		...
		    		if(isAllComplete) 
		    		{
		    			...
		    			//평가상태[3:평가완료] 업데이트
		    			wxp =  new SepoaXmlParser(this,"SR_checkComplete_5");
				    	sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp);
				    	rtnIns = sm.doUpdate(headerdata);
...
//						try{
							headerdata.put("eval_flag",strEvalFlag);
							headerdata.put("doc_no",doc_no);
							headerdata.put("doc_count",doc_count);

							sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp_);
					    	rtnIns = sm.doUpdate(headerdata);
//						}catch(NullPointerException ne){
//							
//						}
