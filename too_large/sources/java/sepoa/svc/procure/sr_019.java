...
public class SR_019 extends SepoaService{
	...
	String QUA_FACTOR_NO_19 = "19";			//재거래 의사
  	...
  	public SR_019( String opt, SepoaInfo info ) throws SepoaServiceException{
		super(opt, info);
		setVersion("1.0.0");
...
public SepoaOut getEvalProperty(String eval_refitem){
	...
	private String[] et_setEvabdupd1(String[][] SetData, String eval_refitem, String fromdate, String todate, String flag) throws Exception 
	{
	 ...
	 public SepoaOut getTmpList(String tmp_name)	{
		...
		public SepoaOut getEvaList(Map<String, String> data)
		{
			String rtn = null;
	   		ConnectionContext ctx = getConnectionContext();			
	        ...
	        data.put("SG_REFITEM_SQL", "AND A.SG_REFITEM IN('"+data.get("sg_refitem")+"')");
			try
			{
				setValue(sm.doSelect(data));
				setStatus(1);
			}catch(Exception e){
				Logger.err.println("Exception e =" + e.getMessage());
				setStatus(0);
		...
		private String[] getEvaInsert(List<Map<String, String>> grid, Map<String, String> header) throws Exception 
		{
			...
			try 
			{
				...
				if(MapUtils.getString(grid.get(0), "qty_yn").equals("Y"))//평가 템플릿이 정량평가 항목으로만 구성되어 있으면 자동평가
					auto = "N";
				else
					auto = "Y";

				
	    		SepoaXmlParser wxp =  new SepoaXmlParser(this,"getEvaInsert_0");
				sm = new SepoaSQLManager(info.getSession( "ID" ),this,ctx,wxp);
	  	    	String rtn = sm.doSelect(new HashMap());
	  	    	wf =  new SepoaFormater(rtn);
				...
				//평가테이블 생성
		    	header.put("max_eval_refitem", max_eval_refitem);
		    	header.put("auto", auto);
		    	header.put("DOC_TYPE", "");
		    	header.put("DOC_NO", "");
		    	header.put("DOC_COUNT", "");

	    		wxp =  new SepoaXmlParser(this,"getEvaInsert_1");				
				sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp);
				...
				for ( int i = 0; i < grid.size(); i++ )
				{
					...
					customdata.put("max_eval_refitem", max_eval_refitem);
					sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp);
