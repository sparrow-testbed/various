...
public class SR_012 extends SepoaService {

    public SR_012(String opt, SepoaInfo info) throws SepoaServiceException {
		super(opt, info);
		setVersion("1.0.0");
	...
	public SepoaOut getChkDetail(String c_factor_refitem) {
		setStatus(1);
		setFlag(true);
		...
		try {
			...
			no.put("c_factor_refitem", c_factor_refitem);
  	...
  	public SepoaOut setChkInsert(Map<String, Object> data) throws Exception{
		...
		Message msg = null;
		double currVal = getCurrVal("icomvcfh", "c_factor_refitem");
		int rtn=0;
		setMessage("성공");
		try {
			...
			header.put("c_factor_refitem", String.valueOf(currVal));
			...
			for(int i=0; i<rtn; i++){
				...
				customheader.put("c_factor_refitem", header.get("c_factor_refitem"));
  	 ...
  	 public SepoaOut setChkUpdate(Map<String, Object> data) throws Exception{
     	setStatus(1);
 		setFlag(true);
 		ConnectionContext ctx = getConnectionContext();
 		int rtn=0;
     	...
     	try{
 			...
 			for(int i=0; i<rtn; i++){
 				...
 				customheader.put("item_name", header.get("itemName_"+(i+1)));
 				customheader.put("seq", String.valueOf(i+1));					
 				customheader.put("item_score", header.get("itemScore"));
 				customheader.put("c_factor_refitem", header.get("c_factor_refitem"));
    ...
    public SepoaOut chkUpdateChkItem(String c_factor_refitem) {
		setStatus(1);
		setFlag(true);
		ConnectionContext ctx = getConnectionContext();
