...
public class SR_021 extends SepoaService{
	...
	String QUA_FACTOR_NO_19 = "19";			//재거래 의사
  	...
  	public SR_021( String opt, SepoaInfo sinfo ) throws SepoaServiceException{
      	super( opt, sinfo );
      	setVersion( "1.0.0" );
  	}
  	...
  	public SepoaOut getEvaList(Map<String, Object> data) {
		...
		String rtn = null;
		String evalname = header.get("evalname");
		String status = header.get("status_sel");
		String from_date = SepoaString.getDateUnSlashFormat(header.get("from_date"));
		String to_date = SepoaString.getDateUnSlashFormat(header.get("to_date"));
	
		try 
		{
			rtn = getEvaList( evalname, status, from_date, to_date);
  	...
  	private String getEvaList(String evalname, String status, String from_date, String to_date) throws Exception 
	{
   		String rtn = null;

   		ConnectionContext ctx = getConnectionContext();
   		Map<String, String> header = new HashMap<String, String>();
		header.put("evalname", evalname);
		header.put("status", status);
		header.put("from_date", from_date);
		header.put("to_date", to_date);
  	 ...
  	 * 평가완료 
  	 */
  	public SepoaOut setEvaComplete(Map<String, Object> data) 
	{
		String rtn[] = null;
		List<Map<String, String>> grid = (List<Map<String, String>>)MapUtils.getObject(data, "gridData");
		Map<String, String> customdata = grid.get(0);
		String eval_refitem = customdata.get("eval_refitem");
	   	try 
	   	{
    		rtn = setEvaComplete(eval_refitem);
  	...
  	private String[] setEvaComplete(String eval_refitem) throws Exception
  	{
  		
    	String returnString[] = new String[ 2 ];
    	ConnectionContext ctx = getConnectionContext();
    	Map<String, String> customdata = new HashMap<String, String>();
    	customdata.put("eval_refitem",eval_refitem);
		...
		try
		{
			...
			SepoaXmlParser wxp =  new SepoaXmlParser(this,"setEvaComplete_1");
			
 	    	sm = new SepoaSQLManager(info.getSession( "ID" ),this,ctx,wxp);
    	    String rtn = sm.doSelect(customdata);
			...
			//평가대상 업체 가져오기
			wxp =  new SepoaXmlParser(this,"setEvaComplete_2");
			sm = new SepoaSQLManager(info.getSession( "ID" ),this,ctx,wxp);
			rtn = sm.doSelect(customdata);
			wf =  new SepoaFormater(rtn);
			...
			for(int i=0; i < cnt; i++) 
			{
				...
				mapdata.put("eval_item_refitem", wf.getValue(i, 0));
				listdata.add(mapdata);
			}
			
			SepoaXmlParser wxp_3 =  new SepoaXmlParser(this,"setEvaComplete_3");
				
			for(int j=0; j < cnt; j++) 
			{
				
				eval_score = "0";
				sm = new SepoaSQLManager(info.getSession( "ID" ),this,ctx,wxp_3);
    	    	rtn = sm.doSelect(listdata.get(j));
				wf =  new SepoaFormater(rtn);
				...
				wxp =  new SepoaXmlParser(this,"setEvaComplete_4");
				customdata.put("eval_score",eval_score);
				customdata.put("eval_item_refitem",listdata.get(j).get("eval_item_refitem"));
		    	sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp);
		    	rtnIns = sm.doUpdate(customdata);
			...
			wxp =  new SepoaXmlParser(this,"setEvaComplete_6");
			sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp);
			rtnIns = sm.doUpdate(customdata);
...
//				try{
					...
					customdata.put("eval_flag", strEvalFlag);
					customdata.put("doc_no", doc_no);
					customdata.put("doc_count", doc_count);
...
//		if(wf != null && wf.getRowCount() > 0) 
  	...
  	*/
  	...
  	public SepoaOut setEvaDelete(Map<String, Object> data) 
  	{
    	String rtn[] = null;
    	List<Map<String, String>> grid = (List<Map<String, String>>)MapUtils.getObject(data, "gridData");
    	try 
    	{
    		rtn = setEvaDelete(grid);
  	...
  	private String[] setEvaDelete(List<Map<String, String>> grid) throws Exception 
  	{
    	String returnString[] = new String[ 2 ];
    	ConnectionContext ctx = getConnectionContext();    	
    	int rtnIns = 0;
		...
		{		
			...
			{
				i_eval_refitem = grid.get(i).get("eval_refitem");
				Map<String,String> customdata = new HashMap<String, String>();
				customdata.put("i_eval_refitem",i_eval_refitem);

				//평가대상업체 평가자 테이블 삭제
				
				SepoaXmlParser wxp =  new SepoaXmlParser(this,"setEvaDelete_1");
	    		sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp);
	    		rtnIns = sm.doInsert(customdata);

				//평가대상업체 테이블 삭제
	    		wxp =  new SepoaXmlParser(this,"setEvaDelete_2");				 
	    		sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp);
	    		rtnIns = sm.doInsert(customdata);

				//평가테이블 삭제
				wxp =  new SepoaXmlParser(this,"setEvaDelete_3");
	    		sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp);
	    		rtnIns = sm.doInsert(customdata);
