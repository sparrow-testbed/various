...
public class SR_016 extends SepoaService {

    public SR_016(String opt, SepoaInfo info) throws SepoaServiceException {
		super(opt, info);
		setVersion("1.0.0");
	...
	public SepoaOut getEvaDetail(String e_factor_refitem) {
		setStatus(1);
		setFlag(true);
		...
		try {
			...
			no.put("e_factor_refitem", e_factor_refitem);
	...
	public SepoaOut setEvaInsert(Map<String, Object> data) throws Exception{
		...
		Message msg = null;
		double currVal = getCurrVal("icomvefh", "e_factor_refitem");
		int rtn=0;
		setMessage("성공");
		try {
			msg = new Message(info, "SR_016");
			
			Map<String, String> header = MapUtils.getMap(data, "headerData");
			header.put("e_factor_refitem", String.valueOf(currVal));
			...
			for(int i=0; i<rtn; i++){
				...
				customheader.put("e_factor_refitem", header.get("e_factor_refitem"));
    ...
    public SepoaOut setEvaUpdate(Map<String, Object> data) throws Exception{
    	setStatus(1);
		setFlag(true);
		ConnectionContext ctx = getConnectionContext();
		int rtn=0;
    ...
    public SepoaOut chkUpdateEvaItem(String e_factor_refitem) {
		setStatus(1);
		setFlag(true);
		ConnectionContext ctx = getConnectionContext();
