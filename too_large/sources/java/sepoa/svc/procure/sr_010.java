...
public class SR_010 extends SepoaService {

    public SR_010(String opt, SepoaInfo info) throws SepoaServiceException {
		super(opt, info);
		setVersion("1.0.0");
	...
	public SepoaOut getTempDetail(String s_template_refitem) {
		setStatus(1);
		setFlag(true);
	...
	public SepoaOut setTempInsert(Map<String, Object> data) throws Exception{
		...
		Message msg = null;
		double currVal = getCurrVal("icomvsth", "s_template_refitem");
		String rtn= "";
		setMessage("성공");
		try {
			...
			header.put("s_template_refitem", String.valueOf(currVal));
			...
			for(int i=0; i<row_cnt; i++){
				...
				customheader.put("s_template_refitem", header.get("s_template_refitem"));
				
				customheader.put("weight", as.nextToken());
				customheader.put("s_factor_refitem", ts.nextToken());
	...
	public SepoaOut chkUpdateTemp(String s_template_refitem) {
		setStatus(1);
		setFlag(true);
		ConnectionContext ctx = getConnectionContext();
