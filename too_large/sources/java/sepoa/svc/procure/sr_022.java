...
public class SR_022 extends SepoaService{
	...
	String QUA_FACTOR_NO_19 = "19";			//재거래 의사
  	...
  	public SR_022( String opt, SepoaInfo sinfo ) throws SepoaServiceException{
      	super( opt, sinfo );
      	setVersion( "1.0.0" );
  	}
...
public SepoaOut getEvaDetailList(Map<String, String>data){
		
		String rtn = null;
		try{
			String eval_refitem = data.get("eval_refitem");
			String user_id = info.getSession("ID");
			
			rtn = getEvaDetailList( eval_refitem );
			setValue(rtn);
			setStatus(1);
	...
	private String getEvaDetailList(String eval_refitem) throws Exception {
	...
	private String et_getEvabdupd2(String eval_item_refitem) throws Exception
	{
		...
		SepoaXmlParser wxp =  new SepoaXmlParser(this,"et_getEvabdupd2");
		Map<String, String> data = new HashMap<String, String>();
	...
	public SepoaOut getEvaInsert(Map<String, Object> data) throws Exception
	{
		setStatus(1);
		setFlag(true);
		String rtn[] = null;
		...
		Map<String, String> header = MapUtils.getMap(data, "headerData");
		header.put("fromdate", SepoaString.getDateUnSlashFormat(header.get("fromdate")));
		header.put("todate", SepoaString.getDateUnSlashFormat(header.get("todate")));
		try
		{
			rtn = getEvaInsert(grid, header);
	...
	private String[] getEvaInsert(List<Map<String, String>> grid, Map<String, String> header) throws Exception 
	{
    	String returnString[] = new String[ 2 ];
    	ConnectionContext ctx = getConnectionContext();
		...
		{
			//평가테이블 수정
			String fromdate = SepoaString.getDateUnSlashFormat(MapUtils.getString(header, "fromdate", ""));
			String todate = SepoaString.getDateUnSlashFormat(MapUtils.getString(header, "todate", ""));
			header.put("fromdate",fromdate);
			header.put("todate",todate);
			SepoaXmlParser wxp =  new SepoaXmlParser(this,"getEvaInsert_1");
			...
			for ( int i = 0; i < grid.size(); i++ )
        	{
				...
				i_sg_refitem = MapUtils.getString(grid.get(i), "sg_refitem");
				i_vendor_code = MapUtils.getString(grid.get(i), "vendor_code");
				i_value_id 	= MapUtils.getString(grid.get(i), "value_id");
				i_eval_item_refitem = MapUtils.getString(grid.get(i), "eval_item_refitem");
				...
				if(i == 0)
				{
		    		...
		    		wxp =  new SepoaXmlParser(this,"getEvaInsert_4");
					...
					*/
					...
					wxp =  new SepoaXmlParser(this,"getEvaInsert_2");
				...
				wxp =  new SepoaXmlParser(this,"getEvaInsert_3");
				customdata.put("i_sg_refitem", i_sg_refitem);
				customdata.put("i_vendor_code", i_vendor_code);
    	    	
	    		sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp);
	    		rtnIns = sm.doInsert(customdata, header);
	        	...
	        	for ( int ii = 0; ii < row_cnt; ii++ )
	        	{
					...
					getCurrVal = Double.toString(getCurrVal("icomvevd", "eval_item_refitem"));
					//평가대상업체 평가자 테이블 생성
					wxp =  new SepoaXmlParser(this,"getEvaInsert_5");
					idata.put("house_code", house_code);
					idata.put("i_dept", i_dept);
					idata.put("i_id", i_id);
					idata.put("getCurrVal", getCurrVal);
					
		    		sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp);
		    		rtnIns = sm.doInsert(idata);
				}
        	}	
