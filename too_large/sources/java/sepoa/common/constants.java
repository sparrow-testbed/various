...
public class Constants {
	...
	public static final String DEFAULT_COMPANY_CODE = "0001"; // 하드코딩을 피하기 위하여
															  // 기본적으로 사용될 회사코드
	/** 하드코딩을 피하기 위하여 기본적으로 사용될 회사명(동아) */
	public static final String DEFAULT_COMPANY_NAME = "동아"; // 하드코딩을 피하기 위하여
															// 기본적으로 사용될 회사명

	/** 하드코딩을 피하기 위하여 기본적으로 사용될 사업장코드(A0) */
	public static final String DEFAULT_PLANT_CODE = "A0"; // 하드코딩을 피하기 위하여 기본적으로
														  // 사용될 사업장코드
	/** 하드코딩을 피하기 위하여 기본적으로 사용될 사업장명(동아) */
	public static final String DEFAULT_PLANT_NAME = "동아"; // 하드코딩을 피하기 위하여 기본적으로
														  // 사용될 사업장명
	...
	public static final String DEFAULT_TO_DATE = "20991231"; // 동아 통합구매시스템에서 추가
															 // CHO
	                                                         // 신규로 단가계약시 계약시작일은
															 // 오늘로 자동입력되나
															 // 계약종료일은 필수입력값이 아님
	...
	public static final String DEFAULT_SELLER_COMPANY_CODE = "S999"; // 하드코딩을
																	 // 피하기 위하여
																	 // 기본적으로
																	 // 사용될 회사코드
	public static final String DEFAULT_SELLER_USER_ID = "SUPPLIER"; // 하드코딩을 피하기
																	// 위하여 기본적으로
																	// 사용될 회사코드
	...
	/**
	 * 동아 추가 2014.03.05 CHO PLANT_CODE 상태에 사용되는 열거형자료 SCODE.미등록 <br/>
	 * Cheonan("25") // 천안 <br/>
	 * ,Dalseong("29") // 달성 <br/>
	 * ,Banwoul("21") // 반월 <br/>
	 * ,DMB("23") // DMB <br/>
	...
	/**
	 * 동아 추가 2014.03.05 CHO PRGubun 상태에 사용되는 열거형자료 SCODE.미등록 <br/>
	 * Usual("AA") // 일반 <br/>
	...
	/**
	 * 동아 추가 2014.03.05 CHO 컴파니코드 상태에 사용되는 열거형자료 SCODE.미등록 <br/>
	 * ST("0001") // ST <br/>
	 ...
	 * , Approved("E") // 승인완료 <br/>
	 * , Rejected("R") // 반려 <br/>
	 * , ApprovalCancellation("A") // 승인취소 <br/>
	 * 
	 * D009 : 그룹전자결재상태
	 */
	...
	public enum PRStatus { // PR_STATUS 상태에 사용되는 열거형자료 SCODE.M157/M159/M420
		SelectSeller("A") // 업체선정
		, ConsultCompleted("B") // 품의완료
		, Estimating("C") // 견적진행
		, ExplanNotice("E") // 현설공고
		, PurchaseRequest("P") // 구매요청
		, PurchaseRejected("R") // 구매반송
		, TTT("T") // ???, SCODE 에는 없는데 소스에는 있는 하드코딩 되어있던 문자이다.
		;
	 ...
	 * <br/>
	 */
	public enum PRProceedingFlag { // PR_PROCEEDING_FLAG 상태에 사용되는 열거형자료
								   // SCODE.M159
		SelectSeller("A") // 업체선정
		, Estimating("C") // 견적진행
		, ExplanNotice("E") // 현설공고
	...
	/**
	 * 
	 * 동아 통합구매시스템 CHO 대분류 상태에 사용되는 열거형자료 <br/>
	 * A01("A01") // A01, 원료 <br/>
	 * ,A02("A02") // A02, 재료 <br/>
	 * ,A03("A03") // A03, 상품 <br/>
	 * ,A04("A04") // A04, 외주가공 <br/>
	...
	/**
	 * 
	 * 동아 통합구매시스템 CHO PR_TYPE 상태에 사용되는 열거형자료 <br/>
	 ...
	 * , Approved ( "E" ) // 승인<br/>
	 */
	public enum mroEstStatus { // EST_SIGN_STATUS 상태에 사용되는 열거형자료 SCODE.M240 ->
							   // MRO품목 승인/조회
		Requested("T") // 견적요청
		, Received("S") // 견적수신
		, ReEstimated("R") // 재견적
	...
	/**
	 * MRO I/F 용 트리거테이블(dbo.KEPTRXL)의 문서구분(DOC_NAME)에 사용되는 열거형자료 PO(발주), GR(입고),
	 * RE(견적)<br/>
	 * PuchaseOrder ( "PO" ) // 발주<br/>
	 * , Warehousing ( "GR" ) // 입고<br/>
	 * , Estimation ( "RE" ) // 견적<br/>
	 */
	public enum mroDocName { // MRO I/F 용 트리거테이블(dbo.KEPTRXL)의 문서구분(DOC_NAME)에
							 // 사용되는 열거형자료 PO(발주), GR(입고), RE(견적)
		PuchaseOrder("PO") // 발주
		, Warehousing("GR") // 입고
		, Estimation("RE") // 견적
		;
	 ...
	 * , Estimation ( "RE" ) // 견적<br/>
	 */
	public enum ApprovalDocType { // MRO I/F 용 트리거테이블(dbo.KEPTRXL)의
								  // 문서구분(DOC_NAME)에 사용되는 열거형자료 PO(발주), GR(입고),
								  // RE(견적)
		PuchaseOrder("PO") // 발주
		, Warehousing("GR") // 입고
		, Estimation("RE") // 견적
	...
	/**
	 * 전자 결재시 사용 되는 form id FormIn0001( "853" ) // (협조)동아쏘시오홀딩스 :H FormIn0002(
	 * "852" ) // (협조)동아ST :E FormIn0003( "854" ) // (협조)동아제약 :O FormCo0001(
	 * "850" ) // (내부)동아쏘시오홀딩스 :H FormCo0002( "849" ) // (내부)동아ST :E FormCo0003(
	 * "851" ) // (내부)동아제약 :O
	 * 
	 */
	...
	/**
	 * 의뢰형태 SCODE M427 Tender("B") 입찰의뢰<br/>
	 * ,Purch_req("P") 구매의뢰<br/>
	 * ,CCC("C") // ???? <br/>
	 * <br/>
	 */
	...
	/**
	 * 낙찰상태 SCODE M121 Success("B") 낙찰<br/>
	 * ,Failure("P") 유찰<br/>
	 */
	...
	public enum ApprovalDept { // 결재 권한 부서 코드 : D012
		codeScodeType("D012"), codeDept01("C230"), codeCcompany01("0002") // 구매부
																		  // 회사코드
		...
		/*
		 * //해당 값 사용시 참조 int foreachIndex = 0; String foreachDept = ""; String
		 * foreachComp = ""; for( sepoa.svc.common.constants.ApprovalDept
		 * approvalDept : sepoa.svc.common.constants.ApprovalDept.values() ){
		 * if( foreachIndex == 0 ){ foreachIndex++; continue;
		 * 
		 * }
		 * 
		 * foreachDept = ""; foreachComp = "";
		 * 
		 * if( ( foreachIndex % 2 ) == 1 ){ foreachDept =
		 * approvalDept.getValue(); System.out.println( "foreachDept : " +
		 * foreachDept );
		 * 
		 * } else { foreachComp = approvalDept.getValue(); System.out.println(
		 * "foreachComp : " + foreachComp );
		 * 
		 * }
		 * 
		 * foreachIndex++;
	...
	/**
	 * D006 : 납품요청일 Code001("7" ) // 발주일 7일 이내 , Code002("15" ) // 발주일 15일 이내 ,
	 * Code003("30" ) // 발주일 30일 이내 , Code004("60" ) // 발주일 60일 이내 ,
	 * Code005("90" ) // 발주일 90일 이내 , Code006("ETC") // 기타 <br/>
	 */
	public enum RdCode {
		Code001("7") // 발주일 7일 이내
