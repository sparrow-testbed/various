...
public class ap_wait_list extends HttpServlet {
	...
	public GridData getWaitList(GridData gdReq, SepoaInfo info)
		...
		try {
			...
			SepoaOut value = ServiceConnector.doService(info, "AP_001",	"CONNECTION", "getWaitList", obj);
		...
		try {
			...
			if (app_type.equals("E")) { // 승인일때 ( 차기결제자가 있을 경우와 마지막 최종 결제일때로 분리
				...
				if (AppNext.size() != 0) {	//차기결재자가 있을 경우
					...
					Object[] obj = { data };
					SepoaOut value = ServiceConnector.doService(info, "AP_001","TRANSACTION", "setUpdate", obj);
					if (value.flag) {
						gdRes.setMessage(message.get("MESSAGE.0001").toString());
						gdRes.setStatus("true");
					...
					Object[] obj = { data };
					SepoaOut value = ServiceConnector.doService(info, "AP_001", "TRANSACTION", "setEndApp", obj);
...
//					ServiceConnector.doService(info, "SMS",  "TRANSACTION", "rqApplyE", smsObj);
				...
				Object[] obj = { data };
				SepoaOut value = ServiceConnector.doService(info, "AP_001",	"TRANSACTION", "setRefund", obj);
				if (value.flag) {
					gdRes.setMessage(message.get("MESSAGE.0001").toString());
					gdRes.setStatus("true");
				...
				if (AppNext.size() != 0) {
					Object[] obj = { data };
					SepoaOut value = ServiceConnector.doService(info, "AP_001", "TRANSACTION", "setInsert", obj);
					if (value.flag) {
						gdRes.setMessage(message.get("MESSAGE.0001").toString());
						gdRes.setStatus("true");
