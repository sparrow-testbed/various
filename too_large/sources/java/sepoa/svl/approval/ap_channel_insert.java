...
public class ap_channel_insert extends HttpServlet {
    ...
    public GridData getChannelList(GridData gdReq, SepoaInfo info) throws Exception {
		...
		try {
			...
			SepoaOut value = ServiceConnector.doService(info, "AP_007", "CONNECTION", "getChannelList", obj);
			...
			for (int i = 0; i < wf.getRowCount(); i++) {
				for(int k=0; k < grid_col_ary.length; k++) {
                	...
                	}else if(grid_col_ary[k] != null && grid_col_ary[k].equals("USER_POP")) {
    ...
    public GridData setChannelInsert(GridData gdReq, SepoaInfo info) throws Exception {
		...
		try {
			...
			Object[] obj = {data};
			SepoaOut value = ServiceConnector.doService(info, "AP_007", "TRANSACTION","setChannelInsert", obj);
			if(value.flag) {
				gdRes.setMessage(message.get("MESSAGE.0001").toString()); 
				gdRes.setStatus("true");
			}else {
    ...
    public GridData setChannelUpdate(GridData gdReq, SepoaInfo info) throws Exception {
		...
		try {
			...
			Map<String, Object> data = SepoaDataMapper.getData(info, gdReq);
			Object[] obj = {data};
			SepoaOut value = ServiceConnector.doService(info, "AP_007", "TRANSACTION","setChannelUpdate", obj);
			if(value.flag) {
				gdRes.setMessage(message.get("MESSAGE.0001").toString()); 
				gdRes.setStatus("true");
			}else {
    ...
    public GridData setChannelDelete(GridData gdReq, SepoaInfo info) throws Exception {
		...
		try {
			...
			Map<String, Object> data = SepoaDataMapper.getData(info, gdReq);
			Object[] obj = {data};
			SepoaOut value = ServiceConnector.doService(info, "AP_007", "TRANSACTION","setChannelDelete", obj);
			if(value.flag) {
				gdRes.setMessage(message.get("MESSAGE.0001").toString()); 
				gdRes.setStatus("true");
			}else {
