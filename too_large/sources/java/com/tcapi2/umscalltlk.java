/* TOBE 2018-11-01 UMS SMS 문자 발송 EAI */
...
public class UMSCALLTLK extends SepoaService implements I_Wms  {
	  
    public IF_UMSCALLTLK_S  SEND      =  null;
    public IF_UMSCALLTLK_R  RECV      =  null;
    public IF_OT0000E        eRECV     =  null;    
    public static String     PGMID     =  null;
    public eaiCom            eaiCom    =  null; //TOBE 2018-11-01 EAI 공통처리부
    ...
    public UMSCALLTLK () {    	
        this.SEND    = new IF_UMSCALLTLK_S();
        this.RECV    = new IF_UMSCALLTLK_R();
        this.eRECV   = new IF_OT0000E();
        PGMID = "UMSCALLTLK";
        this.send_msg        = "";
        this.pay_send_no     = "";

        //TOBE 2018-11-01 공통처리부
    	...
    	SEND.sysHdr.INTF_ID                	     =  "EPSER0000023"           ;   //(문자	12        )  인터페이스 ID            
    	...
    	SEND.trnHdr.TRN_CD                 =  "TLKSMS";                //(문자	9   )  거래코드                   
    	SEND.trnHdr.SRVC_ID                =  "ISGF_000023";           //(문자	11  )  서비스ID                   
	...
	public	int sendMessage(String EventCode, String servAddress, int servPort ) {
		if(EventCode.equals("UMSCALLTLK")) {
			...
			try {  
				    ...
				    socket.setSoTimeout(1000*90); //TOBE 2018-11-01  15 -> 90    
				    /* TCP DATA SEND */
				    ...
				    //test start 
				    //System.out.println("debug:UMSCALLTLK:length:"+slen);
...
//							SepoaOut value = ServiceConnector.doService(info, "IF_001", "CONNECTION", "insert_if_msg", obj);
					    	...
					    	//TOBE 2018-11-01 메세지코드 (오류발생시 에러코드, 정상처리시 정상코드)
					    	RECV.setR(r_data, r_data.length,"EUC-KR");
					    	pvUtil.msglog(ONCNF.LOGNAME," + RECV.msgHdr.MSG_CD = '"+RECV.msgHdr.MSG_CD +"'");
					    	
					    	// 전문응답 오류이면 .. 
					    	//ASIS 2018-11-01 if(!RECV.bizHdr.ERRCODE.trim().equals("")){
					    	//TOBE 2018-11-01
					    	if(!"N".equals(RECV.sysHdr.RSP_RST_DSCD.trim()) ){
					    		//ASIS 2018-11-01 eRECV.set(r_data, r_data.length,"EUC-KR");
