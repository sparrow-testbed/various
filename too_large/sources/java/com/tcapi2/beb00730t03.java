...
//ASIS 2017-07-01 TO08603 집행실행
public class BEB00730T03 extends SepoaService implements I_Wms  { //TOBE 2017-07-01 추가 SepoaService
	  
    public IF_BEB00730T03_S  SEND      =  null;
    public IF_BEB00730T03_R  RECV      =  null;
    ...
    public BEB00730T03 () {    	
        this.SEND    = new IF_BEB00730T03_S();
        this.RECV    = new IF_BEB00730T03_R();
		...
		//if(SEND.bioHdr.IBIO.length()>0){
        //	SEND.bioHdr.iBioLen =  669;
        // }
        /*
     	SEND.comHdr.TOT_SIZE   = "";;
		...
		*/
        ...
        SEND.bizHdr.INLEN                    = "";    //(5)  N전문전체길이-->헤더+전문길이                                                                      
        ...
        SEND.bizHdr.TRM_BNO                  = "002";                //(3)  PDA의단말번호는'20481'->                                                                     
        SEND.bizHdr.USER_TRM_NO              = "4";                //(1)  사용자단말번호                                                                                
        //SEND.bizHdr.UID                      = "19116877";                //(8)  PDA조작자번호88799999                                                                       
        ...
        /*책임자*///SEND.bizHdr.MGR_BIOAUT_YN            = "Y";                //(1)  책임자생체인식여부
        /*
        ...
        */                                              
        /*책임자*///SEND.bizHdr.MGRAPV_DSCD              = "2";                //(1)  책임자승인구분코드-->'1'표준(책임자승인거래아님)  ,'2'항시승인,'3'조건부승인
        /*
        ...
        */
    	...
    	SEND.sysHdr.INTF_ID                	     =  "EPSER0000018"           ;   //(문자	12        )  인터페이스 ID            
    	...
    	SEND.trnHdr.SRVC_ID                =  "BEB00730T03";           //(문자	11  )  서비스ID                   
	...
	public	int sendMessage(String EventCode, String servAddress, int servPort ) {
		if(EventCode.equals("BEB00730T03")) {
		    ...
		    try {
			    	...
			    	/*----------------------------------------*/
//		    	System.out.println("--------SEND.bioHdr.IBIO.length()---------->"+SEND.bioHdr.IBIO.length());
		    	if(SEND.bioHdr.IBIO.length()>0){
		    		SEND.getBEB00730T03B(s_data,"UTF-8", SEND.bioHdr.iBioLen);
		    	}else{
		    		SEND.getBEB00730T03S(s_data,"UTF-8");
		    	}
			...
			try {  
				    ...
				    //int slen = SEND.comHdr.iTLen + SEND.bizHdr.iTLen + SEND.iTLen;
			        int slen = 0;
				    
			        if(SEND.bioHdr.IBIO.length()>0){
				    	slen = SEND.sysHdr.iTLen + SEND.trnHdr.iTLen + SEND.bioHdr.iBioLen + SEND.iTLen ;			//common(34) + header(200) + content() + list() + im(5)
				    }
				    else{
				    	slen = SEND.sysHdr.iTLen + SEND.trnHdr.iTLen + SEND.iTLen ;			//common(34) + header(200) + content() + list() + im(5)
				    }
				    ...
				    //test start 
				    //System.out.println("debug: BEB00730T03 length:"+slen);
