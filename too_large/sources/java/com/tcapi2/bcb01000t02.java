...
import com.tcJun2.IF_BCB01000T02_R;
import com.tcJun2.IF_BCB01000T02_S;
...
public class BCB01000T02 extends SepoaService implements I_Wms  {

	
	public IF_BCB01000T02_S    SEND      = null;
	public LIST_BCB01000T02_S  LISTSEND  = null;			//IM 반복문 LIST
    public IF_BCB01000T02_R    RECV      = null;
    ...
    public BCB01000T02 () {    	
        this.SEND     = new IF_BCB01000T02_S();
        this.LISTSEND = new LIST_BCB01000T02_S(); 
        this.RECV     = new IF_BCB01000T02_R();
    	...
    	SEND.sysHdr.INTF_ID                	     =  "EPSER0000019"           ;   //(문자	12        )  인터페이스 ID            
    	...
    	SEND.trnHdr.SRVC_ID                =  "BCB04000T02";           //(문자	11  )  서비스ID                   
    	SEND.trnHdr.FUNC_CD                =  "30";                    //(문자	2   )  기능 코드  (00:실행,20:조회,30:등록)                
	...
	public int sendMessage(String EventCode, String servAddress, int servPort){
		int rtn = 0;
		if(EventCode.equals("BCB01000T02")) {
...
//System.out.println("debug:BCB01000T02:loopCnt:"+Integer.parseInt(SEND.TTAL.toString()));	
...
//	        SEND.bizHdr.MGRAPV_DSCD              = "1";                //(1)  책임자승인구분코드-->'1'표준(책임자승인거래아님)  ,'2'항시승인,'3'조건부승인                                        
	        ...
	        SEND.bizHdr.SYS_ID_NO                = "0304";                //(4)  CICSSYSID-->'0304'                                                                     
	    	...
	    	int listCnt = Integer.parseInt(SEND.TTAL.toString());
            */
			
			
			int listCnt = Integer.parseInt(SEND.DACC_CST_CNT.toString()); //TOBE 2017-07-01 일계조립건수
	        ...
	        try {
		    	...
		    	//SEND.get870100S(s_data,"UTF-8", (LISTSEND.iTLen*listCnt));
	        	if(SEND.bioHdr.iBioLen > 0){
//		    		SEND.getBCB01000T02B(s_data,"UTF-8", (LISTSEND.iTLen*listCnt), SEND.bioHdr.iBioLen);  //AS-IS [R102104204668] [2021-06-30] 전자구매 자본예산대금지급 취소시 일일감사 개선
	        		SEND.getBCB01000T02B(s_data,"EUC-KR", (LISTSEND.iTLen*listCnt), SEND.bioHdr.iBioLen); //[R102104204668] [2021-06-30] 전자구매 자본예산대금지급 취소시 일일감사 개선
		    		 
		    	}else{
//		    		SEND.getBCB01000T02S(s_data,"UTF-8", (LISTSEND.iTLen*listCnt));  //AS-IS [R102104204668] [2021-06-30] 전자구매 자본예산대금지급 취소시 일일감사 개선
		    		SEND.getBCB01000T02S(s_data,"EUC-KR", (LISTSEND.iTLen*listCnt)); //[R102104204668] [2021-06-30] 전자구매 자본예산대금지급 취소시 일일감사 개선
		    		
		    	} 
		    	/*
		    	//업무로직에 따라 반복문 LOOP 실행
		    	// BS전문 개수가 5개일 때		    	 
		    	*/
		    	
		    	//LISTSEND.get870100L(l_data, "UTF-8", listCnt);
...
//		    		System.out.println(pvUtil.logTime()+PGMID+" :0000: DATA FORMAT ERROR !!.." +ne.toString());
			...
			try {  
			    ...
			    socket.setSoTimeout(1000*180); //TOBE 2017-07-01  15 -> 180
			    /* tcp data recv */
			    ...
			    /* ASIS 2017-07-01
			    if(SEND.bioHdr.iBioLen > 0){
			    	slen = SEND.comHdr.iTLen + SEND.bizHdr.iTLen + SEND.bioHdr.iBioLen + SEND.iOrgLen + LISTSEND.iTLen*listCnt + 5 + 27;			//common(34) + header(200) + content() + list() + im(5)
			    }
			    else{
			    	slen = SEND.comHdr.iTLen + SEND.bizHdr.iTLen + SEND.bioHdr.iBioLen + SEND.iOrgLen + LISTSEND.iTLen*listCnt + 5;			//common(34) + header(200) + content() + list() + im(5)
			    }
			    */

			    
			    /* TOBE 2017-07-01 pay_bd_app.java 에서 SEND.bioHdr.iBioLen 길이 값을 넘겨주고 있음 */
			    slen = SEND.sysHdr.iTLen + SEND.trnHdr.iTLen + SEND.bioHdr.iBioLen + SEND.iOrgLen + LISTSEND.iTLen*listCnt;
...
//test start 
//System.out.println("debug:BCB01000T02:length:"+slen);
...
//					    	System.out.println("receive:"+bos.toString());
					    	...
					    	System.out.println("debug:BCB01000T02_SUC:"+RECV.TEL_NO);
					    	// 전문응답 오류이면 ..
					    	System.out.println("debug:errcode:"+RECV.sysHdr.ERRCODE+"|"+"END");
					    	System.out.println("debug:errcode:"+RECV.sysHdr.ERRCODE);
					    	*/
...
//					    	System.out.println("ONCNF.D_OK:"+ONCNF.D_OK);
