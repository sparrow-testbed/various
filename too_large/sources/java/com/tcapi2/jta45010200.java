/* TOBE 2017-07-01 재무회계 일계처리 (BS처리) EAI */
...
public class JTA45010200 extends SepoaService implements I_Wms {
	  
    public IF_JTA45010200_S  SEND      =  null;
    public IF_JTA45010200_R  RECV      =  null;
    ...
    public JTA45010200 () {    	
        this.SEND    = new IF_JTA45010200_S();
        this.RECV    = new IF_JTA45010200_R();
    	...
    	SEND.sysHdr.MD_KDCD                	     =  "EEPS2"           ;   //(문자	8         )  매체종류코드                 	
    	SEND.sysHdr.INTF_ID                	     =  "EPSER0000021"           ;   //(문자	12        )  인터페이스 ID            
    	...
    	SEND.trnHdr.TRN_CD                 =  "JTA450102";             //(문자	9   )  거래코드                   
    	SEND.trnHdr.SRVC_ID                =  "JTA45010200";           //(문자	11  )  서비스ID                   
    	...
    	//SEND.trnHdr.ACC_MOD_DSCD           =  "0";                     //(문자	1   )  계정모드구분코드   당일 0 맘감전 2         
	...
	public	int sendMessage(String EventCode, String servAddress, int servPort ) {
		if(EventCode.equals("JTA45010200")) {
	    	Logger.sys.println("sendMessage EvnetCode == JTA45010200 002");								
		    ...
		    try {
		    	...
		    	Logger.sys.println("sendMessage s_date [" + s_data  + "]  003" );										    	
			...
			try {  
				    ...
				    //SEND.trnHdr.ACC_MOD_DSCD = 
				    
				    if (SEND.TRN_STCD.equals("1") )
				    {
				    	slen = SEND.sysHdr.iTLen + SEND.trnHdr.iTLen + SEND.iTLen;
				    }
				    else if (SEND.TRN_STCD.equals("2"))
				    {
				    	slen = SEND.sysHdr.iTLen + SEND.trnHdr.iTLen + SEND.iTlen2;	
				    }
				    /////// SEND DATA 보기 //////////////
				    try {
...
//							param.put("IF_NAME", "JTA45010200"); 
//							param.put("SEND_NO", ""); 
//							param.put("CONTS", f1.toString());
//							
//							Object[] obj = { param };
//							SepoaOut value = ServiceConnector.doService(info, "p6030", "CONNECTION", "insert_if_msg", obj);
