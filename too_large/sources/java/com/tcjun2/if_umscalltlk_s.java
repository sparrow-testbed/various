/* TOBE 2018-11-01 UMS SMS 문자 발송 송신 */

package com.tcJun2;

import com.tcLib2.*;

public class IF_UMSCALLTLK_S extends OUT_MSG {
    ...
    public String  SMKT_CMGN_ID       = ""; //세일즈마케팅캠페인IDVARCHAR(16 ) /////////////////////////////////////////  
...
//  public String  CRTF_NO            = ""; //인증번호            VARCHAR(10 )
    public String  MPNG_1             = ""; //매핑_1              VARCHAR(20 )
    public String  MPNG_2             = ""; //매핑_2              VARCHAR(20 )
    public String  MPNG_3             = ""; //매핑_3              VARCHAR(20 )
    public String  MPNG_4             = ""; //매핑_4              VARCHAR(20 )
    public String  MPNG_5             = ""; //매핑_5              VARCHAR(20 )
    ...
    public String  ENCY_MPNG_TXT_1    = ""; //암호화매핑내용_1    VARCHAR(32 ) /////////////////////////////////////////
    public String  ENCY_MPNG_TXT_2    = ""; //암호화매핑내용_2    VARCHAR(32 ) /////////////////////////////////////////
    public String  ENCY_MPNG_TXT_3    = ""; //암호화매핑내용_3    VARCHAR(64 ) /////////////////////////////////////////
    public String  ENCY_MPNG_TXT_4    = ""; //암호화매핑내용_4    VARCHAR(64 ) /////////////////////////////////////////    
    public String  ENCY_CRTF_NO       = ""; //암호화인증번호      VARCHAR(64 ) /////////////////////////////////////////
    public String  UMS_SMS_CNVSD_YN   = ""; //UMSSMS전환발송여부  VARCHAR(1  ) /////////////////////////////////////////
            
    public String  MPNG_ALL_TITL      = ""; //매핑전체제목        VARCHAR(160)
  
    public static String PGMID = "IF_UMSCALLTLK_S";  
    ...
    private String FieldNames[] = {
            ...
            "SMKT_CMGN_ID" ,     //세일즈마케팅캠페인IDVARCHAR(16 )
...
//          "CRTF_NO"      ,     //인증번호            VARCHAR(10 )
            "MPNG_1"       ,     //매핑_1              VARCHAR(20 )
            "MPNG_2"       ,     //매핑_2              VARCHAR(20 )
            "MPNG_3"       ,     //매핑_3              VARCHAR(20 )
            "MPNG_4"       ,     //매핑_4              VARCHAR(20 )
            "MPNG_5"       ,     //매핑_5              VARCHAR(20 )
            "MPNG_6"       ,     //매핑_6              VARCHAR(50 )
            "MPNG_7"       ,     //매핑_7              VARCHAR(50 )
            "MPNG_8"       ,     //매핑_8              VARCHAR(50 )
            "MPNG_9"       ,     //매핑_9              VARCHAR(50 )
            "MPNG_10"      ,     //매핑_10             VARCHAR(50 )            
            "ENCY_MPNG_TXT_1"      ,     //암호화매핑내용_1             VARCHAR(32 )
            "ENCY_MPNG_TXT_2"      ,     //암호화매핑내용_2             VARCHAR(32 )
            "ENCY_MPNG_TXT_3"      ,     //암호화매핑내용_3             VARCHAR(64 )
            "ENCY_MPNG_TXT_4"      ,     //암호화매핑내용_4             VARCHAR(64 )
            "ENCY_CRTF_NO"         ,     //암호화인증번호               VARCHAR(64 )
            "UMS_SMS_CNVSD_YN"     ,     //UMSSMS전환발송여부           VARCHAR(1  )            
    ...
    public void setData(byte[] buff, int pos, String code) throws pvException
    {
        ...
        SMKT_CMGN_ID  = getStr(buff, pos, iLen[i], FieldNames[i], code);  pos += iLen[i]; i++;        
...
//      CRTF_NO       = getStr(buff, pos, iLen[i], FieldNames[i], code);  pos += iLen[i]; i++;
        ...
        ENCY_MPNG_TXT_1       = getStr(buff, pos, iLen[i], FieldNames[i], code);  pos += iLen[i]; i++;
        ENCY_MPNG_TXT_2       = getStr(buff, pos, iLen[i], FieldNames[i], code);  pos += iLen[i]; i++;
        ENCY_MPNG_TXT_3       = getStr(buff, pos, iLen[i], FieldNames[i], code);  pos += iLen[i]; i++;
        ENCY_MPNG_TXT_4       = getStr(buff, pos, iLen[i], FieldNames[i], code);  pos += iLen[i]; i++;
        ENCY_CRTF_NO          = getStr(buff, pos, iLen[i], FieldNames[i], code);  pos += iLen[i]; i++;
        UMS_SMS_CNVSD_YN      = getStr(buff, pos, iLen[i], FieldNames[i], code);  pos += iLen[i]; i++;        
    ...
    public void getData(byte[] buff, int pos, String code) throws pvException
    {
        ...
        objToArry(buff, pos, SMKT_CMGN_ID  , iLen[i], FieldNames[i], code); pos += iLen[i]; i++;
...
//      objToArry(buff, pos, CRTF_NO       , iLen[i], FieldNames[i], code); pos += iLen[i]; i++;
        ...
        objToArry(buff, pos, ENCY_MPNG_TXT_1  , iLen[i], FieldNames[i], code); pos += iLen[i]; i++;
        objToArry(buff, pos, ENCY_MPNG_TXT_2  , iLen[i], FieldNames[i], code); pos += iLen[i]; i++;
        objToArry(buff, pos, ENCY_MPNG_TXT_3  , iLen[i], FieldNames[i], code); pos += iLen[i]; i++;
        objToArry(buff, pos, ENCY_MPNG_TXT_4  , iLen[i], FieldNames[i], code); pos += iLen[i]; i++;
        objToArry(buff, pos, ENCY_CRTF_NO     , iLen[i], FieldNames[i], code); pos += iLen[i]; i++;
        objToArry(buff, pos, UMS_SMS_CNVSD_YN , iLen[i], FieldNames[i], code); pos += iLen[i]; i++;        
    ...
    public void log() {
        ...
        pvUtil.msglog(logname, sThread+PGMID+" +  SMKT_CMGN_ID   = ("+pvUtil.getNum(iLen[i], 3)+")[" + pvUtil.getFormat(iLen[i++],SMKT_CMGN_ID        ) +"]");
...
//      pvUtil.msglog(logname, sThread+PGMID+" +  CRTF_NO        = ("+pvUtil.getNum(iLen[i], 3)+")[" + pvUtil.getFormat(iLen[i++],CRTF_NO             ) +"]");
        ...
        pvUtil.msglog(logname, sThread+PGMID+" +  ENCY_MPNG_TXT_1     = ("+pvUtil.getNum(iLen[i], 3)+")[" + pvUtil.getFormat(iLen[i++],ENCY_MPNG_TXT_1  ) +"]");
        pvUtil.msglog(logname, sThread+PGMID+" +  ENCY_MPNG_TXT_2     = ("+pvUtil.getNum(iLen[i], 3)+")[" + pvUtil.getFormat(iLen[i++],ENCY_MPNG_TXT_2  ) +"]");
        pvUtil.msglog(logname, sThread+PGMID+" +  ENCY_MPNG_TXT_3     = ("+pvUtil.getNum(iLen[i], 3)+")[" + pvUtil.getFormat(iLen[i++],ENCY_MPNG_TXT_3  ) +"]");
        pvUtil.msglog(logname, sThread+PGMID+" +  ENCY_MPNG_TXT_4     = ("+pvUtil.getNum(iLen[i], 3)+")[" + pvUtil.getFormat(iLen[i++],ENCY_MPNG_TXT_4  ) +"]");
        pvUtil.msglog(logname, sThread+PGMID+" +  ENCY_CRTF_NO        = ("+pvUtil.getNum(iLen[i], 3)+")[" + pvUtil.getFormat(iLen[i++],ENCY_CRTF_NO     ) +"]");
        pvUtil.msglog(logname, sThread+PGMID+" +  UMS_SMS_CNVSD_YN    = ("+pvUtil.getNum(iLen[i], 3)+")[" + pvUtil.getFormat(iLen[i++],UMS_SMS_CNVSD_YN ) +"]");        
