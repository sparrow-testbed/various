...
import wisecommon.SignResponseInfo;
	...
	public class I_p0070 extends SepoaService
	{

    	String language = "";
    	String serviceId = "I_p0070";
		Message msg = null; 

    	public I_p0070( String opt, SepoaInfo sinfo ) throws SepoaServiceException
    	{
        	super( opt, sinfo );
        	setVersion( "1.0.0" );
...
//	    	value[0][0] = "N";
			...
			try{
		    	...
		    	*/
	    		...
	    		// 업체정보
				SepoaXmlParser wxp_vngl = new SepoaXmlParser(this, "SR_deleteProgVendor_vngl");
				sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp_vngl.getQuery());//sql.toString() );
	    		rtnIns = sm.doDelete(param);

	    		// 주소정보
	    		wxp_vngl = new SepoaXmlParser(this, "SR_deleteProgVendor_addr");
				sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp_vngl.getQuery());//sql.toString() );
	    		rtnIns = sm.doDelete(param);

	    		// 담당자 정보
	    		wxp_vngl = new SepoaXmlParser(this, "SR_deleteProgVendor_vncp");
				sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp_vngl.getQuery());//sql.toString() );
	    		rtnIns = sm.doDelete(param);

	    		// 수주실적 : 없음
	    		wxp_vngl = new SepoaXmlParser(this, "SR_deleteProgVendor_vnpj");
				sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp_vngl.getQuery());//sql.toString() );
	    		rtnIns = sm.doDelete(param);

	    		//업체세부관리정보
	    		wxp_vngl = new SepoaXmlParser(this, "SR_deleteProgVendor_vnit");
				sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp_vngl.getQuery());//sql.toString() );
	    		rtnIns = sm.doDelete(param);

	    		//사용자정보
	    		wxp_vngl = new SepoaXmlParser(this, "SR_deleteProgVendor_lusr");
				sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp_vngl.getQuery());//sql.toString() );
	    		rtnIns = sm.doDelete(param);
	  	...
	  	/* 등록 진행업체목록 - 팝업 조회 */
	  	/* ICT 사용 */
	  	public SepoaOut getVendorInfo(String vendor_code){
	     	...
	     	try{
	     		...
	     		String user_id = info.getSession("ID");
	      		...
	      		rtn = et_getVendorInfo(user_id, vendor_code);
	  	...
	  	/* 등록 진행업체목록 - 팝업 조회 쿼리 */
	  	/* ICT 사용 */
	  	private String et_getVendorInfo(String user_id, String vendor_code) throws Exception{
	  	...
	  	/* ICT 사용 : 협력사현황 - 삭제 */
    	public SepoaOut delete_approval(Map<String, String> param){

	    	String rtn[] = null;
	    	try{

	    		rtn = SR_delete_approval(param);
	    		setMessage( rtn[ 0 ] );
	    		setValue( "delete Row=" + rtn );
	    		setStatus( 1 );
    	...
    	/* ICT 사용 : 협력사현황 - 삭제 */
		private String[] SR_delete_approval(Map<String, String> param) throws Exception{
	  	...
	  	/* ICT 사용 : 협력사현황 - 복구 */
    	public SepoaOut restore_approval(Map<String, String> param){

	    	String rtn[] = null;
	    	try{

	    		rtn = SR_restore_approval(param);
	    		setMessage( rtn[ 0 ] );
	    		setValue( "delete Row=" + rtn );
	    		setStatus( 1 );
	  	...
	  	/* ICT 사용 : 협력사현황 - 복구 */
		private String[] SR_restore_approval(Map<String, String> param) throws Exception{
	  	...
	  	private String[] SR_setRegister(String vendor_code, String sg_refitem, String is_registry, String sign_status, String creditgrade, Map<String ,String> param ) throws Exception{
			...
			try{
				...
				//sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp.getQuery());//sql.toString() );
	  	    	...
	  	    	//SepoaXmlParser wxp_sgvn = new SepoaXmlParser(this, "SR_setRegister_sgvn");
				//sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp_sgvn.getQuery());//sql.toString() );
	  	    	//rtnIns = sm.doUpdate( param);	  	    	
		  	    ...
		  	    if(is_registry.equals("R")){//업체 반려일 경우
		  	    	...
		  	    	// 업체 상태변경
		  	    	SepoaXmlParser wxp_job = new SepoaXmlParser(this, "SR_setRegister_job");
					sm = new SepoaSQLManager( info.getSession("ID"), this, ctx, wxp_job.getQuery());//sql.toString() );
		  	    	...
		  	    	if("9999".equals(old_vendor_code2)) {
		  	    		if("I".equals(old_vendor_code1)) {
		  	    			new_vendor_code1 = "J";
		  	    		} else if("J".equals(old_vendor_code1)) {
		  	    			new_vendor_code1 = "K";
		  	    		} else if("K".equals(old_vendor_code1)) {
		  	    			new_vendor_code1 = "L";//업체코드가 C9999를 초과되어 생성되지 못할 것 같지만 에러가 날 수도 있기에 D로 변환되도록 처리함
		  	    		}
		  	    	...
		  	    	//Logger.debug.println( info.getSession("ID"), this, "ihstone : 001");
		  	    	rtnIns = updateVendorCodeVNCP(ctx, param, new_vendor_code);//ICOMVNCP : UPDATE 대상이 없어도 예외처리하지 않음
		  	    	//Logger.debug.println( info.getSession("ID"), this, "ihstone : 002");
		  	    	rtnIns = updateVendorCodeVNPJ(ctx, param, new_vendor_code);//ICOMVNPJ : UPDATE 대상이 없어도 예외처리하지 않음
		  	    	//Logger.debug.println( info.getSession("ID"), this, "ihstone : 003");
		  	    	rtnIns = updateVendorCodeVNIT(ctx, param, new_vendor_code);//ICOMVNIT : UPDATE 대상이 없어도 예외처리하지 않음		  
		  	    	//Logger.debug.println( info.getSession("ID"), this, "ihstone : 004");
	  	...
	  	public SepoaOut setApproval(String vendor_code, String sg_refitem, String is_registry, String sign_status, String credit_grade, Map<String, String> param, String sign_flag, String approval){
	    	...
	    	try{
	    	    ...
	    	    try {
					...
					Logger.err.println( info.getSession( "ID" ), this, "Exception e =" + e1.getMessage() );
				}
	    	    Logger.err.println( info.getSession( "ID" ), this, "Exception e =" + e.getMessage() );
	    	    setStatus( 0 );
	    	}
	  	...
	  	private String et_getRegVenLst(Map<String, String> header) throws Exception
	  	{
			...
			String pjt_name 	= param[5];*/
	      	...
	      	try
	      	{
	      		...
	      		SepoaXmlParser wxp = new SepoaXmlParser(this, new Exception().getStackTrace()[0].getMethodName());
	      		//SepoaXmlParser wxp = new SepoaXmlParser(this, "et_getRegVenLst");
	      		...
	      		*/
	          	...
	          	SepoaSQLManager sm = new SepoaSQLManager(user_id,this,ctx,wxp.getQuery());//tSQL.toString());
	          	//SepoaSQLManager sm = new SepoaSQLManager(user_id,this,ctx,wxp);//tSQL.toString());
	          	rtn = sm.doSelect(header);
...
//				}
				...
				}catch(Exception e)
				{
					...
					try{
					Rollback();
					}catch(Exception e1){ Logger.err.println(this,e1.getMessage()); }
					//}
					Logger.err.println("Exception e =" + e.getMessage());
					setStatus(0);
					setMessage(msg.getMessage("4000"));
...
*/
		...
		private String getIrsNo_RegNo(ConnectionContext ctx, String[][] pData) throws Exception {
	  	...
	  	/* ICT 사용 */
		public SepoaOut setBlock(Map<String, String> param){			//거래 개시,중지
...
//				String[] type1 = { "S", "S", "S"};
	...
	private SepoaFormater makeNewVendorCode(ConnectionContext ctx) throws Exception
	{
		...
		try{
			Map<String, String> data =  new HashMap<String, String>();
			//현재 문서아이템의 갯수 조회
			SepoaXmlParser wxp = new SepoaXmlParser(this, "makeNewVendorCode");
			data.put("house_code", house_code);
	...
	private int updateVendorCodeVNGL(ConnectionContext ctx, Map<String, String> setData, String new_vendor_code) throws Exception
	{
		...
		try
		{
			...
			String cur_date   = SepoaDate.getShortDateString();
			String cur_time   = SepoaDate.getShortTimeString();
			SepoaXmlParser wxp = new SepoaXmlParser(this, "updateVendorCodeVNGL");
	...
	private int updateVendorCodeADDR(ConnectionContext ctx, Map<String, String> setData, String new_vendor_code) throws Exception
	{
		...
		try
		{
			...
			SepoaXmlParser wxp2 = new SepoaXmlParser(this, "updateVendorCodeADDR_2");
			...
			if(cnt.equals("0")){
				SepoaXmlParser wxp1 = new SepoaXmlParser(this, "updateVendorCodeADDR_1");
...
//			e.printStackTrace();
	...
	private int updateVendorCodeVNCP(ConnectionContext ctx, Map<String, String> setData, String new_vendor_code) throws Exception
	{
		...
		try
		{
			...
			String cur_date   = SepoaDate.getShortDateString();
			String cur_time   = SepoaDate.getShortTimeString();
			SepoaXmlParser wxp = new SepoaXmlParser(this, "updateVendorCodeVNCP");
				Map<String, String> data =  new HashMap<String, String>();
	...
	private int updateVendorCodeVNPJ(ConnectionContext ctx, Map<String, String> setData, String new_vendor_code) throws Exception
	{
		...
		try
		{
			...
			String cur_date   = SepoaDate.getShortDateString();
			String cur_time   = SepoaDate.getShortTimeString();
			SepoaXmlParser wxp = new SepoaXmlParser(this, "updateVendorCodeVNPJ");
				Map<String, String> data =  new HashMap<String, String>();
...
//				throw new Exception("입력된 데이타가 없습니다.");
	...
	private int updateVendorCodeVNIT(ConnectionContext ctx, Map<String, String> setData, String new_vendor_code) throws Exception
	{
		...
		try
		{
			...
			String cur_date   = SepoaDate.getShortDateString();
			String cur_time   = SepoaDate.getShortTimeString();
			SepoaXmlParser wxp = new SepoaXmlParser(this, "updateVendorCodeVNIT");
				Map<String, String> data =  new HashMap<String, String>();
