...
public class rfq_bd_lis1 extends HttpServlet{
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
		...
		try{
			...
			if ("getRfqList".equals(mode)){				// 견적요청접수현황 조회
				gdRes = getRfqList(gdReq,info);
			}else if ("getQtaProList".equals(mode)){				// 견적요청접수현황 조회
				gdRes = getQtaProList(gdReq,info);
			}		       		
		} catch (Exception e) {
	...
	private GridData getRfqList(GridData gdReq, SepoaInfo info) throws Exception {
        ...
        try{
            ...
            value        = ServiceConnector.doService(info, "I_s2011", "CONNECTION", "getRfqList", obj);
   			if(value.flag) {
...
private GridData getQtaProList(GridData gdReq, SepoaInfo info) throws Exception {
        ...
        try{
            ...
            // DB  : CONNECTION, TRANSACTION, NONDBJOB
            //System.out.println("header:"+header);
            value        = ServiceConnector.doService(info, "I_s2011", "CONNECTION", "getQtaProList", obj);
   			if(value.flag) {
   			    gdRes.setMessage(message.get("MESSAGE.0001").toString()); 
   			    gdRes.setStatus("true");
   			} else {
