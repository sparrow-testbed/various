...
public class I_SBD_010 extends SepoaService {
    ...
    public I_SBD_010( String opt, SepoaInfo info ) throws SepoaServiceException {
        super( opt, info );
        setVersion( "1.0.0" );
        //20131212 sendakun
        try {
//            msg = new Message( info, "MESSAGE" );
        	...
        	Logger.err.println("I_SBD_010: = " + e.getMessage());
        }
    }
	...
	public SepoaOut getBdQtSubmitList(Map< String, String > headerData)
	{
		...
		try {
			...
			sxp = new SepoaXmlParser(this, "getBdQtSubmitList");
	...
	public SepoaOut setBDQTSubmit(Map< String, String > headerData)
	{
		ConnectionContext ctx = getConnectionContext(); 
		String company_code   = info.getSession("COMPANY_CODE"); 
		...
		try {
			...
			if ("Y".equals(status)){
				setFlag( false );
				setStatus(0);
				setMessage("견적서류 적합이되어 다시 제출불가합니다.");
				return getSepoaOut();
			}
			
			headerData.put("ATTACH_NO", 	MapUtils.getString( headerData, "attach_no" )); 
			rtn = et_setBdQtSubmit(headerData);
			
			
			setFlag( true );
			setStatus(1);
			setValue(rtn+""); 
			setMessage("견적서류가 제출되었습니다");
	...
	private int et_setBdQtSubmit(Map< String, String > headerData ) throws Exception 
    {
    	ConnectionContext ctx = getConnectionContext(); 
    	int rtn = 0;
    	...
    	try {
    		...
    		sxp = new SepoaXmlParser(this, "et_delBdQtSubmit");
    		ssm = new SepoaSQLManager(info.getSession("ID"), this, ctx, sxp); 
    		rtn = ssm.doInsert(headerData); 
    		
    		sxp = new SepoaXmlParser(this, "et_setBdQtSubmit");
    		ssm = new SepoaSQLManager(info.getSession("ID"), this, ctx, sxp); 
    		rtn = ssm.doInsert(headerData); 
	...
	public SepoaOut setBDQTSubmitCancel(Map< String, String > headerData)
	{
		ConnectionContext ctx = getConnectionContext(); 
		String company_code   = info.getSession("COMPANY_CODE"); 
		String USER_ID         = info.getSession("ID");
		...
		try {
			...
			if ("Y".equals(status)){
				setFlag( false );
				setStatus(0);
				setMessage("견적서류 적합이되어 취소 불가합니다.");
				return getSepoaOut();
			}
			
			headerData.put("ATTACH_NO", 	MapUtils.getString( headerData, "attach_no" )); 
			rtn = et_setBDQTSubmitCancel(headerData);
			
			
			setFlag( true );
			setStatus(1);
			setValue(rtn+""); 
			setMessage("견적서제출 취소되었습니다.");
	...
	private int et_setBDQTSubmitCancel(Map< String, String > headerData ) throws Exception 
    {
    	ConnectionContext ctx = getConnectionContext(); 
    	int rtn = 0;
