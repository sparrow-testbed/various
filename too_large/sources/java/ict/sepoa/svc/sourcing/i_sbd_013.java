...
public class I_SBD_013 extends SepoaService {
    ...
    public I_SBD_013( String opt, SepoaInfo info ) throws SepoaServiceException {
        super( opt, info );
        setVersion( "1.0.0" );
        //20131212 sendakun
        try {
//            msg = new Message( info, "MESSAGE" );
	...
	public SepoaOut getBdPySubmitList(Map< String, String > headerData)
	{
		...
		try {
			...
			sxp = new SepoaXmlParser(this, "getBdPySubmitList");
	...
	public SepoaOut setBdPySubmit(Map< String, String > headerData)
	{
		ConnectionContext ctx = getConnectionContext(); 
		String company_code   = info.getSession("COMPANY_CODE"); 
		...
		try {
			...
			if ("Y".equals(status)){
				setFlag( false );
				setStatus(0);
				setMessage("대금지급서류 적합이되어 다시 제출불가합니다.");
				return getSepoaOut();
			}
			
			headerData.put("ATTACH_NO", 	MapUtils.getString( headerData, "attach_no" )); 
			rtn = et_setBdPySubmit(headerData);
			
			
			setFlag( true );
			setStatus(1);
			setValue(rtn+""); 
			setMessage("대금지급서류가 제출되었습니다");
	...
	private int et_setBdPySubmit(Map< String, String > headerData ) throws Exception 
    {
		ConnectionContext ctx = getConnectionContext(); 
    	int rtn = 0;
    	...
    	try {
    		...
    		sxp = new SepoaXmlParser(this, "et_delBdPySubmit");
    		ssm = new SepoaSQLManager(info.getSession("ID"), this, ctx, sxp); 
    		rtn = ssm.doInsert(headerData); 
    		
    		sxp = new SepoaXmlParser(this, "et_setBdPySubmit");
    		ssm = new SepoaSQLManager(info.getSession("ID"), this, ctx, sxp); 
    		rtn = ssm.doInsert(headerData); 
	...
	public SepoaOut setBdPySubmitCancel(Map< String, String > headerData)
	{
		ConnectionContext ctx = getConnectionContext(); 
		String company_code   = info.getSession("COMPANY_CODE"); 
		String USER_ID         = info.getSession("ID");
		...
		try {
			...
			if ("Y".equals(status)){
				setFlag( false );
				setStatus(0);
				setMessage("대금지급서류 적합이되어 취소 불가합니다.");
				return getSepoaOut();
			}
			
			headerData.put("ATTACH_NO", 	MapUtils.getString( headerData, "attach_no" )); 
			rtn = et_setBdPySubmitCancel(headerData);
			
			
			setFlag( true );
			setStatus(1);
			setValue(rtn+""); 
			setMessage("대금지급서류 업로드 취소되었습니다.");
	...
	private int et_setBdPySubmitCancel(Map< String, String > headerData ) throws Exception 
    {
    	ConnectionContext ctx = getConnectionContext(); 
    	int rtn = 0;
