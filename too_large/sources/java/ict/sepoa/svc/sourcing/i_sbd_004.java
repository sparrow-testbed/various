...
public class I_SBD_004 extends SepoaService {
    ...
    public I_SBD_004( String opt, SepoaInfo info ) throws SepoaServiceException {
        super( opt, info );
        setVersion( "1.0.0" );
        //20131212 sendakun
        try {
//            msg = new Message( info, "MESSAGE" );
        	...
        	Logger.err.println("I_SBD_004: = " + e.getMessage());
        }
    }
	...
	public SepoaOut setBDAPinsert(Map< String, Object > allData)
    {
        ...
        try {
            ...
            String status3         = et_chkBidBeginDate(headerData); // 입찰제출시작시간 체크
            if (status3.equals("N")){
                setStatus(0);
                setMessage("입찰제출시작 시간 이전입니다.");
                return getSepoaOut();
            }
    ...
    private String et_chkBidBeginDate( Map< String, String > headerData ) throws Exception 
    {
        ConnectionContext ctx = getConnectionContext(); 
        ...
        try { 
    		...
    		SepoaXmlParser sxp = new SepoaXmlParser(this, "et_chkBidBeginDate");
			SepoaSQLManager ssm = new SepoaSQLManager(info.getSession("ID"), this, ctx, sxp); 
			value = ssm.doSelect(headerData);
            SepoaFormater wf = new SepoaFormater(value);
            rtn = wf.getValue(0,0);
