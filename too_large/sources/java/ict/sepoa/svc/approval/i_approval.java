...
public class I_Approval extends SepoaService{
	private String m_PO_PR_PROCEEDING_FLAG = "B";
    public I_Approval(String opt,SepoaInfo info) throws SepoaServiceException
    {
        super(opt,info);
        setVersion("1.0.0");
...
/*			//결재완료시 sms, mail 발송.
	...
	private SepoaOut et_Approval_8(SignResponseInfo inf) throws Exception
	{
		...
		try
		{
			...
			//취소공문 담당자가 결재취소시 (상신내역)
			if(reportCancel(objBDHD) > 0){
				setStatus(1);
				setMessage("");
				return getSepoaOut();
			}
						
			//입찰종료 또는 입찰진행건 공문승인 불가
			if("E".equals(sign_status)) {
				if(et_chkBid(objBDHD) > 0){
					setStatus(0);
					setMessage("입찰종료 또는 입찰진행건 입니다.");
					return getSepoaOut();
				}
			}
			...
			for(int	i=0;i <	DOC_NO.length;i++){
				...
				if("D".equals(sign_status)){ //TOBE 2017-07-01 결재상신-결재취소시 입찰공고SMS 발송 방지
					...
					new SMS("NONDBJOB", info).bd1Process_ICT(ctx, param);
				}
			}
	...
	private int et_chkBid( String[][] args ) throws Exception 
    {
    	ConnectionContext ctx = getConnectionContext(); 
    	...
    	try { 
    		...
    		for(int i=0;i<args.length;i++){
            	...
            	data.put("BID_NO", args[i][5]);
            	
            	sxp = new SepoaXmlParser(this, "et_chkBid");
				ssm = new SepoaSQLManager(info.getSession("ID"),this,ctx,sxp);
				
				String rtnSql = ssm.doSelect(data);
				wf = new SepoaFormater(rtnSql);
				
				iVal = Integer.parseInt(wf.getValue(0,0));	
	...
	private int delPreBdDelete( String[][] args ) throws Exception 
    {
        ConnectionContext ctx = getConnectionContext(); 
    	...
    	try { 
			...
			for(int i=0;i<args.length;i++){
            	...
            	data.put("HOUSE_CODE", args[i][4]);
            	data.put("BID_NO", args[i][5]);
            	data.put("BID_COUNT", args[i][7]);
            	
    			if(args[i][0].equals("E")){    				
			...
			rtn = 1;
   			...
   			throw new Exception("delPreBdDelete:"+e.getMessage());
   		}
   		return rtn;
    }
	
	private int reportCancel( String[][] args ) throws Exception 
    {
        ConnectionContext ctx = getConnectionContext(); 
        ...
        try { 
			...
			for(int i=0;i<args.length;i++){
            	...
            	sxp = new SepoaXmlParser(this, "et_getBidStatus");
				ssm = new SepoaSQLManager(info.getSession("ID"),this,ctx,sxp);
				
				String rtnSql = ssm.doSelect(data);
				wf = new SepoaFormater(rtnSql);
				BID_STATUS = wf.getValue(0,0);
				
				
    			if(args[i][0].equals("D") && BID_STATUS.equals("CP")){    				
	    			sxp = new SepoaXmlParser(this, "et_reportCancel_1");
					ssm = new SepoaSQLManager(info.getSession("ID"), this, ctx, sxp); 
					ssm.doUpdate(data);
	
					sxp = new SepoaXmlParser(this, "et_reportCancel_2");
					ssm = new SepoaSQLManager(info.getSession("ID"), this, ctx, sxp); 
					ssm.doUpdate(data);
					
					sxp = new SepoaXmlParser(this, "et_reportCancel_3");
					ssm = new SepoaSQLManager(info.getSession("ID"), this, ctx, sxp); 
					ssm.doUpdate(data);
					
					sxp = new SepoaXmlParser(this, "et_reportCancel_4");
					ssm = new SepoaSQLManager(info.getSession("ID"), this, ctx, sxp); 
					ssm.doUpdate(data);
					
					sxp = new SepoaXmlParser(this, "et_reportCancel_5");
					ssm = new SepoaSQLManager(info.getSession("ID"), this, ctx, sxp); 
					ssm.doUpdate(data);
					
					sxp = new SepoaXmlParser(this, "et_reportCancel_6");
					ssm = new SepoaSQLManager(info.getSession("ID"), this, ctx, sxp); 
					ssm.doUpdate(data);	
   			...
   			throw new Exception("reportCancel:"+e.getMessage());
   		}
   		return rtn;
    }
...
//			Rollback();
		...
		try {
			if ("RFQ".equals(MapUtils.getString(map, "APPROVAL_TYPE"))) {
				...
				SepoaSQLManager ssm = new SepoaSQLManager(
						info.getSession("ID"), this, ctx, sxp);
				sf = new SepoaFormater(ssm.doSelect(map));
			} else if ("BD_ICT".equals(MapUtils.getString(map, "APPROVAL_TYPE"))) {
				SepoaXmlParser sxp = new SepoaXmlParser(this,
						"approvalBdIctAttach");
				SepoaSQLManager ssm = new SepoaSQLManager(
						info.getSession("ID"), this, ctx, sxp);
				...
				for (int i = 0; i < sf.getRowCount(); i++) {
					...
					if (rs.next()) {
						oriFilePath = conf.getString("sepoa.attach.path."
								+ rs.getString("TYPE"))
								+ "/" + rs.getString("DES_FILE_NAME").replaceAll("\\.\\./", "");
						
						f5 = new File(oriFilePath);
						
						// CLOB column????븳 ?ㅽ듃由쇱쓣 ?삳뒗??
						try {
							fileName1 = rs.getString("DES_FILE_NAME").replaceAll("\\.\\./", "") + ".B"
									+ createDate;
	...
	//							System.out.println(filePath1 + "_DEC");
							...
							l = 0;
							fileName2 = rs.getString("DES_FILE_NAME").replaceAll("\\.\\./", "") + ".C"
									+ createDate;
...
//						e.printStackTrace();
						...
						try {
							...
							Logger.err.println("setFileDec: = " + e.getMessage());
						}
					}
				}
			}
