...
public class I_AD_132 extends SepoaService
{
...
//    Message msg = new Message("STDCOMM");
    ...
    public I_AD_132(String opt,SepoaInfo info) throws SepoaServiceException
    {
        super(opt,info);
        setVersion("1.0.0");
        //20131217 sendakun
...
//            msg = new Message(info, "STDCOMM");
        	...
        	Logger.sys.println("I_AD_132 error : " + e.getMessage());
        }
    ...
    public SepoaOut getMainternace(Map <String,String>header) throws Exception
    {
        ...
        try
        {
            ...
            String s_status_flag = MapUtils.getString( header, "s_status_flag", "" );
        	StringBuffer tSQL = new StringBuffer();
        	ParamSql sm = new ParamSql(info.getSession("ID"), this, ctx);
        	

        	
        	String strSeller   = sepoa.svc.common.constants.UserType.Seller.getValue();
            String strPartner  = sepoa.svc.common.constants.UserType.Partner.getValue();
            String strFlagNo   = sepoa.fw.util.CommonUtil.Flag.No.getValue();
            
            String strLang     = info.getSession("LANGUAGE");
            String strUserType = info.getSession("USER_TYPE");
            String strProfile  = info.getSession("MENU_PROFILE_CODE");
            
            tSQL.append(" select                                                                                                                                                               \n");
            tSQL.append("             USER_ID                                                                                                                                                  \n");
            tSQL.append("           , USER_NAME_LOC                                                                                                                                            \n");
            tSQL.append("           ,(case when (" + SEPOA_DB_OWNER + "CNV_NULL(USER_TYPE,'NULL') = '" + strSeller + "')                                                           			   \n");
            tSQL.append("                          then (select VENDOR_NAME_LOC  from ICOMVNGL_ICT where VENDOR_CODE  = A1.COMPANY_CODE)                                                       \n");
            tSQL.append("                          else (select COMPANY_NAME_LOC from ICOMCMGL     where COMPANY_CODE = A1.COMPANY_CODE)                       				                   \n");
			tSQL.append("             end)                                                                                                                                  as COMPANY_NAME    \n");
            tSQL.append("           ," + DB_NULL_FUNCTION + "((select PLANT_NAME_LOC from ICOMOGPL where COMPANY_CODE = A1.COMPANY_CODE and PLANT_CODE = A1.PLANT_CODE),'') as PLANT_NAME      \n");
            tSQL.append("           ," + SEPOA_DB_OWNER   + "getcodetext2('M104',WORK_TYPE, '" + strLang + "')                                                              as TEXT_WORK_TYPE  \n");
            tSQL.append("           ,(select DEPT_NAME_LOC from ICOMOGDP where DEPT=A1.DEPT and COMPANY_CODE = A1.COMPANY_CODE)                                             as DEPT_NAME       \n");
            tSQL.append("           ,(case when (" + SEPOA_DB_OWNER + "CNV_NULL(USER_TYPE,'NULL') = '" + strSeller + "')                                                                       \n");
            tSQL.append("	                          then POSITION                                                                                                                            \n");
            tSQL.append("	                          else " + SEPOA_DB_OWNER + "getcodetext2('M106',POSITION, '" + strLang + "')                                                              \n");
            tSQL.append(" 	          end)                                                                                                                                  as POSITION        \n");
            tSQL.append("           ,(case when (" + SEPOA_DB_OWNER + "CNV_NULL(USER_TYPE,'NULL') = '" + strSeller + "')                                                                       \n");
            tSQL.append("                       then " + SEPOA_DB_OWNER + "getcodetext2('M106', POSITION, '" + strLang + "')                                                                   \n");
            tSQL.append("                       else " + SEPOA_DB_OWNER + "getcodetext2('M106',POSITION, '" + strLang + "')                                                                    \n");
            tSQL.append("             end) as MANAGER_POSITION                                                                                                                                 \n");
            tSQL.append("           ," + SEPOA_DB_OWNER + "getaddrattr2(user_id,'3','PHONE_NO1')                                                                            as PHONE_NO        \n");
            tSQL.append("           ,(select max(Z.MENU_NAME) as MENU_NAME from SMUPD Z where (Z.MENU_PROFILE_CODE = A1.MENU_PROFILE_CODE))                                 as MENU_NAME       \n");
            tSQL.append("           ,case when '" + strUserType + "' in ('" + strSeller + "', '" + strPartner + "')                                                                            \n");
            tSQL.append("                         then (case when " + DB_NULL_FUNCTION + "(menu_profile_code, ' ') = ' '                                                                       \n");
            tSQL.append("                                          then '" + strProfile + "'                                                                                                   \n");
            ...
            tSQL.append("           ,case when STATUS = 'D' then '삭제' else '정상' end                                                                                     as STATUS_NAME     \n");
            tSQL.append("   from  ICOMLUSR_ICT A1                                                                                                                                              \n");
            tSQL.append("   where " + DB_NULL_FUNCTION + "(DEL_FLAG, '" + strFlagNo + "') = '" + strFlagNo + "'                                                                                \n");
            ...
            if(s_status_flag.equals("C"))
            	tSQL.append( " and STATUS IN ('C','R') \n");		
            else if(s_status_flag.equals("D"))
            	tSQL.append( " and STATUS IN ('D') \n");		            
                    
            tSQL.append( " order by USER_ID             \n");		//R : 등록  A:승인
    ...
    public SepoaOut setDelete(List<Map < String , String >> gridData)throws Exception, DBOpenException
    {
        ...
        try
        {
            ...
            for(int i=0; i < gridData.size(); i++){
                ...
                //tSQL.append( " DELETE FROM ICOMLUSR_ICT        \n");               
                //tSQL.append( " WHERE USER_ID = ?        \n");sm.addStringParameter(MapUtils.getString( grid, "USER_ID", "" ));    
                
                tSQL.append( " update ICOMLUSR_ICT        \n");               
              ...
              /*  sm.removeAllValue();
                ...
                //tSQL.append(" DELETE FROM ICOMADDR_ICT         \n");    			
...
//          setMessage(msg.getMessage("0000"));
    ...
    /* ICT 수정 : 사용자 복구 */
    public SepoaOut setRestore(List<Map < String , String >> gridData)throws Exception, DBOpenException
    {
        int rtn = -1;
        ConnectionContext ctx = getConnectionContext();
        Map <String , String > grid = null;
    ...
    public String et_getDisplay(String[] args) throws Exception
    {
        ...
        try
        {
           ...
           // tSQL.append("     (case when (a.USER_TYPE = '" + sepoa.svc.common.constants.UserType.Seller.getValue() + "') then A.DEPT_NAME     \n");
            ...
            tSQL.append("   from ICOMLUSR_ICT a left join ICOMOGDP b                      \n");
            ...
            tSQL.append("   left join ICOMVNGL_ICT v                                   \n");
            tSQL.append("     on a.COMPANY_CODE = v.vendor_CODE                 \n");
            tSQL.append("   left join ICOMADDR_ICT d                                   \n");
            tSQL.append("     on  a.user_id = d.CODE_NO                          \n");
            tSQL.append("     and d.CODE_TYPE = '3'                              \n");
    ...
    */
    private int et_setUserInfoUpdate2(String pwd, String[][] args_user, String[][] args_addr, String user_info_flag) throws Exception, DBOpenException {
        ...
        try {
            ...
            if(result<1)
                throw new Exception("ICOMLUSR_ICT UPDATE ERROR");
            ...
            for(int i = 0; i < args_addr.length; i++)
            {
                ...
                sql.append(" SELECT COUNT(*) CNT FROM ICOMADDR_ICT         \n ");
                ...
                if(icomaddr_cnt <= 0)
                {
                    pm.removeAllValue();
                    sql.delete(0, sql.length());
                    sql.append(" INSERT INTO ICOMADDR_ICT (    \n ");
                    sql.append("  ZIP_CODE              \n ");
                ...
                else
                {
                    pm.removeAllValue();
                    sql.delete(0, sql.length());
                    sql.append(" UPDATE ICOMADDR_ICT SET              \n");
                    sql.append("     ZIP_CODE       = ?        \n"); pm.addStringParameter(zip_code);
                    sql.append("   , PHONE_NO1      = ?        \n"); pm.addStringParameter(SepoaString.encString (phone_no1  ,"PHONE"));
                    sql.append("   , PHONE_NO2      = ?        \n"); pm.addStringParameter(SepoaString.encString (phone_no2  ,"PHONE"));
                    sql.append("   , FAX_NO         = ?        \n"); pm.addStringParameter(SepoaString.encString (fax_no  ,"PHONE"));
