...
public class bd_req_prepare_list_ict extends HttpServlet {
    ...
    public GridData getBdReqPrepareList( GridData gdReq, SepoaInfo info ) throws Exception {
        ...
        try {
			...
			SepoaOut value = ServiceConnector.doService(info, "I_BD_010", "CONNECTION", "getBdReqPrepareList", obj);
    ...
    public GridData setBdEsCancel(GridData gdReq, SepoaInfo info) throws Exception {
		...
		try {
			...
			for(int i = 0; i < grid.size(); i++){
				...
				Object[] obj = {gridInfo};
				SepoaOut value = ServiceConnector.doService(info, "I_BD_010", "TRANSACTION","setBdEsCancel", obj);
				if(value.flag) {
					gdRes.setMessage(message.get("MESSAGE.0001").toString()); 
					gdRes.setStatus("true");
				}else {
