...
public class ap_complete_list_ict extends HttpServlet {
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
        ...
        try {
       		...
       		if ("getCompleteList".equals(mode))
       		{	
       			gdRes = getCompleteList(gdReq, info);		//조회
       		}
        } catch (Exception e) {
    ...
    public GridData getCompleteList(GridData gdReq, SepoaInfo info) throws Exception {
		GridData gdRes   		= new GridData();
		...
		try {
			...
			SepoaOut value = ServiceConnector.doService(info, "I_AP_004", "CONNECTION", "getCompleteList", obj);
			...
			for (int i = 0; i < wf.getRowCount(); i++) {
				...
				singRemark = wf.getValue("SIGN_REMARK", i);
				...
				for(int k=0; k < grid_col_ary.length; k++) {
					if(grid_col_ary[k] != null && grid_col_ary[k].equals("SELECTED")) {
						...
						if(!("".equals(wf.getValue("SIGN_REMARK", i).trim())) && wf.getValue(grid_col_ary[k], i)!=null){
							gdRes.addValue(grid_col_ary[k], "<img src='/images/icon/detail.gif' align='absmiddle' border='0' alt=''>");
						}else{
