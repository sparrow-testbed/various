...
public class s6006 extends SepoaService
{
   String language = "";
   ...
   public s6006( String opt, SepoaInfo sinfo ) throws SepoaServiceException
   {
       super( opt, sinfo );
       setVersion( "1.0.0" );
   }
...
//		sql.append("	and use_flag = 'Y'  order by seq asc \n");
	...
	public SepoaOut insertScrItem2( String[][] pSetData, String[][] pData )
    {
    	String rtn[] = null;
    	try
    	{
    			if (pData != null) {
    	    	rtn = SR_insertScrItem3( pSetData, pData );
    	    } else {
    	    	rtn = SR_insertScrItem4(pSetData);
    	    }
...
//				}
	 ...
	 private String[] SR_insertScrItem3( String[][] pSetData, String[][] pData ) throws Exception
	    {
	        String returnString[] = new String[ 2 ];
	        ConnectionContext ctx = getConnectionContext();
//	        StringBuffer sql = new StringBuffer();
	        ...
	        try
	        {
	        	...
	        	SepoaXmlParser wxp = new SepoaXmlParser(this, new Exception().getStackTrace()[0].getMethodName()+"_sgvn3");
				wxp.addVar("house_code", house_code);
...
//					}
    ...
    private String[] SR_insertScrItem4( String[][] pSetData) throws Exception
    {
        String returnString[] = new String[ 2 ];
        ConnectionContext ctx = getConnectionContext();
        String house_code =info.getSession("HOUSE_CODE")  ;
...
//		           }   // end of for
	...
	public SepoaOut updateNewReg(Map<String, Object> data) throws Exception{ 
		...
		try{
            ...
            //소싱정보 등록여부 체크로직 필요
            sxp = new SepoaXmlParser(this, "et_chkSsgvn");
			ssm = new SepoaSQLManager(info.getSession("ID"), this, ctx, sxp); 		 
            wf = new SepoaFormater(ssm.doSelect(map));
            SSGVN = wf.getValue(0,0);            
            if(!SSGVN.equals("Y")){
            	throw new Exception("소싱정보를 입력하셔야 신규가입이 가능합니다.\r\n\r\n※ 신규가입방법\r\n1.기본정보(저장) -> 2.영업담당(저장) -> 3.소싱정보등록(저장) -> 4.가입신청");
            }
            
            
            
			            
            sxp = new SepoaXmlParser(this, "et_updSSGVN");
			ssm = new SepoaSQLManager(info.getSession("ID"), this, ctx, sxp); 		 
