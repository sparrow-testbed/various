...
public class sta_bd_lis02 extends HttpServlet{
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
    	...
    	try {
    		...
    		if("getPoList".equals(mode)){
    			gdRes = this.getPoList(gdReq, info);
    		}
     ...
     * @modify 2014-11-24
     */
	private GridData getPoList(GridData gdReq, SepoaInfo info) {
	    ...
	    try{
	    	...
	    	value    = ServiceConnector.doService(info, "p7008", "CONNECTION", "getPoList", obj);
	    	isStatus = value.flag;
	
	    	if(isStatus){
	    		gdRes        = this.getPoListGdRes(gdReq, gdRes, value);
	 ...
	 @SuppressWarnings({ "rawtypes", "unchecked" })
	    ...
	    private Object[] requestListHeader(GridData gdReq, SepoaInfo info) throws Exception{
	    	...
	    	String po_from_date      	= header.get("po_from_date"		    );
			String po_to_date        	= header.get("po_to_date"			);
			...
			String confirm_flag 		= header.get("confirm_flag"		    );
			String complete_mark    	= header.get("complete_mark"   	    );
			String po_status 			= header.get("po_status"			);
			...
			String cont_from_date    	= header.get("cont_from_date"   	);
			String cont_to_date    		= header.get("cont_to_date"   		);
			String cont_seq    			= header.get("cont_seq"   			);
			String cont_sign_from_date  = header.get("cont_sign_from_date"	);
			String cont_sign_to_date    = header.get("cont_sign_to_date"	);
			String cont_title    		= header.get("cont_title"   		);
			String cont_status    		= header.get("cont_status"   		);
			String cust_code    		= header.get("cust_code"   		    );
			String sales_dept    		= header.get("sales_dept"   		);
			String req_dept    			= header.get("req_dept"   			);
			String wbs_name    			= header.get("wbs_name"   		    );
			String po_name    			= header.get("po_name"   		    );
			String ct_name    			= header.get("ct_name"   		    );
			String maker_name    		= header.get("maker_name"   	    );
			String ctrl_code_session	= header.get("ctrl_code"   	        );
			String BID_DIV				= header.get("BID_DIV"              );
			
			
			resultInfo.put("po_from_date"             ,   po_from_date      	);
			resultInfo.put("po_to_date"               ,   po_to_date        	);
            resultInfo.put("po_no"                    ,   po_no          		);
			...
			resultInfo.put("confirm_flag"             ,   confirm_flag 		);
			resultInfo.put("complete_mark"            ,   complete_mark    	);
			resultInfo.put("po_status"                ,   po_status 			);
			...
			resultInfo.put("cont_from_date"           ,   cont_from_date    	);
			resultInfo.put("cont_to_date"             ,  	cont_to_date    	);
			resultInfo.put("cont_seq"                 ,  	cont_seq    		);
			resultInfo.put("cont_sign_from_date"      ,   cont_sign_from_date );
			resultInfo.put("cont_sign_to_date"        ,   cont_sign_to_date   );
			resultInfo.put("cont_title"               ,   cont_title    		);
			resultInfo.put("cont_status"              ,   cont_status    		);
			resultInfo.put("cust_code"                ,   cust_code    		);
			resultInfo.put("sales_dept"               ,   sales_dept    		);
			resultInfo.put("req_dept"                 ,  	req_dept    		);
			resultInfo.put("wbs_name"                 ,  	wbs_name    		);
			resultInfo.put("po_name"                  ,   po_name    			);
			resultInfo.put("ct_name"                  ,   ct_name    			);
			resultInfo.put("maker_name"               ,   maker_name    		);
			resultInfo.put("ctrl_code_session"        ,   ctrl_code_session	);
			resultInfo.put("bid_div"                  ,   BID_DIV				);
	    ...
	    private GridData getPoListGdRes(GridData gdReq, GridData gdRes, SepoaOut value) throws Exception{
