var __verifyvid = function(__SANDBOX) {
...
verifyvid_tk1 = null; var gDialogObj;
var gDialogParam;
var gInputHandler;

XWC.getLocaleResource("verifyvid", XWC.lang());
...
function onload(aDialogObj) {
	...
	if (gDialogParam.inputType == "lite") {
		gInputHandler = new __SANDBOX.inputKeyHandler ("verifyvid", __22, 1, 78, 0, "number_crt", 30, 240, "lite");
	} else if (gDialogParam.inputType == "xfs") {
		gInputHandler = new __SANDBOX.inputKeyHandler ("verifyvid", __22, 1, 78, 0, "number_crt", 30, 240, "e2e");
	} else {
		gInputHandler = new __SANDBOX.inputKeyHandler ("verifyvid", __22, 1, 78, 0, "number_crt", 30, 240);
	}
	...
	if (!(__SANDBOX.isIE() && navigator.userAgent.indexOf('Windows NT 6.2') >= 0)) {
		if (__SANDBOX.IEVersion <= 7) {
			setCapsLockToolTip(__22, __14, 0, -8);
		} else  {
			setCapsLockToolTip(__22, __14, 0, 0);
		}
	}
	...
	if (AnySign.mWBStyleApply)
	{
		...
		label.setAttribute('for', 'xwup_verifyvid_tek_check1', 0);
		label.setAttribute('id', 'xwup_verifyvid_tek_label1', 0);
		...
		if (__SANDBOX.IEVersion <= 8) {
			checkbox.mergeAttributes(XWC.UI.createElement("<INPUT name='verifyvid_tek_check1'>"), false);
		} else {
			checkbox.setAttribute('name', 'verifyvid_tek_check1', 0);
		}

		checkbox.setAttribute('id', 'xwup_verifyvid_tek_check1', 0);
...
function onOKButtonClick(e)
{
    if (gInputHandler.getLength() == 0) {
		alert(XWC.S.idnumlengtherr);
		return;
    }

		_inputHandlerCallback = function (result)
	{
		var aKeyword = gInputHandler.getValue(result);
...
function onInputMouseCheckBoxClick(aEvent) {
	...
	aTransKey = verifyvid_tk1;

	if (aChecked) {
		aTransKey.useTransKey = true;
		__22.readOnly = true;
		showTransKeyBtn("verifyvid_tk1");
		aTransKey.clear();
	}
	else {
...
__10.setAttribute('src', AnySign.mBasePath+'/img/main_verifyvid.png', 0);
__10.setAttribute('width', '52', 0);
__10.setAttribute('height', '32', 0);
__10.setAttribute('alt', XWC.S.idnum, 0);
__9.appendChild(__10);
__8.appendChild(__9);
__7.appendChild(__8);
var __11 = XWC.UI.createElement('TD');
__11.appendChild(document.createTextNode(XWC.S.info1));
var __12 = XWC.UI.createElement('BR');
...
if (__SANDBOX.IEVersion <= 8) {
__19.mergeAttributes(XWC.UI.createElement("<FORM name='xwup_verifyvid_tek_form'>"),false);
} else {
__19.setAttribute('name', 'xwup_verifyvid_tek_form', 0);
}
__19.setAttribute('id', 'xwup_verifyvid_tek_form', 0);
__19.className = 'xwup-veriid-sec';
__19.onsubmit = function(event) {return false;};
var __20 = XWC.UI.createElement('DIV');
__20.className = 'xwup-password-field2';
__20.style.textAlign = 'left';
__20.setAttribute('id', 'verifyvid_tk1', 0);
var __21 = XWC.UI.createElement('LABEL');
__21.htmlFor = 'xwup_verifyvid_tek_input1';
__21.className = 'xwup-input-name';
...
if (__SANDBOX.IEVersion <= 8) {
__22.mergeAttributes(XWC.UI.createElement("<INPUT name='verifyvid_tek_input1'>"),false);
} else {
__22.setAttribute('name', 'verifyvid_tek_input1', 0);
}
__22.setAttribute('id', 'xwup_verifyvid_tek_input1', 0);
__22.setAttribute('type', 'password', 0);
__22.className = 'xwup-input-pwd';
__22.setAttribute('kbd', 'number_crt', 0);
...
__24.onclick = function(event) {onOKButtonClick(event)};
