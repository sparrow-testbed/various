...
<%
	...
	String material_code	= JSPUtil.nullToEmpty(request.getParameter("material_number").replaceAll ( "&#64;" , "@" ));
...
/*****************************************************************
	if(mode.equals("S")) 
	{
        ...
        if(bid_flag == null) bid_flag = "";

		Object[] obj = {rfq_no, rfq_count, rfq_seq, bid_flag};
		SepoaOut value = ServiceConnector.doService(info, "PU_119", "CONNECTION", "getVendorList", obj); //p1005
		...
		if(wf != null) {
			if(wf.getRowCount() > 0) { 
				...
				IRS_NO	    = new String[wf.getRowCount()];
	...
	else if(mode.equals("R") || mode.equals("M")) 
	{
		...
		SepoaOut value = ServiceConnector.doService(info, "PU_119", "CONNECTION", "re_getVendorList_all", obj);
		...
		if(wf != null) 
		{
			if(wf.getRowCount() > 0) 
			{
				...
				NO          = new String[wf.getRowCount()];
				NAME        = new String[wf.getRowCount()];
				PHONE_NO1 	= new String[wf.getRowCount()];
				PURCHASE_BLOCK_FLAG  = new String[wf.getRowCount()];
				IRS_NO	    = new String[wf.getRowCount()];
				...
				for(int i=0; i<wf.getRowCount(); i++) 
				{
					...
					NO[i]          			= wf.getValue("NO",i);
					NAME[i]        			= wf.getValue("NAME",i);
					PHONE_NO1[i] 	   		= wf.getValue("PHONE_NO1",i);
					PURCHASE_BLOCK_FLAG[i] 	= wf.getValue("PURCHASE_BLOCK_FLAG",i);
					IRS_NO[i]	   = wf.getValue("IRS_NO",i);
				}
			}
		}
	...
	else if(mode.equals("A"))
	{
		...
		Object[] obj = {material_code};	
		
		SepoaOut value = ServiceConnector.doService(info, "PU_119", "CONNECTION", "re_MList_all", obj);
...
/* 
	...
	function leftArrow() 
	{
		...
		for(row = left_grid_array.length-1; row >= 0; row--) 
		{
			...
			if(LEFT_PURCHASE_BLOCK_FLAG == "Y"){
				flag = "false";
			  	alert(LEFT_SELLER_NAME_LOC + " 업체는 거래중지 입니다.");
			  	continue;
			}
			
			if(LEFT_PURCHASE_BLOCK_FLAG == "B"){
				flag = "false";
			  	alert(LEFT_SELLER_NAME_LOC + " 업체는 입찰금지 입니다.");
			  	continue;
			}
	...
	function getSupiSelect(RESET_FLAG){
		...
		for(var i = 0 ; i < selectGrid.length ; i++){
			var LEFT_SELLER_CODE         = GridObj.cells(selectGrid[i], GridObj.getColIndexById("SELLER_CODE")).getValue(); // 업체코드
		   	left_seller_code += LEFT_SELLER_CODE + "@";
		}
	...
	function doSave(flag)
	{
		...
		var message = "<%=text.get("PU_112_2.0002")%>";
		if(flag == 2){
			message = "업체지정을 완료하시겠습니까?";
		}

		/* tytolee
		var sellerDuplicationFlag = false;
		for(var x = 0 ; x < centerGrid.length ; x++){
			for(var z = 0 ; z < selectGrid.length ; z++){
				
				var selectSeller = parent.bottomFrame.GridObj.cells(selectGrid[z], parent.bottomFrame.GridObj.getColIndexById("SELLER_CODE")).getValue();
				var centerSeller = parent.mainFrame  .GridObj.cells(centerGrid[x], parent.mainFrame  .GridObj.getColIndexById("SELLER_CODE")).getValue();
				if(selectSeller == centerSeller){
					sellerDuplicationFlag = true;
				}
			}
			if(sellerDuplicationFlag){
				sellerDuplicationFlag = false;
			}else{
				alert("업체별 담당자는 필수 선택사항입니다.");
				return;
			}
		}
		*/
		...
		for(var j = 0 ; j < selectGrid.length ; j++){
			supiSelected += parent.bottomFrame.GridObj.cells(selectGrid[j], parent.bottomFrame.GridObj.getColIndexById("USER_SEQ")).getValue() + "!";

			for(var k = 0 ; k < parent.opener.supiFlame.GridObj.getRowsNum() ; k++){
				var temp1 = parent.opener.supiFlame.GridObj.cells(k+1, parent.opener.supiFlame.GridObj.getColIndexById("SELLER_CODE")).getValue() ;
				var temp2 = parent.opener.supiFlame.GridObj.cells(k+1, parent.opener.supiFlame.GridObj.getColIndexById("USER_SEQ")).getValue();
				
				var s_temp1 = parent.bottomFrame.GridObj.cells(selectGrid[j], parent.bottomFrame.GridObj.getColIndexById("SELLER_CODE")).getValue();
				var s_temp2 = parent.bottomFrame.GridObj.cells(selectGrid[j], parent.bottomFrame.GridObj.getColIndexById("USER_SEQ")).getValue();
				
				if(temp1 == s_temp1 && temp2 == s_temp2){
		   		    parent.opener.supiFlame.GridObj.cells(k+1, parent.opener.supiFlame.GridObj.getColIndexById("SELECTED")).cell.wasChanged = true;
		   	    	parent.opener.supiFlame.GridObj.cells(k+1, parent.opener.supiFlame.GridObj.getColIndexById("SELECTED")).setValue("1");
		   	    	
		   	    	selectUserData += s_temp2 + "|@|";
...
<%--
선택한 인원의 정보가 없어 디비상 등록된 기본 담당자에게만 메일 전송....
화면과 실제 기능이 동이 하지 않음.
...
--%>		
		...
		try{
			parent.opener.document.form.selectUserData.value = selectUserData;
		} catch( e ) {}
		*/
		...
		for(var i =0; i < rowcount; i++ )
		{
    	    ...
    	    parent.opener.setVendorList("<%=szRow%>", VENDOR_SELECTED, linecount);
...
<form name="form">
	...
	<table width="98%" border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td align="center">
				<a href="javascript:leftArrow();"><img src="../images/icon_next.gif" width="15" height="15" border="0" alt=""></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				<br><br>
				<a href="javascript:rightArrow();"> <img src="../images/icon_next2.gif" width="15" height="15" border="0" alt=""></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			</td>
			<td>
			<table width="100" border="0" cellspacing="0" cellpadding="0">
				<tr>
			  		<td width="78%" >&nbsp;&nbsp;&nbsp;<b><%-- 지정업체 --%><%=text.get("PU_112_2.TEXT_0002")%></b>
			  		</td>
				</tr>
				<tr>
