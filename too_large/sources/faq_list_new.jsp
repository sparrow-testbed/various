...
<%
	...
	Object[] obj =
	{
	 ...
	 multilang_id.addElement("FQ_002");
...
// 		var dept_type = encodeUrl(LRTrim(document.form1.dept_type.value));
		...
		GridObj.post("<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.admin.bulletin_list_new","gubun="+gubun+"&quest="+quest+"&company_code="+company_code+"&view_user_type="+view_user_type+"&language="+language+"&mode=getSupFaq&grid_col_id="+grid_col_id);
	...
	function checkRows()
	{
		...
		alert("<%=text.get("FQ_002.MSG_0101")%>");
		return false;
	}
	...
	function doUpdate()
	{
		...
		if(parseInt(sel_row) > 1) {
			//alert('한행만 선택하십시요.');
			alert("<%=text.get("FQ_002.MSG_0102")%>");
			return;
		}

		var select_seq = LRTrim(GridObj.cells(cnt, GridObj.getColIndexById("SEQ")).getValue());

		location.href = "faq_list_upd_new.jsp?select_seq="+select_seq;
	...
	function doDelete()
	{
	   ...
	   if (confirm("<%=text.get("FQ_002.0001")%>")){
		    var cols_ids = "<%=grid_col_id%>";

		    myDataProcessor = new dataProcessor("<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.admin.bulletin_list_new?cols_ids="+cols_ids+"&mode=setFaqDelete");
...
function popup(seq)
{
	var url = "../admin/faq_list_popup_new.jsp?seq="+seq;

	CodeSearchCommon(url,'faq_list_popup_new','0','0','700','600');
	doSelect();
}
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <%	} %>
				<tr>
			      ...
			      <td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("FQ_002.MSG_8801")%></td>
...
<%--
				...
				<tr>
		        	<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("FQ_002.COMPANY_NAME")%>
		               </td>
		            <td class="data_td">
...
<!-- 				</tr>				    -->
<!-- 				<tr> -->
<%-- 		        	<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("FQ_002.DEPT_TYPE")%> --%>
<!-- 		               </td> -->
<!-- 		            <td class="data_td"> -->
						...
						<TR>
							...
							<TABLE cellpadding="2" cellspacing="0">
							  <TR>
								...
								<td><script language="javascript">btn("javascript:doUpdate()","<%=text.get("FQ_002.button_update")%>")</script></td>
								<td><script language="javascript">btn("javascript:doDelete()","<%=text.get("FQ_002.button_delete")%>")</script></td>			                            	
			                   <%--  <% } %>		 --%>			
							  </TR>
						    </TABLE>
						  </td>
