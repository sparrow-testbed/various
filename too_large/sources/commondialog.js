var __commondialog = function(__SANDBOX) {
...
XWC.getLocaleResource("commondialog", XWC.lang(), "UTF8");
...
function onload(aDialogObj) {
	...
	var aTitle, aMessage;
	if(gDialogParam.dialogType == "alert") {
		__8.style.display = 'none';
	}

	if(gDialogParam.align) {
		__5.align = gDialogParam.align;
	}

	if(gDialogParam.message) {
		aTitle = gDialogParam.title || "";
		aMessage = gDialogParam.message;
	} else {
		aTitle = eval("XWC.S." + gDialogParam.messageType + ".title");
		aMessage = eval("XWC.S." + gDialogParam.messageType + ".message");
	}

	__2.firstChild.appendChild(document.createTextNode(aTitle));


	if (gDialogParam.postMessage) {
		aMessage += gDialogParam.postMessage;
	}

	__5.innerHTML = aMessage;
...
function onOKButtonClick()
{
	...
	if(gDialogParam.dialogType == "confirm" && gDialogParam.failedCount >= 3)
	{
		__SANDBOX.upInterface().installCertificateByLDAP (gDialogParam.ldapURL,
														  gDialogParam.issuerRDN,
                                                          _CB_installCertificateByLDAP);
	}
	else
	{
		gDialogObj.onconfirm();
...
__5.setAttribute('id', 'xwup_message', 0);
__5.className = 'xwup-dialog-sec';
__5.setAttribute('readonly', '', 0);
__4.appendChild(__5);
var __6 = XWC.UI.createElement('DIV');
__6.setAttribute('align', 'center', 0);
