...
// 		System.out.println("ORR_PR_DATA : " + MapUtils.getString(hashMap, "PR_DATA", ""));
		if(!"".equals(MapUtils.getString(hashMap, "PR_DATA", "").trim())){
			SepoaOut prValue = ServiceConnector.doService(info, "BD_001", "TRANSACTION","getDefaultPrData", prObj);
			if(prValue.flag){
		...
		if(!"".equals(BID_NO)){
		    ...
		    if(wf.getRowCount() > 0) {
				...
				if (!(origin_bid_status).equals("IAR")) { // 입찰공고 작성을 하려는 경우외에는 기존에 data를 조회해 온다.
					...
					CNST_BATCH_GB	    = wf.getValue("CNST_BATCH_GB", 0);
				}
			}
		}
	}
...
function init() {
	...
	<%if (SCR_FLAG.equals("I") && !BID_STATUS.equals("UR")) {%>
		...
		<%if(BID_TYPE.equals("D")) {%>
				document.form.X_BASIC_ADD.value = "낙찰자는 낙찰통지를 받은 후 10일 이내에 계약을 체결해야 함. ";
				document.form.X_RELATIVE_ADD.value = "(1) 총액입찰(입찰품목에 대한 품목별 단가에 제조예정 수량을 곱하여 합산한 금액)\r\n(2) 비공개 1회 투찰방식";
				document.form.BID_ETC.value = "(1)입찰금액은 이윤, 부가가치세, 포장비, 배송비 등 일체의 관련비용을 포함한 금액임\r\n(2)예정수량, 입찰일시, 납품기일 등 기타 모든사항은 우리은행 사정에 따라 변경 될 수 있음\r\n(3)담합 및 경쟁참가를 방해한 자의 입찰은 무효로 함\r\n(4)기타 계약에 관한 일반적인 사항은 (주)우리은행 계약사무지침에 따름";
		<% }%>
		
		document.form.LIMIT_CRIT.value = "o 참가 자격을 구비한 업체로 소정 기일내에 입찰참가 등록을 필하여야 함.";

		//입찰 설명회에 참석한 자에 한함" 문구 삭제 요청
...
<%-- 		document.form.REMARK.value = "<%//=questtel%>"; --%>
	...
	if ("<%=BID_TYPE%>" == "C" && "<%=SCR_FLAG%>" == "I" && "<%=BID_STATUS%>" == "AR") {
		document.form.PROM_CRIT.value = "B";
	}
	
	<%if(!isAdmin || "D".equals(SCR_FLAG)){%>$("#comp_td").hide();<%}%>
	
	if ("<%=BID_INPUT_TYPE%>" == "T"|| "<%=BID_INPUT_TYPE%>" == "") {
		
		document.getElementById("r_type1").checked = true;
	}
	else{
		document.getElementById("r_type2").checked = true;
...
--%>
		...
		var r_type = "";
		var bidInputType = document.getElementsByName("r_type");
		
		
		//alert(bid_input_type.length);
		for( var i=0; i < bidInputType.length; i++)
		{
			if( bidInputType[i].checked == true){
				
				bid_input_type = bidInputType[i].value;
			}
		
		}
		//alert("bid_input_type=="+bid_input_type);
		
		document.forms[0].BID_INPUT_TYPE.value = bid_input_type;
		
		//alert("setvalue=="+document.forms[0].BID_INPUT_TYPE.value);
		
		if(LRTrim(document.form.PROM_CRIT.value) == "D" && bid_input_type == "T") {
			alert("TCO 입찰방법은 입찰서 작성기준은 단가(내역)을 선택해주세요.     ");
			//document.getElementById("r_type2").checked = true;
			//document.forms[0].PROM_CRIT.focus();
			return false;
		}	
		...
		{
			alert("품목수가 1개 이상인 경우 입찰서 작성기준은 단가(내역)을 선택해주세요.");
			//document.getElementById("r_type2").checked = true;
			return false;
		}
		...
		for(var i = 0; i < grid_array.length; i++)
		{
				...
				if( bid_input_type == "T" && QTY > 1 )
				{
					alert("수량이 1개 이상인 경우 입찰서 작성기준은 단가(내역)을 선택해주세요.");
					//document.getElementById("r_type2").checked = true;
					return false;
				}
...
<%--             alert("<%=text.get("MESSAGE.1002")%>"); --%>
	...
	function avengerkim() {
		...
		if(f.PROM_CRIT.value=='D'){			
			for(var i = 0; i < grid_array.length; i++)
			{					
				var TCO_FLAG = GridObj.cells(grid_array[i], GridObj.getColIndexById("TCO_FLAG")).getValue();
				if(TCO_FLAG=="Y"){
					tcocount++;
				}
			}
			...
			if(rowcount <2){
				alert("자재가 하나이하일 경우에는 TCO를 적용하실 수 없습니다.");
				return;
			}
			
			if(tcocount == 0){
				alert("자재의 TCO FLAG값이 NULL일 경우에는 TCO를 적용하실 수 없습니다.");
				return;
			}
		}
					...
					<td  width="80%">
						<table width="100%" border="0" cellspacing="1" cellpadding="0" style="border-collapse: collapse">
							...
							<tr>
								...
								<td width="80%" class="data_td">
									<SPAN CLASS=".general_padding_left">공개여부</SPAN>&nbsp;&nbsp;<input type="checkbox" id="X_ESTM_CHECK" name="X_ESTM_CHECK" <% if (X_ESTM_CHECK.equals("Y")) {%>checked="checked"<%}%><%=abled%> style="border: 0px;">
								</td>
							</tr>							
						</table>
					</td>
					...
					<tr>
						...
						if(BID_TYPE.equals("D")) {
							...
							<tr>
								...
								<td width="80%" class="data_td">
									<textarea id="LIMIT_CRIT" name="LIMIT_CRIT" cols="70" rows="3" maxlength="500" class="input_re_2" <%=script%> onKeyUp="return chkMaxByte(1500, this, '참가자격');"><%=LIMIT_CRIT%></textarea>
								</td>
							</tr>
							<%
						} else {
							...
							<tr>
								...
								<td width="80%" class="data_td">
									<select id="LIMIT_CRIT" name="LIMIT_CRIT" class="input_re_2" <%=script%> onKeyUp="return chkMaxByte(1500, this, '참가자격');">
										<%
...
<%-- 							<input type="text" id="X_DOC_SUBMIT_TIME_HOUR_MINUTE"  name="X_DOC_SUBMIT_TIME_HOUR_MINUTE"  value="<%=X_DOC_SUBMIT_TIME_HOUR_MINUTE%>" size="4"    maxlength="4"  class="input_re_2" <%=script%>> 까지 --%>
					...
					<tr>
		  				...
		  				<td width="80%" class="data_td">
							<textarea id="BID_JOIN_TEXT" name="BID_JOIN_TEXT" cols="70" rows="7" maxlength="500" class="input_re_2" <%=script%> onKeyUp="return chkMaxByte(2000, this, '제출서류');"><%=BID_JOIN_TEXT%></textarea>
						</td>
					</tr>
					<tr>
		  				<td  width="20%" class="title_td">
		  				...
		  				<td width="80%" class="data_td">
							<textarea id="BID_CANCEL_TEXT" name="BID_CANCEL_TEXT" cols="70" rows="3" maxlength="500" class="input_re_2" <%=script%> onKeyUp="return chkMaxByte(2000, this, '유의사항');"><%=BID_CANCEL_TEXT%></textarea>
						</td>
					</tr>
				</table>
		  </td>
		  		...
		  		<table width="100%" border="0" cellspacing="1" cellpadding="0" style="border-collapse: collapse">
		  			<tr>
		  				...
		  				<td width="60%" class="data_td" colspan="3">
							  ...
							  <table style="<%=displayMode%>">
							  	<tr>
							  		<td>

									<td width="30%" class="data_td" id="comp_td">
									 	<span class="general_padding_left" id="h1">입찰서 작성기준
										</span>
										
										
						 			</td>	
						 			<td width="70%" class="data_td" id="comp_td" align="left">
									 	총액(가격)<input type="radio" value="T" name="r_type" id="r_type1">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									 	단가(내역)<input type="radio" value="I" name="r_type" id="r_type2" checked>
				  ...
				  <td  width="20%" class="title_td">9. 계약관련사항</td>
				  	<td width="80%">
			  			<table width="100%" border="0" cellspacing="1" cellpadding="0" style="border-collapse: collapse">
			  				...
			  				<td width="80%" class="data_td">
			  					<input type="text" id="hidCOST_SETTLE_TERMS" name="hidCOST_SETTLE_TERMS" size="70" value="<%=COST_SETTLE_TERMS.replaceAll("\"", "&quot;")%>" class="input_re_2" <%=script%> onKeyUp="return chkMaxByte(4000, this, '대금지급방법');">
				  			</td>
						</tr>
					</table>
			  </td>
...
%>									
						...
						<tr>
							...
							<td width="80%" class="data_td">
				  				<select id="CNST_BATCH_GB" name="CNST_BATCH_GB" class="input_re" <%=abled%>>
									<%
										String com9 = ListBox(request, "SL0018", info.getSession("HOUSE_CODE") + "#" +"M225", CNST_BATCH_GB);
						    			out.println(com9);
									%>
								</select>
							</td>
