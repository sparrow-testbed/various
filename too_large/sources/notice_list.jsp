...
<%
    ...
    int pg_row = 5;
    int rec_cnt = 0;
    String pg_no    = JSPUtil.nullToRef(request.getParameter("st_page"),"1");
    String flag     = JSPUtil.nullToEmpty(request.getParameter("flag"));
    String gubun    = JSPUtil.nullToEmpty(request.getParameter("gubun"));
    String quest    = JSPUtil.nullToEmpty(request.getParameter("quest"));
    String search   = JSPUtil.nullToEmpty(request.getParameter("search"));    
    String con_type = JSPUtil.nullToRef(request.getParameter("con_type"),"N");	// 'N'은 공지사항, 'Q'는 Q&A, 'D'는 자료실
    String cnt      = JSPUtil.nullToEmpty(request.getParameter("cnt"));
    ...
    String module = "01";  // 01 : 국내사용자, 02 : 국외사용자

    Object[] obj = {pg_no, String.valueOf(pg_row), gubun, quest, search, con_type}; // "M" 은 PR자료
...
function setPage_Change(page, type){
	...
	document.notfrm.method = "post";
	var qaPageNo = document.notfrm.st_page.value;
        
	//document.notfrm.action = "<%=request.getRequestURI()%>?st_page=" + page + "&st_page=" + qaPageNo;
	
	if(type == "N"){
		document.notfrm.action = "<%=request.getRequestURI()%>?st_page=" + page;
	} else if(type == "Q"){
		document.notfrm.action = "<%=request.getRequestURI()%>?faq_page=" + page;
	} else {
		document.notfrm.action = "<%=request.getRequestURI()%>?dataStore_page=" + page;
	}
...
function goNews(seq){
	...
	var con_type = "<%=con_type%>";
	//var url="/intro/sub/popup.jsp?seq="+seq+"&module="+module+"&con_type="+con_type+"&search=N";
	var url="/intro/sub/notice_view.jsp?seq="+seq+"&module="+module+"&con_type="+con_type+"&search=N";
	//var width = 531;
	//var height = 300;
	var width = 750;
	var height =750;
	...
	var scrollbars = 'no';
	var resizable = 'no';
	var library = window.open( url, 'notice', 'left='+left+', top='+top+', width='+width+', height='+height+', toolbar='+toolbar+', menubar='+menubar+', status='+status+', scrollbars='+scrollbars+', resizable='+resizable);

	//library.moveTo  ( Math.ceil( windowX ) , Math.ceil( windowY ) )
	*/

	var url = "/admin/bulletin_list_popup_new_index.jsp?seq="+seq;
...
function goList() {
	var con_type = "<%=con_type%>";
	location.href = "notice_list.jsp?con_type="+con_type;
}
    
function Create(){
	var con_type = "<%=con_type%>";
	location.href = "not_bd_ins1.jsp?con_type="+con_type
}
    
function goNotice(flag) {
	var url="/intro/sub/notice_list.jsp?search=Y&con_type="+flag;
	var ret  = window.open( url, 'notice', 'left=0, top=0, width=750, height=700, toolbar=no, menubar=no, status=no, scrollbars=no, resizable=no');
}
...
function fnNoticePop(){
	...
	var top    = dim[0];
	var left   = dim[1];
	var url    = "/common/index_buyer_notice_pop.jsp";
	var name   = "Notice";
...
<form name="notfrm">
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td width="100%" height="25"><img src="/images/title_notice.gif" alt="공지사항" onclick="fnNoticePop()" style="cursor: pointer;"/></td>
		</tr>
		<tr>
...
<%
	try {
		SepoaOut value11 = ServiceConnector.doService(info, "CO_004", "CONNECTION", "getQuery_ICOMMACH", obj);
...
%>
	...
	<input type="hidden" name="st_page" value="<%=pg_no%>">
</form>
<%-- 
