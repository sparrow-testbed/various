...
<%
	...
	multilang_id.addElement("WO_103");
...
var G_SERVLETURL    = "<%=POASRM_CONTEXT_NAME%>/servlets/srm.sg_supplycompany_list";
    ...
    function doQuery()
	{
	    ...
	    var sg_type1	= LRTrim(f.sg_type1.value);
	    var sg_type2	= LRTrim(f.sg_type2.value);
	    var sg_type3	= LRTrim(f.sg_type3.value);
		...
		GridObj.post("<%=POASRM_CONTEXT_NAME%>/servlets/srm.sg_supplycompany_list?mode=query&grid_col_id="+grid_col_id+ param);
	...
	function initAjax(){
		...
		//doRequestUsingPOST( 'W001', '2' ,'daebun2', '' );
		//doRequestUsingPOST( 'W001', '3' ,'daebun3', '' );
	}
    ...
    function setdoAddRow( ischkNum, sg_type1, sg_type2, sg_type3, company_code, company_name ){
    	...
    	if( ischkNum == 0 ){
    		for( var i = 1; i <= nMaxRow2; i++ ){
	    		...
	    		var data_company_code = LRTrim(GridObj.cells(i, GridObj.getColIndexById("vendor_code")).getValue());
	    	...
	    	if( ischkNum2 == 0 ){
				...
				GridObj.cells(nMaxRow2, GridObj.getColIndexById("vendor_code")     ).setValue(company_code);
				GridObj.cells(nMaxRow2, GridObj.getColIndexById("vendor_name_loc") ).setValue(company_name);
...
<%
	 ...
	 thisWindowPopupScreenName = "소싱그룹/공급업체연결현황 ";
		...
		<td width="100%" valign="top">
			<table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#DBDBDB">
				<tr>
					...
					<td width="85%" class="se_cell_data" colspan="5">
						<input type="text" id="company_code" name="company_code" class="inputsubmit" readonly="readonly">
						...
						<a href="javascript:getVendor_code('','')">
							<img src="<%=POASRM_CONTEXT_NAME%>/images/ico_x2.gif" align="absmiddle" border="0" alt="">
						</a>
						<input type="text" id="company_name" name="company_name" class="input_empty" readonly="readonly">
					</td>					
				</tr>
...
<%-- <s:grid screen_id="WO_103" grid_obj="GridObj" grid_box="gridbox"/> --%>
