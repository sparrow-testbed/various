...
<%
	...
	multilang_id.addElement("I_RQ_444");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "I_RQ_444";
	...
	String WISEHUB_PROCESS_ID="I_RQ_444";
...
function doSelect(){
	...
	var param = "mode=getRfqList2&grid_col_id="+grid_col_id;
	    param += dataOutput();
	var url = "<%=POASRM_CONTEXT_NAME%>/servlets/ict.dt.rfq.rfq_bd_lis2";
...
function doConfirm(){
	...
	for(row=rowcount-1; row>=0; row--){
		...
		if( GD_GetCellValueIndex(GridObj,row, IDX_RFQ_STATUS) == "E" ) { 
			alert("진행상태가 [견적마감] 인 경우 확인이 불가합니다.");
		
			return;
		}
		
		if("0" == GD_GetCellValueIndex(GridObj,row, IDX_SUBMIT_COUNT)) { // 견적서를 제출한 업체가 있는 경우 확인불가
			alert("견적서를 제출한 업체가 없습니다.");
	...
	var url="rfq_bd_upd02_ict.jsp?rfq_no=" + rfq_no + "&rfq_count=" + rfq_count + "&item_no=" + item_no;	
	var win1 = window.open(url,"rfq_bd_upd02_ict","toolbar=no, location=no, directories=no, status=yes,menubar=no,scrollbars=yes,resizable=yes,width=1024,height=650,left=0,top=0");
	win1.focus();
}
	...
	for(row=rowcount; row>=0; row--) {
		if( true == GD_GetCellValueIndex(GridObj,row, IDX_SEL)) {
			...
			if(GD_GetCellValueIndex(GridObj,row, IDX_SUBMIT_COUNT) == 0){
				alert("견적마감은 견적서를 제출한 업체가 있어야 가능 합니다.");
...
function setRFQCloseCancel(){ // 견적마감
	rowcount = GridObj.GetRowCount() -1;
	...
	for(row=rowcount; row>=0; row--) {
		if( true == GD_GetCellValueIndex(GridObj,row, IDX_SEL)) {
			...
			if("E" != GD_GetCellValueIndex(GridObj,row, IDX_RFQ_STATUS)) {
				alert("진행상태가 [견적마감] 인 경우에만 마감취소 가능합니다.");
...
function doOnRowSelected(rowId,cellInd){
	...
	else if(cellInd == GridObj.getColIndexById("CONFIRM_COUNT")) {	//확인업체수
		url      = '/ict/kr/dt/rfq/rfq_pp_dis01_ict.jsp?rfq_no=' + encodeUrl(rfq_no) + '&rfq_count=' + encodeUrl(rfq_count) + '&screen_flag=search&popup_flag=true';
		popUpOpen(url, 'GridCellClick', '650', '350');
	}
...
btn("javascript:doSelect()","조 회");
							...
							<TD>
<script language="javascript">
btn("javascript:doConfirm()","견적서 (확인)");
</script>
							</TD>
							<TD>
<script language="javascript">
btn("javascript:setRFQClose()","견적요청 (마감)");
</script>
							</TD>
							<TD>
<script language="javascript">
btn("javascript:setRFQCloseCancel()","견적마감 (취소)");
</script>
							</TD>						
						</TR>
					</TABLE>
...
<%-- <s:grid screen_id="I_RQ_444" grid_obj="GridObj" grid_box="gridbox"/> --%>
