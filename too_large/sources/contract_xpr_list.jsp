...
<%
	...
	String from_date = SepoaDate.addSepoaDateMonth(to_day,-3);
	//String to_date = to_day;
	String to_date = SepoaDate.addSepoaDateMonth(to_day,3);
	...
	multilang_id.addElement("CT_021");
...
var myDataProcessor = null;
    ...
    function doOnRowSelected(rowId,cellInd)
    {
    	...
    	if(header_name == "GW_COD_NO") {
    		
    		var	docLink      = LRTrim(GridObj.cells(rowId, GridObj.getColIndexById("DOC_LINK")).getValue());
    		...
    		window.open(docLink, "gwPop", status);
    	}
    ...
    function doQuery()
	{
		var from_date1      = del_Slash(document.form.from_date1.value);
		var to_date1        = del_Slash(document.form.to_date1.value);
		var from_date2      = del_Slash(document.form.from_date2.value);
		var to_date2        = del_Slash(document.form.to_date2.value);
		...
		var gw_cod_no       = document.form.gw_cod_no.value;	    // G/W품의번호
		var gw_title        = document.form.gw_title.value;			// G/W품의제목
		
		
		
		var param  = "";
			param += "&from_date1="		+ encodeUrl(from_date1);
	    	param += "&to_date1="		+ encodeUrl(to_date1);
	    	param += "&from_date2="		+ encodeUrl(from_date2);
	    	param += "&to_date2="		+ encodeUrl(to_date2);
			...
			param += "&gw_cod_no="	    + encodeUrl(gw_cod_no);
			param += "&gw_title="	    + encodeUrl(gw_title);
			
		var grid_col_id = "<%=grid_col_id%>";
		var SERVLETURL  = G_SERVLETURL + "?mode=query3&grid_col_id="+grid_col_id + param;
...
<%--  		doRequestUsingPOST( 'SL0018', '<%=info.getSession("HOUSE_CODE")%>#M287' ,'status', '' ); --%>
	...
	function getApproval(Approval_str) {
		...
		if (Approval_str == "") {
			alert("<%=text.get("CT_021.MSG_001")%>");//결재자를 지정해 주세요.
			return;
		}
		...
		if (confirm("<%=text.get("CT_021.MSG_002")%>")){	//결재요청 하시겠습니까?
...
// 			sign_status 	= encodeUrl(LRTrim(GridObj.cells(grid_array[i], GridObj.getColIndexById("SIGN_STATUS")).getValue()));
	...
	function doRemove( type ){
	    ...
	    if( type == "date1" ) {
	    	document.forms[0].to_date1.value = "";
	        document.forms[0].from_date1.value = "";
	    }
	    if( type == "date2" ) {
	    	document.forms[0].to_date2.value = "";
...
<body leftmargin="15" topmargin="6"
								...
								<td width="100%">
									<table width="100%" border="0" cellspacing="0" cellpadding="0">
										<tr>
											...
											<td width="30%" class="data_td"><s:calendar
													id_from="from_date2" id_to="to_date2"
													default_from=""
													default_to=""
													format="%Y/%m/%d" />
											<a href="javascript:doRemove('date2')"><img
														src="<%=POASRM_CONTEXT_NAME%>/images/button/ico_x2.gif"
														align="absmiddle" border="0"></a></td>
										...
										<tr>
											<td width="20%" class="title_td">&nbsp;<img
												src="/images/blt_srch.gif" width="7" height="7"
												align="absmiddle">&nbsp;&nbsp; G/W품의번호
											</td>
											<td width="30%" class="data_td"><input type="text"
												id="gw_cod_no" name="gw_cod_no" style="ime-mode: inactive"
												onkeydown='entKeyDown()'></td>											
											<td width="20%" class="title_td">&nbsp;<img
												src="/images/blt_srch.gif" width="7" height="7"
												align="absmiddle">&nbsp;&nbsp; G/W품의제목
											</td>
											<td width="30%" class="data_td"><input type="text"
												id="gw_title" name="gw_title" style="width: 150px;"
												onkeydown='entKeyDown()' /></td>
										</tr>																			
									</table>
								</td>
					...
					<td style="padding: 5 5 5 0" align="right">
						<TABLE cellpadding="2" cellspacing="0">
							<TR>
								...
								<td><script language="javascript">btn("javascript:doExcelDown()","엑셀다운")		</script></td>																				
							</TR>
						</TABLE>
					</td>
				</TR>
	...
	<s:grid screen_id="CT_021" grid_box="gridbox" grid_obj="GridObj" />
	<s:footer />
</body>
