...
<%
	...
	multilang_id.addElement("I_AP_004");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "I_AP_004";
...
<% String WISEHUB_PROCESS_ID="I_AP_004";%>
<%@ include file="/include/code_common.jsp"%>
...
function Query(type){
 ...
 if(type == "close")
 {
	...
	var param ="mode=getCompleteList&grid_col_id="+grid_col_id;
	var url	= "<%=POASRM_CONTEXT_NAME%>/servlets/ict.sepoa.svl.approval.ap_complete_list_ict";
	param += dataOutput();
...
function doOnRowSelected(rowId,cellInd)
{
	...
	if(header_name=="ATTACH_NO_IMG"){
    	if(GridObj.cells(rowId, GridObj.getColIndexById("ATTACH_NO")).getValue() != null && GridObj.cells(rowId, GridObj.getColIndexById("ATTACH_NO")).getValue() != ""){
//     		FileAttach('NOT',GridObj.cells(rowId, GridObj.getColIndexById("ATTACH_NO")).getValue(),'VI', false);
    ...
    if(header_name=="SIGN_PATH_IMG"){
		...
		CodeSearchCommon(url,'BKWin','550','800','600','400');
    }
...
// 			winpopup = window.open(vm_url ,"doExplanationMvd","toolbar=no, location=no, directories=no, status=no,menubar=no,scrollbars=yes,resizable=yes,width=800,height=748,left=0,top=0");			
			...
			var default_url = "approval_bid_ict2.jsp?doc_no="+doc_no+"&doc_seq="+doc_seq+"&doc_type="+doc_type+"&sign_enable=T"+"&attach_no="+attach_no+"&doc_status="+doc_status+"&proceeding_flag="+proceeding_flag+"&doc_seq="+doc_seq+"&sign_path_seq="+sign_path_seq+"&add_user_name="+add_user_name+"&add_date="+add_date+"&subject="+subject;
			winpopup = window.open(default_url, "_blank", "width=800, height=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no" );			
		}		
			...
			winpopup = window.open(default_url, "_blank", "width=950, height=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no" );			
		}	
...
<%-- <s:grid screen_id="I_AP_004" grid_obj="GridObj" grid_box="gridbox"/> --%>
