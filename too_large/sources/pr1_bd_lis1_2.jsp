...
<%
	...
	multilang_id.addElement("PR_008_2");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String  screen_id             = "PR_008_2";
	String  grid_obj              = "GridObj";
	String  WISEHUB_PROCESS_ID    = "PR_008_2";
	String  house_code            = info.getSession("HOUSE_CODE");
	String  company_code          = info.getSession("COMPANY_CODE");
	String  menu_profile_code          = info.getSession("MENU_PROFILE_CODE");//MUP141000001
...
function doModify(){
    ...
    var pr_no = "";
    //새로만든 스크립트(2015.02.24 next1210)
    var grid_array = getGridChangedRows(GridObj, "SELECTED");
    if(grid_array.length > 1) {
		alert("1개의 구매요청건만 수정할 수 있습니다.");
		return;
	}
	...
	for(var i = 0; i < grid_array.length; i++) {
		...
		var PR_PROCEEDING_FLAG 		= GridObj.cells(grid_array[i], GridObj.getColIndexById("PR_PROCEEDING_FLAG")).getValue();
...
//     }
    ...
    var menu_profile_code = "<%= menu_profile_code%>";
    //프로파일코드가 내부관리자인 경우만 수정가능하게 함. 구매요청자의 경우, 
    if(menu_profile_code == "MUP141000001") {
	...
	//s내부관리자가 아닌 일반구매요청자인 경우, PR테이블과 결재라인테이블의 해당 데이터를 전부 지우고 다시 카드에 Insert하여 되돌려 신청내역으로 되돌려 보낸다.	
    } else {
    	
    	if(confirm("해당 구매요청건에 속한 품목이 전부 신청내역으로 되돌아갑니다. \n진행하시겠습니까?")) {
	        var cols_ids = "<%=grid_col_id%>";
	        var params;
	        
	        params = "?mode=doReturnCart";
	        params += "&cols_ids=" + cols_ids;
	        params += dataOutput();
							...
							<td width="100%">
	  							<table width="100%" border="0" cellspacing="0" cellpadding="0">
									...
									<tr>
										...
										<td class="data_td">
											<input type="text" name="add_user_id" id="add_user_id" style="ime-mode:inactive" size="15" maxlength="10" onkeydown='entKeyDown()' class="inputsubmit" value='<%=info.getSession("ID")%>' >
	...
	<TABLE width="100%" border="0" cellspacing="0" cellpadding="0">
		<TR>
		    <TD height="30" align="left">
		    	<a href="javascript:topMenuClick('/approval/ap_report_list.jsp','MUO141000015', '2','')" class="summary_no">&nbsp;* <font color="red">결재취소</font>는 <font color="blue">"결재/상신함/상신내역"</font> 에서 하세요. <font color="red">(클릭)</font></a>
		    </TD>
...
<%-- <s:grid screen_id="PR_008_2" grid_obj="GridObj" grid_box="gridbox"/> --%>
