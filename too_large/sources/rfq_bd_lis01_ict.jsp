...
<%
	...
	multilang_id.addElement("I_RQ_443");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "I_RQ_443";
	...
	String WISEHUB_PROCESS_ID="I_RQ_443";
...
function setHeader() {
	...
	IDX_SUBMIT_COUNT                        = GridObj.GetColHDIndex("SUBMIT_COUNT");
}
...
function doSelect(){
	...
	var param = "mode=getRfqList&grid_col_id="+grid_col_id;
	    param += dataOutput();
	var url = "<%=POASRM_CONTEXT_NAME%>/servlets/ict.dt.rfq.rfq_bd_lis1";
...
function doCopy(){
	...
	for(row=rowcount-1; row>=0; row--){
		...
		/*
		if( GD_GetCellValueIndex(GridObj,row, IDX_RFQ_STATUS) != "T" ) { // 진행상태에 따라 견적 수정불가(작성중)
	...
	//var url="rfq_bd_upd01_ict.jsp?rfq_no=" + rfq_no + "&rfq_count=" + rfq_count + "&item_no=" + item_no;	
	//window.open(url,"rfq_bd_upd01_ict","toolbar=no, location=no, directories=no, status=yes,menubar=no,scrollbars=yes,resizable=yes,width=1024,height=650,left=0,top=0");
	
	var url    = '/ict/kr/dt/rfq/rfq_bd_copy01_ict.jsp';
	var title  = '견적작성복사';
	var param  = 'popup=Y';
	param     += '&rfq_no=' + rfq_no;
...
function doModify(){
	...
	for(row=rowcount-1; row>=0; row--){
		...
		if("0" != GD_GetCellValueIndex(GridObj,row, IDX_SUBMIT_COUNT)) { // 견적서를 제출한 업체가 있는 경우 수정불가
	...
	var url    = '/ict/kr/dt/rfq/rfq_bd_upd01_ict.jsp';
	var title  = '견적작성수정';
	var param  = 'popup=Y';
	param     += '&rfq_no=' + rfq_no;
	param     += '&rfq_count=' + rfq_count;
...
function doDelete(){
	...
	for(row=rowcount-1; row>=0; row--){
		if( true == GD_GetCellValueIndex(GridObj,row, IDX_SEL)){
			checked_count++;				
			RFQ_FLAG = LRTrim(GD_GetCellValueIndex(GridObj,row, IDX_RFQ_STATUS));
			
			if( RFQ_FLAG != "T" ) {
				alert("진행상태가 [작성중] 인 경우에만 삭제 가능합니다.");				
				return;
			}
	...
	//var url="rfq_bd_dis01_ict.jsp?flag=D&rfq_no=" + rfq_no + "&rfq_count=" + rfq_count + "&item_no=" + item_no;	
	//window.open(url,"rfq_bd_upd01_ict","toolbar=no, location=no, directories=no, status=yes,menubar=no,scrollbars=yes,resizable=yes,width=1024,height=650,left=0,top=0");
	
	var url    = '/ict/kr/dt/rfq/rfq_bd_dis01_ict.jsp';
	var title  = '견적작성삭제';
	var param  = 'popup=Y';
	param     += '&flag=D';
	param     += '&rfq_no=' + rfq_no;
...
function setRFQCancel(){ // 견적취소
	...
	for(row=rowcount; row>=0; row--) {
		if( true == GD_GetCellValueIndex(GridObj,row, IDX_SEL)) {
			...
			if(!("B" == GD_GetCellValueIndex(GridObj,row, IDX_RFQ_STATUS))) {
				alert("진행상태가 [견적중] 인 경우에만 회수 가능합니다.");
				
				return;
			}

			if(GD_GetCellValueIndex(GridObj,row, IDX_SUBMIT_COUNT) != 0){
				alert("견적서를 제출한 업체가 있어 회수 안됩니다.");			
				return;
			}
		}
	}
	...
	if(!confirm("견적요청을 회수 하시겠습니까?")) {
		return;
	}

	var servletUrl  = "<%=POASRM_CONTEXT_NAME%>/servlets/ict.dt.rfq.rfq_bd_lis1";
	var grid_array = getGridChangedRows(GridObj, "SEL");
	var cols_ids = "<%=grid_col_id%>";
	var params;
	
	params = "?mode=setRFQCancel";
	params += "&cols_ids=" + cols_ids;
	params += dataOutput();
...
function doOnRowSelected(rowId,cellInd){
	...
	if(cellInd == GridObj.getColIndexById("RFQ_NO")) {
	    ...
	    url = '/ict/kr/dt/rfq/rfq_pp_dis01_ict.jsp?rfq_no=' + encodeUrl(rfq_no) + '&rfq_count=' + encodeUrl(rfq_count) + '&screen_flag=search&popup_flag=true'; 	    
    	popUpOpen(url, 'GridCellClick', '650', '350');
    }
	else if(cellInd == GridObj.getColIndexById("SUBMIT_COUNT")) {	//제안업체수
		url      = '/ict/kr/dt/rfq/rfq_pp_dis01_ict.jsp?rfq_no=' + encodeUrl(rfq_no) + '&rfq_count=' + encodeUrl(rfq_count) + '&screen_flag=search&popup_flag=true';
		popUpOpen(url, 'GridCellClick', '650', '350');
	}
...
btn("javascript:doSelect()","조 회");
							...
							<TD>
<script language="javascript">
btn("javascript:doCopy()","견적복사");
</script>
							</TD>							
							<TD>
<script language="javascript">
btn("javascript:doModify()","견적작성 (수정)");
</script>
							</TD>
							<TD>
<script language="javascript">
btn("javascript:doDelete()","견적작성 (삭제)");
</script>
							</TD>
							<TD>
<script language="javascript">
btn("javascript:setRFQCancel()","견적요청 (회수)");
</script>
							</TD>
<%--							
							<TD>
...
<%-- <s:grid screen_id="I_RQ_443" grid_obj="GridObj" grid_box="gridbox"/> --%>
