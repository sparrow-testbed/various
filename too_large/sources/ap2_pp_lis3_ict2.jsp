...
<%
	...
	if ( from.equals("P") ) {
%>
	    opener.<%=fnc_name%>("UrgentFlag==="+ARGENT_FLAG+"|||SignUserId==="+app_person+"|||SignRemark==="+urlEncode(remark)+"|||auto_flag===<%=auto_flag%>|||strategy===<%=strategy%>|||attach_no==="+attach_no+"|||Sign_Path==="+Sign_Path);
<%
	} else {
%>
...
function doOnRowSelected(rowId,cellInd)
{
	...
	if( header_name == "USER_POP" ){
		...
		//CodeSearchCommon(url,'getUser','','','','');
		window.open("/common/CO_008_ict.jsp?callback=getUser", "SP9113", "toolbar=no, location=no, directories=no, status=yes,menubar=no,scrollbars=yes,resizable=no,width=450,height=550,left=0,top=0");
    }  
