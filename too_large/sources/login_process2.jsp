...
String goUrl     = "/common/idpwd2.jsp?prev_url=login_fail";
...
if("setDirectLogin".equals(mode) ){	//그룹웨어 직접 로그인
	if("true".equals(logStatus)){
		...
		if("ICT2".equals(from_site)){
			goUrl = "/common/index_buyer_ict.jsp";
		}else{
			goUrl = "/common/index_buyer.jsp?f=1";
		}
		goFnc = "startDirectPage();";
		
	}else{
		goUrl = "/common/idpwd2.jsp?prev_url=login_fail";
		goFnc = "failDirectPage();";
	}
	
}else if("setXecureLogin".equals(mode)){	//공인인증서 로그인
	if("true".equals(logStatus)){
		goUrl = "/common/index_seller.jsp?f=1";
		goFnc = "startDirectPage();";
		
	}else{
		goUrl = "/";
		goFnc = "failXecurePage();";
	}
	
}else if("setDateOver".equals(msg)){	//날짜 3개월 초과
	msg2 = "사용자 암호변경주기가 도래하였습니다. (분기당1회) 암호를 변경하여 주십시오.";
	goUrl = "/common/idpwd_pass2.jsp?FromSite=GAD2";
	goFnc = "failPage();";
	...
	if("true".equals(logStatus)){
		if("ICT2".equals(from_site)){
			...
			if("WOORI".equals(user_type)){
				goUrl = "/common/index_buyer.jsp";
			}else{
				goUrl = "/common/index_seller.jsp";
			}
		}
