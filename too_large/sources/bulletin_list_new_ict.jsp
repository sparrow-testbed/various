...
<%
	...
	multilang_id.addElement("I_MT_014");
...
var myDataProcessor = null;
	...
	function doSelect()
	{
		...
		GridObj.post("<%=POASRM_CONTEXT_NAME%>/servlets/ict.sepoa.svl.admin.bulletin_list_new_ict","gubun="+gubun+"&quest="+quest+"&company_code="+company_code+"&dept_type="+dept_type+"&view_user_type="+view_user_type+"&language="+language+"&mode=getSupNotice&grid_col_id="+grid_col_id);
	...
	function checkRows()
	{
		...
		alert("<%=text.get("I_MT_014.MSG_0101")%>");
		return false;
	}
	...
	function doUpdate()
	{
		...
		if(parseInt(sel_row) > 1) {
			//alert('한행만 선택하십시요.');
			alert("<%=text.get("I_MT_014.MSG_0102")%>");
			return;
		}

		var select_seq = LRTrim(GridObj.cells(cnt, GridObj.getColIndexById("SEQ")).getValue());

		location.href = "bulletin_list_upd_new_ict.jsp?select_seq="+select_seq;
	...
	function doDelete()
	{
	   ...
	   if (confirm("<%=text.get("I_MT_014.0001")%>")){
		    var cols_ids = "<%=grid_col_id%>";

		    myDataProcessor = new dataProcessor("<%=POASRM_CONTEXT_NAME%>/servlets/ict.sepoa.svl.admin.bulletin_list_new_ict?cols_ids="+cols_ids+"&mode=setDelete");
...
function view_count(seq){
	...
	jqa.action = "bulletin_view_count_update_ict.jsp";
	jqa.submit();
...
function popup(seq)
{
	var url = "../admin/bulletin_list_popup_new_ict.jsp?seq="+seq;
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <%	} %>
				<tr>
			      ...
			      <td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("I_MT_014.MSG_8801")%></td>
	              <td width="35%" class="data_td">
				...
				<tr>
		        	<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("I_MT_014.COMPANY_NAME")%>
		               </td>
		            <td class="data_td">
				...
				<tr>
		        	<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;<%=text.get("I_MT_014.DEPT_TYPE")%>
		               </td>
		            <td class="data_td">
						...
						<TR>
							...
							<TABLE cellpadding="2" cellspacing="0">
							  <TR>
								...
								<td><script language="javascript">btn("javascript:doUpdate()","<%=text.get("I_MT_014.button_update")%>")</script></td>
								<td><script language="javascript">btn("javascript:doDelete()","<%=text.get("I_MT_014.button_delete")%>")</script></td>			                            	
			                   <%--  <% } %>		 --%>			
							  </TR>
						    </TABLE>
						  </td>
