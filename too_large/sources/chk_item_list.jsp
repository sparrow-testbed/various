...
<%
	...
	multilang_id.addElement("SR_011");
	multilang_id.addElement("BUTTON");
	multilang_id.addElement("MESSAGE");
	
	HashMap text = MessageUtil.getMessage(info,multilang_id);
	
	String screen_id = "SR_011";
...
<script language="javascript" type="text/javascript">
	...
	function setHeader() {
		...
		INDEX_c_factor_refitem = GridObj.GetColHDIndex("c_factor_refitem");
	...
	function getQuery() {
		...
		var servletUrl = "<%=getWiseServletPath("master.template.chk_item_lst")%>";
 --%>
		...
		var param = "mode=getChkList&grid_col_id="+grid_col_id;
		    param += dataOutput();
		var url = "<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.procure.chk_item_list";
	...
	function updateChkItem() {
		...
		c_factor_refitem = GD_GetCellValueIndex(GridObj,cnt,INDEX_c_factor_refitem);

		var url = "chk_item_insert.jsp?mode=update&c_factor_refitem=" + c_factor_refitem;
		popup(url);
	}
	...
	function delChkItem(){
		...
		for(var i=0;i<GridObj.GetRowCount();i++){
			...
			if(temp == "1") {
				sel_row++;
				c_factor_refitem = c_factor_refitem + "," + GD_GetCellValueIndex(GridObj,i,INDEX_c_factor_refitem);
			}
		}
		if(sel_row == 0) {
			alert("항목을 선택해주세요.");
		...
		c_factor_refitem = c_factor_refitem.substring(1);

		var value = confirm('삭제 하시겠습니까?');
		if(value){
			/* this.hiddenframe.location.href = "chk_item_ins.jsp?mode=delete&c_factor_refitem=" + c_factor_refitem;
 */
 		
			var cols_ids = "<%=grid_col_id%>";
	    	var url = "<%=POASRM_CONTEXT_NAME%>/servlets/sepoa.svl.procure.chk_item_list";
			var params = "mode=setChkDelete&cols_ids="+cols_ids;
				params += dataOutput();
...
setHeader();
  	...
  	function JavaCall(msg1,msg2,msg3,msg4,msg5){
		...
		if(msg3 == INDEX_factor_name){
			var c_factor_refitem = GD_GetCellValueIndex(GridObj,msg2,INDEX_c_factor_refitem);
			var url = "chk_item_pop.jsp?mode=view&c_factor_refitem=" + c_factor_refitem;
			popup(url);
...
var c_factor_refitem = GridObj.cells(rowId, GridObj.getColIndexById("c_factor_refitem")).getValue();	

window.open("chk_item_detail.jsp?mode=view&c_factor_refitem=" +c_factor_refitem, "_blank", "width=800, height=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no" );
...
setHeader();" bgcolor="#FFFFFF" text="#000000" >
			...
			<TABLE cellpadding="0">
	      		<TR>
					<td><script language="javascript">btn("javascript:getQuery()","조 회")</script></td>
    	  			...
    	  			<td><script language="javascript">btn("javascript:delChkItem()","삭 제")</script></td>
				</TR>
  			</TABLE>
  		</td>
	</tr>
...
<s:grid screen_id="SR_011" grid_obj="GridObj" grid_box="gridbox"/>
<s:footer/>
</body>
</html>
