...
<%
	...
	if(mode.equals("")) {		// status = 2(신용평가요청), status =3(신용평가완료) status = 4(실사통보)
	...
	}else if(mode.equals("register")) {  // 반려
		...
		value    = ServiceConnector.doService(info, "p0070", "TRANSACTION", "setRegister", obj3);
	...
	}else if(mode.equals("kg")){		//협력사 입찰경고/입찰경고해제
		
		Object[] obj5 = { paramStr };
		value    = ServiceConnector.doService(info, "p0070", "TRANSACTION", "setBlock", obj5);

	}else if(mode.equals("nbd")){		//협력사 입찰금지/입찰금지 해제
		
		Object[] obj5 = { paramStr };
		value    = ServiceConnector.doService(info, "p0070", "TRANSACTION", "setBlock", obj5);
		
	}else if(mode.equals("block")){		//협력사 거래개시
		...
		String nickName= "p0070";
		String conType = "TRANSACTION";
		String MethodName = "setBlock";

		String[] arg = vendor_code.split(",");
		String[][] param = new String[arg.length][2];
		for(int i=0; i < arg.length; i++) {
			param[i][0] = PURCHASE_BLOCK_FLAG;
			param[i][1] = arg[i];
		}
		...
		value    = ServiceConnector.doService(info, "p0070", "TRANSACTION", "deleteProgVendor", obj6);
...
%>
	<%if(value.message == null || value.message.equals("")){%>
		...
		<%}else if(mode.equals("register")) {%>
			<%if(flag.equals("Y")){%>
			<script>alert('등록 처리 되었습니다.');</script>
			<%}else if(flag.equals("R")){%>
		...
		<%}else if(mode.equals("kg")){%>
			...
			%>
			<script>alert('입찰경고 되었습니다.');</script>
			<%
			}else{
			%>
			<script>alert('입찰경고 해제 되었습니다.');</script>
			<%
			}
			%>
			<script>parent.doSelect();</script>
		<%}else if(mode.equals("nbd")){%>
			<%
			if("B".equals(purchase_block_flag)){
			%>
			<script>alert('입찰금지 되었습니다.');</script>
			<%
			}else{
			%>
			<script>alert('입찰금지 해제 되었습니다.');</script>
			<%
			}
			%>
			<script>parent.doSelect();</script>					
		<%}else if(mode.equals("delete")){%>
			...
			<script>parent.setHeader();</script>
