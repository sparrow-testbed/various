...
<%
	...
	Object[]    obj = { mapData };
	SepoaOut    so  = ServiceConnector.doService( info, "I_SBD_012", "CONNECTION", "getBDHeader", obj );
	SepoaFormater wf = new SepoaFormater( so.result[0] );
        ...
        so  = ServiceConnector.doService( info, "I_SBD_012", "CONNECTION", "getBDHD_VnInfo", obj );
...
var G_SERVLETURL   = "<%=POASRM_CONTEXT_NAME%>/servlets/ict.sepoa.svl.sourcing.bd_ct_submit_list_seller_ict2";
...
// 	var rowcount = grid_array.length;
    	...
    	$.post(
    		...
    		function(arg){
    			...
    			if(result[1] == "true"){
    				alert(result[0]);
	    			location.href="bd_ct_submit_list_seller_ict2.jsp";
    			}
  ...
  <table width="98%" border="0" cellspacing="0" cellpadding="0">
    <tr> 
      <td width="98%" height="30">
        ...
        <table><tr>
         <% if("S".equals(MODE)){ %>
          <td><script language="javascript">btn("javascript:doSubmit()", "계약서류제출")</script></td>
         <% }else if("C".equals(MODE)){ %>          
          <td><script language="javascript">btn("javascript:doCancel()", "계약서류제출취소")</script></td>
          <% } %>         
		  </tr></table>  
        </div>
      </td>
