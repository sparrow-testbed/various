...
//////////////////ClipReport4 선언부 시작///////////////////////////////////////////////////////////
String _rptName          = "020644/rpt_app_bd_ins1"; //리포트명
...
<%
	...
	String PR_TYPE = "I";	//JSPUtil.nullToEmpty(request.getParameter("pr_type"));
...
<%
	...
	if(!"".equals(exec_no)){
		...
		data.put("pr_data"		, pr_data);
		
		Object[] obj3 = {data};
		SepoaOut value3 = ServiceConnector.doService(info, "p1062", "CONNECTION", "getEXDTInfo", obj3);
		SepoaFormater wf3 = new SepoaFormater(value3.result[0]);
	
		int iRowCount = wf3.getRowCount();
	    
	    if(iRowCount>0)
	    {
	    	ATTACH_NO           	= wf3.getValue("H_ATTACH_NO",0);
	    	ATTACH_COUNT           	= wf3.getValue("H_ATTACH_COUNT",0);
	    }
...
function init()
{
	...
	//GridObj.setColumnExcellType( GridObj.getColIndexById( LRTrim( "QTY        " ) ), LRTrim( "ron " ) );//edn 속성을 ron으로 변경
	GridObj.setColumnExcellType( GridObj.getColIndexById( LRTrim( "UNIT_PRICE " ) ), LRTrim( "ron " ) );//edn 속성을 ron으로 변경
	//GridObj.setColumnExcellType( GridObj.getColIndexById( LRTrim( "RD_DATE    " ) ), LRTrim( "ro  " ) );//dhxCalendar 속성을 ro로 변경
}
function setHeader1()
{
...
// 	GridObj.SetDateFormat("ENT_TO_DATE"			,"yyyy/MM/dd");
	...
	INDEX_SAVE_ITEM_DETAIL	= GridObj.GetColHDIndex("SAVE_ITEM_DETAIL"	);
...
// 			document.forms[0].save_item_detail.value 	= GridObj.GetCellValue("SAVE_ITEM_DETAIL"	, 0);
...
//			document.forms[0].ctrl_person_name.value 	= GridObj.GetCellValue("USER_NAME"		, 0);
			...
			var contFlag = GridObj.GetCellValue("CONTRACT_FLAG", 0);
		<%  if ("R".equals(screenMode) || !pre_cont_seq.equals("")) {  %>
				if(contFlag == ""){
					contFlag = "N";
				}
		<%	}else{	%>
		...
		<%  }	%>
			...
			GridObj.cells(GridObj.getRowId(0), GridObj.getColIndexById("CONTRACT_FLAG")).setValue(contFlag);
			
 			for(var i = 0; i < document.forms[0].contract_flag.options.length; i++){
 				if(document.forms[0].contract_flag.options[i].value == contFlag){
			...
			for(var i = 0; i < document.forms[0].po_yn.options.length; i++){
				if(GridObj.GetCellValue("PO_YN", 0) != ""){
					if(document.forms[0].po_yn.options[i].value == GridObj.GetCellValue("PO_YN", 0)){
						document.forms[0].po_yn.options[i].selected = true;
						break;
					}
				}
			}
		...
		var tmp_vendor_code = GridObj.GetCellValue("VENDOR_CODE",0);
		var vendor_cnt  = 0;
		//alert("tmp_vendor_code==="+tmp_vendor_code);
		
		for(var i = 0; i < GridObj.GetRowCount(); i++){
			
			if( tmp_vendor_code != GridObj.GetCellValue("VENDOR_CODE",i) )
			{
				vendor_cnt++; 		
				tmp_vendor_code =  GridObj.GetCellValue("VENDOR_CODE",i)
			}
		...
		//alert("vendor_cnt==="+vendor_cnt);
		
		if(vendor_cnt != 0 ){
			document.getElementById("vendor_cnt").value = "외"+vendor_cnt;
			document.getElementById("vendor_cnt").style.display="inline";
		}	
...
// 			wise.SetCellValue("ITEM_VAT_AMT", msg2 , RoundEx(wise.GetCellValue("ITEM_AMT", msg2) * 0.1 , 3));
			...
			document.forms[0].amt.value = add_comma(RoundEx(tot_item_amt, 3), 2);
    ...
    if(msg1=="t_imagetext")
    {
        ...
        if (msg3 == INDEX_PR_NO) {
      		...
      		window.open("/kr/dt/pr/pr1_bd_dis1I.jsp?pr_no="+img_pr_no,"pr1_bd_dis1","left=0,top=0,width=1024,height=650,resizable=yes,scrollbars=yes");
		}
...
function Approval(sign_status)
{
	...
	*/
	...
	if(document.forms[0].delay_remark.value != ''){
		if(Number(document.forms[0].delay_remark.value) > 1000){
			alert("지체상금율은 1000을 넘을 수 없습니다.");
			return;
		}
	}
...
// 	}
	...
	if(item_amt != dp_amt){
		alert("기안금액과 대금지불 금액이 올바르지 않습니다.\n기안(" + add_comma(item_amt, 2) + ") !=  대금지불(" + add_comma(dp_amt, 2) + ")");
		return;
	}
	...
	function getApprovalSend(approval_str) {
		...
		//alert("gridModifyFlag : " + gridModifyFlag)
...
<%String pr_no_name = "P".equals(REQ_TYPE) ? "구매요청번호 " : "사전지원요청번호 ";
			...
			if(prNo_prAmt_itemAmt[i]["ITEM_AMT"] != sum_grid_item_amt){
				...
				//retrun;
				if(!confirm("<%=pr_no_name%>" + prNo_prAmt_itemAmt[i]["PR_NO"] + "의 기존계약금액과(" + add_comma(prNo_prAmt_itemAmt[i]["ITEM_AMT"], 2) + ") 과 수정된 계약금액(" + add_comma(sum_grid_item_amt, 2) + ") 이 일치하지 않습니다.\r\n\r\n무시하고 계속 진행하시겠습니까?")){
					return;
				}
			}
		}
...
// 	GridObj.SetCellHiddenValue("SOURCING_NO", iMaxRow, modifySourcingNo);
	...
	GD_SetCellValueIndex(GridObj,iMaxRow, INDEX_SAVE_ITEM_DETAIL	,wise.GetCellValue("SAVE_ITEM_DETAIL", modifySeq));
...
var myDataProcessor = null;
    ...
    {
    	GridObj_setGridDraw();
    	GridObj.attachHeader("#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,단가,금액,단가,금액,단가,금액,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan");//히든컬럼도 포함
    	GridObj.setSizes();
    }
...
function doOnRowSelected(rowId,cellInd)
{
	...
	if( header_name == "VENDOR_NAME" ) {
		...
		var vendor_code = SepoaGridGetCellValueId(GridObj, rowId, "PO_VENDOR_CODE");
	...
	if( header_name == "SAVE_ITEM_DETAIL" ) {
		...
		var item_amt = SepoaGridGetCellValueId(GridObj, rowId, "ITEM_AMT");
		var detail_doc_no = SepoaGridGetCellValueId(GridObj, rowId, "DETAIL_DOC_NO");
		
		var url    = '/kr/dt/app/bd_item_mgt.jsp';
		var title  = '상세품목등록';
		var param  = 'popup=Y';
		param     += '&item_no=' + item_no;
		param     += '&description_loc=' + description_loc;
		param     += '&item_amt=' + item_amt;
		param     += '&detail_doc_no=' + detail_doc_no;
		param     += '&p_row_id=' + rowId;
		popUpOpen01(url, title, '750', '600', param);
...
// 	else if(header_name == "SOURCING_NO") {
		
// 		var sourcing_no 	= LRTrim(GridObj.cells(rowId, GridObj.getColIndexById("SOURCING_NO")).getValue());
// 		var sourcing_count 	= LRTrim(GridObj.cells(rowId, GridObj.getColIndexById("SOURCING_COUNT")).getValue());
// 		var sourcing_type	= LRTrim(GridObj.cells(rowId, GridObj.getColIndexById("SOURCING_TYPE")).getValue());
		
// 		if(sourcing_type == "RFQ"){
// 			url = "/kr/dt/rfq/rfq_bd_dis1.jsp?rfq_no=" + sourcing_no + "&rfq_count=" + sourcing_count;
// 		}else if(sourcing_type == "RAT"){
// 			url= "/ebid_doc/reveracution.jsp?RA_NO=" + sourcing_no + "&RA_COUNT=" + sourcing_count;
// 		}else if(sourcing_type == "BID"){//소싱번호 페이지 없음
// 			url= "/ebid_doc/inchaldetail.jsp?BID_NO=" + sourcing_no + "&BID_COUNT=" + sourcing_count
// 		}else if(sourcing_type == ""){
// 			return;
// 		}

// 		window.open(url ,"windowopen1","toolbar=no, location=no, directories=no, status=yes,menubar=no,scrollbars=yes,resizable=no,width=1021,height=768,left=0,top=0");
// 	}
...
// 상세품목등록 팝업에서 저장시 상세문서번호 세팅
function setDetailDocNo(rowId, detail_doc_no) {
	GridObj.cells(rowId, GridObj.getColIndexById('DETAIL_DOC_NO')).setValue(detail_doc_no);
}
...
function doQueryEnd() {
    ...
    for(var i = 0; i < GridObj.GetRowCount(); i++) {//이미지 세팅
		GridObj.cells(GridObj.getRowId(i), GridObj.getColIndexById("SAVE_ITEM_DETAIL")).setValue("<img src='/images/icon/icon_data_a.gif'>");
    }
...
function setOnBlur(obj) {
    ...
    target.value = Comma(target.value);
...
function clipPrint(rptAprvData,approvalCnt) {
	...
	sRptData += document.form1.add_date.value;
	sRptData += rf;
	sRptData += document.form1.subject.value;
	sRptData += rf;
	sRptData += document.form1.ctrl_person_name.value;
	sRptData += " (";	
	sRptData += document.form1.ctrl_person_id.value;
	sRptData += ")";
	sRptData += rf;	
	sRptData += document.form1.add_user_name.value;
	sRptData += " (";	
	sRptData += document.form1.add_user_id.value;
	sRptData += ")";
	sRptData += rf;	
	sRptData += document.form1.vendor_name.value;
	sRptData += " (";	
	sRptData += document.form1.vendor_code.value;
	sRptData += ")";
	sRptData += rf;	
	sRptData += document.form1.amt.value;
	sRptData += rf;	
	sRptData += document.form1.po_type.options[document.form1.po_type.selectedIndex].text;
	sRptData += rf;	
	sRptData += document.form1.contract_flag.options[document.form1.contract_flag.selectedIndex].text;
	sRptData += rf;		
	sRptData += document.form1.i_dp_type_text.value;
	sRptData += " ";	
	sRptData += document.form1.i_pay_amt.value;
	sRptData += rf;	
	sRptData += document.form1.po_yn.options[document.form1.po_yn.selectedIndex].text;
	sRptData += rf;		
	sRptData += document.form1.dely_terms.options[document.form1.dely_terms.selectedIndex].text;
	sRptData += rf;	
	sRptData += document.form1.delay_remark.value;
	sRptData += rf;	
	sRptData += document.form1.valid_from_date.value;
	sRptData += " ~ ";	
	sRptData += document.form1.valid_to_date.value;
	sRptData += rf;		
	sRptData += document.form1.warranty_month.value;
	sRptData += rf;		
	sRptData += document.form1.dely_to_location.value;
	sRptData += rf;	
	sRptData += document.form1.dely_to_address.value;
	sRptData += rf;	
	sRptData += document.form1.dely_to_user.value;
	sRptData += rf;	
	sRptData += document.form1.dely_to_phone.value;
	...
	for(var i = 0; i < GridObj.GetRowCount(); i++){	
		...
		sRptData += rf;		
		sRptData += GridObj.GetCellValue("QTY",i);
		sRptData += rf;		
		sRptData += GridObj.GetCellValue("CUR",i);
		sRptData += rf;		
		sRptData += GridObj.GetCellValue("UNIT_PRICE",i);
		sRptData += rf;		
		...
		sRptData += rf;		
		sRptData += GridObj.GetCellValue("SOURCING_NO",i);
		sRptData += rf;		
		sRptData += GridObj.GetCellValue("VENDOR_NAME",i);
		sRptData += rf;		
		sRptData += GridObj.GetCellValue("DETAIL_DOC_NO",i);	
		sRptData += rl;				
	}	
	sRptData += rd;	
...
<%}else { %>
			<td height="20" align="left" class="title_page" vAlign="bottom">
				<span id="preferred_bidder"><%=("C".equals(screenMode)) ? "기안생성" : (("R".equals(screenMode)) ? "기안상세" : (("U".equals(screenMode)) ? "기안수정" : (("E".equals(screenMode)) ? "변경기안생성" : "")))%></span>
<%} %>					
			</td>
		</tr>
	</table>
...
<td width="100%">
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td width="16%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;
				<%=("B".equals(REQ_TYPE)) ? "생성일자":"기안일자"%><font style="color: red;">*</font>
			</td>
			<td width="34%" class="data_td" colspan="3">
				<input type="text" id="add_date" name="add_date"  readOnly>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td width="16%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;
				<%=("B".equals(REQ_TYPE)) ? "사전지원명":"기안명"%><font style="color: red;">*</font>
			</td>
			<td width="34%" class="data_td" colspan="3">
				<input type="text" id="subject" name="subject" style="width: 98%" <%if ("R".equals(screenMode)) {%>  readOnly <%} else {%> class="input_re" <%}%> onKeyUp="return chkMaxByte(500, this, '기안명');" <%=("B".equals(REQ_TYPE)) ? "readOnly":""%>>
			</td>
		</tr>
		<tr>
			<td colspan="4" height="1" bgcolor="#dedede"></td>
		...
		<tr>
			<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;
				담당자<font style="color: red;">*</font>
			</td>
			<td class="data_td">
				<b>
					<input type="text" id="ctrl_person_id" name="ctrl_person_id" size="10" <%if ("R".equals(screenMode)) {%>  readOnly <%} else {%> class="input_re" <%}%> class="inputsubmit" readOnly> 
<%
if (!"R".equals(screenMode)) {
%> 							<a href="javascript:SP0023_Popup();"><img src="<%=G_IMG_ICON%>" align="absmiddle" border="0"></a> 
<%
	}
%> 
					<input type="text" id="ctrl_person_name" name="ctrl_person_name" size="10"  readOnly style="width: 50%">
					
				</b> 
				<input type="hidden" id="ctrl_code" name="ctrl_code" size="5" maxlength="5" class="inputsubmit" readOnly> 
				<input type="hidden" id="ctrl_name" name="ctrl_name" size="25"  readOnly>
			</td>
			<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;
				요청자<font style="color: red;">*</font>
			</td>
			<td class="data_td">
				<input type="text" id="add_user_id"   name="add_user_id"   size="10" value="" readOnly > 
				<input type="text" id="add_user_name" name="add_user_name" size="20" maxlength="12" value="" readOnly  style="width: 50%;">
			</td>
		</tr>
		<tr style="display: none;">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;
			<td class="title_td">
				프로젝트
			</td>
			<td class="data_td">
				<input type="text" id="wbs_info" name="wbs_info" style="width: 100%; color: #0000ff; cursor: ;"  readOnly onClick="openSCMSProject(GridObj.GetCellValue('WBS_NO', 0), GridObj.GetCellValue('ORDER_NO', 0), GridObj.GetCellValue('ORDER_COUNT', 0), GridObj.GetCellValue('CUST_CODE', 0));">
		...
		<tr>
			<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;
				<%=("B".equals(REQ_TYPE)) ? "담당자":"기안담당자"%><font style="color: red;">*</font>
			</td>
			<td class="data_td" colspan="3">
				<b>
					<input type="text" id="ctrl_person_id" name="ctrl_person_id" size="8" <%if ("R".equals(screenMode)) {%>  readOnly <%} else {%> class="input_re" <%}%> class="inputsubmit" readOnly> 
<%
if (!"R".equals(screenMode)) {
%> 						<a href="javascript:SP0023_Popup();"><img src="<%=G_IMG_ICON%>" align="absmiddle" border="0"> </a> 
<%
	}
%> 						<input type="text" id="ctrl_person_name" name="ctrl_person_name" size="10"  readOnly>
				</b> 
				<input type="hidden" id="ctrl_code" name="ctrl_code" size="5" maxlength="5" class="inputsubmit" readOnly> 
				<input type="hidden" id="ctrl_name" name="ctrl_name" size="25"  readOnly> 
				<input type="hidden" id="wbs_info" name="wbs_info" style="width: 100%;"  readOnly> 
				<select id="exec_flag" name="exec_flag" class="inputsubmit" style="display: none;">
					<%=LB_EXEC_FLAG%>
				</select>
			</td>
		...
		<tr>
			...
			<td class="data_td">
				<input type="text" id="vendor_code" name="vendor_code" size="10"  readOnly value="" >   
				<input type="text" id="vendor_cnt" name="vendor_cnt"  readOnly value="" class="input_data2" style="display:none;width:5%;line-height:25px;">  
				<input type="text" id="vendor_name" name="vendor_name" size="15"  readOnly value="" style="width: 50%;">
			</td>
			<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;
				<%=("B".equals(REQ_TYPE)) ? "확인금액":"기안금액"%>(VAT 포함)<font style="color: red;">*</font>
			</td>
			<td class="data_td"><input type="text" id="amt" name="amt" size="20" value="" readOnly class="inputsubmit_right" onfocus="javascript:setOnFocus(this);" onblur="javascript:setOnBlur(this);" style="text-align: right;"></td>
		</tr>
		<tr style="display: none">
			...
			<td class="data_td">
				<input type="text" id="sale_amt" name="sale_amt" size="20" value="" readOnly >
			</td>
			<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;
				절감율
			</td>
			<td class="data_td">
				<input type="text" id="sale_per" name="sale_per" size="6" value="" readOnly >%
			</td>
		</tr>
		<%
			if ("P".equals(REQ_TYPE)) {
		...
		<tr style="display: none">
			...
			<td class="data_td">
				<input type="checkbox" id="ahead_flag_header" name="ahead_flag_header" onChange="setAhead_flag(this);" <%if ("R".equals(screenMode)) {%> disabled <%}%>>
			</td>
		</tr>
		<tr>
			<td colspan="4" height="1" bgcolor="#dedede"></td>
		...
		<tr>
			<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;
				기안구분<font style="color: red;">*</font>
			</td>
			<td class="data_td">
			...
			<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;
				계약여부<font style="color: red;">*</font>
			</td>
			<td class="data_td">
				<select id="contract_flag"  name="contract_flag" <%if ("R".equals(screenMode)) {%> class="inputsubmit" <%} else {%> class="input_re" <%}%> onchange="changeContract()" <%if ("R".equals(screenMode) || !pre_cont_seq.equals("")) {%> disabled <%}%>>
					<option value="">선택</option>
					<option value="N">N</option>
					<option value="Y" <%if (!pre_cont_seq.equals("")) { //변경계약기안일경우 계약서작성은 필수!%> selected <%}%>>Y</option>
				</select>
			</td>
		</tr>
		<%
		...
		<tr style="display: none;">
			...
			<td class="data_td">
				<input type="text" id="pr_subject" name="pr_subject"  size="40" readOnly>
			</td>
		</tr>
		<tr>
			<td colspan="4" height="1" bgcolor="#dedede"></td>
...
<%-- 				<a href="javascript:openDPinfo('S');"><img src="<%=G_IMG_ICON%>" align="absmiddle" border="0"></a>  --%>
<!-- 				<input type="hidden" id="s_dp_count" 		name="s_dp_count" 		size="2" value="" readOnly >  -->
<!-- 				<input type="text"   id="s_dp_type_text" 	name="s_dp_type_text" 	size="14" value="" readOnly >  -->
<!-- 				<input type="hidden" id="s_dp_type" 		name="s_dp_type" 		size="3" value="" readOnly >  -->
<!-- 				<input type="hidden" id="s_insurance" 	 	name="s_insurance" 		size="3" value="Y" readOnly >  -->
<!-- 				<input type="text"   id="s_pay_amt" 		name="s_pay_amt" 		 readOnly> -->
<!-- 			</td> -->
			<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;
<!-- 				물품 대금지불 -->
				대금지불<font style="color: red;">*</font>
			</td>
			<td class="data_td">
				<a href="javascript:openDPinfo('I');"><img src="<%=G_IMG_ICON%>" align="absmiddle" border="0"></a> 
				<input type="hidden" id="i_dp_count" 		name="i_dp_count" 		size="2" value="" readOnly > 
				<input type="text" 	 id="i_dp_type_text" 	name="i_dp_type_text" 	size="14" value="" readOnly class="input_re"> 
				<input type="hidden" id="i_dp_type" 		name="i_dp_type" 		size="3" value="" readOnly > 
				<input type="hidden" id="i_insurance" 		name="i_insurance" 		size="3" value="Y" readOnly > 
				<input type="text"   id="i_pay_amt" 		name="i_pay_amt" 		 readOnly class="input_re">
			</td>
			<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;
				발주여부<font style="color: red;">*</font>
			</td>
			<td class="data_td">
				<select id="po_yn" name="po_yn" <%if ("R".equals(screenMode)) {%> class="inputsubmit" <%} else {%> class="input_re" <%}%><%if ("R".equals(screenMode)) {%> disabled <%}%>>
		...
		<tr style="display: none">
			...
			%>
			<td class="data_td" colspan="3">
				<input type="text" id="pre_exec_no" name="pre_exec_no" style="width: 20%; color: #0000ff; cursor: hand;"  readOnly onClick="openPreExec();">
			</td>
			<%
				} else {
			%>
			<td class="data_td" colspan="3">
				<input type="text" id="pre_exec_no" name="pre_exec_no" class="input_data" size="20">
			</td>
			<%
				}
			%>
		...
		<tr>
			<td width="15%" class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;
				인도조건<font style="color: red;">*</font>
			</td>
			<td width="35%" class="data_td">
			...
			<td width="30%" class="data_td">
				1000 분의&nbsp; 
				<input type="text" id="delay_remark" name="delay_remark" size="7" style="ime-mode:disabled;"  onKeyUp="return chkMaxByte(4, this, '지체상금율');" onKeyPress="checkNumberFormat('[0-9|/./]', this);">
			</td>
		</tr>
		<tr>
			<td colspan="4" height="1" bgcolor="#dedede"></td>
		...
		<tr>
			<td class="title_td">&nbsp;<img src="/images/blt_srch.gif" width="7" height="7" align="absmiddle">&nbsp;&nbsp;
				계약기간<font style="color: red;">*</font>
			</td>
			<td class="data_td">
				<%
					if ("R".equals(screenMode)) {
				%> 
				<input type="text" id="valid_from_date" name="valid_from_date" size="10" maxlength="10" value=""  readOnly> 
				~ 
				<input type="text" id="valid_to_date" name="valid_to_date" size="10" maxlength="10" value=""  readOnly> 
				<%
					} else {
				%> 
			...
			<td width="35%" class="data_td">최종검수일자로부터
				<input type="text" id="warranty_month" style="text-align:right; ime-mode:disabled;" name="warranty_month" size="5" maxlength="5"  <%="R".equals(screenMode) ? "class='input_data2_right' readonly" : "class='inputsubmit_right'"%>  onKeyUp="return chkMaxByte(2, this, '지체상금율');" onKeyPress="checkNumberFormat('[0-9]', this);" >개월
			</td>
		</tr>
		<tr>
			<td colspan="4" height="1" bgcolor="#dedede"></td>
		...
		<tr>
			...
			<td class="data_td">
				<input type="text" id="dely_to_phone" name="dely_to_phone" style="width: 35%; ime-mode:disabled;" onKeyPress="checkNumberFormat('[0-9.]', this)" onKeyUp="return chkMaxByte(13, this, '수령인연락처');" <%="R".equals(screenMode) ? "class='input_data2' readonly" : "class='inputsubmit'"%> >
			</td>
		</tr>
	</table>
</td>
				...
				<TABLE cellpadding="0">
					<TR>
						<%
							if ("R".equals(screenMode)) {
						%>
						<TD><script language="javascript">btn("javascript:clipPrint();", "출 력")   </script>
...
<!-- 				<iframe name="attachFrame" width="620" height="150" marginwidth="0" marginheight="0" frameborder="0" scrolling="no"></iframe> <br>&nbsp; -->
<!-- 			</td> -->
<!-- 		</tr> -->
		...
		%>
   		<tr>
			...
			<td class="se_cell_data" colspan="3">				
				...
				<input type="text" size="3" readOnly class="input_empty" value="<%=ATTACH_COUNT%>" name="sign_attach_no_count" id="sign_attach_no_count"/>
				<input type="hidden" name="sign_attach_no" id="sign_attach_no" value="<%=ATTACH_NO%>">
			</td>
		</tr>
		<%
			}
